...
В следующих примерах показано, как создавать и отменять точки восстановления с помощью функции [**срсетресторепоинт**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) .

Первым шагом при использовании восстановления системы является настройка вызовов COM для [**CoInitializeEx**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializeex) и [**CoInitializeSecurity**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializesecurity). Это необходимо для любого процесса, использующего функцию [**срсетресторепоинт**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) . Сетевая служба, локальная служба и система должны быть разрешены для выполнения вызовов процесса. Следующая функция **инитиализекомсекурити** является примером инициализации безопасности COM. Может потребоваться изменить параметры для приложения в функции **CoInitializeSecurity** .
...
## <a name="example-1-create-a-restore-point"></a>Пример 1. Создание точки восстановления.
...
## <a name="example-2-create-and-cancel-a-restore-point"></a>Пример 2. Создание и отмена точки восстановления.
