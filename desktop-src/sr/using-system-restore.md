---
title: Использование восстановления системы
description: В следующих примерах показано, как создавать и отменять точки восстановления с помощью функции Срсетресторепоинт.
...
---
# <a name="using-system-restore"></a><span data-ttu-id="d4cfd-104">Использование восстановления системы</span><span class="sxs-lookup"><span data-stu-id="d4cfd-104">Using System Restore</span></span>

<span data-ttu-id="d4cfd-105">В следующих примерах показано, как создавать и отменять точки восстановления с помощью функции [**срсетресторепоинт**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) .</span><span class="sxs-lookup"><span data-stu-id="d4cfd-105">The following examples demonstrate how to create and cancel restore points using the [**SRSetRestorePoint**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) function.</span></span>

<span data-ttu-id="d4cfd-106">Первым шагом при использовании восстановления системы является настройка вызовов COM для [**CoInitializeEx**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializeex) и [**CoInitializeSecurity**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializesecurity).</span><span class="sxs-lookup"><span data-stu-id="d4cfd-106">The first step to using using System Restore is setting up the COM calls to [**CoInitializeEx**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializeex) and [**CoInitializeSecurity**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializesecurity).</span></span> <span data-ttu-id="d4cfd-107">Это необходимо для любого процесса, использующего функцию [**срсетресторепоинт**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) .</span><span class="sxs-lookup"><span data-stu-id="d4cfd-107">This is required for any process that uses the [**SRSetRestorePoint**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) function.</span></span> <span data-ttu-id="d4cfd-108">Сетевая служба, локальная служба и система должны быть разрешены для выполнения вызовов процесса.</span><span class="sxs-lookup"><span data-stu-id="d4cfd-108">The NetworkService, LocalService, and System must be allowed make calls to the process.</span></span> <span data-ttu-id="d4cfd-109">Следующая функция **инитиализекомсекурити** является примером инициализации безопасности COM.</span><span class="sxs-lookup"><span data-stu-id="d4cfd-109">The following **InitializeCOMSecurity** function is an example of how to initialize COM security.</span></span> <span data-ttu-id="d4cfd-110">Может потребоваться изменить параметры для приложения в функции **CoInitializeSecurity** .</span><span class="sxs-lookup"><span data-stu-id="d4cfd-110">You may need to modify the parameters to the **CoInitializeSecurity** function for your application.</span></span>
...
## <a name="example-1-create-a-restore-point"></a><span data-ttu-id="d4cfd-111">Пример 1. Создание точки восстановления.</span><span class="sxs-lookup"><span data-stu-id="d4cfd-111">Example 1: Create a restore point.</span></span>
...
## <a name="example-2-create-and-cancel-a-restore-point"></a><span data-ttu-id="d4cfd-112">Пример 2. Создание и отмена точки восстановления.</span><span class="sxs-lookup"><span data-stu-id="d4cfd-112">Example 2: Create and cancel a restore point.</span></span>
