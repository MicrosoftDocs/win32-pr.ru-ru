---
description: В следующем примере данные передаются из файла журнала счетчиков, созданного средством Performance, в формат с разделителями-запятыми (.csv).
...
ms.locfileid: "122880909"
---
# <a name="converting-data-from-a-binary-format-log-file-to-a-csv-format-log-file"></a>Преобразование данных из файла журнала в двоичном формате в файл журнала формата CSV

В следующем примере данные передаются из файла журнала счетчиков, созданного средством Performance, в формат с разделителями-запятыми (.csv). Этот пример передает данные счетчика времени процессора, собранные с локального компьютера. Чтобы указать другой тип данных счетчика, измените переменную Сзкаунтерпас. Если собранные данные счетчика относятся к определенному компьютеру, добавьте имя компьютера в путь (например, " \\ \\ \\ \\ &lt; &gt; \\ \\ процессор ComputerName (0) \\ \\ % загруженности процессора").
