---
description: Если требуется отобразить список объектов и их счетчиков в пользовательском интерфейсе, необходимо получить данные о производительности.
ms.assetid: 0d122fa8-2ed8-4bd8-a52e-6cb20fe81741
title: Отображение имен объектов, экземпляров и счетчиков
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103987973"
---
# <a name="displaying-object-instance-and-counter-names"></a><span data-ttu-id="a96fc-103">Отображение имен объектов, экземпляров и счетчиков</span><span class="sxs-lookup"><span data-stu-id="a96fc-103">Displaying Object, Instance, and Counter Names</span></span>

<span data-ttu-id="a96fc-104">Если требуется отобразить список объектов и их счетчиков в пользовательском интерфейсе, необходимо получить данные о производительности.</span><span class="sxs-lookup"><span data-stu-id="a96fc-104">If you want to display a list of objects and their counters in a user interface, you must retrieve the performance data.</span></span> <span data-ttu-id="a96fc-105">Данные производительности содержат переменное число объектов производительности, их экземпляры и счетчики.</span><span class="sxs-lookup"><span data-stu-id="a96fc-105">The performance data contains a variable number of performance objects and their instances and counters.</span></span> <span data-ttu-id="a96fc-106">Сведения о формате данных производительности см. в разделе [Формат данных производительности](performance-data-format.md).</span><span class="sxs-lookup"><span data-stu-id="a96fc-106">For information on the format of the performance data, see [Performance Data Format](performance-data-format.md).</span></span>

<span data-ttu-id="a96fc-107">Данные производительности не содержат имена объектов и счетчиков.</span><span class="sxs-lookup"><span data-stu-id="a96fc-107">The performance data does not contain the object and counter names.</span></span> <span data-ttu-id="a96fc-108">Вместо этого данные содержат значения индекса, используемые для получения имен объектов и счетчиков, которые хранятся в ресурсах, известных инфраструктуре счетчиков производительности.</span><span class="sxs-lookup"><span data-stu-id="a96fc-108">Instead, the data contains index values that you use to retrieve the names of the objects and counters, which are stored in resources known to the performance counters infrastructure.</span></span> <span data-ttu-id="a96fc-109">Однако имена экземпляров хранятся в данных о производительности.</span><span class="sxs-lookup"><span data-stu-id="a96fc-109">However, the instance names are stored in performance data.</span></span> <span data-ttu-id="a96fc-110">Пример, считывающий имена объектов и счетчиков из реестра и создающий таблицу индексов для доступа к именам позднее, см. в разделе [Получение имен счетчиков и текста справки](retrieving-counter-names-and-help-text.md).</span><span class="sxs-lookup"><span data-stu-id="a96fc-110">For an example that reads the object and counter names from the registry and creates an index table for accessing the names later, see [Retrieving Counter Names and Help Text](retrieving-counter-names-and-help-text.md).</span></span>

<span data-ttu-id="a96fc-111">В следующем примере показано, как получить блоки объектов, экземпляров и счетчиков из данных о производительности.</span><span class="sxs-lookup"><span data-stu-id="a96fc-111">The following example shows how to retrieve the object, instance, and counter blocks from the performance data.</span></span> <span data-ttu-id="a96fc-112">После получения имен объектов, имен экземпляров и имен счетчиков в примере сортируются объекты по имени и каждому объекту, сортируются экземпляры и счетчики по имени.</span><span class="sxs-lookup"><span data-stu-id="a96fc-112">After retrieving the object names, instance names, and counter names, the example sorts the objects by name and for each object, sorts its instances and counters by name.</span></span> <span data-ttu-id="a96fc-113">Затем в примере выводятся отсортированные списки.</span><span class="sxs-lookup"><span data-stu-id="a96fc-113">The example then prints the sorted lists.</span></span>
