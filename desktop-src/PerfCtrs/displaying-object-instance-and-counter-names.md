...
ms.locfileid: "120033924"
---
# <a name="displaying-object-instance-and-counter-names"></a>Отображение имен объектов, экземпляров и счетчиков

Если требуется отобразить список объектов и их счетчиков в пользовательском интерфейсе, необходимо получить данные о производительности. Данные производительности содержат переменное число объектов производительности, их экземпляры и счетчики. Сведения о формате данных производительности см. в разделе [Формат данных производительности](performance-data-format.md).

Данные производительности не содержат имена объектов и счетчиков. Вместо этого данные содержат значения индекса, используемые для получения имен объектов и счетчиков, которые хранятся в ресурсах, известных инфраструктуре счетчиков производительности. Однако имена экземпляров хранятся в данных о производительности. Пример, считывающий имена объектов и счетчиков из реестра и создающий таблицу индексов для доступа к именам позднее, см. в разделе [Получение имен счетчиков и текста справки](retrieving-counter-names-and-help-text.md).

В следующем примере показано, как получить блоки объектов, экземпляров и счетчиков из данных о производительности. После получения имен объектов, имен экземпляров и имен счетчиков в примере сортируются объекты по имени и каждому объекту, сортируются экземпляры и счетчики по имени. Затем в примере выводятся отсортированные списки.
