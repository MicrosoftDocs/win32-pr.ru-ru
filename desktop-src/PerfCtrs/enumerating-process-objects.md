---
description: В следующем примере вызывается функция Пдхенумобжектитемс для перечисления экземпляров и счетчиков объектов процесса на локальном компьютере.
ms.assetid: d7518ba6-a0f1-4985-aa2c-1ca15a0ceb02
title: Перечисление объектов процессов
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105663235"
---
# <a name="enumerating-process-objects"></a><span data-ttu-id="afc55-103">Перечисление объектов процессов</span><span class="sxs-lookup"><span data-stu-id="afc55-103">Enumerating Process Objects</span></span>

<span data-ttu-id="afc55-104">В следующем примере вызывается функция [**пдхенумобжектитемс**](/windows/desktop/api/Pdh/nf-pdh-pdhenumobjectitemsa) для перечисления экземпляров и счетчиков объектов процесса на локальном компьютере.</span><span class="sxs-lookup"><span data-stu-id="afc55-104">The following example calls the [**PdhEnumObjectItems**](/windows/desktop/api/Pdh/nf-pdh-pdhenumobjectitemsa) function to enumerate the instances and counters of the process objects on the local computer.</span></span>
