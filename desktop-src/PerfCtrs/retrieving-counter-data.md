---
description: Объекты производительности могут определять один или несколько счетчиков.
ms.assetid: a3a598b2-5623-4472-a814-620c6a003a7e
title: Получение данных счетчиков
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105663157"
---
# <a name="retrieving-counter-data"></a><span data-ttu-id="1c2c3-103">Получение данных счетчиков</span><span class="sxs-lookup"><span data-stu-id="1c2c3-103">Retrieving Counter Data</span></span>

<span data-ttu-id="1c2c3-104">Объекты производительности могут определять один или несколько счетчиков.</span><span class="sxs-lookup"><span data-stu-id="1c2c3-104">Performance objects can define one or more counters.</span></span> <span data-ttu-id="1c2c3-105">Данные счетчика для счетчиков находятся в блоке [**памяти \_ \_ блока счетчика производительности**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_block) .</span><span class="sxs-lookup"><span data-stu-id="1c2c3-105">The counter data for the counters is located in the [**PERF\_COUNTER\_BLOCK**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_block) memory block.</span></span> <span data-ttu-id="1c2c3-106">Расположение блока счетчиков в блоке объектов зависит от того, содержит ли объект счетчики одного экземпляра или несколько экземпляров.</span><span class="sxs-lookup"><span data-stu-id="1c2c3-106">The location of the counter block within the object block depends on whether the object contains single instance counters or multiple instance counters.</span></span> <span data-ttu-id="1c2c3-107">Дополнительные сведения см. в разделе [Формат данных производительности](performance-data-format.md).</span><span class="sxs-lookup"><span data-stu-id="1c2c3-107">For details, see [Performance Data Format](performance-data-format.md).</span></span>

<span data-ttu-id="1c2c3-108">Для доступа к данным счетчика в блоке счетчиков используются члены **каунтероффсет** и **каунтерсизе** [**\_ \_ определения счетчика производительности**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) .</span><span class="sxs-lookup"><span data-stu-id="1c2c3-108">You use the **CounterOffset** and **CounterSize** members of [**PERF\_COUNTER\_DEFINITION**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) to access the counter's data within the counter block.</span></span> <span data-ttu-id="1c2c3-109">В настоящее время данные счетчика ограничены типами данных **DWORD** и **улонглонг** (это единственные типы, поддерживаемые средством производительности).</span><span class="sxs-lookup"><span data-stu-id="1c2c3-109">Currently, counter data is limited to **DWORD** and **ULONGLONG** data types (these are the only types that the Performance tool supports).</span></span>

<span data-ttu-id="1c2c3-110">Элемент **CounterType** в [**\_ \_ определении счетчика производительности**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) сообщает, какие другие сведения нужны из объекта производительности, чтобы использовать данные счетчика.</span><span class="sxs-lookup"><span data-stu-id="1c2c3-110">The **CounterType** member of [**PERF\_COUNTER\_DEFINITION**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) tells you what other information you need from the performance object in order to use the counter data.</span></span> <span data-ttu-id="1c2c3-111">Для некоторых счетчиков можно использовать данные счетчика напрямую, но для других пользователей может потребоваться указать базовую информацию времени или данные из другого счетчика, чтобы вычислить отображаемое значение.</span><span class="sxs-lookup"><span data-stu-id="1c2c3-111">For some counters, you can use the counter data directly, but for others, you may need time base information or data from another counter in order to calculate a displayable value.</span></span>

<span data-ttu-id="1c2c3-112">В следующем примере показано, как использовать тип счетчика для определения сведений, которые необходимо получить из данных о производительности для счетчика, чтобы вычислить отображаемое значение счетчика.</span><span class="sxs-lookup"><span data-stu-id="1c2c3-112">The following example shows how to use the counter type to determine the information you need to retrieve from the performance data for a counter in order to calculate a displayable counter value.</span></span> <span data-ttu-id="1c2c3-113">Пример, в котором вычисляется отображаемое значение на основе типа счетчика, см. в разделе [Вычисление значений счетчиков](calculating-counter-values.md).</span><span class="sxs-lookup"><span data-stu-id="1c2c3-113">For an example that calculates a displayable value based on the counter type, see [Calculating Counter Values](calculating-counter-values.md).</span></span>
