...
ms.locfileid: "119962304"
---
# <a name="retrieving-counter-data"></a>Получение данных счетчиков

Объекты производительности могут определять один или несколько счетчиков. Данные счетчика для счетчиков находятся в блоке [**памяти \_ \_ блока счетчика производительности**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_block) . Расположение блока счетчиков в блоке объектов зависит от того, содержит ли объект счетчики одного экземпляра или несколько экземпляров. Дополнительные сведения см. в разделе [Формат данных производительности](performance-data-format.md).

Для доступа к данным счетчика в блоке счетчиков используются члены **каунтероффсет** и **каунтерсизе** [**\_ \_ определения счетчика производительности**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) . В настоящее время данные счетчика ограничены типами данных **DWORD** и **улонглонг** (это единственные типы, поддерживаемые средством производительности).

Элемент **CounterType** в [**\_ \_ определении счетчика производительности**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) сообщает, какие другие сведения нужны из объекта производительности, чтобы использовать данные счетчика. Для некоторых счетчиков можно использовать данные счетчика напрямую, но для других пользователей может потребоваться указать базовую информацию времени или данные из другого счетчика, чтобы вычислить отображаемое значение.

В следующем примере показано, как использовать тип счетчика для определения сведений, которые необходимо получить из данных о производительности для счетчика, чтобы вычислить отображаемое значение счетчика. Пример, в котором вычисляется отображаемое значение на основе типа счетчика, см. в разделе [Вычисление значений счетчиков](calculating-counter-values.md).
