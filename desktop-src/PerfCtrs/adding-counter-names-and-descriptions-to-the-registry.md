...
- [Создайте инициализацию (. INI-файл)](#creating-an-initialization-ini-file) , содержащий строки.
...
## <a name="creating-an-initialization-ini-file"></a>Создание инициализации (. INI-файл)

Инициализация (. INI) содержит имя и строки справки для каждого объекта и счетчика, определенные в файле символов. Тот. INI-файл используется в качестве входных данных для **lodctr** во время установки поставщика.

Тот. Файл INI должен быть закодирован как UTF-16LE (с меткой порядка следования байтов) и содержать следующие разделы и ключи:
...
|**симболфиле**| Укажите файл заголовка. h, содержащий символьные значения объектов и счетчиков поставщика. Во время установки (при вызове **lodctr**) файл заголовка должен находиться в том же каталоге, что и. INI-файл.
...
Для загрузки имен и строк справки, определенных в. INI-файл (во время установки поставщика) запустите средство **lodctr** из папки, в которой находится. INI-файл и заголовочный файл. Средство входит в состав компьютера. Необходимо запустить **lodctr** с повышенными привилегиями. Параметр для **lodctr** — это путь к. INI-файл. Например, `lodctr "C:\Program Files\MyCompany\MyProvider\MyProvider.ini"`.
...
Служебная программа **lodctr** копирует строки из. INI-файл к **счетчикам** и значения реестра **справки** в подразделах соответствующего языка. Если соответствующий языковой ключ не существует, строки для этого языка не копируются. Программа также обновляет **последний счетчик** и Последнее значение **справки** . Имена и описания счетчиков производительности хранятся в следующем расположении в реестре.
