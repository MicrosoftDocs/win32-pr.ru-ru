...
---
# <a name="prototypes"></a>Прототипы

[Итфконтекстрендерингмаркуп](/windows/desktop/TSF/itfcontextrenderingmarkup), [иенумтфрендерингмаркуп](/windows/desktop/TSF/ienumtfrenderingmarkup)и [tf \_ рендерингмаркуп](/windows/desktop/TSF/tf-renderingmarkup) , описанные в этой ссылке, не определены в файлах IDL или в заголовочном файле. Для того чтобы получить файл заголовка, необходимо, чтобы компилятор MIDL выполнял следующие прототипы.
