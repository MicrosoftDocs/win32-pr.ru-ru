---
title: WMT_VIDEOIMAGE_TRANSITION_REVEAL (Вмсдкидл. h)
description: Переход «Показать» раскрывает новый образ вдоль прямой линии, полученной с одной стороны рамки.
ms.assetid: 75ff6155-6b28-474a-b5d1-c3f1b3873b8e
keywords:
- WMT_VIDEOIMAGE_TRANSITION_REVEAL формат Windows Media
topic_type:
- apiref
api_name:
- WMT_VIDEOIMAGE_TRANSITION_REVEAL
api_location:
- wmsdkidl.h
api_type:
- HeaderDef
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: b916a5142f09628852a016754f9fb3ad691882731466d802b8367e03837b9699
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/11/2021
ms.locfileid: "119083738"
---
# <a name="wmt_videoimage_transition_reveal"></a>ВМТ \_ видеоимаже \_ Переход \_

Переход «Показать» раскрывает новый образ вдоль прямой линии, полученной с одной стороны рамки.

## <a name="parameters"></a>Параметры

В следующей таблице описаны параметры, используемые этим переходом, и перечислены члены структуры [**ВМТ \_ видеоимаже \_ SAMPLE2**](/previous-versions/windows/desktop/api/Wmsdkidl/ns-wmsdkidl-wmt_videoimage_sample2) , которой они назначены.



<table>
<colgroup>
<col style="width: 33%" />
<col style="width: 33%" />
<col style="width: 33%" />
</colgroup>
<thead>
<tr class="header">
<th>Параметр</th>
<th>Член структуры</th>
<th>Описание</th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td>расстояние;</td>
<td><strong>fEffectPara0</strong></td>
<td>Размер нового изображения, выданного в пикселях. Это значение задается относительно исходной стороны кадра.</td>
</tr>
<tr class="even">
<td>Направление</td>
<td><strong>fEffectPara1</strong></td>
<td>Направление раскрытия. Задайте одно из следующих значений:<br/>
<ul>
<li>0 — отображается справа; исходит от левой части рамки.</li>
<li>1 — раскрывается слева; исходит от правой части кадра.</li>
<li>2. Отображение вверх; исходит от нижней части рамки.</li>
<li>3 — раскрытие вниз; исходит от верхней части рамки.</li>
</ul></td>
</tr>
<tr class="odd">
<td>Композиция</td>
<td><strong>fEffectPara2</strong></td>
<td>Задайте одно из следующих значений:
<ul>
<li>0 — задает нормальную композицию, в которой предыдущее изображение является фоном, а текущее изображение — передним планом.</li>
<li>1 — указывает обратную композицию, в которой текущий рисунок является фоновым изображением, а предыдущее изображение — передний план.</li>
</ul></td>
</tr>
</tbody>
</table>



 

## <a name="requirements"></a>Требования



| Требование | Значение |
|-------------------|---------------------------------------------------------------------------------------|
| Заголовок<br/> | <dl> <dt>Вмсдкидл. h</dt> </dl> |



## <a name="see-also"></a>См. также раздел

<dl> <dt>

[**Переходы по изображениям видео**](video-image-transitions.md)
</dt> </dl>

 

 





