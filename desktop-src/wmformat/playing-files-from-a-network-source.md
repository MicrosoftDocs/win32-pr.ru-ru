...
keywords:
- Windows Пакет SDK для формата мультимедиа, чтение данных ASF
- Windows Пакет SDK для формата мультимедиа, воспроизведение файлов из сетевых источников
...
-   Потоки. Данные передаются в течение времени для воспроизведения на локальном компьютере. серверы, на которых работает службы Windows Media, могут предоставлять потоковые данные. Если содержимое с несколькими потоками (MBR) передается потоком, клиент может запросить другую скорость передачи данных с сервера при потоковой передаче.
