---
title: Поиск форматов аудио
description: Поиск форматов аудио
ms.assetid: f2001ed5-f07d-45a5-a566-45697024870e
keywords:
- потоки, поиск звуковых форматов
- звуковые потоки, поиск аудио форматов
- потоки, форматы аудио
- звуковые потоки, форматы аудио
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104338297"
---
# <a name="to-find-audio-formats"></a><span data-ttu-id="a941b-107">Поиск форматов аудио</span><span class="sxs-lookup"><span data-stu-id="a941b-107">To Find Audio Formats</span></span>

<span data-ttu-id="a941b-108">В следующем примере кода показано, как найти звуковой формат, соответствующий заданному критерию.</span><span class="sxs-lookup"><span data-stu-id="a941b-108">The following example code demonstrates how to find an audio format that matches criteria you specify.</span></span> <span data-ttu-id="a941b-109">Функция **финдаудиоформат** принимает указатель на структуру [**вавеформатекс**](/previous-versions/windows/desktop/legacy/dd757720(v=vs.85)) , содержащую количество каналов, бит на выборку и частоту выборки, которую вы хотите использовать.</span><span class="sxs-lookup"><span data-stu-id="a941b-109">The **FindAudioFormat** function accepts a pointer to a [**WAVEFORMATEX**](/previous-versions/windows/desktop/legacy/dd757720(v=vs.85)) structure that contains the number of channels, bits per sample, and sample rate that you want to use.</span></span> <span data-ttu-id="a941b-110">Функция находит формат, соответствующий этим требованиям, и имеет наибольшую скорость потока, которая не превышает параметр *двмаксрате* .</span><span class="sxs-lookup"><span data-stu-id="a941b-110">The function finds the format that matches those requirements and has the highest bit rate that does not exceed the *dwMaxRate* parameter.</span></span> <span data-ttu-id="a941b-111">Если для *фавсинк* задано **значение true**, функция проверяет только форматы, которые можно синхронизировать с видео.</span><span class="sxs-lookup"><span data-stu-id="a941b-111">If you set *fAVSync* to **TRUE**, the function only validates formats that can be synchronized with video.</span></span> <span data-ttu-id="a941b-112">Для простоты эта функция работает только с 1-проходным форматом CBR.</span><span class="sxs-lookup"><span data-stu-id="a941b-112">For simplicity, this function only works with 1-pass CBR formats.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="a941b-113">См. также</span><span class="sxs-lookup"><span data-stu-id="a941b-113">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="a941b-114">**Настройка звуковых потоков**</span><span class="sxs-lookup"><span data-stu-id="a941b-114">**Configuring Audio Streams**</span></span>](configuring-audio-streams.md)
</dt> <dt>

[<span data-ttu-id="a941b-115">**Перечисление форматов кодеков**</span><span class="sxs-lookup"><span data-stu-id="a941b-115">**To Enumerate Codec Formats**</span></span>](to-enumerate-codec-formats.md)
</dt> </dl>
