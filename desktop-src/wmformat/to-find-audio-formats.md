...
ms.locfileid: "118196510"
---
# <a name="to-find-audio-formats"></a>Поиск форматов аудио

В следующем примере кода показано, как найти звуковой формат, соответствующий заданному критерию. Функция **финдаудиоформат** принимает указатель на структуру [**вавеформатекс**](/previous-versions/windows/desktop/legacy/dd757720(v=vs.85)) , содержащую количество каналов, бит на выборку и частоту выборки, которую вы хотите использовать. Функция находит формат, соответствующий этим требованиям, и имеет наибольшую скорость потока, которая не превышает параметр *двмаксрате* . Если для *фавсинк* задано **значение true**, функция проверяет только форматы, которые можно синхронизировать с видео. Для простоты эта функция работает только с 1-проходным форматом CBR.
...
[**Перечисление форматов кодеков**](to-enumerate-codec-formats.md)
</dt> </dl>
