---
title: WM/Composer
description: атрибут WM/Composer содержит имя музыкального композитора.
ms.assetid: ec606961-0196-4a11-81f7-066496e6e413
keywords:
- формат windows Media WM/Composer
topic_type:
- apiref
api_name:
- WM/Composer
api_type:
- NA
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 548257945d4b8a6606136247dfdc82daff232d1c8fd0a8219f74c42c3eac7433
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/11/2021
ms.locfileid: "118431814"
---
# <a name="wmcomposer"></a>WM/Composer

атрибут **WM/Composer** содержит имя музыкального композитора.

## <a name="global-constant"></a>Глобальная константа

g \_ всзвмкомпосер

## <a name="data-type"></a>Тип данных

**\_Строка типа \_ ВМТ**

## <a name="see-also"></a>См. также раздел

<dl> <dt>

[**Список атрибутов**](attribute-list.md)
</dt> </dl>

 

 




