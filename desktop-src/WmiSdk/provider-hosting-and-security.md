...
Инструментарий WMI находится в узле общей службы с несколькими другими службами. Чтобы избежать остановки всех служб при сбое поставщика, поставщики загружаются в отдельный ведущий процесс с именем "Wmiprvse.exe". Может выполняться более одного процесса с таким именем. Каждый из них может выполняться под другой учетной записью с различными уровнями безопасности. имейте в виду, что начиная с Windows Vista, используйте команду [**winmgmt**](winmgmt.md) для запуска инструментария WMI в отдельном процессе с помощью фиксированного порта. Дополнительные сведения см. [в статье удаленное подключение к WMI, начиная с ОС Vista](connecting-to-wmi-remotely-starting-with-vista.md).
