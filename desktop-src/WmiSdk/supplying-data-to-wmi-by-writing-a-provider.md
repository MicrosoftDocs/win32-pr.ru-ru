...
---
# <a name="supplying-data-to-wmi-by-writing-a-provider"></a>Предоставление данных в WMI путем написания поставщика

Поставщик WMI создает группу поддерживаемых классов, экземпляров и событий для передачи данных в WMI. Приложение управления или сценарий, в свою очередь, могут вызывать [*методы поставщика*](gloss-p.md) для управления данными, предоставляемыми поставщиком.

На следующей схеме показана связь между поставщиком, связанным с WMI и приложением.

![отношение между WMI, связанным поставщиком и приложением](images/coupledprov.png)

В следующей процедуре описывается создание простого поставщика, поддерживающего набор экземпляров. Поставщик, описанный здесь, зарегистрирован для запуска в рамках процесса WMI. В некоторых случаях используйте [*несвязанный поставщик*](gloss-d.md) , который выполняется в другом процессе. Дополнительные сведения о моделях размещения поставщиков см. в разделе [размещение и безопасность поставщика](provider-hosting-and-security.md). Шаги 1 и 2 в следующей процедуре отличаются для несвязанных поставщиков, но в других отношениях они используют те же интерфейсы, что и внутрипроцессный поставщик. Дополнительные сведения см. [в разделе Включение поставщика в приложение](incorporating-a-provider-in-an-application.md).

**Создание поставщика экземпляров**

1.  Создайте экземпляр класса [**\_ \_ Win32Provider**](--win32provider.md) с кодом [MOF-файл (MOF)](managed-object-format--mof-.md) , указав имя и **идентификатор CLSID** поставщика. Дополнительные сведения см. в разделе [Конструирование классов MOF-файл (MOF)](designing-managed-object-format--mof--classes.md).

    В следующем примере кода показано, как создать экземпляр класса [**\_ \_ Win32Provider**](--win32provider.md) .
    ...
    Дополнительные сведения см. в разделе [Создание экземпляра](creating-an-instance.md) и [Регистрация поставщика экземпляра](registering-an-instance-provider.md).

2.  Создайте экземпляр класса [**\_ \_ инстанцепровидеррегистратион**](--instanceproviderregistration.md) , описывающий функции поставщика экземпляров.

    В следующем примере кода показано, как создать экземпляр класса [**\_ \_ инстанцепровидеррегистратион**](--instanceproviderregistration.md) .
    ...
    Дополнительные сведения о свойствах в этом разделе MOF-кода см. в разделе [**\_ \_ инстанцепровидеррегистратион**](--instanceproviderregistration.md) и [**\_ \_ обжектпровидеррегистратион**](--objectproviderregistration.md).

    Дополнительные сведения см. в разделе [Регистрация поставщика экземпляров](registering-an-instance-provider.md).

3.  Используйте MOF-код для создания динамического класса, для которого поставщик предоставляет экземпляры.

    Динамический класс — это класс, экземпляры которого получают обновления от поставщика. Эти обновления могут быть обычными или связаны с постоянными изменениями в объектах, представляемых экземплярами. Вы можете просматривать изменения в экземплярах динамических классов с помощью собственного приложения управления или [обозревателя объектов WMI](further-information.md).

    В следующем примере кода описывается динамический класс, поддерживаемый поставщиком "Инстпровсамп".
...
4.  Зарегистрируйте классы с помощью WMI с помощью компилятора MOF.

    В командной строке в каталоге поставщика введите следующую команду, чтобы зарегистрировать пример MOF-кода с помощью инструментария WMI.

    **mofcomp** *инстпров. mof*

    Дополнительные сведения см. в разделе [Компиляция MOF-файлов](compiling-mof-files.md).

5.  Определите COM-объект, который будет содержать поставщик. Пример кода для этого шага находится в полном примере в конце этого раздела.

    1.  Как и в случае с любым COM-объектом, необходимо реализовать конструктор и деконструктор, а также методы [**QueryInterface**](/windows/win32/api/unknwn/nf-unknwn-iunknown-queryinterface(q)), [**AddRef**](/windows/win32/api/unknwn/nf-unknwn-iunknown-addref)и [**Release**](/windows/win32/api/unknwn/nf-unknwn-iunknown-release) .

    2.  Реализуйте метод [**ивбемпровидеринит:: Initialize**](/windows/desktop/api/Wbemprov/nf-wbemprov-iwbemproviderinit-initialize) в COM-объекте.

        Основной целью [**инициализации**](/windows/desktop/api/Wbemprov/nf-wbemprov-iwbemproviderinit-initialize) в этом примере является присвоение элементу **m \_ пнамеспаце** текущего пространства имен. Дополнительные сведения о пространствах имен см. [в разделе Создание иерархий в WMI](creating-hierarchies-within-wmi.md).

        Приемник передается через параметр *пинитсинк* в методе [**Ивбемпровидеринит:: Initialize**](/windows/desktop/api/Wbemprov/nf-wbemprov-iwbemproviderinit-initialize) , чтобы указать, что поставщик успешно инициализирован. Дополнительные сведения о приемниках см. в разделе [**ивбемобжектсинк**](iwbemobjectsink.md) и [вызов метода](calling-a-method.md).

    3.  Реализуйте метод [**IWbemServices:: креатеинстанцеенумасинк**](/windows/desktop/api/WbemCli/nf-wbemcli-iwbemservices-createinstanceenumasync) в объекте com (хотя можно реализовать различные интерфейсы [**IWbemServices**](/windows/desktop/api/WbemCli/nn-wbemcli-iwbemservices) , пример в конце этого раздела реализует только **креатеинстанцеенумасинк**). В частности, **креатеинстанцеенумасинк** возвращает экземпляры указанных объектов в WMI.

    4.  Реализуйте метод [**жетобжектасинк**](/windows/desktop/api/WbemCli/nf-wbemcli-iwbemservices-getobjectasync) в объекте com.

        В примере в конце этого раздела [**жетобжектасинк**](/windows/desktop/api/WbemCli/nf-wbemcli-iwbemservices-getobjectasync) проверяет параметры управляемого объекта, извлекает указанный объект, выполняет проверку ошибок и возвращает соответствующие коды приемнику.

6.  Зарегистрируйте поставщик в виде COM-объекта с помощью RegSvr32.

    В командной строке в каталоге поставщика введите следующее:

    **regsvr32** *InstProv.dll*

Поставщик теперь готов. На этом этапе у вас должна быть возможность доступа к экземплярам класса **инстпровсамп** из WMI.

В следующем примере кода создается COM-объект, который будет содержать поставщик, как описано в шаге 5 выше. Полный пример содержит код из файла заголовка Sample. h и исходных файлов с именем Инстпров. cpp, utils. cpp, Классфак. cpp и Маиндлл. cpp.
...
Исходный файл Инстпро. cpp содержит код, реализующий функции для поставщика, объявленного в файле заголовка Sample. h.
...
Исходный файл utils. cpp содержит код, реализующий служебные функции для поставщика, объявленного в файле заголовка Sample. h.
...
Исходный файл Классфак. cpp содержит код, создающий объекты при запросе соединений.
...
Исходный файл Маиндлл. cpp содержит код, который определяет, можно ли выгрузить библиотеку DLL путем отслеживания количества объектов и блокировок, а также подпрограмм, поддерживающих самостоятельную регистрацию.
