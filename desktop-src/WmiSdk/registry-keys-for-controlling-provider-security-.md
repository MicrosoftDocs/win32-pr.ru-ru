...
Для повышения безопасности wmiprvse.exe процесса узла общего поставщика инструментарий управления Windows (WMI) (WMI) были внесены изменения в платформы Windows, которые защищают хост-процесс поставщика с помощью [*идентификатора безопасности (SID) службы*](gloss-s.md). Эти изменения посвящены следующим режимам работы для общего узла WMI: безопасный и совместимый.
...
Этот ключ управляет поведением отдельных поставщиков. Все поставщики, перечисленные в этом разделе, всегда выполняются в безопасном режиме. Все входящие в состав Windows поставщики входящих сообщений перечислены в этом разделе и по умолчанию запускаются в безопасном режиме.
