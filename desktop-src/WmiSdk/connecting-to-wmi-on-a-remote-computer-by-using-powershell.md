...
В следующем примере Windows PowerShell показано, как подключиться к удаленному компьютеру с другими учетными данными и установить уровень олицетворения равным 3, что является олицетворением.
...
Следующий пример Windows PowerShell подключается к группе удаленных компьютеров в том же домене, создавая массив имен удаленных компьютеров, а затем отображая имена самонастраивающийся устройств — экземпляры [**Win32 \_ пнпентити**](/windows/desktop/CIMWin32Prov/win32-pnpentity)— на каждом компьютере:
