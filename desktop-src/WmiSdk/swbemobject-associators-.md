...
Метод- **соединители \_** объекта [**SWbemObject**](swbemobject.md) Возвращает коллекцию объектов (классов или экземпляров), связанных с текущим объектом. Эти возвращенные объекты называются конечными точками. Этот метод выполняет ту же функцию, что и СОЕДИНИТЕЛи WQL-запроса.
...
Строка, содержащая имя квалификатора. Этот параметр, если он указан, указывает, что возвращенные конечные точки должны быть связаны с исходным объектом через класс ассоциации, включающий указанный квалификатор.
...
Заставляет WMI возвращать данные о поправности класса с определением базового класса. Включение этого флага делает текст квалификатора локализованного описания доступным для классов, свойств и методов. Дополнительные сведения об измененных квалификаторах см. в разделе [Локализация сведений о классе WMI](localizing-wmi-class-information.md).
...
После завершения метода **сопоставления \_** объект **Err** может содержать один из кодов ошибок из следующего списка.
