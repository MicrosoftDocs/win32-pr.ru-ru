---
title: show sslcert
description: Список привязок сертификатов сервера SSL и соответствующих политик сертификатов клиента для IP-адреса и порта.
ms.assetid: 8e20f55e-a357-4f9c-a24e-e234607c3196
keywords:
- показывать sslcert HTTP
topic_type:
- apiref
api_name:
- show sslcert
api_type:
- NA
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: b71c2faf370066f5ce8d5ce6a20b173a74d0f645
ms.sourcegitcommit: 476861130ea63675206d1f06e517059705b930ed
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 11/06/2019
ms.locfileid: "104335059"
---
# <a name="show-sslcert"></a>show sslcert

Список привязок сертификатов сервера SSL и соответствующих политик сертификатов клиента для IP-адреса и порта.

``` syntax
show sslcert [ipport=]IP Address:port
 
```

## <a name="parameters"></a>Параметры

<dl> <dt>

<span id="_ipport__IP_Address_port"></span><span id="_ipport__ip_address_port"></span><span id="_IPPORT__IP_ADDRESS_PORT"></span>**\[иппорт = \] * * * IP-адрес: порт*
</dt> <dd>

Указывает адрес или порт IPv4 или IPv6, для которого будут отображаться привязки SSL-сертификатов. Не указывая иппорт, перечисляются все привязки.

</dd> </dl>

## <a name="examples"></a>Примеры

**показывать sslcert иппорт = \[ FE80:: 1 \] : 443**

**show sslcert ipport=1.1.1.1:443**

**show sslcert ipport=0.0.0.0:443**

**показывать sslcert иппорт = \[ ::: \] 443**

 

 




