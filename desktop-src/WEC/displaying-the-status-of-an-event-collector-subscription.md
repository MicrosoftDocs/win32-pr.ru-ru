...
Можно отобразить состояние подписки сборщика событий. Состояние включает доступность подписки, последнюю ошибку, возникшую для подписки, время последней ошибки и следующий раз, когда подписка будет повторно выполнена.

> [!Note]
>
> С помощью этого примера можно отобразить состояние подписки или ввести в командной строке следующую команду:
>
...
Ниже приведен пример процедуры для просмотра состояния подписки сборщика событий.
...
5.  Печать сведений о состоянии подписки.
6.  Закройте подписку, вызвав функцию [**екклосе**](/windows/desktop/api/Evcoll/nf-evcoll-ecclose) .

В следующем примере кода C++ показано, как отобразить состояние подписки сборщика событий.
