...
Подписки, инициированные источником, позволяют определить подписку на компьютере сборщика событий, не определяя исходные компьютеры событий, а затем можно настроить несколько удаленных компьютеров источника событий (с помощью параметра групповой политики) для перенаправления событий на компьютер сборщика событий. Прежде чем локальный компьютер сможет подписаться на события, а удаленный компьютер может пересылать события, оба компьютера должны быть настроены для сбора событий и пересылки событий. Дополнительные сведения о настройке компьютеров см. в разделе [Настройка инициированной источником подписки](setting-up-a-source-initiated-subscription.md).

В следующем примере кода используется последовательность действий для создания исходной инициированной подписки, в которой источники событий находятся в том же домене, что и компьютер сборщика событий.
...
В следующем примере C++ показано, как создать исходную подписку.
