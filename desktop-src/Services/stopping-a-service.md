...
---
# <a name="stopping-a-service"></a>Остановка службы

[Программа управления службами](service-control-programs.md) может прерывать работу службы с помощью функции [**ControlService**](/windows/desktop/api/Winsvc/nf-winsvc-controlservice) для отправки \_ запроса на завершение управления службой \_ . Если [Диспетчер управления службами](service-control-manager.md) (SCM) получает \_ запрос на \_ остановку управления службой, он указывает службе на остановку, перенаправляя запрос в функцию [**ServiceMain**](/windows/win32/api/winsvc/nc-winsvc-lpservice_main_functiona) службы. Однако если SCM определяет, что другие работающие службы зависят от указанной службы, запрос на остановку не будет перенаправлен. Вместо этого он возвращает \_ выполняемые службы, зависящие от ошибок \_ \_ . Таким образом, чтобы программно предотвратить такую службу, необходимо сначала перечислить и отключить зависимые от нее службы.

Функция Достопсвк в следующем примере показывает, как можно прерывать службу и зависимые от нее службы. Переменная Сзсвкнаме представляет собой глобальную переменную, которая содержит имя останавливаемой службы. Полный пример, задающий эту переменную, см. в разделе [свкконтрол. cpp](svccontrol-cpp.md).
