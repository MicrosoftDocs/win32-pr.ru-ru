---
description: В следующем примере показано, как кодировать простой обработчик обратного вызова для получения установщик Windows сообщений о ходе установки.
ms.assetid: ae1589ae-0ad7-4314-8bf1-c8ad51eac5a2
title: Обработка сообщений о ходе выполнения с помощью Мсисетекстерналуи
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103910931"
---
# <a name="handling-progress-messages-using-msisetexternalui"></a><span data-ttu-id="772f8-103">Обработка сообщений о ходе выполнения с помощью Мсисетекстерналуи</span><span class="sxs-lookup"><span data-stu-id="772f8-103">Handling Progress Messages Using MsiSetExternalUI</span></span>

<span data-ttu-id="772f8-104">В следующем примере показано, как кодировать простой обработчик обратного вызова для получения установщик Windows сообщений о ходе установки.</span><span class="sxs-lookup"><span data-stu-id="772f8-104">The following sample demonstrates how to code a simple callback handler to receive Windows Installer progress messages during an installation.</span></span>

> [!Note]  
> <span data-ttu-id="772f8-105">При использовании [**мсисетекстерналуи**](/windows/desktop/api/Msi/nf-msi-msisetexternaluia) с типом сообщений инсталлмессаже \_ филесинусе сообщение, отправленное в функцию внешнего обработчика пользовательского интерфейса, не содержит никаких сведений о файлах, используемых или в заголовках окон, которые используются в диалоговом окне " [филесинусе](filesinuse-dialog.md) ".</span><span class="sxs-lookup"><span data-stu-id="772f8-105">When using [**MsiSetExternalUI**](/windows/desktop/api/Msi/nf-msi-msisetexternaluia) with a message type of INSTALLMESSAGE\_FILESINUSE, the message sent to the external UI handler function does not contain any information about files in use or window titles used by the [FilesInUse](filesinuse-dialog.md) dialog box.</span></span> <span data-ttu-id="772f8-106">Для получения сведений следует использовать [**мсисетекстерналуирекорд**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) .</span><span class="sxs-lookup"><span data-stu-id="772f8-106">You should use [**MsiSetExternalUIRecord**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) to obtain information.</span></span>
