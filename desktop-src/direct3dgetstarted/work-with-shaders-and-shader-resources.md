...
Научитесь работать с шейдерами и ресурсами шейдеров в процессе разработки игры Microsoft DirectX для Windows 8. Мы рассмотрели, как настроить графическое устройство и ресурсы, и, возможно, вы даже начали изменять его конвейер. Теперь давайте взглянем на шейдеры пикселей и вершин.
...
-   **b * * *\#* : регистр для буфера констант (** кбуффер * *).
-   **t * * *\#* : регистр для буфера текстуры (** тбуффер * *).
-   **s * * \#**: регистрация для образца. (Образец определяет поведение поиска для пикселей текстуры в ресурсе текстуры.)
