...
До Windows 10 версии 1809 (10,0; Сборка 17763). Правильная идентификация пользователя (удостоверение вспомогательного маркера) используется для обнаружения прокси-сервера на основе сети (WPAD) и проверки подлинности прокси, но фактическое обнаружение локальных параметров прокси-сервера (IE) всегда выполняется с помощью маркера владельца задания, даже если настроен вспомогательный токен. Чтобы обойти этот недостаток, можно выполнить следующие действия.
