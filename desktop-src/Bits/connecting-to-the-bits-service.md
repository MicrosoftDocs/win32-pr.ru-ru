...
Чтобы подключиться к системной службе BITS, создайте экземпляр объекта Баккграундкопиманажер, как показано в следующем примере. Системная служба BITS — это системная служба Windows, выполняемая на клиентском компьютере, в которой реализована возможность передачи в фоновом режиме.
