...
| PSH_PROPTITLE (0x00000001) | Указывает, что *псзкаптион* — это имя объекта, для которого отображаются свойства. Windows делает корректировку заголовка в зависимости от версии и языка. Например, на английском языке фраза "свойства для" добавляется в начало непустого *псзкаптион* (и если *псзкаптион* создает пустой заголовок, заголовок — просто "Properties"). Если этот флаг опущен, то Псзкаптион используется без изменений.  |
