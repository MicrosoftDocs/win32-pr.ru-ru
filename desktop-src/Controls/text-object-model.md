...
TOM определяет значительный набор интерфейсов обработки текста. Текстовые решения, такие как Microsoft Word и элементы управления Rich Edit, поддерживают набор компонентов TOM. На том было сильно влияло на WordBasic (язык программирования, используемый для Word), и его легко использовать в Microsoft Visual Basic для приложений (VBA). Эта совместимость имеет несколько преимуществ.
