...
> [!Note]  
> Начиная с Windows 8, по умолчанию используется быстрый запуск, когда запрашивается завершение работы системы. Полное завершение работы (S5) возникает при запросе перезагрузки системы (или при вызове приложением API завершения работы).
...
> [!Note]  
> Протокол WOL официально не поддерживается при мягком выключении (S5). Однако BIOS в некоторых системах может поддерживать арминг сетевые карты для пробуждения, даже если Windows не участвует в процессе.
