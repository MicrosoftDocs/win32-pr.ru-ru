...
Поведение Windows. Этот Протокол реализован в Windows 2000, Windows XP, Windows Server 2003 и Windows Vista.
...
*Поведение Windows. Обычно приложения взаимодействуют с оболочкой интерфейса OLE DB \[ MSDN-OLEDB \] (например, клиент протокола), а не напрямую с протоколом.*
...
CISP предназначен для запросов и управления каталогами на удаленном сервере от клиента. CISP является устаревшим в Windows Vista.
...
*Поведение Windows. в Windows используются только значения, указанные в разделе 4.1.3 из \[ MS-sys \] .*
...
*Поведение Windows. Клиент всегда задает \_ для поля состояния значение 0x00000000.*
...
*Поведение Windows*. обычно это значение равно нулю, за исключением случаев, когда было начато индексирование или после того, как очередь уведомлений переполняется.
...
*Поведение Windows. в клиентах Windows параметр иклиентверсион устанавливается следующим* образом:
...
| 0x00000008 | Клиентская ОС — 32-разрядная версия Windows XP или 32-разрядная версия Windows Server 2003. |
| 0x00010008 | Клиентская ОС — 64-разрядная версия Windows XP или 64-разрядная версия Windows Server 2003. |
...
**\_ wid**: 32-разрядное целое число без знака, представляющее идентификатор документа, определяющий документ, для которого должно быть выбрано свойство.
...
*Поведение Windows: для этого поля задано значение 0x00004000 для всех версий Windows.*
...
4.  Сервер проверяет правильность **\_ улчекксум** , проверяет, имеет ли пользователь разрешение на выполнение этого запроса, и реагирует на сообщение кпмконнектаут.
    ...
    -   для поля **\_ serverVersion** установлено значение 0x00000007 (32-разрядная версия windows XP или 32-разрядная версия Windows Server 2003).
...
5.  Клиент готовит сообщение Кпмкреатекуерин.
    ...
    -   **Пидмаппер** имеет значение:
        ...
        -   **\_ апропспек** имеет значение GUID B725F130-47EF-101A-A5-F1-02608C9EEBAC/0X00000001 (для прспек \_ PropID)/0x0000000c, которое представляет свойство размера файла Windows.
...
7.  Клиент выдает сообщение запроса Кпмсетбиндингсин для определения формата строки.
    ...
    -   -   **\_ Пропспек** имеет значение GUID b725f130-47ef-101A-A5-F1-02608c9eebac/0X00000001 (для прспек \_ PropID)/0x0000000c, которое представляет свойство размера файла Windows.
        ...
        **\_ паноде \[ 1 \]** содержит:
        ...
        -   -   для **\_ ултипе** задано значение 0x00000004 (ртконтент).
            ...
            -   Оставшаяся часть поля содержит структуру Кконтентрестриктион:
                ...
                -   для **\_ пвксфрасе** задана строка "Office".
                -   для **\_ LCID** задано значение 0X409 (для английского языка).
                -   для **\_ улженератемесод** задано значение 0x00000000 (точное совпадение).
...
|---------------------------------------------------------------------------------------------------|-----------|--------------|------------|---------------------|
| Этот Протокол реализован в Windows 2000, Windows XP, Windows Server 2003 и Windows Vista. | 1.3.2     | X            | X          | X                   |
