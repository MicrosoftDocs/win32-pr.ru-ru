...
ms.locfileid: "119519704"
---
# <a name="gltexsubimage2d-function"></a>Функция glTexSubImage2D
...
Высота подизображения текстуры.
...
Формат точечных данных. Может принимать одно из следующих символьных значений.
...
| <span id="GL_RGB"></span><span id="gl_rgb"></span><dl> <dt>**RGB в формате GL \_**</dt> </dl>                                      | Каждый элемент — это тройной RGB. Он преобразуется в формат с плавающей запятой и собираются в элемент RGBA путем присоединения 1,0 для альфа. Затем каждый компонент умножается на коэффициент масштабирования со знаком «GL \_ c \_ », добавленный к смещению «смещение на языке c» со знаком, а также в \_ \_ диапазон \[ 0, 1 \] (см. **глпикселтрансфер**).<br/>                                                                                                                                                                                                                                     |
...
| Имя                                                                                                  | Значение                                                                                                                                                                                                                                                                                                                                                                                                      |
|-------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <dl> <dt>**\_недействительное \_ перечисление GL**</dt> </dl>      | *целевой объект* не является \_ плоской текстурой GL \_ .<br/>                                                                                                                                                                                                                                                                                                                                                                 |
...
| <dl> <dt>**\_недопустимое \_ значение GL**</dt> </dl>     | *ксоффсет* был меньше-*b*; или *ксоффсет*  +  *Ширина* больше *w*  -  *b*; или *йоффсет* была меньше-*b*; или   +  *Высота* йоффсет была больше *h*  -  *b*, где *w* — Ширина текстуры GL, \_ \_ *h* — \_ Высота текстуры GL \_ , а *b* — ширина \_ границы текстуры текстуры \_ для изменяемого изображения текстуры. <br/> Обратите внимание, что *w* и *h* имеют вдвое больше ширины границы.<br/> |
...
| <dl> <dt>**\_Недопустимая \_ Операция GL**</dt> </dl> | Массив текстуры не был определен предыдущей операцией **glTexImage2D** .<br/>                                                                                                                                                                                                                                                                                                                       |
...
Двухмерный текстурирование для примитива включается с помощью [**гленабле**](glenable.md) и **глдисабле** с аргументом GL \_ текстуры \_ 2D. Во время текстурирования часть указанного изображения текстуры сопоставляется с каждым включенным примитивом. Используйте функцию **glTexSubImage2D** для указания непрерывного подизображения существующего двухмерного изображения текстуры для текстурирования.

Пикселей текстуры, на который ссылается *точка* , заменяет регион существующего массива текстуры на *x* индексов *ксоффсет* и *ксоффсет* + (*Width* 1) включительно и *y* индексов *йоффсет* и *йоффсет* + (*Высота* 1) включительно. Этот регион не может включать пикселей текстуры за пределами диапазона изначально заданного массива текстуры.
...
Следующие функции извлекают сведения, относящиеся к **glTexSubImage2D**:
