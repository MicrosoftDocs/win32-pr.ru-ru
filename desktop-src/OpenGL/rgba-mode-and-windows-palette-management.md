...
Режим RGBA использует значения цвета красного, зеленого и синего (R, G и B), которые вместе определяют цвет каждого пикселя в дисплее. Значения R, G и B определяют интенсивность каждого цвета (красный, зеленый и синий); диапазон значений — от 0,0 (наименьшее значение) до 1,0 (самый сильный). Количество бит для каждого компонента зависит от используемого оборудования (2, 3, 5, 6 и 8 бит). Отображаемый цвет является результатом суммы трех значений цвета. Если все три значения равны 0,0, результат будет черным. Если все три значения равны 1,0, результат будет белым. Другие цвета являются результатом сочетания значений R, G и B, которые находятся в диапазоне от 0 до 1,0. Бит A (альфа) не используется для указания цвета.

Стандартный дисплей Super-VGA использует палитры с восемью цветными битами на пиксель. Восемь бит считываются из буфера и используются в качестве индекса в системной палитре для получения значений R, G и B. Если палитра RGB выбрана и реализована в контексте устройства, OpenGL может быть отображен с использованием режима RGBA.

Так как в каждом пикселе имеется восемь цветовых битов, OpenGL подчеркивает использование палитры типа "три-три-две". "Три-три-два" — это способ обработки цветовых битов данных оборудованием или физической палитрой. Красный (R) и зеленый (G) каждый, заданный тремя битами; синий (B) задается двумя битами. Красный — это наименьший значащий бит, а синий — самый значащий бит.

Вы определяете цвета логической палитры приложения с помощью структур [палеттинтри](/previous-versions//dd162769(v=vs.85)) . Обычно создается массив структур **палеттинтри** для указания всей таблицы ввода палитры логической палитры.

## <a name="rgba-mode-palette-sample"></a>Образец палитры для режима RGBA

В следующем фрагменте кода показано, как создать тройную палитру RGBA.
