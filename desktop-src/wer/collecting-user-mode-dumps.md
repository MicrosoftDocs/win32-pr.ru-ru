...
Эта функция отключена по умолчанию. Для включения функции требуются права администратора. Чтобы включить и настроить функцию, используйте следующие значения реестра в разделе **\_ \_ программное обеспечение для локального компьютера HKEY \\ \\ Microsoft \\ Windows \\ отчеты об ошибках Windows \\ локалдумпс** Key.
