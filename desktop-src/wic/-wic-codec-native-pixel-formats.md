...
В этом разделе описываются форматы пикселей, предоставляемые компонентом Windows Imaging Component (WIC).
...
**\[GUID \_ викпикселформат \] \[ бит на пиксельный \] \[ \] \[ тип хранилища для порядка каналов\]**
...
-   **Тип хранилища**. Числовая кодировка данных изображения. Это значение может быть целым числом без знака (UINT), числом с фиксированной запятой (Фикседпоинт) или числом с плавающей запятой (float).
...
Каждый из поставляемых в Windows кодеков WIC поддерживает подмножество форматов пикселей WIC. Для каждого кодека Поддерживаемые форматы декодирования могут отличаться от поддерживаемых форматов кодирования.
...
В следующих таблицах показаны форматы пикселей, поддерживаемые каждым из кодеков WIC, предоставляемых Windows.
...
> [!Note]  
> GUID \_ WICPixelFormat32bppBGRA поддерживается только в Windows 8, в [обновлении платформы для Windows 7](/windows/desktop/direct3darticles/platform-update-for-windows-7)и более поздних версиях.
>
...
> [!Note]  
> GUID \_ WICPixelFormat96bppRGBFloat поддерживается только в Windows 8, в [обновлении платформы для Windows 7](/windows/desktop/direct3darticles/platform-update-for-windows-7)и более поздних версиях.
...
> [!Note]  
> Кодек Windows, предоставленный DDS, поддерживает файлы DDS, закодированные с помощью следующих \_ значений формата DXGI:
