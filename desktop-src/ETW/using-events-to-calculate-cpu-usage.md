---
description: В следующем примере показано, как использовать события для вычисления загрузки ЦП для набора инструкций. В этом примере предполагается, что поставщик создает оболочку для набора инструкций с типом события Start и типом завершающего события.
ms.assetid: 7bc15ed3-d299-40dd-9838-520f896921e3
title: Использование событий для вычисления загрузки ЦП
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104984553"
---
# <a name="using-events-to-calculate-cpu-usage"></a><span data-ttu-id="cab7c-104">Использование событий для вычисления загрузки ЦП</span><span class="sxs-lookup"><span data-stu-id="cab7c-104">Using Events to Calculate CPU Usage</span></span>

<span data-ttu-id="cab7c-105">В следующем примере показано, как использовать события для вычисления загрузки ЦП для набора инструкций.</span><span class="sxs-lookup"><span data-stu-id="cab7c-105">The following example shows you how to use events to calculate the CPU usage for a set of instructions.</span></span> <span data-ttu-id="cab7c-106">В этом примере предполагается, что поставщик создает оболочку для набора инструкций с типом события Start и типом завершающего события.</span><span class="sxs-lookup"><span data-stu-id="cab7c-106">This example assumes the provider wraps the instruction set with a start event type and an end event type.</span></span>
