...
| **string**             | **Формат**, **стрингтерминатион** | Объявляет строковое значение. Чтобы указать, что строка является строкой расширенных символов, используйте квалификатор **формата** и задайте для его значения значение w. Строка считается строкой ANSI, если не указан квалификатор **формата** . Чтобы указать, как завершается строка, используйте квалификатор **стрингтерминатион** . <br/> |



 

Чтобы указать массив, можно использовать квадратные скобки, \[ \] . Квадратные скобки могут включать размер массива. Например:

`[WmiDataId(1), read] uint8 MyGuid[16];`

Квалификатор **Max** также можно использовать для указания размера массива. Например:
