...
ms.locfileid: "119582264"
---
# <a name="retrieving-event-data-using-mof"></a>Получение данных о событиях с помощью MOF

Чтобы использовать данные событий, потребитель должен иметь представление о формате данных события. Если поставщик использовал MOF для публикации формата данных события, можно использовать класс MOF для анализа данных события. Все события ядра используют MOF для публикации формата данных события. Сведения о публикации событий см. в статье [Публикация схемы событий](publishing-your-event-schema.md).

для анализа данных события требуется использовать API инфраструктуры управления Windows (WMI). Пространство имен ETW, где поставщики публикуют свой класс MOF, — это root \\ WMI. Пространство имен ETW содержит три типа классов MOF: класс MOF поставщика, класс MOF события и класс MOF типа события. Класс событий MOF логически группирует один или несколько классов MOF типа событий. Класс MOF типа события определяет фактические данные события.

Класс MOF события содержит квалификатор класса **GUID** , значение которого должно совпадать со значением в элементе **Header. GUID** структуры [**\_ трассировки событий**](/windows/win32/api/evntrace/ns-evntrace-event_trace) . Чтобы обеспечить правильную версию класса, также Сравните квалификатор класса **евентверсион** с элементом **Header. class. Version** структуры **\_ трассировки событий** .

После поиска правильного класса событий перечислите его дочерние классы типов событий, чтобы найти класс, который содержит формат данных события. Класс правильного типа событий содержит квалификатор класса **EventType** , значение которого соответствует значению элемента **Header. class. Type** структуры [**\_ трассировки событий**](/windows/win32/api/evntrace/ns-evntrace-event_trace) .

Затем можно использовать API инструментария WMI для перечисления свойств класса MOF. Используйте квалификаторы и тип данных каждого свойства, чтобы определить размер элемента данных в данных события для чтения и способ их форматирования. Список квалификаторов MOF, поддерживаемых ETW, см. в разделе [Квалификаторы MOF трассировки событий](event-tracing-mof-qualifiers.md).

Поскольку ETW не выполняет принудительное выравнивание значений данных событий, приведение или присваивание значения непосредственно из буфера может привести к сбою выравнивания. не следует создавать структуру из класса MOF и пытаться использовать ее для использования данных события. Например, если имеется символ, за которым следует УЛОНГЛОНГ, УЛОНГЛОНГ не будет выровнен по 8-байтной границе, поэтому присваивание вызовет исключение выравнивания. (На 64-разрядных компьютерах это происходит чаще.) По этой причине следует использовать Копимемори для копирования данных из буфера в локальную переменную. Кроме того, при последующем исправлении события потребитель может не работать, если вы попытаетесь использовать структуру.

начиная с Windows Vista, рекомендуется использовать функции модуля поддержки данных трассировки (TDH) для использования событий, опубликованных с помощью классов MOF. Дополнительные сведения см. в разделе [Получение данных о событиях с помощью TDH](retrieving-event-data-using-tdh.md).

В следующем примере показано, как использовать события, определенные классом MOF.
