...
---
# <a name="retrieving-event-data-using-mof"></a><span data-ttu-id="2cf38-103">Получение данных о событиях с помощью MOF</span><span class="sxs-lookup"><span data-stu-id="2cf38-103">Retrieving Event Data Using MOF</span></span>

<span data-ttu-id="2cf38-104">Чтобы использовать данные событий, потребитель должен иметь представление о формате данных события.</span><span class="sxs-lookup"><span data-stu-id="2cf38-104">To consume event specific data, the consumer must know the format of the event data.</span></span> <span data-ttu-id="2cf38-105">Если поставщик использовал MOF для публикации формата данных события, можно использовать класс MOF для анализа данных события.</span><span class="sxs-lookup"><span data-stu-id="2cf38-105">If the provider used MOF to publish the format of the event data, you can use the MOF class to parse the event data.</span></span> <span data-ttu-id="2cf38-106">Все события ядра используют MOF для публикации формата данных события.</span><span class="sxs-lookup"><span data-stu-id="2cf38-106">All the kernel events use MOF to publish the format of the event data.</span></span> <span data-ttu-id="2cf38-107">Сведения о публикации событий см. в статье [Публикация схемы событий](publishing-your-event-schema.md).</span><span class="sxs-lookup"><span data-stu-id="2cf38-107">For information on publishing events, see [Publishing Your Event Schema](publishing-your-event-schema.md).</span></span>

<span data-ttu-id="2cf38-108">Анализ данных событий требует использования API инфраструктуры управления Windows (WMI).</span><span class="sxs-lookup"><span data-stu-id="2cf38-108">Parsing the event data requires using the Windows Management Infrastructure (WMI) API.</span></span> <span data-ttu-id="2cf38-109">Пространство имен ETW, где поставщики публикуют свой класс MOF, — это root \\ WMI.</span><span class="sxs-lookup"><span data-stu-id="2cf38-109">The ETW namespace where providers publish their MOF class is root\\wmi.</span></span> <span data-ttu-id="2cf38-110">Пространство имен ETW содержит три типа классов MOF: класс MOF поставщика, класс MOF события и класс MOF типа события.</span><span class="sxs-lookup"><span data-stu-id="2cf38-110">The ETW namespace contains three types of MOF classes: the provider MOF class, the event MOF class, and the event type MOF class.</span></span> <span data-ttu-id="2cf38-111">Класс событий MOF логически группирует один или несколько классов MOF типа событий.</span><span class="sxs-lookup"><span data-stu-id="2cf38-111">The event MOF class logically groups one or more event type MOF classes.</span></span> <span data-ttu-id="2cf38-112">Класс MOF типа события определяет фактические данные события.</span><span class="sxs-lookup"><span data-stu-id="2cf38-112">The event type MOF class defines the actual event data.</span></span>

<span data-ttu-id="2cf38-113">Класс MOF события содержит квалификатор класса **GUID** , значение которого должно совпадать со значением в элементе **Header. GUID** структуры [**\_ трассировки событий**](/windows/win32/api/evntrace/ns-evntrace-event_trace) .</span><span class="sxs-lookup"><span data-stu-id="2cf38-113">An event MOF class contains a **Guid** class qualifier whose value must match the value in the **Header.Guid** member of the [**EVENT\_TRACE**](/windows/win32/api/evntrace/ns-evntrace-event_trace) structure.</span></span> <span data-ttu-id="2cf38-114">Чтобы обеспечить правильную версию класса, также Сравните квалификатор класса **евентверсион** с элементом **Header. class. Version** структуры **\_ трассировки событий** .</span><span class="sxs-lookup"><span data-stu-id="2cf38-114">To ensure you have the correct version of the class, also compare the **EventVersion** class qualifier to the **Header.Class.Version** member of the **EVENT\_TRACE** structure.</span></span>

<span data-ttu-id="2cf38-115">После поиска правильного класса событий перечислите его дочерние классы типов событий, чтобы найти класс, который содержит формат данных события.</span><span class="sxs-lookup"><span data-stu-id="2cf38-115">After you find the correct event class, enumerate its child event type classes to find the class that contains the format of the event data.</span></span> <span data-ttu-id="2cf38-116">Класс правильного типа событий содержит квалификатор класса **EventType** , значение которого соответствует значению элемента **Header. class. Type** структуры [**\_ трассировки событий**](/windows/win32/api/evntrace/ns-evntrace-event_trace) .</span><span class="sxs-lookup"><span data-stu-id="2cf38-116">The correct event type class contains an **EventType** class qualifier whose value matches the value in the **Header.Class.Type** member of the [**EVENT\_TRACE**](/windows/win32/api/evntrace/ns-evntrace-event_trace) structure.</span></span>

<span data-ttu-id="2cf38-117">Затем можно использовать API инструментария WMI для перечисления свойств класса MOF.</span><span class="sxs-lookup"><span data-stu-id="2cf38-117">You can then use the WMI API to enumerate the properties of the MOF class.</span></span> <span data-ttu-id="2cf38-118">Используйте квалификаторы и тип данных каждого свойства, чтобы определить размер элемента данных в данных события для чтения и способ их форматирования.</span><span class="sxs-lookup"><span data-stu-id="2cf38-118">Use the qualifiers and data type of each property to determine the size of the data element in the event data to read and how to format it.</span></span> <span data-ttu-id="2cf38-119">Список квалификаторов MOF, поддерживаемых ETW, см. в разделе [Квалификаторы MOF трассировки событий](event-tracing-mof-qualifiers.md).</span><span class="sxs-lookup"><span data-stu-id="2cf38-119">For a list of MOF qualifiers that ETW supports, see [Event Tracing MOF Qualifiers](event-tracing-mof-qualifiers.md).</span></span>

<span data-ttu-id="2cf38-120">Поскольку ETW не выполняет принудительное выравнивание значений данных событий, приведение или присваивание значения непосредственно из буфера может привести к сбою выравнивания. не следует создавать структуру из класса MOF и пытаться использовать ее для использования данных события.</span><span class="sxs-lookup"><span data-stu-id="2cf38-120">Because ETW does not force an alignment between event data values, typecasting or assigning the value directly from a buffer may cause an alignment fault; you should not create a structure from the MOF class and try to use it to consume event data.</span></span> <span data-ttu-id="2cf38-121">Например, если имеется символ, за которым следует УЛОНГЛОНГ, УЛОНГЛОНГ не будет выровнен по 8-байтной границе, поэтому присваивание вызовет исключение выравнивания.</span><span class="sxs-lookup"><span data-stu-id="2cf38-121">For example, if you have a character followed by ULONGLONG, the ULONGLONG would not be aligned to an 8-byte boundary, so an assignment would cause an alignment exception.</span></span> <span data-ttu-id="2cf38-122">(На 64-разрядных компьютерах это происходит чаще.) По этой причине следует использовать Копимемори для копирования данных из буфера в локальную переменную.</span><span class="sxs-lookup"><span data-stu-id="2cf38-122">(On 64-bit computers, this happens more often.) For this reason, you should use CopyMemory to copy the data from the buffer to a local variable.</span></span> <span data-ttu-id="2cf38-123">Кроме того, при последующем исправлении события потребитель может не работать, если вы попытаетесь использовать структуру.</span><span class="sxs-lookup"><span data-stu-id="2cf38-123">Also, if the event is later revised, your consumer may not work if you try to use a structure.</span></span>

<span data-ttu-id="2cf38-124">Начиная с Windows Vista, для использования событий, опубликованных с помощью классов MOF, рекомендуется использовать функции TDH данных трассировки.</span><span class="sxs-lookup"><span data-stu-id="2cf38-124">Beginning with Windows Vista, you are encouraged to use the trace data helper (TDH) functions to consume events that were published using MOF classes.</span></span> <span data-ttu-id="2cf38-125">Дополнительные сведения см. в разделе [Получение данных о событиях с помощью TDH](retrieving-event-data-using-tdh.md).</span><span class="sxs-lookup"><span data-stu-id="2cf38-125">For details, see [Retrieving Event Data Using TDH](retrieving-event-data-using-tdh.md).</span></span>

<span data-ttu-id="2cf38-126">В следующем примере показано, как использовать события, определенные классом MOF.</span><span class="sxs-lookup"><span data-stu-id="2cf38-126">The following example shows how to consume events that are defined by a MOF class.</span></span>
