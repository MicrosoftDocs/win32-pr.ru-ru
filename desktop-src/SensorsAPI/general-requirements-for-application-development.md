...
Чтобы создать приложение API датчика, необходимо установить Windows 7 и пакет средств разработки программного обеспечения (SDK) Windows 7 на компьютере. В следующей таблице описаны конкретные файлы, которые потребуются.
...
| Портабледевицегуидс. lib | Статическая библиотека, содержащая определения **GUID** для объектов переносимых устройств Windows.                   |
...
Приложения API датчика будут работать во всех выпусках Windows 7, за исключением выпуска Windows 7 Starter Edition.

## <a name="windows-portable-devices-interfaces"></a>Интерфейсы переносных устройств Windows

API датчика использует определенные объекты переносных устройств Windows (WPD) для инкапсуляции ключей и значений свойств. В следующей таблице описаны интерфейсы для этих объектов.
