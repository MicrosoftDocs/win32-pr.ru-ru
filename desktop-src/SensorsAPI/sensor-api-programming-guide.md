...
В примере кода в этом разделе используются следующие константы для категорий датчиков, типов и полей данных. Эти константы представляют собой пользовательские значения, определяемые образцом драйвера Тимесенсор в комплекте драйверов Windows. Обратите внимание, что, хотя платформа датчика позволяет определять и использовать пользовательские типы, такие как, следует использовать типы, определяемые платформой, когда это возможно.
