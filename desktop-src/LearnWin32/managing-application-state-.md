...
Последний параметр [**CreateWindowEx**](/windows/desktop/api/winuser/nf-winuser-createwindowexa) является указателем типа **void \***. В этом параметре можно передать любое значение указателя. Когда процедура окна обрабатывает сообщение [**WM \_ Нккреате**](/windows/desktop/winmsg/wm-nccreate) или [**WM \_ CREATE**](/windows/desktop/winmsg/wm-create) , оно может извлекать это значение из данных сообщения.
...
Многие из существующих платформ программирования Windows, таких как Microsoft Foundation Classes (MFC) и библиотека ATL, используют подходы, похожие на показанные здесь. Конечно, полностью обобщенная платформа, такая как MFC, сложнее, чем этот сравнительно простой пример.
