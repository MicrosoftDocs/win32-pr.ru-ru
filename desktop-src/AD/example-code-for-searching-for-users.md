---
title: Пример кода для поиска пользователей
description: В следующем примере кода осуществляется поиск пользователей в домене учетной записи пользователя, под которой выполняется вызывающий процесс. Пример C++ Ексамплевисуал Basic
...
---
# <a name="example-code-for-searching-for-users"></a><span data-ttu-id="5f171-104">Пример кода для поиска пользователей</span><span class="sxs-lookup"><span data-stu-id="5f171-104">Example Code for Searching for Users</span></span>

<span data-ttu-id="5f171-105">В следующем примере кода осуществляется поиск пользователей в домене учетной записи пользователя, под которой выполняется вызывающий процесс.</span><span class="sxs-lookup"><span data-stu-id="5f171-105">The following code examples search for users in the domain of the user account under which the calling process is running.</span></span>

-   [<span data-ttu-id="5f171-106">Пример C++</span><span class="sxs-lookup"><span data-stu-id="5f171-106">C++ Example</span></span>](#c-example)
-   [<span data-ttu-id="5f171-107">Пример Visual Basic</span><span class="sxs-lookup"><span data-stu-id="5f171-107">Visual Basic Example</span></span>](#visual-basic-example)

## <a name="c-example"></a><span data-ttu-id="5f171-108">Пример C++</span><span class="sxs-lookup"><span data-stu-id="5f171-108">C++ Example</span></span>

<span data-ttu-id="5f171-109">В следующем примере кода C++ выполняется поиск всех пользователей в текущем домене или определенного пользователя на основе указанного фильтра.</span><span class="sxs-lookup"><span data-stu-id="5f171-109">The following C++ code example searches the current domain for all users, or a specific user based on a specified filter.</span></span>
...
## <a name="visual-basic-example"></a><span data-ttu-id="5f171-110">Пример Visual Basic</span><span class="sxs-lookup"><span data-stu-id="5f171-110">Visual Basic Example</span></span>

<span data-ttu-id="5f171-111">В следующем примере кода Visual Basic выполняется поиск пользователей с указанной фамилией в текущем домене и возвращение атрибутов Name и [**distinguishedName**](/windows/desktop/ADSchema/a-distinguishedname) для найденных объектов.</span><span class="sxs-lookup"><span data-stu-id="5f171-111">The following Visual Basic code example searches the current domain for users with the specified surname and returns the name and [**distinguishedName**](/windows/desktop/ADSchema/a-distinguishedname) attributes for the objects that are found.</span></span> <span data-ttu-id="5f171-112">В следующем примере кода для выполнения поиска используется ADO.</span><span class="sxs-lookup"><span data-stu-id="5f171-112">The following code example uses ADO to perform the search.</span></span>
