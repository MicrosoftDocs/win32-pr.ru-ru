...
ms.locfileid: "118186329"
---
# <a name="modifying-the-defaultsecuritydescriptor-for-an-object-class"></a>Изменение Дефаултсекуритидескриптор для класса Object

В следующем примере кода извлекается дескриптор безопасности по умолчанию для класса объекта, добавляется ACE в список DACL, а затем задается измененный дескриптор безопасности для класса объекта.

имейте в виду, что изменение схемы по умолчанию отключено на всех контроллерах домена Windows 2000. Чтобы включить изменение схемы на определенном контроллере домена, установите \_ значение reg DWORD с именем "обновление схемы разрешено" в следующем разделе реестра:

**HKey \_ \_** \\ Параметры NTDS локальных **системных систем** \\ **CurrentControlSet** \\ **Services** \\  \\ 

Добавьте это значение, если оно еще не существует. Задайте для этого параметра значение 1, чтобы включить изменение схемы. Если это значение равно нулю, изменение схемы отключено. Оснастка MMC "Диспетчер схемы" предоставляет флажок, который позволяет выбрать или очистить этот раздел реестра.
