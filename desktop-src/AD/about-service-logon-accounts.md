...
Учетная запись входа определяет удостоверение безопасности службы во время выполнения, то есть основной контекст безопасности службы. Контекст безопасности определяет способность службы к доступу к локальным и сетевым ресурсам. Например, служба, работающая в контексте безопасности локальной учетной записи пользователя, не может получить доступ к сетевым ресурсам. и наоборот, служба, работающая в контексте безопасности учетной записи LocalSystem на контроллере домена Windows 2000 (dc), будет иметь неограниченный доступ к dc. Дополнительные сведения и описание преимуществ и ограничений учетных записей пользователей и LocalSystem см. в разделе [контексты безопасности и службы домен Active Directory](security-contexts-and-active-directory-domain-services.md).
