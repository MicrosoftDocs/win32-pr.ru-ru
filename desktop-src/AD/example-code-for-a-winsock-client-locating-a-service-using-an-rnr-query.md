...
---
# <a name="example-code-for-locating-a-service-using-an-rnr-query"></a><span data-ttu-id="ccb2f-106">Пример кода для поиска службы с помощью запроса РНР</span><span class="sxs-lookup"><span data-stu-id="ccb2f-106">Example Code for Locating a Service Using an RnR Query</span></span>

<span data-ttu-id="ccb2f-107">Следующий пример кода находит пример службы Winsock и подключается к ней.</span><span class="sxs-lookup"><span data-stu-id="ccb2f-107">The following code example locates the example Winsock service and connects to it.</span></span>
