...
Поскольку удаленные объекты имеют большинство атрибутов, удаляемых при их удалении, невозможно выполнить привязку непосредственно к удаленному объекту. При привязке к удаленному объекту необходимо указать параметр **\_ быстрой \_ привязки ADS** . Это означает, что интерфейсы ADSI, используемые для работы с объектом домен Active Directory Services, например [**iAds**](/windows/desktop/api/iads/nn-iads-iads) и [**иадспропертилист**](/windows/desktop/api/iads/nn-iads-iadspropertylist), не могут использоваться в контейнере удаленных объектов.
