...
---
# <a name="reading-an-objects-security-descriptor"></a>Считывание дескриптора безопасности объекта

В следующем примере кода используются интерфейсы [**iAds**](/windows/desktop/api/iads/nn-iads-iads) для перечисления свойств дескриптора безопасности объекта каталога, списка DACL и ACE списка DACL.

В следующем примере кода метод [**iAds. Get**](/windows/desktop/api/iads/nf-iads-iads-get) используется для получения свойства **нтсекуритидескриптор** объекта каталога. Версия C++ этого метода возвращает **значение типа Variant** , содержащее указатель [**IDispatch**](/windows/win32/api/oaidl/nn-oaidl-idispatch) . Затем в примере кода метод **QueryInterface** вызывается для этого указателя **IDispatch** , чтобы получить интерфейс [**иадссекуритидескриптор**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) для дескриптора безопасности объекта.

Затем в примере кода используются методы [**иадссекуритидескриптор**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) для получения данных из дескриптора безопасности. Имейте в виду, что данные, доступные через **иадссекуритидескриптор**, зависят от прав доступа вызывающей стороны. Свойства [**иадссекуритидескриптор. дискретионарякл**](/windows/desktop/ADSI/iadssecuritydescriptor-property-methods) и **иадссекуритидескриптор. Owner** завершатся ошибкой, если вызывающий объект не имеет **\_ доступа на чтение** для объекта. Аналогичным образом вызов метода **Get \_ системакл** завершится ошибкой, если для вызывающего объекта не включено право на **\_ \_ имя безопасности SE** .
