...
ms.locfileid: "118189697"
---
# <a name="example-code-to-retrieve-changes-using-usnchanged"></a>Пример кода для получения изменений с помощью USNChanged

В следующем примере кода используется атрибут **uSNChanged** объекта в службах домен Active Directory Services для получения изменений с момента предыдущего запроса. В примере кода можно выполнить полную или добавочную синхронизацию. Для полной синхронизации пример приложения подключается к rootDSE контроллера домена и считывает следующие параметры, которые он хранит для использования при следующей добавочной синхронизации:

-   DNS-имя контроллера домена. Добавочные синхронизации должны выполняться на том же контроллере домена, что и Предыдущая синхронизация.
-   Идентификатор GUID **invocationID** контроллера домена. В примере кода это значение используется для обнаружения того, что контроллер домена был восстановлен из резервной копии. в этом случае пример должен выполнить полную синхронизацию.
-   **Хигхесткоммиттедусн**. Это значение станет нижней границей для фильтра **uSNChanged** при следующей добавочной синхронизации.

В примере кода используется интерфейс [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) , указывающий различающееся имя базы для поиска, область поиска и фильтр. Ограничения для базы или области поиска отсутствуют. Помимо указания интересующих объектов, в фильтре также должно быть указано сравнение **uSNChanged** , например "uSNChanged >= <lower bound USN> ". Для полной синхронизации " &lt; Нижний предел USN &gt; " равен нулю. Для добавочной синхронизации это 1 плюс значение **хигхесткоммиттедусн** из предыдущего поиска.

Имейте в виду, что этот пример приложения предназначен только для того, чтобы продемонстрировать, как использовать **uSNChanged** для получения изменений с сервера Active Directory. Он выводит изменения и не выполняет фактическую синхронизацию данных во вторичном хранилище. Следовательно, он не показывает, как выполнять такие проблемы, как перемещенные объекты или условия "нет родителя". Он показывает, как извлекать удаленные объекты, но не показывает, как приложение использует **objectGUID** удаленных объектов, чтобы определить соответствующий объект для удаления в хранилище.

Кроме того, пример кэширует имя контроллера домена, **invocationID** и **хигхесткоммиттедусн** в реестре. В реальном приложении синхронизации параметры необходимо хранить в том же хранилище, которое соответствует серверу Active Directory. Это гарантирует, что параметры и данные объекта будут синхронизированы, если база данных когда-либо восстанавливается из резервной копии.
