---
title: Пример кода для получения изменений с помощью USNChanged
description: В следующем примере кода используется атрибут uSNChanged объекта в службах домен Active Directory Services для получения изменений с момента предыдущего запроса.
ms.assetid: 519fd20f-9eb4-4702-9338-8cda899604bd
ms.tgt_platform: multiple
keywords:
- Active Directory примеры Active Directory, получение изменений с помощью USNChanged
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104336804"
---
# <a name="example-code-to-retrieve-changes-using-usnchanged"></a><span data-ttu-id="4408c-104">Пример кода для получения изменений с помощью USNChanged</span><span class="sxs-lookup"><span data-stu-id="4408c-104">Example Code to Retrieve Changes Using USNChanged</span></span>

<span data-ttu-id="4408c-105">В следующем примере кода используется атрибут **uSNChanged** объекта в службах домен Active Directory Services для получения изменений с момента предыдущего запроса.</span><span class="sxs-lookup"><span data-stu-id="4408c-105">The following code example uses the **uSNChanged** attribute of an object in Active Directory Domain Services to retrieve changes since a previous query.</span></span> <span data-ttu-id="4408c-106">В примере кода можно выполнить полную или добавочную синхронизацию.</span><span class="sxs-lookup"><span data-stu-id="4408c-106">The code example can perform either a full synchronization or an incremental update.</span></span> <span data-ttu-id="4408c-107">Для полной синхронизации пример приложения подключается к rootDSE контроллера домена и считывает следующие параметры, которые он хранит для использования при следующей добавочной синхронизации:</span><span class="sxs-lookup"><span data-stu-id="4408c-107">For a full synchronization, the sample application connects to the rootDSE of a domain controller and reads the following parameters that it stores to be used in the next incremental synchronization:</span></span>

-   <span data-ttu-id="4408c-108">DNS-имя контроллера домена.</span><span class="sxs-lookup"><span data-stu-id="4408c-108">The DNS name of the DC.</span></span> <span data-ttu-id="4408c-109">Добавочные синхронизации должны выполняться на том же контроллере домена, что и Предыдущая синхронизация.</span><span class="sxs-lookup"><span data-stu-id="4408c-109">Incremental synchronizations must be performed on the same DC as the previous synchronization.</span></span>
-   <span data-ttu-id="4408c-110">Идентификатор GUID **invocationID** контроллера домена.</span><span class="sxs-lookup"><span data-stu-id="4408c-110">The **invocationID** GUID of the DC.</span></span> <span data-ttu-id="4408c-111">В примере кода это значение используется для обнаружения того, что контроллер домена был восстановлен из резервной копии. в этом случае пример должен выполнить полную синхронизацию.</span><span class="sxs-lookup"><span data-stu-id="4408c-111">The code example uses this value to detect that the DC has been restored from a backup, in which case, the sample must perform a full synchronization.</span></span>
-   <span data-ttu-id="4408c-112">**Хигхесткоммиттедусн**.</span><span class="sxs-lookup"><span data-stu-id="4408c-112">The **highestCommittedUSN**.</span></span> <span data-ttu-id="4408c-113">Это значение станет нижней границей для фильтра **uSNChanged** при следующей добавочной синхронизации.</span><span class="sxs-lookup"><span data-stu-id="4408c-113">This value becomes the lower bound for the **uSNChanged** filter on the next incremental synchronization.</span></span>

<span data-ttu-id="4408c-114">В примере кода используется интерфейс [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) , указывающий различающееся имя базы для поиска, область поиска и фильтр.</span><span class="sxs-lookup"><span data-stu-id="4408c-114">The code example uses the [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) interface, specifying the distinguished name of the base of the search, a search scope, and a filter.</span></span> <span data-ttu-id="4408c-115">Ограничения для базы или области поиска отсутствуют.</span><span class="sxs-lookup"><span data-stu-id="4408c-115">There are no restrictions on the search base or scope.</span></span> <span data-ttu-id="4408c-116">Помимо указания интересующих объектов, в фильтре также должно быть указано сравнение **uSNChanged** , например "uSNChanged >= <lower bound USN> ".</span><span class="sxs-lookup"><span data-stu-id="4408c-116">In addition to specifying the objects of interest, the filter must also specify a **uSNChanged** comparison, such as "uSNChanged >= <lower bound USN>".</span></span> <span data-ttu-id="4408c-117">Для полной синхронизации " &lt; Нижний предел USN &gt; " равен нулю.</span><span class="sxs-lookup"><span data-stu-id="4408c-117">For a full synchronization, "&lt;lower bound USN&gt;" is zero.</span></span> <span data-ttu-id="4408c-118">Для добавочной синхронизации это 1 плюс значение **хигхесткоммиттедусн** из предыдущего поиска.</span><span class="sxs-lookup"><span data-stu-id="4408c-118">For an incremental synchronization, it is the 1 plus the **highestCommittedUSN** value from the previous search.</span></span>

<span data-ttu-id="4408c-119">Имейте в виду, что этот пример приложения предназначен только для того, чтобы продемонстрировать, как использовать **uSNChanged** для получения изменений с сервера Active Directory.</span><span class="sxs-lookup"><span data-stu-id="4408c-119">Be aware that this sample application is intended only to show how to use **uSNChanged** to retrieve changes from the Active Directory server.</span></span> <span data-ttu-id="4408c-120">Он выводит изменения и не выполняет фактическую синхронизацию данных во вторичном хранилище.</span><span class="sxs-lookup"><span data-stu-id="4408c-120">It prints the changes and does not actually synchronize the data in a secondary storage.</span></span> <span data-ttu-id="4408c-121">Следовательно, он не показывает, как выполнять такие проблемы, как перемещенные объекты или условия "нет родителя".</span><span class="sxs-lookup"><span data-stu-id="4408c-121">Consequently, it does not show how to handle issues like moved objects or "no parent" conditions.</span></span> <span data-ttu-id="4408c-122">Он показывает, как извлекать удаленные объекты, но не показывает, как приложение использует **objectGUID** удаленных объектов, чтобы определить соответствующий объект для удаления в хранилище.</span><span class="sxs-lookup"><span data-stu-id="4408c-122">It does show how to retrieve deleted objects, but it does not show how an application uses the **objectGUID** of the deleted objects to determine the corresponding object to delete in the storage.</span></span>

<span data-ttu-id="4408c-123">Кроме того, пример кэширует имя контроллера домена, **invocationID** и **хигхесткоммиттедусн** в реестре.</span><span class="sxs-lookup"><span data-stu-id="4408c-123">Also, the sample caches the DC name, **invocationID**, and **highestCommittedUSN** in the registry.</span></span> <span data-ttu-id="4408c-124">В реальном приложении синхронизации параметры необходимо хранить в том же хранилище, которое соответствует серверу Active Directory.</span><span class="sxs-lookup"><span data-stu-id="4408c-124">In a real synchronization application, you must store the parameters in the same storage that are kept consistent with the Active Directory server.</span></span> <span data-ttu-id="4408c-125">Это гарантирует, что параметры и данные объекта будут синхронизированы, если база данных когда-либо восстанавливается из резервной копии.</span><span class="sxs-lookup"><span data-stu-id="4408c-125">This ensures that the parameters and object data remain synchronized if your database is ever restored from a backup.</span></span>
