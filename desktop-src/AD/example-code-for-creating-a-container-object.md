...
В следующем примере кода создаются два объекта. Сначала — объект контейнера и, во-вторых, объект подконтейнера. Значение для объекта подконтейнера добавляется в свойство [**otherWellKnownObjects**](/windows/desktop/ADSchema/a-otherwellknownobjects) объекта контейнера. Пример кода привязывается к объекту вложенного контейнера с помощью привязки ВКГУИД и отображает его значение ADsPath. Затем он переименовывает объект подконтейнера и привязывает его еще раз, используя ту же привязку ВКГУИД.
