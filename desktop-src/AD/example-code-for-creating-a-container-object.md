---
title: Пример кода для создания объекта-контейнера
description: В следующем примере кода создаются два объекта.
...
---
# <a name="example-code-for-creating-a-container-object"></a><span data-ttu-id="b7ea8-104">Пример кода для создания объекта-контейнера</span><span class="sxs-lookup"><span data-stu-id="b7ea8-104">Example Code for Creating a Container Object</span></span>

<span data-ttu-id="b7ea8-105">В следующем примере кода создаются два объекта.</span><span class="sxs-lookup"><span data-stu-id="b7ea8-105">The following code example creates two objects.</span></span> <span data-ttu-id="b7ea8-106">Сначала — объект контейнера и, во-вторых, объект подконтейнера.</span><span class="sxs-lookup"><span data-stu-id="b7ea8-106">First, a container object and, second, a sub-container object.</span></span> <span data-ttu-id="b7ea8-107">Значение для объекта подконтейнера добавляется в свойство [**otherWellKnownObjects**](/windows/desktop/ADSchema/a-otherwellknownobjects) объекта контейнера.</span><span class="sxs-lookup"><span data-stu-id="b7ea8-107">A value for the sub-container object is added to the [**otherWellKnownObjects**](/windows/desktop/ADSchema/a-otherwellknownobjects) property of the container object.</span></span> <span data-ttu-id="b7ea8-108">Пример кода привязывается к объекту вложенного контейнера с помощью привязки ВКГУИД и отображает его значение ADsPath.</span><span class="sxs-lookup"><span data-stu-id="b7ea8-108">The code example binds to the sub-container object using the WKGUID binding and displays its ADsPath.</span></span> <span data-ttu-id="b7ea8-109">Затем он переименовывает объект подконтейнера и привязывает его еще раз, используя ту же привязку ВКГУИД.</span><span class="sxs-lookup"><span data-stu-id="b7ea8-109">It then renames the sub-container object and binds again using the same WKGUID binding.</span></span>
