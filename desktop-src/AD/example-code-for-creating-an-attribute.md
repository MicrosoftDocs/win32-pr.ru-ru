...
ms.locfileid: "118962304"
---
# <a name="example-code-for-creating-an-attribute"></a>Пример кода для создания атрибута

В следующем примере кода создается объект [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema) в контейнере схемы.

Функция **CreateAttribute** создает объект [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema) в контейнере схемы, но не фиксирует его в каталоге. Вызовите метод [**iAds. сетинфо**](/windows/desktop/api/iads/nf-iads-iads-setinfo) , чтобы зафиксировать новый объект **attributeSchema** в каталоге.

Функция **битестовариантаррай** — это служебная функция, которая упаковывает строку октета в массив Variant.
