---
title: Пример кода для создания атрибута
description: В следующем примере кода создается объект attributeSchema в контейнере схемы.
ms.assetid: 5f679a88-29d1-44ec-918b-091522188ae9
ms.tgt_platform: multiple
keywords:
- Пример кода для создания атрибута AD
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104487448"
---
# <a name="example-code-for-creating-an-attribute"></a><span data-ttu-id="23c29-104">Пример кода для создания атрибута</span><span class="sxs-lookup"><span data-stu-id="23c29-104">Example Code for Creating an Attribute</span></span>

<span data-ttu-id="23c29-105">В следующем примере кода создается объект [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema) в контейнере схемы.</span><span class="sxs-lookup"><span data-stu-id="23c29-105">The following code example creates an [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema) object in the schema container.</span></span>

<span data-ttu-id="23c29-106">Функция **CreateAttribute** создает объект [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema) в контейнере схемы, но не фиксирует его в каталоге.</span><span class="sxs-lookup"><span data-stu-id="23c29-106">The **CreateAttribute** function creates an [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema) object in the schema container, but does not commit it to the directory.</span></span> <span data-ttu-id="23c29-107">Вызовите метод [**iAds. сетинфо**](/windows/desktop/api/iads/nf-iads-iads-setinfo) , чтобы зафиксировать новый объект **attributeSchema** в каталоге.</span><span class="sxs-lookup"><span data-stu-id="23c29-107">Call the [**IADs.SetInfo**](/windows/desktop/api/iads/nf-iads-iads-setinfo) method to commit the new **attributeSchema** object to the directory.</span></span>

<span data-ttu-id="23c29-108">Функция **битестовариантаррай** — это служебная функция, которая упаковывает строку октета в массив Variant.</span><span class="sxs-lookup"><span data-stu-id="23c29-108">The **BytesToVariantArray** function is a utility function that packs an octet string into a variant array.</span></span>
