...
Этот раздел содержит примеры кода, в которых отображаются члены группы.

В следующем примере кода C++ для вывода элементов группы используются классы [**иадсграуп**](/windows/desktop/api/iads/nn-iads-iadsgroup) и [**иадсмемберс**](/windows/desktop/api/iads/nn-iads-iadsmembers) .
