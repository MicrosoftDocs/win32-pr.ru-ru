...
---
# <a name="example-code-for-checking-for-rights-to-create-schema-objects"></a><span data-ttu-id="604ac-104">Пример кода для проверки прав на создание объектов схемы</span><span class="sxs-lookup"><span data-stu-id="604ac-104">Example Code for Checking for Rights to Create Schema Objects</span></span>

<span data-ttu-id="604ac-105">В следующем примере кода C/C++ показана функция, которая проверяет атрибут **алловедчилдклассесеффективе** в контейнере схемы (указатель iAds на контейнер схемы передается в качестве параметра) для классов **attributeSchema** и **classSchema** .</span><span class="sxs-lookup"><span data-stu-id="604ac-105">The following C/C++ code example shows a function that checks the **allowedChildClassesEffective** attribute on the schema container (IADs pointer to schema container is passed as a parameter) for the **attributeSchema** and **classSchema** classes.</span></span> <span data-ttu-id="604ac-106">Он возвращает **\_ ОК** , если оба класса перечислены в **алловедчилдклассесеффективе**.</span><span class="sxs-lookup"><span data-stu-id="604ac-106">It returns **S\_OK** if both classes are listed in **allowedChildClassesEffective**.</span></span> <span data-ttu-id="604ac-107">Если эти значения не равны, возвращается **\_ значение S false**.</span><span class="sxs-lookup"><span data-stu-id="604ac-107">If both are not, it returns **S\_FALSE**.</span></span>
