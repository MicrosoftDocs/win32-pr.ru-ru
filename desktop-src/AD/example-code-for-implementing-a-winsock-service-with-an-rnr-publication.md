...
В следующем примере кода реализуется пример службы Winsock с публикацией РНР.

В этом примере используется функция **серверрегистер** , определенная в [примере кода для публикации раздела точки подключения РНР](example-code-for-publishing-the-rnr-connection-point.md) , и функция **Серверунрегистер** , определенная в [примере кода для удаления раздела подключения РНР](example-code-for-removing-the-rnr-connection-point.md) .
