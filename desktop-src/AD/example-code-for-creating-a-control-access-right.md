---
title: Пример кода для создания права доступа к элементу управления
description: Этот раздел содержит пример кода, который создает объект Контролакцессригхт в контейнере Extended-Rights.
...
ms.locfileid: "103986153"
---
# <a name="example-code-for-creating-a-control-access-right"></a><span data-ttu-id="c8241-103">Пример кода для создания права доступа к элементу управления</span><span class="sxs-lookup"><span data-stu-id="c8241-103">Example Code for Creating a Control Access Right</span></span>

<span data-ttu-id="c8241-104">В следующем примере Visual Basic создается объект **контролакцессригхт** в контейнере Extended-Rights.</span><span class="sxs-lookup"><span data-stu-id="c8241-104">The following Visual Basic example creates a **controlAccessRight** object in the Extended-Rights container.</span></span>
...
<span data-ttu-id="c8241-105">Следующий пример кода C++ — это функция, которая создает объект **контролакцессригхт** в контейнере Extended-Rights.</span><span class="sxs-lookup"><span data-stu-id="c8241-105">The following C++ code example is a function that creates a **controlAccessRight** object in the Extended-Rights container.</span></span> <span data-ttu-id="c8241-106">При вызове этой функции используйте следующий формат, чтобы указать строку GUID для параметра *псзригхтсгуид* .</span><span class="sxs-lookup"><span data-stu-id="c8241-106">When you call this function, use the following format to specify the GUID string for the *pszRightsGUID* parameter.</span></span>
...
<span data-ttu-id="c8241-107">Массив **адсвалуе** для свойства **appliesTo** использует тот же формат GUID и задает для элемента **двтипе** значение **адстипе \_ Case \_ Ignore \_**.</span><span class="sxs-lookup"><span data-stu-id="c8241-107">The **ADSVALUE** array for the **appliesTo** property uses the same GUID format and sets the **dwType** member to **ADSTYPE\_CASE\_IGNORE\_STRING**.</span></span>
...
<span data-ttu-id="c8241-108">Этот пример функции **креатикстендедригхт** можно вызвать с помощью следующего примера кода.</span><span class="sxs-lookup"><span data-stu-id="c8241-108">This **CreateExtendedRight** sample function can be called with the following code example.</span></span>
