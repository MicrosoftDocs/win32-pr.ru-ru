...
---
# <a name="example-code-for-creating-a-control-access-right"></a>Пример кода для создания права доступа к элементу управления

В следующем примере Visual Basic создается объект **контролакцессригхт** в контейнере Extended-Rights.
...
Следующий пример кода C++ — это функция, которая создает объект **контролакцессригхт** в контейнере Extended-Rights. При вызове этой функции используйте следующий формат, чтобы указать строку GUID для параметра *псзригхтсгуид* .
...
Массив **адсвалуе** для свойства **appliesTo** использует тот же формат GUID и задает для элемента **двтипе** значение **адстипе \_ Case \_ Ignore \_**.
...
Этот пример функции **креатикстендедригхт** можно вызвать с помощью следующего примера кода.
