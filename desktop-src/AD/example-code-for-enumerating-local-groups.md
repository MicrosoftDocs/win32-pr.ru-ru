...
Этот раздел содержит пример кода, который перечисляет все объекты указанного класса.

В следующем примере кода C++ перечисляются все объекты указанного класса с помощью интерфейсов ADSI.
