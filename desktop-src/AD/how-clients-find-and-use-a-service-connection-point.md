...
---
# <a name="how-clients-find-and-use-a-service-connection-point"></a>Как клиенты находят и используют точку подключения службы

В следующем примере кода показано, как клиентское приложение ищет в глобальном каталоге точку подключения службы (SCP). В этом примере кода клиентское приложение имеет жестко запрограммированную строку GUID, идентифицирующую службу. Установщик службы хранил ту же строку GUID как одно из значений атрибута запрашивая с несколькими значениями **ключевых слов** .

Этот пример состоит из двух подпрограмм. Подпрограммы **жетгк** получают указатель [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) для глобального каталога (GC). Для поиска в GC подпрограммы **скплокате** используют методы **IDirectorySearch** .

GC содержит частичную реплику каждого объекта в лесу, но не содержит все атрибуты SCP, необходимые клиенту. Сначала клиент должен выполнить поиск в GC, чтобы найти SCP и получить его DN. Затем клиент использует DN SCP для привязки к указателю [**идиректорйобжект**](/windows/desktop/api/iads/nn-iads-idirectoryobject) на SCP. Затем клиент вызывает метод [**идиректорйобжект:: жетобжектаттрибутес**](/windows/desktop/api/iads/nf-iads-idirectoryobject-getobjectattributes) для получения остальных атрибутов.
