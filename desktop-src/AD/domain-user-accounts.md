...
description: Учетная запись пользователя домена позволяет службе использовать все преимущества функций безопасности служб Windows и доменных служб Microsoft Active Directory.
...
Учетная запись пользователя домена имеет два формата имен: различающееся имя объекта пользователя в каталоге и &lt; Формат "домен &gt; \\ &lt; имя_пользователя", &gt; используемый локальным диспетчером управления службами. Дополнительные сведения и пример кода, который преобразует из одного формата в другой, см. в разделе [Преобразование форматов имени учетной записи домена](converting-domain-account-name-formats.md).
