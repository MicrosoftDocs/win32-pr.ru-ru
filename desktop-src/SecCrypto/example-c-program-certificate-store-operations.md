...
В следующем примере показано несколько общих операций [*хранилища сертификатов*](../secgloss/c-gly.md) , а также следующие задачи и функции [*CryptoAPI*](../secgloss/c-gly.md) :

-   Открытие и закрытие памяти и системных хранилищ с помощью [**цертопенсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore) и [**цертклосесторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore).
-   Дублирование открытого хранилища с помощью [**цертдупликатесторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatestore).
-   Поиск в хранит сертификаты, удовлетворяющие некоторым критериям, с помощью [**цертфиндцертификатеинсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore).
-   Создание нового контекста сертификата из закодированной части существующего сертификата с помощью [**церткреатецертификатеконтекст**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcreatecertificatecontext).
...
В этом примере в памяти создается хранилище сертификатов. Системное хранилище открывается и дублируется. Сертификат извлекается из системного хранилища. Новый сертификат создается из закодированной части полученного сертификата. Полученный сертификат добавляется в хранилище памяти. Второй сертификат извлекается из хранилища My Store, а в хранилище памяти добавляется ссылка на этот сертификат. Затем сертификат и ссылка извлекаются из хранилища памяти, а память сохраняется на диск. Все магазины и файлы закрыты. Затем хранилище файлов открывается повторно и выполняется поиск по ссылке на сертификат. Успешное выполнение этой программы зависит от доступного хранилища. Это хранилище должно содержать сертификат с темой "вставить сертификат \_ \_ субъекта \_ Name1" и второй сертификат с темой "вставить \_ сертификат \_ субъекта \_ 2.". Имена субъектов необходимо изменить на имена субъектов сертификатов, известных в хранилище My.
