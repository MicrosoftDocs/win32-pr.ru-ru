...
Следующий пример работает совместно с программой в [примере программы на языке C: отправка и получение подписанного и зашифрованного сообщения](example-c-program-sending-and-receiving-a-signed-and-encrypted-message.md). Он считывает подписанное и зашифрованное сообщение, затем расшифровывает и проверяет сообщение.

Чтобы расшифровать и проверить сообщение, должен быть доступен [*закрытый ключ*](../secgloss/p-gly.md) предполагаемого получателя сообщения. Сертификат для подписи включен в [*объект*](../secgloss/b-gly.md) сообщения, считанный из файла.

В этом примере показаны следующие задачи:

-   Открытие и закрытие хранилищ сертификатов системы.
-   Чтение [**\_ \_ большого двоичного объекта имени сертификата**](/previous-versions/windows/desktop/legacy/aa381414(v=vs.85)) из файла.
-   Инициализация структур данных, необходимых для расшифровки и проверки сообщения.
...
В этом примере используется [**михандлиррор**](myhandleerror.md) для корректного выхода из программы в случае любого сбоя. Код **михандлиррор** включен в пример, и его также можно найти вместе с другими вспомогательными функциями в разделе [функции общего назначения](general-purpose-functions.md).
