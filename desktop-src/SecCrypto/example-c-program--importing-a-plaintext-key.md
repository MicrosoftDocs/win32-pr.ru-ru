...
Многие функции в этом пакете SDK должны указывать ключ с помощью обработчика **хкрипткэй** . Если ключ содержится в массиве байтов, можно создать маркер с помощью функции [**криптимпорткэй**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptimportkey) , как показано в следующем примере.
...
-   Импорт открытого ключа в контейнер ключей CSP путем вызова [**криптимпорткэй**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptimportkey).
-   Экспорт ключа из контейнера ключей путем вызова [**циптекспорткэй**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptexportkey).
-   Печать экспортированного ключа на консоли, чтобы убедиться в том, что ключ в виде открытого текста действительно был импортирован в контейнер.
-   Освобождение памяти, зарезервированной для ключа открытого текста.
-   Освобождение CSP путем вызова [**криптрелеасеконтекст**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptreleasecontext).
