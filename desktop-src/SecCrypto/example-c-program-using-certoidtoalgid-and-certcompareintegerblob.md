...
ms.locfileid: "119007452"
---
# <a name="example-c-program-using-certoidtoalgid-and-certcompareintegerblob"></a>Пример программы на C: использование Цертоидтоалгид и Церткомпареинтежерблоб

В следующем примере демонстрируется использование функций [**цертоидтоалгид**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) и [**церткомпареинтежерблоб**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) .

Во первых, все доступные OID перечисляются с помощью [**криптенумоидинфо**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo). Код, используемый с этой функцией, также демонстрирует использование функции обратного вызова. Функция обратного вызова демонстрирует логику прерывания для приостановки между каждой группой OID и после представления информации о заданном количестве идентификаторов объектов.

Во-вторых, три строки [*идентификатора объекта*](../secgloss/o-gly.md) (OID) преобразуются в целые числа идентификатора алгоритма **типа DWORD** с помощью [**цертоидтоалгид**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid). В коде также показано, что все строки OID не имеют связанных идентификаторов алгоритмов.

Наконец, в примере показано сравнение целочисленных больших двоичных объектов. В этом примере демонстрируется усечение ведущих 0x00's от положительных чисел и ведущих 0xFF's от отрицательных чисел.

Он также показывает, что целые числа сравниваются, как если бы они хранились в форме с [*прямым порядком байтов*](../secgloss/l-gly.md) с наиболее значащими цифрами справа.
