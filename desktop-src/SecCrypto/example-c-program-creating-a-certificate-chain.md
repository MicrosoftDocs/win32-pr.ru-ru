...
---
# <a name="example-c-program-creating-a-certificate-chain"></a>Пример программы на языке C. Создание цепочки сертификатов

В следующем примере создается и устанавливается механизм цепочки сертификатов, отличный от используемого по умолчанию. Подсистема используется для создания цепочек сертификатов для каждого из сертификатов в [*хранилище сертификатов*](../secgloss/c-gly.md).

В этом примере показаны следующие задачи и функции [*CryptoAPI*](../secgloss/c-gly.md) :

-   Подготовка к созданию механизма цепочки сертификатов, не используемого по умолчанию, путем объявления и инициализации структуры данных [**\_ \_ \_ конфигурации механизма цепочки сертификатов**](/windows/desktop/api/Wincrypt/ns-wincrypt-cert_chain_engine_config) .
-   Создание поисковой системы с помощью [**церткреатецертификатечаиненгине**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcreatecertificatechainengine).
-   Использование [**цертопенсистемсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea) для открытия моего хранилища системы.
-   Извлечение всех сертификатов из открытого хранилища с помощью [**цертенумцертификатесинсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore) в цикле.
-   Для каждого сертификата в открытом хранилище — получение имени субъекта из сертификата с помощью [**цертжетнаместринг**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).
-   Создание цепочки сертификатов для каждого сертификата с помощью [**CertGetCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetcertificatechain).
-   Создание дубликата цепочки сертификатов с помощью [**цертдупликатецертификатечаин**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatechain).
-   Использование [**цертфрицертификатечаин**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfreecertificatechain) для освобождения каждой цепочки перед построением следующей цепочки.
