...
В следующем примере перечисляются CSP, доступные на компьютере, и используются следующие функции [*CryptoAPI*](../secgloss/c-gly.md) :

-   [**криптенумпровидертипес**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumprovidertypesa)
-   [**криптенумпровидерс**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumprovidersa)
-   [**криптжетдефаултпровидер**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetdefaultprovidera)
-   [**криптжетпровпарам**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetprovparam)

В этом примере используется функция [**михандлиррор**](myhandleerror.md). В этот пример включен код для этой функции. Код для этой и других вспомогательных функций также указан в разделе [общего назначения функции](general-purpose-functions.md).

В следующем примере показано, как перечислить CSP и типы поставщиков.
