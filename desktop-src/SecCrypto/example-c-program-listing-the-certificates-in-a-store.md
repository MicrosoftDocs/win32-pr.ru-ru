...
ms.locfileid: "119007602"
---
# <a name="example-c-program-listing-the-certificates-in-a-store"></a>Пример программы на языке C. Вывод списка сертификатов в хранилище

В следующем примере кода выводится список всех сертификатов в системном [*хранилище сертификатов*](../secgloss/c-gly.md) , имя субъекта и все свойства [*контекста сертификата*](../secgloss/c-gly.md) каждого из этих сертификатов. Пример получает имя хранилища сертификатов от пользователя и, таким образом, может использоваться для получения списка содержимого любого системного хранилища сертификатов. Кроме того, в этом примере показано использование двух новых функций пользовательского интерфейса, в которых отображается сертификат, а другой — пользовательский интерфейс, позволяющий пользователю выбрать сертификат из списка сертификатов в хранилище.

В этом примере кода показаны следующие задачи и функции [*CryptoAPI*](../secgloss/c-gly.md) :

-   Открытие системного хранилища с помощью [**цертопенсистемсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea).
-   В цикле перечислите все сертификаты в открытом хранилище с помощью [**цертенумцертификатесинсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore).
-   Отображение сертификата с помощью [**криптуидлгвиевконтекст**](/windows/desktop/api/Cryptuiapi/nf-cryptuiapi-cryptuidlgviewcontext).
-   Получение имени субъекта сертификата с помощью [**цертжетнаместринг**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).
-   В цикле с помощью [**цертенумцертификатеконтекстпропертиес**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatecontextproperties) можно получить идентификаторы свойств всех свойств, связанных с сертификатом.
-   Использование [**цертжетцертификатеконтекстпроперти**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetcertificatecontextproperty) для получения каждого свойства.
-   Отображение списка сертификатов в хранилище и предоставление пользователю возможности выбора одного из них с помощью [**криптуидлгселектцертификатефромсторе**](/windows/desktop/api/Cryptuiapi/nf-cryptuiapi-cryptuidlgselectcertificatefromstore).
-   Закрытие хранилища сертификатов с помощью [**цертклосесторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore).
...
В следующем примере показано перечисление и отображение сертификатов в хранилище. Чтобы скомпилировать этот пример, необходимо настроить компилятор для использования многобайтовой кодировки.
