...
В следующем примере задается и получается свойство хранилища сертификатов, имя локализованного хранилища. Это свойство не сохраняется при закрытии хранилища.
...
-   Задание локализованного имени хранилища с помощью [**цертсетсторепроперти**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsetstoreproperty).
-   Получение локализованного имени хранилища с помощью [**цертжетсторепроперти**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetstoreproperty).
-   Получение предопределенного локализованного имени хранилища с помощью [**криптфиндлокализеднаме**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptfindlocalizedname).
-   Сохраните хранилище сертификатов как сообщение [*PKCS \# 7*](../secgloss/p-gly.md) в файл с помощью [**цертсавесторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsavestore).
-   Сохраните хранилище сертификатов в [*большой двоичный объект*](../secgloss/b-gly.md) в памяти с помощью [**цертсавесторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsavestore).
-   Определите число подписывающих \# сообщений для сообщения PKCS 7 с помощью [**криптжетмессажесигнеркаунт**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount).
-   Откройте хранилище сертификатов из \# сообщения PKCS 7 в памяти с помощью [**криптжетмессажецертификатес**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagecertificates).
-   Инициализируйте [**\_ \_ идентификатор алгоритма шифрования**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) и [**отшифрованное \_ хэш- \_ сообщение \_**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_hash_message_para) , необходимое для [*хэширования*](../secgloss/h-gly.md) сообщения
-   Хеширование и кодирование сообщения с помощью [**крипсашмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-crypthashmessage).
-   Определите, были ли внесены изменения в открытое хранилище сертификатов, и синхронизируйте хранилище при необходимости с помощью [**цертконтролсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcontrolstore).
-   Закрытие хранилища сертификатов с помощью [**цертклосесторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore) и \_ \_ флага закрытия Store \_ \_ .
