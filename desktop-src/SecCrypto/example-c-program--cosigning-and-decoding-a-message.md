...
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a>Пример программы на языке C. Создание и декодирование сообщения

Для подписывания сообщения можно использовать функцию [**криптсигнмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) . Это достигается путем вызова [**криптсигнмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) один раз для подписывания исходного сообщения, а затем вызовите **криптсигнмессаже** еще раз, чтобы подписать подписанное сообщение.

При проверке подписи подписанного сообщения используется функция [**криптжетмессажесигнеркаунт**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) для получения числа подписывающихй сообщения и последующего вызова [**криптверифимессажесигнатуре**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) для каждой подписи. Если все подписи проверены, значит, сообщение соподписано допустимым.

В следующем примере показано, как подписать сообщение несколькими пользователями (соподписать сообщение), проверить все подписи и декодировать сообщение.
