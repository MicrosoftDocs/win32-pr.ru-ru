---
description: Для подписывания сообщения можно использовать функцию Криптсигнмессаже.
ms.assetid: b400436f-a71f-426a-ac8a-7fdcfa6d7575
title: Пример программы на языке C. Создание и декодирование сообщения
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104343592"
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a><span data-ttu-id="6096b-103">Пример программы на языке C. Создание и декодирование сообщения</span><span class="sxs-lookup"><span data-stu-id="6096b-103">Example C Program: Cosigning and Decoding a Message</span></span>

<span data-ttu-id="6096b-104">Для подписывания сообщения можно использовать функцию [**криптсигнмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) .</span><span class="sxs-lookup"><span data-stu-id="6096b-104">You can use the [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) function to cosign a message.</span></span> <span data-ttu-id="6096b-105">Это достигается путем вызова [**криптсигнмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) один раз для подписывания исходного сообщения, а затем вызовите **криптсигнмессаже** еще раз, чтобы подписать подписанное сообщение.</span><span class="sxs-lookup"><span data-stu-id="6096b-105">This is accomplished by calling [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) once to sign the original message, and then call **CryptSignMessage** again to cosign the signed message.</span></span>

<span data-ttu-id="6096b-106">При проверке подписи подписанного сообщения используется функция [**криптжетмессажесигнеркаунт**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) для получения числа подписывающихй сообщения и последующего вызова [**криптверифимессажесигнатуре**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) для каждой подписи.</span><span class="sxs-lookup"><span data-stu-id="6096b-106">When you verify the signature of a cosigned message, you use the [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) function to get the number of signers of the message and then call the [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) for each signature.</span></span> <span data-ttu-id="6096b-107">Если все подписи проверены, значит, сообщение соподписано допустимым.</span><span class="sxs-lookup"><span data-stu-id="6096b-107">If all of the signatures are verified, then you know the cosigned message is valid.</span></span>

<span data-ttu-id="6096b-108">В следующем примере показано, как подписать сообщение несколькими пользователями (соподписать сообщение), проверить все подписи и декодировать сообщение.</span><span class="sxs-lookup"><span data-stu-id="6096b-108">The following example shows how to sign a message by more than one person (cosign the message), verify all signatures, and decode the message.</span></span>
