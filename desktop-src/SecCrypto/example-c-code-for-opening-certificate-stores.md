...
В следующих примерах представлен код для открытия различных распространенных хранилищ сертификатов. Эти примеры представляют собой ряд фрагментов кода и не являются отдельными программами.

В следующем примере открывается системное хранилище My.
...
В следующем примере открывается хранилище памяти.
...
В следующем примере открывается хранилище с диска. В примере используется функция **креатемидакл** , определенная в разделе [Создание DACL](../secbp/creating-a-dacl.md) , чтобы обеспечить создание открытого файла с соответствующим списком DACL.
...
В следующем примере показано открытие файлового хранилища с помощью \_ хранилища CERT \_ Prov \_ filename.
...
В следующем примере открывается хранилище коллекции.
...
В следующем примере открывается хранилище Register.
...
В следующем примере показано, как открыть хранилище сертификатов на основе \# сообщения PKCS 7.
