---
description: В следующих примерах представлен код для открытия различных распространенных хранилищ сертификатов. Эти примеры представляют собой ряд фрагментов кода и не являются отдельными программами.
...
---
# <a name="example-c-code-for-opening-certificate-stores"></a><span data-ttu-id="571d6-104">Пример кода на языке C для открытия хранилищ сертификатов</span><span class="sxs-lookup"><span data-stu-id="571d6-104">Example C Code for Opening Certificate Stores</span></span>

<span data-ttu-id="571d6-105">В следующих примерах представлен код для открытия различных распространенных хранилищ сертификатов.</span><span class="sxs-lookup"><span data-stu-id="571d6-105">The following examples provide code to open a variety of common certificate stores.</span></span> <span data-ttu-id="571d6-106">Эти примеры представляют собой ряд фрагментов кода и не являются отдельными программами.</span><span class="sxs-lookup"><span data-stu-id="571d6-106">These examples are a series of code fragments and are not stand-alone programs.</span></span>

<span data-ttu-id="571d6-107">В следующем примере открывается системное хранилище My.</span><span class="sxs-lookup"><span data-stu-id="571d6-107">The following example opens the My system store.</span></span>
...
<span data-ttu-id="571d6-108">В следующем примере открывается хранилище памяти.</span><span class="sxs-lookup"><span data-stu-id="571d6-108">The following example opens a memory store.</span></span>
...
<span data-ttu-id="571d6-109">В следующем примере открывается хранилище с диска.</span><span class="sxs-lookup"><span data-stu-id="571d6-109">The following example opens a store from disk.</span></span> <span data-ttu-id="571d6-110">В примере используется функция **креатемидакл** , определенная в разделе [Создание DACL](../secbp/creating-a-dacl.md) , чтобы обеспечить создание открытого файла с соответствующим списком DACL.</span><span class="sxs-lookup"><span data-stu-id="571d6-110">The example uses the **CreateMyDACL** function, defined in the [Creating a DACL](../secbp/creating-a-dacl.md) topic, to ensure the open file is created with a proper DACL.</span></span>
...
<span data-ttu-id="571d6-111">В следующем примере показано открытие файлового хранилища с помощью \_ хранилища CERT \_ Prov \_ filename.</span><span class="sxs-lookup"><span data-stu-id="571d6-111">The following example opens a file-based store using CERT\_STORE\_PROV\_FILENAME.</span></span>
...
<span data-ttu-id="571d6-112">В следующем примере открывается хранилище коллекции.</span><span class="sxs-lookup"><span data-stu-id="571d6-112">The following example opens a collection store.</span></span>
...
<span data-ttu-id="571d6-113">В следующем примере открывается хранилище Register.</span><span class="sxs-lookup"><span data-stu-id="571d6-113">The following example opens a register store.</span></span>
...
<span data-ttu-id="571d6-114">В следующем примере показано, как открыть хранилище сертификатов на основе \# сообщения PKCS 7.</span><span class="sxs-lookup"><span data-stu-id="571d6-114">The following example opens a certificate store based on a PKCS \#7 message.</span></span>
