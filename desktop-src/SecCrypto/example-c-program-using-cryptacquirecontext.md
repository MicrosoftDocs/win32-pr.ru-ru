...
---
# <a name="example-c-program-using-cryptacquirecontext"></a>Пример программы C: использование CryptAcquireContext

В следующем примере показано несколько различных способов использования [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) и связанных функций CryptoAPI для работы с [*поставщиком служб шифрования*](../secgloss/c-gly.md) (CSP) и [*контейнером ключей*](../secgloss/k-gly.md).

В этом примере демонстрируются следующие задачи и функции CryptoAPI:

-   Используйте функцию [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) для получения маркера для CSP по умолчанию и контейнера ключей по умолчанию. Если контейнер ключей по умолчанию не существует, используйте функцию **CryptAcquireContext** для создания контейнера ключей по умолчанию.
-   Используйте функцию [**криптжетпровпарам**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetprovparam) для получения сведений о CSP и контейнере ключей.
-   Увеличьте [*число ссылок*](../secgloss/r-gly.md) в поставщике с помощью функции [**криптконтекстаддреф**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptcontextaddref) .
-   Выпустите CSP с помощью функции [**криптрелеасеконтекст**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptreleasecontext) .
-   Создайте именованный контейнер ключа с помощью функции [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) .
-   Получите маркер для CSP с помощью созданного контейнера ключей.
-   Удалите контейнер ключей с помощью функции [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) .
