...
Следующий пример сочетает подписывание и кодирование сообщения и декодирование подписанного сообщения и проверку подписи. Эти две операции обычно находятся в отдельных программах. В примере кодировки можно создать закодированное сообщение, сохранить его в файл на диске или каким-либо другим способом отправить его другому пользователю. Пример декодирования может получить закодированное сообщение, декодировать его и проверить подпись. Эти два процесса объединены здесь, чтобы отобразить обе процедуры.
...
-   Инициализация структуры с [**\_ \_ сообщением \_ о знаке шифрования**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_sign_message_para) , которая будет использоваться при вызове [**криптсигнмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage).
-   Подписывание и кодирование сообщения с помощью [**криптсигнмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage).
...
-   Добавление закодированного [*BLOB-объекта*](../secgloss/b-gly.md) в сообщение для декодирования с помощью [**криптмсгупдате**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate).
-   Декодирование сообщения с помощью [**криптмсгжетпарам**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam).
-   Открытие хранилища сертификатов в памяти с [**цертопенсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore) , используя полученное и декодированное сообщение.
-   Использование [**цертжетсубжектцертификатефромсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetsubjectcertificatefromstore) для получения сертификата для подписания сообщения.
-   Проверка подписи сообщения с помощью [**криптмсгконтрол**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcontrol).
-   Освобождение памяти, закрытие [*хранилищ сертификатов*](../secgloss/c-gly.md)и освобождение [*контекста сертификата*](../secgloss/c-gly.md).

Пример выполнения этих аналогичных операций с помощью обратного вызова потока см. в разделе [Пример программы C: кодирование и декодирование сообщения с помощью потока](example-c-program--encoding-and-decoding-a-message-using-a-stream.md).
