...
В следующем примере выводится список сертификатов в [*хранилище системных сертификатов*](../secgloss/c-gly.md), в котором отображается имя субъекта каждого сертификата, и он позволяет пользователю выбрать удаление сертификатов из хранилища. Пример получает имя хранилища сертификатов от пользователя и, таким образом, может использоваться для хранения содержимого любого системного хранилища сертификатов.

В этом примере показаны следующие задачи и функции [*CryptoAPI*](../secgloss/c-gly.md) :

-   Открытие хранилища сертификатов системы с помощью [**цертопенсистемсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea).
-   Перечисление сертификатов в хранилище сертификатов с помощью [**цертенумцертификатесинсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore).
...
-   Сравнение имени субъекта сертификата с именем издателя сертификата с помощью [**церткомпарецертификатенаме**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcomparecertificatename).
-   Проверка того, соответствует ли открытый ключ текущего сертификата открытому ключу предыдущего сертификата с помощью [**церткомпарепубликкэйинфо**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcomparepublickeyinfo).
-   Дублирование указателя на [*контекст сертификата*](../secgloss/c-gly.md) с помощью [**цертдупликатецертификатеконтекст**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatecontext).
-   Сравнение членов [**сертификата \_**](/windows/desktop/api/Wincrypt/ns-wincrypt-cert_info) каждого сертификата с помощью [**церткомпарецертификате**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcomparecertificate).
-   Удаление сертификата из хранилища с помощью [**цертделетецертификатефромсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certdeletecertificatefromstore).
-   Закрытие хранилища сертификатов с помощью [**цертклосесторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore).

Этот пример получает имя системного хранилища сертификатов от пользователя, открывает хранилище и проходит по сертификатам в этом хранилище. Для каждого сертификата отображается имя субъекта сертификата, и пользователю предоставляется возможность удалить этот сертификат.
