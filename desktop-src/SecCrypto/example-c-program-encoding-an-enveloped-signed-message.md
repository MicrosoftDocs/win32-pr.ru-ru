...
ms.locfileid: "119007742"
---
# <a name="example-c-program-encoding-an-enveloped-signed-message"></a>Пример программы на языке C. кодирование подписанного сообщения с подписью

В следующем примере создается, подписывается и записываются сообщения, а также демонстрируются следующие задачи и функции [*CryptoAPI*](../secgloss/c-gly.md) :

-   Получение маркера CSP с помощью [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta).
-   Открытие системного хранилища с помощью [**цертопенсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore).
-   Поиск сертификатов подписывающих и получателей с помощью [**цертфиндцертификатеинсторе**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore).
-   Инициализация подходящих структур данных для подписи упакованного сообщения.
-   Определение длины запечатанного сообщения с помощью [**криптмсгкалкулатинкодедленгс**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcalculateencodedlength).
-   Создание и подписывание сообщения с помощью [**криптмсгопентоенкоде**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode), [**криптмсгупдате**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)и [**криптмсгжетпарам**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam).
-   Запечатывание подписанное и закодированное сообщение для получателя с помощью [**криптмсгопентоенкоде**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode), [**криптмсгупдате**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)и [**криптмсгжетпарам**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam).

Этот пример завершится ошибкой, если в [*контейнере ключей*](../secgloss/k-gly.md)по умолчанию не существует пригодного к использованию закрытого ключа. Если необходимый закрытый ключ недоступен, код, использующий [**криптаккуирецертификатеприватекэй**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey), как показано в [примере кода пример программы C: отправка и получение подписанного и зашифрованного сообщения](example-c-program-sending-and-receiving-a-signed-and-encrypted-message.md), может использоваться.
