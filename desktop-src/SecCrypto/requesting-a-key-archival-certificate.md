...
---
# <a name="requesting-a-key-archival-certificate"></a>Запрос архивного сертификата ключа

В следующем примере показано создание и отправка [*запроса на сертификат*](../secgloss/c-gly.md) и получение результирующего архивного сертификата ключа. Чтобы выполнить этот пример, [*центр сертификации*](../secgloss/c-gly.md) (ЦС), получающий запрос на сертификат, должен быть настроен для архивирования ключей.

Следующие шаги описывают создание и отправку запроса на сертификат.

**Создание и отправка запроса на сертификат**

1.  Получите сертификат Exchange ЦС с помощью метода [**ICertRequest2:: жеткацертификате**](/windows/desktop/api/Certcli/nf-certcli-icertrequest-getcacertificate) .
2.  Укажите, что полученный сертификат обмена ЦС является сертификатом резервного архива с помощью свойства [**ICEnroll4::P риватекэйарчивецертификате**](/windows/desktop/api/Xenroll/nf-xenroll-icenroll4-get_privatekeyarchivecertificate) .
3.  Создайте запрос на сертификат CMC с помощью метода [**ICEnroll4:: креатерекуест**](/windows/desktop/api/Xenroll/nf-xenroll-icenroll4-createrequest) .
4.  Отправьте запрос сертификата в центр сертификации с помощью метода [**ICertRequest2:: submit**](/windows/desktop/api/Certcli/nf-certcli-icertrequest-submit) . ЦС должен быть настроен для поддержки архивирования ключей.

Следующие шаги описывают, как получить выданный сертификат для архивирования ключей.

**Получение выданного сертификата для архивирования ключей**

1.  Получите полный ответ, включая выданный сертификат, с помощью метода [**ICertRequest2:: жетфуллреспонсепроперти**](/windows/desktop/api/Certcli/nf-certcli-icertrequest2-getfullresponseproperty) .
2.  Установите выданный сертификат с помощью метода [**ICEnroll4:: акцептреспонсе**](/windows/desktop/api/Xenroll/nf-xenroll-icenroll4-acceptresponse) .

В следующем примере показано создание и отправка запроса на сертификат и получение результирующего архивного сертификата ключа.
