...
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a><span data-ttu-id="43850-103">Пример программы на C: использование Криптенкриптмессаже и Криптдекриптмессаже</span><span class="sxs-lookup"><span data-stu-id="43850-103">Example C Program: Using CryptEncryptMessage and CryptDecryptMessage</span></span>

<span data-ttu-id="43850-104">[**Криптенкриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) — это единственный вызов функции, необходимый для выполнения всех задач, перечисленных в [шифровании сообщения](../secauthn/encrypting-a-message.md).</span><span class="sxs-lookup"><span data-stu-id="43850-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) is the only function call necessary to accomplish all of the tasks listed in [Encrypting a Message](../secauthn/encrypting-a-message.md).</span></span> <span data-ttu-id="43850-105">Инициализация структур данных необходима.</span><span class="sxs-lookup"><span data-stu-id="43850-105">Initialization of data structures is necessary.</span></span> <span data-ttu-id="43850-106">На следующем рисунке показана связь между параметрами функции, которые указывают на структуры или массивы и их инициализированные данные.</span><span class="sxs-lookup"><span data-stu-id="43850-106">The following illustration shows the relationship between those function parameters that point to structures or arrays and their initialized data.</span></span> <span data-ttu-id="43850-107">В этом примере также расшифровывается сообщение с помощью [**криптдекриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span><span class="sxs-lookup"><span data-stu-id="43850-107">This example also decrypts the message using [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span></span>

![Схема инициализации для вызова криптенкриптмессаже](images/crypenc.png)

<span data-ttu-id="43850-109">**Шифрование данных**</span><span class="sxs-lookup"><span data-stu-id="43850-109">**To encrypt data**</span></span>

1.  <span data-ttu-id="43850-110">Получите указатель на содержимое для шифрования.</span><span class="sxs-lookup"><span data-stu-id="43850-110">Get a pointer to the content to be encrypted.</span></span>
2.  <span data-ttu-id="43850-111">Определите размер содержимого для шифрования.</span><span class="sxs-lookup"><span data-stu-id="43850-111">Determine the size of the content to be encrypted.</span></span>
3.  <span data-ttu-id="43850-112">Получение маркера для поставщика служб шифрования.</span><span class="sxs-lookup"><span data-stu-id="43850-112">Acquire a handle to a cryptographic provider.</span></span>
4.  <span data-ttu-id="43850-113">Откройте [*хранилище сертификатов*](../secgloss/c-gly.md).</span><span class="sxs-lookup"><span data-stu-id="43850-113">Open a [*certificate store*](../secgloss/c-gly.md).</span></span>
5.  <span data-ttu-id="43850-114">Получите сертификат получателя.</span><span class="sxs-lookup"><span data-stu-id="43850-114">Get the recipient certificate.</span></span>
6.  <span data-ttu-id="43850-115">Создайте массив сертификатов получателей.</span><span class="sxs-lookup"><span data-stu-id="43850-115">Create the recipient certificate array.</span></span>
7.  <span data-ttu-id="43850-116">Инициализируйте [**структуру \_ \_ идентификатора алгоритма шифрования**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) .</span><span class="sxs-lookup"><span data-stu-id="43850-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) structure.</span></span>
8.  <span data-ttu-id="43850-117">Инициализируйте [**структуру \_ шифрования \_ сообщения \_ с шифрованием**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) .</span><span class="sxs-lookup"><span data-stu-id="43850-117">Initialize the [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) structure.</span></span>
9.  <span data-ttu-id="43850-118">Вызовите [**криптенкриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) , чтобы зашифровать содержимое и создать сообщение с цифровой подписью.</span><span class="sxs-lookup"><span data-stu-id="43850-118">Call [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) to encrypt the content and create a digitally enveloped message.</span></span>

<span data-ttu-id="43850-119">В следующем примере реализуется эта процедура.</span><span class="sxs-lookup"><span data-stu-id="43850-119">The following example implements this procedure.</span></span>

<span data-ttu-id="43850-120">Комментарии связывают фрагменты кода с каждым шагом в процедуре.</span><span class="sxs-lookup"><span data-stu-id="43850-120">Comments relate code fragments to each step in the procedure.</span></span> <span data-ttu-id="43850-121">Дополнительные сведения о функции см. в разделе [**криптенкриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span><span class="sxs-lookup"><span data-stu-id="43850-121">For more information about the function, see [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span></span> <span data-ttu-id="43850-122">Дополнительные сведения о структурах данных см. в [**разделе \_ \_ идентификатор алгоритма**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) шифрования и [**\_ Шифрование зашифрованного сообщения с помощью \_ \_ абзаца**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span><span class="sxs-lookup"><span data-stu-id="43850-122">For more information about the data structures, see [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span></span>

<span data-ttu-id="43850-123">В этом примере используются функции [**михандлиррор**](myhandleerror.md) и [**битетостр**](bytetostr.md).</span><span class="sxs-lookup"><span data-stu-id="43850-123">This example uses the functions [**MyHandleError**](myhandleerror.md) and [**ByteToStr**](bytetostr.md).</span></span> <span data-ttu-id="43850-124">Код для этой функции включен в пример.</span><span class="sxs-lookup"><span data-stu-id="43850-124">The code for this function is included with the sample.</span></span> <span data-ttu-id="43850-125">Код для этой и других вспомогательных функций также указан в разделе [общего назначения функции](general-purpose-functions.md).</span><span class="sxs-lookup"><span data-stu-id="43850-125">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
