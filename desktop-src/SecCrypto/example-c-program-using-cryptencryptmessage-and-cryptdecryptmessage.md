...
[**Криптенкриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) — это единственный вызов функции, необходимый для выполнения всех задач, перечисленных в [шифровании сообщения](../secauthn/encrypting-a-message.md). Инициализация структур данных необходима. На следующем рисунке показана связь между параметрами функции, которые указывают на структуры или массивы и их инициализированные данные. В этом примере также расшифровывается сообщение с помощью [**криптдекриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).

![Схема инициализации для вызова криптенкриптмессаже](images/crypenc.png)

**Шифрование данных**

1.  Получите указатель на содержимое для шифрования.
2.  Определите размер содержимого для шифрования.
3.  Получение маркера для поставщика служб шифрования.
4.  Откройте [*хранилище сертификатов*](../secgloss/c-gly.md).
5.  Получите сертификат получателя.
6.  Создайте массив сертификатов получателей.
7.  Инициализируйте [**структуру \_ \_ идентификатора алгоритма шифрования**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) .
8.  Инициализируйте [**структуру \_ шифрования \_ сообщения \_ с шифрованием**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) .
9.  Вызовите [**криптенкриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) , чтобы зашифровать содержимое и создать сообщение с цифровой подписью.

В следующем примере реализуется эта процедура.

Комментарии связывают фрагменты кода с каждым шагом в процедуре. Дополнительные сведения о функции см. в разделе [**криптенкриптмессаже**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage). Дополнительные сведения о структурах данных см. в [**разделе \_ \_ идентификатор алгоритма**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) шифрования и [**\_ Шифрование зашифрованного сообщения с помощью \_ \_ абзаца**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).

В этом примере используются функции [**михандлиррор**](myhandleerror.md) и [**битетостр**](bytetostr.md). Код для этой функции включен в пример. Код для этой и других вспомогательных функций также указан в разделе [общего назначения функции](general-purpose-functions.md).
