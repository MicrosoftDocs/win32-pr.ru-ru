...
Если задано **значение true**, передача данных в двоичном режиме включается для последовательного порта. компьютерные системы, работающие Windows, разрешают только двоичные передачи через последовательные порты, поэтому это значение всегда равно **TRUE**.
