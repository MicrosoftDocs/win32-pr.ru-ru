---
description: '&Win32 \_ параллелпорт \# 32; Класс WMI представляет свойства параллельного порта в компьютерной системе, на которой работает Windows.'
...
ms.locfileid: "119972254"
---
# <a name="win32_parallelport-class"></a>\_Класс Win32 параллелпорт

[Класс WMI](../wmisdk/retrieving-a-class.md) **\_ параллелпорт для Win32** представляет свойства параллельного порта в компьютерной системе, на которой работает Windows.
...
Класс **Win32 \_ параллелпорт** имеет следующие типы членов:
...
Класс **Win32 \_ параллелпорт** содержит следующие методы.



| Метод            | Описание                                                                                                                                                                                              |
|:------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Сброс**         | Не реализован. Чтобы реализовать этот метод, см. метод [**Reset**](reset-method-in-class-cim-controller.md) в [**CIM \_ параллелконтроллер**](cim-parallelcontroller.md).<br/>                 |
| **SetPowerState** | Не реализован. Чтобы реализовать этот метод, см. метод [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) в [**CIM \_ параллелконтроллер**](cim-parallelcontroller.md).<br/> |
...
Класс **Win32 \_ параллелпорт** имеет следующие свойства.
...
Квалификаторы: [**arrayType**](../wmisdk/standard-qualifiers.md) ("индексированный"), [**Маппингстрингс**](../wmisdk/standard-qualifiers.md) ("MIF. \|Параллельные порты DMTF \| 003,8 "), [**Моделкорреспонденце**](../wmisdk/standard-qualifiers.md) ("[**CIM \_ параллелконтроллер**](cim-parallelcontroller.md).**Капабилитидескриптионс**")
</dt> </dl>

Массив возможностей параллельного контроллера.

Это свойство наследуется от [**CIM \_ параллелконтроллер**](cim-parallelcontroller.md).
...
Квалификаторы: [**arrayType**](../wmisdk/standard-qualifiers.md) ("индексированный"), [**Моделкорреспонденце**](../wmisdk/standard-qualifiers.md) ("[**CIM \_ параллелконтроллер**](cim-parallelcontroller.md).**Возможности**")
</dt> </dl>

Массив более подробных объяснений для любой из функций параллельного контроллера, указанных в массиве **capabilities** . Каждая запись этого массива связана с записью в массиве **capabilities** , расположенном по тому же индексу.
...
Квалификаторы: [**маппингстрингс**](../wmisdk/standard-qualifiers.md) (MIF. \|Параллельные порты DMTF \| 003,7 ")
</dt> </dl>

Если **значение — true**, параллельный порт поддерживает DMA.
...
Если **значение — true**, ошибка, обнаруженная в **ластерроркоде** , удаляется.
...
Если **значение равно true**, то параллельный порт был автоматически обнаружен операционной системой. Если **значение равно false**, порт был обнаружен другими средствами (например, вручную добавленными через панель управления).
...
Это свойство наследуется [**от \_ контроллера CIM**](cim-controller.md). Значения показаны в следующем списке.
...
Дата и время последнего сброса контроллера. Это может означать, что контроллер был отключен или инициализирован повторно.
...
Класс **Win32 \_ параллелпорт** является производным от [**CIM \_ параллелконтроллер**](cim-parallelcontroller.md).
