...
Класс **CIM \_ DMA** представляет архитектуру компьютера с прямым доступом к памяти (DMA).
...
Размер адреса канала DMA в битах. Допустимые значения: 8, 16, 32 и 64. Если значение неизвестно, введите 0.
...
Используется, но доступно (совместное использование).
...
Если **значение — true**, канал DMA поддерживает пакетный режим.
...
Указывает, может ли DMA выполняться в режиме Count-to-Byte.
...
Невозможно выполнить в режиме подсчета по байтам.
...
<span id="ISA_Compatible"></span><span id="isa_compatible"></span><span id="ISA_COMPATIBLE"></span>**Совместимость с ISA** (3)
...
<span id="Type_A"></span><span id="type_a"></span><span id="TYPE_A"></span>**Введите A** (4)
...
<span id="Type_B"></span><span id="type_b"></span><span id="TYPE_B"></span>**Тип B** (5)
...
<span id="Type_F"></span><span id="type_f"></span><span id="TYPE_F"></span>**Тип F** (6)
...
Массив, указывающий все значения ширины передачи в битах, поддерживаемые этим каналом DMA. Допустимые значения: 8, 16, 32, 64 и 128 бит. Если значение неизвестно, введите 0 (ноль).
...
Указывает, поддерживается ли синхронизация типа C (пакетная).
...
<span id="Not_Supported"></span><span id="not_supported"></span><span id="NOT_SUPPORTED"></span>**Не поддерживается** (4)
...
<span id="Supported"></span><span id="supported"></span><span id="SUPPORTED"></span>**Поддерживается** (5)
...
Указывает, можно ли выполнить DMA в режиме подсчета слов.
...
Невозможно выполнить в режиме подсчета слов.
...
Возможность выполнения в режиме подсчета слов.
...
Инструментарий WMI не реализует этот класс. Классы, производные от **CIM \_ DMA**, см. в разделе [Классы Win32](win32-provider.md).
