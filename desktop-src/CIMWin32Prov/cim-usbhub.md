...
---
# <a name="cim_usbhub-class"></a>\_Класс CIM усбхуб

Класс **CIM \_ усбхуб** представляет возможности и управление USB-концентратором.
...
Класс **CIM \_ усбхуб** имеет следующие типы членов:
...
Класс **CIM \_ усбхуб** содержит следующие методы.



| Метод                                                            | Описание                                                                                                                              |
|:------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Двухбайтовый**](getdescriptor-method-in-class-cim-usbhub.md) | Возвращает дескриптор USB-устройства. Не реализовано инструментарием WMI.<br/>                                                                    |
| [**Перезапуск**](reset-method-in-class-cim-usbhub.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-usbhub.md) | Определяет требуемое состояние электропитания для логического устройства, а также когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
...
Класс **CIM \_ усбхуб** имеет следующие свойства.
...
Это свойство наследуется от [**CIM \_ усбдевице**](cim-usbdevice.md).
...
Квалификаторы: [**моделкорреспонденце**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ усбдевице**](cim-usbdevice.md).**Куррентконфигвалуе**")
</dt> </dl>

Альтернативные параметры USB для каждого интерфейса в текущей выбранной конфигурации (указывается свойством **куррентконфигвалуе** ). Этот массив содержит одну запись для каждого интерфейса в конфигурации. Если свойство **куррентконфигвалуе** имеет значение 0 (ноль), которое указывает, что устройство не настроено, массив не определен. Дополнительные сведения о синтаксическом анализе этой строки октета см. в спецификации USB.
...
Квалификаторы: [**моделкорреспонденце**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ усбдевице**](cim-usbdevice.md).**Курренталтернатесеттингс**")
</dt> </dl>

Конфигурация, выбранная в настоящее время для устройства. Если это значение равно 0 (нулю), устройство не настроено.
...
Если **значение равно true**, питание переключается на все порты в концентраторе одновременно. Если **значение равно false**, питание для каждого порта переключается по отдельности.
...
Число нижестоящих портов в концентраторе, включая те, которые встроены в кристалл концентратора.
...
Последняя версия USB, поддерживаемая USB-устройством. Это свойство выражается в виде двоичного десятичного числа (BCD), где между второй и третьей цифрами подразумевается десятичная точка. Например, значение 0x201 указывает, что поддерживается версия 2,01.
...
Класс **CIM \_ усбхуб** является производным от [**CIM \_ усбдевице**](cim-usbdevice.md).

Инструментарий WMI не реализует этот класс. Классы WMI, производные от **CIM \_ усбхуб**, см. в разделе [Классы Win32](win32-provider.md).
