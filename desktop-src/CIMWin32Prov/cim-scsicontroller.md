...
---
# <a name="cim_scsicontroller-class"></a>\_Класс CIM сксиконтроллер

Класс **CIM \_ сксиконтроллер** представляет возможности и управление ЛОГИЧЕСКИМ устройством контроллера SCSI.
...
Класс **CIM \_ сксиконтроллер** имеет следующие типы членов:
...
Класс **CIM \_ сксиконтроллер** содержит следующие методы.



| Метод                                                                    | Описание                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Перезапуск**](reset-method-in-class-cim-scsicontroller.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-scsicontroller.md) | Определяет требуемое состояние электропитания для логического устройства, а также когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
...
Класс **CIM \_ сксиконтроллер** имеет следующие свойства.
...
Количество тайм-аутов контроллера SCSI, произошедших с момента последнего сброса.
...
Максимальная ширина данных в битах, поддерживаемая контроллером SCSI.
...
Максимальная скорость передачи в битах в секунду, поддерживаемая контроллером SCSI.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -контроллер \| 001,3 ")
</dt> </dl>

Указывает, обеспечивает ли SCSI Controller избыточность или защиту от сбоев устройств.
...
<span id="Unprotected"></span><span id="unprotected"></span><span id="UNPROTECTED"></span>Не **защищено** (3)
...
<span id="Protected"></span><span id="protected"></span><span id="PROTECTED"></span>**Защищено** (4)
...
<span id="Protected_through_SCC__SCSI-3_Controller_Command_"></span><span id="protected_through_scc__scsi-3_controller_command_"></span><span id="PROTECTED_THROUGH_SCC__SCSI-3_CONTROLLER_COMMAND_"></span>**Защищено с помощью SCC (команда контроллера SCSI-3)** (5)


</dt> <dd>

Защищено SCC (команда контроллера SCSI-3)
...
<span id="Protected_through_SCC-2__SCSI-3_Controller_Command_"></span><span id="protected_through_scc-2__scsi-3_controller_command_"></span><span id="PROTECTED_THROUGH_SCC-2__SCSI-3_CONTROLLER_COMMAND_"></span>**Защищено с помощью SCC-2 (команда контроллера SCSI-3)** (6)


</dt> <dd>

Защищено SCC-2 (команда контроллера SCSI-3)
...
Класс **CIM \_ сксиконтроллер** является производным от [**\_ контроллера CIM**](cim-controller.md).

Инструментарий WMI не реализует этот класс. Классы WMI, производные от **CIM \_ сксиконтроллер**, см. в разделе [Классы Win32](win32-provider.md).
