...
Статический метод класса **ReleaseDHCPLeaseAll** [WMI](/windows/desktop/WmiSdk/retrieving-a-class) освобождает IP-адреса, привязанные ко всем сетевым адаптерам с поддержкой DHCP.

> [!Note]  
> Предупреждение. Если на локальном компьютере включен DHCP, параметр приведет к прерыванию всех TCP/IP-подключений.
...
Следующий пример кода VBScript освобождает все аренды DHCP, используемые в настоящее время на компьютере.
