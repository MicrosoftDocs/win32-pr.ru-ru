...
Класс **CIM \_ кдромдриве** представляет устройство чтения компакт-дисков на компьютере.

> [!Note]  
> Имя диска не соответствует букве логического диска, назначенной устройству. это имя логического устройства хранения, которое зависит от диска.
...
Класс **CIM \_ кдромдриве** имеет следующие типы членов:
...
Класс **CIM \_ кдромдриве** содержит следующие методы.
...
Класс **CIM \_ кдромдриве** имеет следующие свойства.
...
Массив строк произвольной формы, который содержит подробные пояснения для доступа к функциям устройств, указанным в массиве **capabilities** .
...
Произвольная строка, указывающая алгоритм или инструмент, используемый для сжатия логического файла. Если невозможно описать схему сжатия (так как она неизвестна), используйте следующую команду: Если, используйте "Unknown". Если значение равно, используйте "сжатый". , используйте "not сжатый".
...
Максимальный размер носителя, поддерживаемого этим устройством (в килобайтах). Килобайты интерпретируется как число байтов, умноженное на 1000 (а не на число байтов, умноженное на 1024).
...
Максимальное количество отдельных носителей, которые могут быть поддерживаются или вставлены.
...
Инструментарий WMI не реализует этот класс. Дополнительные сведения о классах, производных от **CIM \_ кдромдриве**, см. в разделе [Классы Win32](win32-provider.md).
