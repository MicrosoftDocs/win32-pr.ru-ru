---
description: '\_Класс WMI Win32 вмисеттинг Singleton содержит рабочие параметры для службы WMI. этот класс может иметь только один экземпляр, который всегда существует для каждой Windows системы и не может быть удален. Невозможно создать дополнительные экземпляры.'
...
[Класс WMI](../wmisdk/retrieving-a-class.md) **Win32 \_ вмисеттинг** Singleton содержит рабочие параметры для службы WMI. этот класс может иметь только один экземпляр, который всегда существует для каждой Windows системы и не может быть удален. Невозможно создать дополнительные экземпляры.
...
Если **значение — true**, скрипты WMI можно использовать на Active Server страницах (ASP). это свойство допустимо только в системах с неподдерживаемыми версиями Windows. для поддерживаемых Windows систем сценарии WMI всегда разрешены в ASP.
...
пример кода на языке VBScript " [Modify WMI Параметры](https://Gallery.TechNet.Microsoft.Com/aa80d174-3592-4fed-9c50-11f34e541445) " в коллекции TechNet использует класс **Win32 \_ вмисеттинг** для настройки интервала резервного копирования wmi и уровня ведения журнала.
...
в [списке всех](https://Gallery.TechNet.Microsoft.Com/29c04301-e9b2-46d1-8714-2dbb51014c92) примеров кода Параметры VBSCript для wmi используется ряд свойств в **Win32 \_ вмисеттинг** для возврата списка параметров WMI, настроенных на компьютере.
