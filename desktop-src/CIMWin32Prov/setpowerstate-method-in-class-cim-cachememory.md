...
ms.locfileid: "119439674"
---
# <a name="setpowerstate-method-of-the-cim_cachememory-class"></a>Метод SetPowerState \_ класса CIM качемемори

Метод **SetPowerState** задает требуемое состояние электропитания для логического устройства и когда устройство должно быть переведено в это состояние. В подклассе укажите набор возможных кодов возврата с помощью квалификатора **ValueMap** в методе. Строки, в которые преобразуются содержимое **ValueMap** , также должны быть указаны в подклассе как квалификатор массива **Values** . Этот метод наследуется [**от \_ CIM**](cim-logicaldevice.md)-ого модели.
...
[\_КАЧЕМЕМОРИ CIM](setpowerstate-method-in-class-cim-cachememory.md)
</dt> <dt>
