...
---
# <a name="win32_idecontroller-class"></a>\_Класс Win32 идеконтроллер

[Класс WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ идеконтроллер для Win32** управляет возможностями устройства контроллера IDE.
...
Класс **Win32 \_ идеконтроллер** имеет следующие типы членов:
...
Класс **Win32 \_ идеконтроллер** содержит следующие методы.



| Метод            | Описание                                                                                                                                                                                                |
|:------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Сброс**         | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**Reset**](reset-method-in-class-cim-controller.md) в [**CIM \_ Controller**](cim-controller.md) .<br/>                 |
| **SetPowerState** | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) в [**CIM \_ Controller**](cim-controller.md) .<br/> |
...
Класс **Win32 \_ идеконтроллер** имеет следующие свойства.
...
Уникальный идентификатор контроллера IDE с другими устройствами в системе.
...
Дополнительные сведения об ошибке, записанной в свойстве **ластерроркоде** , и сведения о любых корректирующих действиях, которые могут быть выполнены.
...
Изготовитель устройства IDE Controller.
...
ИДЕНТИФИКАТОР устройства Windows самонастраивающийся для логического устройства.
...
Если **значение — true**, Управление питанием устройства может осуществляться в режиме приостановки и т. д. Свойство не указывает на то, что функции управления питанием в настоящее время включены, только что логическое устройство поддерживает управление питанием.
...
Дата и время последнего сброса этого контроллера. Это может означать, что контроллер был отключен или инициализирован повторно.
...
Класс **Win32 \_ идеконтроллер** является производным от [**CIM \_ Controller**](cim-controller.md).
