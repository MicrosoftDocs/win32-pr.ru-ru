...
Класс **\_ батареи CIM** представляет возможности и управление логическим устройством с батареей. Этот класс применяется к батареям в ноутбуках и других внутренних и внешних батареях.
...
Класс **\_ батареи CIM** имеет следующие методы.
...
Класс **\_ батареи CIM** имеет следующие свойства.
...
Описание состояния зарядки аккумулятора. Значение 10 недопустимо в схеме CIM, которая представляет отсутствие батареи, установленной в интерфейсе управления рабочим столом (DMI). В этом случае не следует создавать экземпляры объекта.
...
<span id="Lead_Acid"></span><span id="lead_acid"></span><span id="LEAD_ACID"></span>**Ведущий ACID** (3)
...
<span id="Nickel_Cadmium"></span><span id="nickel_cadmium"></span><span id="NICKEL_CADMIUM"></span>**Никель кадмиум** (4)
...
<span id="Nickel_Metal_Hydride"></span><span id="nickel_metal_hydride"></span><span id="NICKEL_METAL_HYDRIDE"></span>**Никель металла хидриде** (5)


</dt> <dd>

Никель металла хидриде.

</dd> <dt>

<span id="Lithium-ion"></span><span id="lithium-ion"></span><span id="LITHIUM-ION"></span>

<span id="Lithium-ion"></span><span id="lithium-ion"></span><span id="LITHIUM-ION"></span>**Литий-ионный** (6)
...
<span id="Zinc_air"></span><span id="zinc_air"></span><span id="ZINC_AIR"></span>**Зинк воздуха** (7)
...
<span id="Lithium_Polymer"></span><span id="lithium_polymer"></span><span id="LITHIUM_POLYMER"></span>**Литий-Polymer** (8)
...
Емкость аккумулятора в диаграмме милливаттах-часах. Если это свойство не поддерживается, введите 0.
...
Спроектированное напряжение батареи в милливольтах. Если этот атрибут не поддерживается, введите 0.
...
Предполагаемый процент оставшегося полного платежа.
...
Предполагаемое время в минутах, пока заряд батареи не будет исчерпан в условиях текущей нагрузки, если выключена программа, будет утрачена и останется отключенной, или если ноутбук отключен от источника питания.
...
Ожидаемое время работы батареи (в минутах), предполагая, что батарея полностью заряжена. Это свойство представляет общий ожидаемый срок работы батареи, а не текущий оставшийся срок, который указывается свойством **EstimatedRunTime** .
...
Полная емкость батареи в диаграмме милливаттах-часах. Сравните это значение со свойством **десигнкапаЦити** , чтобы определить, когда батарея требует замены. Окончание работы батареи обычно происходит, когда свойство **фуллчаржекапаЦити** становится менее 80% от свойства **десигнкапаЦити** . Если это свойство не поддерживается, введите 0.
...
Максимальное время (в минутах) для полной зарядки аккумулятора. Это свойство представляет время, которое будет выставлять полностью исчерпанную батарею, а не текущее оставшееся время зарядки, которое указывается в свойстве **тиметофуллчарже** .
...
Номер версии спецификации данных Smart аккумулятора, поддерживаемый этим аккумулятором. Если батарея не поддерживает эту функцию, значение должно остаться пустым.
...
Время в секундах, прошедшее с момента последнего переключения ИБП компьютера на питание от аккумулятора или от времени последнего перезапуска системы или ИБП, в зависимости от того, какое значение меньше. Если батарея находится в оперативном режиме, возвращается значение 0.
...
Оставшееся время (в минутах), в течение которого батарея полностью заряжена на текущий коэффициент зарядки и используется.
...
Инструментарий WMI не реализует этот класс. Дополнительные сведения о классах, производных от **\_ аккумулятора CIM**, см. в разделе [Классы Win32](win32-provider.md).
