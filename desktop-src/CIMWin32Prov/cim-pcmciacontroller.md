...
Класс **CIM \_ пкмЦиаконтроллер** представляет возможности и Управление контроллером международной связи с картой памяти персональных компьютеров (PCMCIA).
...
Класс **CIM \_ пкмЦиаконтроллер** имеет следующие типы членов:
...
Класс **CIM \_ пкмЦиаконтроллер** содержит следующие методы.
...
Класс **CIM \_ пкмЦиаконтроллер** имеет следующие свойства.
...
Устройство не работает, но может быть включено в полную работу.
...
Имя изготовителя PCMCIA Controller.
...
Поддерживается метод [**SetPowerState**](setpowerstate-method-in-class-cim-pcmciacontroller.md) .
...
Метод [**SetPowerState**](setpowerstate-method-in-class-cim-pcmciacontroller.md) может быть вызван с параметром *PowerState* , установленным в значение 5 ("цикл электропитания").
...
Метод [**SetPowerState**](setpowerstate-method-in-class-cim-pcmciacontroller.md) может быть вызван с параметром *PowerState* , установленным в значение 5 ("цикл электропитания"), и параметром *времени* , установленным на определенную дату и время (или интервал) для включения питания.
...
Дата и время последнего сброса контроллера, означающее, что контроллер был выключен или повторно инициализирован.
...
Инструментарий WMI не реализует этот класс. Сведения о классах WMI, которые являются производными от **CIM \_ пкмЦиаконтроллер**, см. в разделе [Классы Win32](win32-provider.md).
