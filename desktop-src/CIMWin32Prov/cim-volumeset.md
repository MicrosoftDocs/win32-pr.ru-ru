...
Класс **набора \_ томов CIM** представляет непрерывный диапазон логических блоков, представленных в операционной среде для чтения и записи пользовательских данных. Наборы томов не должны перекрывать друг друга и основаны на одном или нескольких физических экстентах, защищенных экстентах или агрегатных экстентах (все они одного типа). При необходимости экземпляры на основе сопоставления должны быть созданы или подклассаны.
...
Класс **\_ томов CIM** содержит эти методы.



| Метод                                                               | Описание                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Перезапуск**](reset-method-in-class-cim-volumeset.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-volumeset.md) | Определяет требуемое состояние электропитания для логического устройства, а также когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
...
Класс **\_ тома CIM** имеет следующие свойства.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| Volume Set \| 001,5 ")
</dt> </dl>

Количество экстентов защищенного пространства, которое можно распределить как совокупный набор. В SCC это значение определяется как число полос, подлежащих подсчету, прежде чем продолжить соотнесение со следующим непрерывным набором экстентов, за пределами текущей полосы.

Дополнительные сведения об использовании значений **UInt64** в скриптах см. [в разделе Создание сценариев в WMI](/windows/desktop/WmiSdk/creating-a-wmi-script).

</dd> <dt>

**псекстентстрипеленгс**
</dt> <dd> <dl> <dt>
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| Volume Set \| 001,4 ")
</dt> </dl>

Количество смежных областей защищенного пространства, подсчитанных до возвращения в первую область защищенного пространства текущей полосы. Это количество экстентов, формирующих полосу данных пользователя.
