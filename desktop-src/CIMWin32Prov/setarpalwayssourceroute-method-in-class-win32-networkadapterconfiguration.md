...
ms.locfileid: "119752714"
---
# <a name="setarpalwayssourceroute-method-of-the-win32_networkadapterconfiguration-class"></a>Метод Сетарпалвайссаурцерауте \_ класса Win32 NetworkAdapterConfiguration

Статический метод класса **сетарпалвайссаурцерауте** [WMI](/windows/desktop/WmiSdk/retrieving-a-class) используется для настройки передачи запросов ARP по протоколу TCP/IP.
...
*Арпалвайссаурцерауте* \[ окне\]
</dt> <dd>

Если задано **значение true**, TCP/IP вынужден ПЕРЕДАВАТЬ запросы ARP с включенной маршрутизацией источника в сетях Token Ring. По умолчанию стек передает запросы ARP без маршрутизации исходного кода, а затем выполняет повторную попытку с включенной маршрутизацией источника, если ответ не получен.
...
Пример " [изменение запросов ARP для использования исходной маршрутизации](https://Gallery.TechNet.Microsoft.Com/e0298c96-acc2-4809-9365-fce3000db00e) " в коллекции TechNet использует **СЕТАРПАЛВАЙССАУРЦЕРАУТЕ** для настройки TCP/IP, чтобы всегда использовать исходную маршрутизацию во всех сетях Token Ring.
