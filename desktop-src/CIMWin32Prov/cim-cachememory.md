...
Класс **CIM \_ качемемори** определяет возможности и управление кэшовой памятью.

Кэш памяти — это выделенная или выделенная память, в которой процессор сначала выполняет поиск данных. Данные в кэш-памяти быстро доставляются процессору и обычно описываются по сходству с процессором (например, первичным или вторичным кэшем). Дисковый накопитель, который включает ОЗУ, выделенный для хранения самых последних прочитанных или соседних данных диска (для ускорения извлечения), также моделируется как кэш-память.

> [!Note]  
> Кэш памяти не является буфером операционной системы или уровня приложения; это ОЗУ, выделенный для кэширования данных процессора.
...
Класс **CIM \_ качемемори** содержит следующие методы.



| Метод                                                                 | Описание                                                                                                                                |
|:-----------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------|
| [**Перезапуск**](reset-method-in-class-cim-cachememory.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                                 |
| [**SetPowerState**](setpowerstate-method-in-class-cim-cachememory.md) | Определяет требуемое состояние электропитания для логического устройства и когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
...
Класс **CIM \_ качемемори** имеет следующие свойства.
...
Описание свойств чтения и записи носителя.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -системный кэш \| 003,15 ")
</dt> </dl>

Перечисление, определяющее ассоциативность системного кэша.
...
**2-направленный набор-ассоциативный** (4)
...
**4-направленный набор-ассоциативный** (5)
...
**8-направленное множество-ассоциативное** (7)
...
**16-направленный набор-ассоциативный** (8)
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -системный кэш \| 003,9 ")
</dt> </dl>

Указывает кэширование инструкций, кэширование данных или и то, и другое. Можно также определить "Other" и "Unknown".
...
Конечный адрес, на который ссылается приложение или операционная система и который сопоставляется контроллером памяти для этого объекта памяти. Конечный адрес указывается в килобайтах.
...
Операция доступа к памяти, которая привела к последней ошибке. Тип ошибки описан в свойстве **errorInfo** . Если свойство **errorInfo** равно 3 ("ОК"), это свойство не имеет смысла.
...
Порядок данных, хранящихся в свойстве **ErrorData** . Если **еррортрансферсизе** имеет значение 0, это свойство не имеет смысла.
...
<span id="Least_Significant_Byte_First"></span><span id="least_significant_byte_first"></span><span id="LEAST_SIGNIFICANT_BYTE_FIRST"></span>**Наименьший значащий байт первым** (1)


</dt> <dd>

Сначала следует наименее значащий байт.

</dd> <dt>

<span id="Most_Significant_Byte_First"></span><span id="most_significant_byte_first"></span><span id="MOST_SIGNIFICANT_BYTE_FIRST"></span>

<span id="Most_Significant_Byte_First"></span><span id="most_significant_byte_first"></span><span id="MOST_SIGNIFICANT_BYTE_FIRST"></span>**Первый старший байт** (2)


</dt> <dd>

Первый значащий байт.
...
Тип ошибки, которая возникла в последнее время. Значения от 12 до 14 не определены в схеме CIM, так как DMI смешивает семантику типа ошибки и является ли она исправимой. Указывает, является ли ошибка исправимой, указывается в свойстве **корректаблиррор** .
...
Указывает, используются ли алгоритмы четности или CRC, ECC или другие механизмы. Также можно указать сведения об алгоритме.
...
Указывает диапазон в байтах, до которого может быть разрешена Последняя ошибка. Например, если адреса ошибок разрешаются в бит 11 (то есть на типичном уровне страницы), ошибки могут быть разрешены до 4 КБ, а значение этого свойства равно 4000. Если свойство **errorInfo** равно 3 ("ОК"), это свойство не имеет смысла.
...
Дата и время возникновения последней ошибки памяти. Тип ошибки описан в свойстве **errorInfo** . Если свойство **errorInfo** равно 3 ("ОК"), это свойство не имеет смысла.
...
Размер передачи данных в битах, которая привела к последней ошибке. Значение 0 указывает на отсутствие ошибки. Если свойство **errorInfo** равно 3 ("ОК"), то этому свойству должно быть присвоено значение 0.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| System Cache \| 003,14 "), [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) (" секунды ")
</dt> </dl>

Максимальное количество времени (в секундах), в течение которого изменившиеся строки или контейнеры могут остаться в кэше перед очисткой. Значение 0 указывает, что очистка кэша не контролируется таймером записи на диск.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -системный кэш \| 003,2 ")
</dt> </dl>

Указывает, является ли это первичным, вторичным или третичным кэшем. Можно также определить «other», «Unknown» и «not применимым».
...
<span id="Primary"></span><span id="primary"></span><span id="PRIMARY"></span>**Основной** (3)
...
<span id="Secondary"></span><span id="secondary"></span><span id="SECONDARY"></span>**Вторичная** (4)
...
<span id="Tertiary"></span><span id="tertiary"></span><span id="TERTIARY"></span>**Третичная** (5)
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -системный кэш \| 003,10 "), [**единиц**](/windows/desktop/WmiSdk/standard-qualifiers) (" байт ")
</dt> </dl>

Размер (в байтах) одного контейнера или строки кэша.
...
Количество последовательных блоков, каждый из которых заблокирует размер значения, содержащегося **в свойстве, которое** формирует экстент хранения. Общий размер области хранения можно вычислить путем умножения **значения свойства размерности на значение** этого свойства. Если **значение параметра DataSize равно 1** (один), это свойство является общим размером экстента хранения.
...
Строка свободной формы, которая предоставляет сведения, если свойство **ErrorType** имеет значение 1 ("другое"). Если значение не равно 1, то эта строка не имеет смысла.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -системный кэш \| 003,13 ")
</dt> </dl>

Политика, применяемая кэшем для обработки запросов на чтение. Если политика чтения определяется отдельно, то есть для каждого запроса, следует указать значение 6 ("определение на операцию ввода-вывода"). Значения "Other" и "Unknown" также являются допустимыми.
...
<span id="Read"></span><span id="read"></span><span id="READ"></span>**Чтение** (3)
...
<span id="Read-Ahead"></span><span id="read-ahead"></span><span id="READ-AHEAD"></span>**Упреждающее чтение** (4)
...
<span id="Read_and_Read-Ahead"></span><span id="read_and_read-ahead"></span><span id="READ_AND_READ-AHEAD"></span>**Чтение и упреждающее** чтение (5)


</dt> <dd>

Чтение и чтение с опережением.

</dd> <dt>

<span id="Determination_Per_I_O"></span><span id="determination_per_i_o"></span><span id="DETERMINATION_PER_I_O"></span>

<span id="Determination_Per_I_O"></span><span id="determination_per_i_o"></span><span id="DETERMINATION_PER_I_O"></span>**Определение для операций ввода-вывода** (6)


</dt> <dd>

Определение для операций ввода-вывода.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -системный кэш \| 003,12 ")
</dt> </dl>

Целочисленное перечисление, описывающее алгоритм, который определяет, какие строки или контейнеры кэша следует использовать повторно.
...
<span id="Least_Recently_Used__LRU_"></span><span id="least_recently_used__lru_"></span><span id="LEAST_RECENTLY_USED__LRU_"></span>**Наименее недавно использованный (LRU)** (3)
...
<span id="First_In_First_Out__FIFO_"></span><span id="first_in_first_out__fifo_"></span><span id="FIRST_IN_FIRST_OUT__FIFO_"></span>**Первым поступил (FIFO)** (4)


</dt> <dd>

Первым поступил, первым обслужен (FIFO).

</dd> <dt>

<span id="Last_In_First_Out__LIFO_"></span><span id="last_in_first_out__lifo_"></span><span id="LAST_IN_FIRST_OUT__LIFO_"></span>

<span id="Last_In_First_Out__LIFO_"></span><span id="last_in_first_out__lifo_"></span><span id="LAST_IN_FIRST_OUT__LIFO_"></span>**Last-First Out (ЛИФО)** (5)


</dt> <dd>

Последнее пошаговое извлечение (ЛИФО).
...
<span id="Least_Frequently_Used__LFU_"></span><span id="least_frequently_used__lfu_"></span><span id="LEAST_FREQUENTLY_USED__LFU_"></span>**Наименее часто используемые (ЛФУ)** (6)


</dt> <dd>

Наименее часто используемые (ЛФУ.)

</dd> <dt>

<span id="Most_Frequently_Used__MFU_"></span><span id="most_frequently_used__mfu_"></span><span id="MOST_FREQUENTLY_USED__MFU_"></span>

<span id="Most_Frequently_Used__MFU_"></span><span id="most_frequently_used__mfu_"></span><span id="MOST_FREQUENTLY_USED__MFU_"></span>**Наиболее часто используемые (** 7)


</dt> <dd>

Наиболее часто используемые (наиболее популярные программные).
...
<span id="Data_Dependent_Multiple_Algorithms"></span><span id="data_dependent_multiple_algorithms"></span><span id="DATA_DEPENDENT_MULTIPLE_ALGORITHMS"></span>**Несколько алгоритмов, зависящих от данных** (8)


</dt> <dd>

Зависящие от данных множественные алгоритмы.
...
Начальный адрес, на который ссылается приложение или операционная система, сопоставленный контроллером памяти для этого объекта памяти. Начальный адрес указывается в килобайтах.
...
Указывает, является ли информация об адресе в свойстве **еррораддресс** адресом системного уровня (**true**) или физическим адресом (**false**). Если свойство **errorInfo** равно 3 ("ОК"), это свойство не имеет смысла.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. DMTF \| -системный кэш \| 003,5 ")
</dt> </dl>

Определяет, производится ли в кэше обратная запись или запись в кэш, а также сведения об отдельности для каждого ввода-вывода. Можно также указать "Other" и "Unknown".
...
<span id="Write_Back"></span><span id="write_back"></span><span id="WRITE_BACK"></span>**Обратная запись** (3)
...
<span id="Write_Through"></span><span id="write_through"></span><span id="WRITE_THROUGH"></span>Сквозная **запись** (4)
...
<span id="Varies_with_Address"></span><span id="varies_with_address"></span><span id="VARIES_WITH_ADDRESS"></span>**Зависит от адреса** (5)
...
Инструментарий WMI не реализует этот класс. Дополнительные сведения о классах, производных от **CIM \_ качемемори**, см. в разделе [Классы Win32](win32-provider.md).
