...
Статический метод класса **сетднссуффикссеарчордер** [WMI](/windows/desktop/WmiSdk/retrieving-a-class) использует массив строковых элементов для задания порядка поиска суффиксов.
...
*Днсдомаинсуффикссеарчордер* \[ окне\]
</dt> <dd>

Список суффиксов сервера для запроса DNS-серверов. В реестре DNS-суффикса находится путь к папке **hKey \_ Local \_ Machine** \\ **System** \\ **CurrentControlSet** \\ **Services** \\ **tcpip \| nameserver** .

Пример: "suffix1.company.com", "suffix2.company.com"
...
Не все аренды DHCP освобождаются или возобновляются.
