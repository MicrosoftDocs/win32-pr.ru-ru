...
---
# <a name="uncompressex-method-of-the-cim_devicefile-class"></a>Метод Ункомпрессекс \_ класса CIM девицефиле

Метод **ункомпрессекс** распаковывает файл логического устройства (или каталог), указанный в пути объекта. Этот метод является расширенной версией метода [**uncompressия**](uncompress-method-in-class-cim-devicefile.md) . Этот метод наследуется от [**CIM \_ LogicalFile**](cim-logicalfile.md).
...
Строка, представляющая дочерний файл (или каталог), используемый в качестве отправной точки для метода. Как правило, параметр *StartFileName* — это параметр *стопфиленаме* , указывающий файл или каталог, в котором произошла ошибка из предыдущего вызова метода. Если этот параметр имеет **значение NULL**, операция выполняется над файлом или каталогом, указанным в вызове метода [**ExecMethod**](/windows/desktop/WmiSdk/swbemservices-execmethod) .
...
Если **значение — true**, этот метод также применяется рекурсивно к файлам и каталогам в каталоге, указанном в экземпляре [**CIM \_ девицефиле**](cim-devicefile.md) . Для экземпляров файлов этот параметр игнорируется.
...
[\_ДЕВИЦЕФИЛЕ CIM](uncompressex-method-in-class-cim-devicefile.md)
</dt> <dt>
