...
ms.locfileid: "119439884"
---
# <a name="setmtu-method-of-the-win32_networkadapterconfiguration-class"></a>Метод Сетмту \_ класса Win32 NetworkAdapterConfiguration

Статический метод класса **сетмту** [WMI](/windows/desktop/WmiSdk/retrieving-a-class) используется для установки максимального значения MTU для сетевого интерфейса по умолчанию.
...
Максимальный размер блока передачи (MTU) для сетевого интерфейса по умолчанию. Диапазон этого значения охватывает минимальный размер пакета (68) до MTU, поддерживаемый базовой сетью.
...
Успешное завершение. Потребуется перезагрузка компьютера.
...
MTU — это максимальный размер пакета (в байтах), передаваемого транспортом в базовой сети. Размер включает заголовок транспорта.

Обратите внимание, что IP-датаграмма может охватывать несколько пакетов. Значения, превышающие значение по умолчанию для базового результата сети в транспортном потоке, с использованием MTU по умолчанию сети. Значения меньше 68 приводят к транспортировке с использованием MTU 68.

## <a name="examples"></a>Примеры

В примере [изменения MTU для всех сетевых адаптеров](https://Gallery.TechNet.Microsoft.Com/49c26363-d46c-4288-9c8d-feb0a1982998) на языке VBScript настраивается максимальная единица передачи для всех сетевых адаптеров, установленных на компьютере.
