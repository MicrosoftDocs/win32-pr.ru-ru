...
---
# <a name="cim_logicaldevice-class-cimwin32-wmi-providers"></a>Класс CIM_LogicalDevice (поставщики WMI CIMWin32)

Класс **CIM \_** -класса представляет собой аппаратную сущность, которая может быть или не реализована в физическом оборудовании.
...
| Метод                                                                   | Описание                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Перезапуск**](reset-method-in-class-cim-logicaldevice.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-logicaldevice.md) | Определяет требуемое состояние электропитания для логического устройства, а также когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
...
Поддерживается метод **SetPowerState** . Этот метод находится в родительском классе **класса \_ CIM** и может быть реализован. Дополнительные сведения см. в разделе [Конструирование классов MOF-файл (MOF)](/windows/desktop/WmiSdk/designing-managed-object-format--mof--classes).
...
Характеристики логического устройства, которые управляют операцией или конфигурацией, содержатся в объекте **CIM \_** , или связываются с ним. Например, в качестве рабочих свойств принтера поддерживаются размеры бумаги или обнаруженные ошибки. Например, свойства конфигурации устройства датчика являются пороговыми значениями. Для логического устройства могут существовать различные конфигурации, которые содержатся в объектах [**\_ параметров CIM**](cim-setting.md) , связанных с логическим устройством.

Класс **CIM \_** -класса является производным от [**CIM \_ логикалелемент**](cim-logicalelement.md).

Инструментарий WMI не реализует этот класс. Классы, производные **от \_ CIM**"классического класса", см. в разделе [Классы Win32](win32-provider.md).
