...
ms.locfileid: "119021032"
---
# <a name="cim_networkadapter-class"></a>\_Класс CIM сетевого адаптера

Класс **CIM \_ сетевого адаптера** является абстрактным классом, который определяет основные понятия сетевого оборудования (например, постоянный адрес или скорость работы). Сведения передаются с помощью ассоциации [**CIM \_ девицесапимплементатион**](cim-devicesapimplementation.md) .

Сетевые адаптеры и связанные конечные точки представляют потенциальную возможность подключения между одноранговыми узлами. Возможность подключения существенно отличается от главного или подчиненного контроллера, а также от контроллеров [**CIM \_**](cim-controller.md)или управляемых отношений. Однако иногда одно устройство действует как сетевой адаптер, так и контроллер, например, когда адаптер волоконного канала работает в качестве контроллера SCSI компьютерной системы. В этом случае аспекты устройства ориентированы на сеть, а другие являются ориентированными на контроллер. Для связывания различных аспектов устройства необходимо создать экземпляр контроллера и классов адаптеров, а также связь удостоверения устройства.
...
Класс **CIM \_ сетевого адаптера** имеет следующие типы членов:
...
Класс **CIM \_ сетевого адаптера** содержит следующие методы.



| Метод                                                                    | Описание                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Перезапуск**](reset-method-in-class-cim-networkadapter.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | Определяет требуемое состояние электропитания для логического устройства, а также когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
...
Класс **CIM \_ сетевого адаптера** имеет следующие свойства.
...
Если **значение — true**, сетевой адаптер может автоматически определить скорость или другие характеристики связи подключенного сетевого носителя.
...
Максимальная скорость в битах в секунду (бит/с) для сетевого адаптера.
...
Список сетевых адресов для адаптера.
...
Сетевой адрес жестко закодирован в адаптере. Жестко заданный адрес можно изменить с помощью обновления встроенного по или конфигурации программного обеспечения. В случае изменения поле должно быть обновлено после внесения изменений. Если для сетевого адаптера не существует жестко заданный адрес, это свойство следует оставить пустым.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| RFC1213-MIB. ифспид "," MIF. DMTF \| , сетевой адаптер 802 \| , порт 001,5 "), [**единиц**](/windows/desktop/WmiSdk/standard-qualifiers) (" бит в секунду ")
</dt> </dl>

Оценка текущей пропускной способности в бит/с. Для конечных точек, которые зависят от пропускной способности или для тех, для которых не может быть выполнена точная оценка, это свойство должно содержать номинальную пропускную способность.
...
Класс **CIM \_ сетевого адаптера** является производным от [**CIM \_**](cim-logicaldevice.md)-класса.

Инструментарий WMI не реализует этот класс. Классы, производные от **CIM \_ сетевого адаптера**, см. в разделе [Классы Win32](win32-provider.md).
