...
Метод **такеовнершипекс** получает владение логическим файлом данных, указанным в пути к объекту. Этот метод является расширенной версией метода [**такеовнершип**](takeownership-method-in-class-cim-datafile.md) и наследуется от [**CIM \_ LogicalFile**](cim-logicalfile.md). Если логический файл является каталогом, то этот метод будет работать рекурсивно, перепринимая владение всеми файлами и подкаталогами, которые содержит каталог.
...
Имя файла (или каталога), в котором произошел сбой метода. Если метод завершится с ошибкой, этот параметр будет иметь значение null.
...
[\_Файл CIM](takeownershipex-method-in-class-cim-datafile.md)
</dt> <dt>
