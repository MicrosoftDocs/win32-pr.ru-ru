...
| [**сетипксфраметипенетворкпаирс**](win32-networkadapterconfiguration-setipxframetypenetworkpairs.md)                         | задает пары номеров или кадров сети ip-Exchange пакетов (IPX) для этого сетевого адаптера.<br/>                                            |
| [**сетипксвиртуалнетворкнумбер**](win32-networkadapterconfiguration-setipxvirtualnetworknumber.md)                           | задает номер виртуальной сети пакета Exchange (IPX) на целевом компьютере.<br/>                                       |
...
| [**сетвинссервер**](setwinsserver-method-in-class-win32-networkadapterconfiguration.md)                                     | задает основной и дополнительный Windows серверы интернета Служба именования (WINS) в этом сетевом адаптере, привязанном к TCP/IP.<br/>                        |
...
допустимый Windows путь к файлам стандартных баз данных интернета (узлы, lmhosts, сети и протоколы). путь к файлу используется интерфейсом сокетов Windows.
...
если **значение — TRUE**, система доменных имен (DNS) включает разрешение имен по сравнению с разрешением Windows Internet Служба именования (WINS). Если имя не удается разрешить с помощью DNS, запрос имени перенаправляется в WINS для разрешения.
...
номер индекса Windows конфигурации сетевого адаптера. Номер индекса используется, если доступно более одной конфигурации.
...
квалификаторы: не [**рекомендуется**](../wmisdk/standard-wmi-qualifiers.md), [**маппингстрингс**](../wmisdk/standard-qualifiers.md) ("Win32API \| Windows сокеты версии 2 \| [**жетсоккопт**](/windows/win32/api/winsock/nf-winsock-getsockopt) \| IPX \_ -адрес")
</dt> </dl>

технология Exchange IPX больше не поддерживается, и это свойство не содержит полезных данных.
...
число попыток TCP повторно передать запрос Подключение перед завершением соединения. Начальное время ожидания повторной передачи — 3 секунды. Время ожидания повторной передачи удваивается для каждой попытки. Значение по умолчанию: 3, допустимый диапазон: 0 – 0xFFFFFFFF.
...
квалификаторы: [**маппингстрингс**](../wmisdk/standard-qualifiers.md) ("Win32API \| Сведения о системе функции \| [**GetSystemDirectory**](/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemdirectorya) \| \\ \\ драйверы \\ \\ и т. д. \\ \\ )
</dt> </dl>
