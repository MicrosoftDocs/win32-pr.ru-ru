...
Класс **CIM \_ копифилеактион** представляет перемещение или копирование файлов из компьютерной системы в новое место.

Сведения о расположении для копирования задаются с помощью ассоциаций [**CIM \_ ТодиректориспеЦификатион**](cim-todirectoryspecification.md) и [**CIM \_ фромдиректориспеЦификатион**](cim-fromdirectoryspecification.md) , а также ассоциаций [**CIM \_ тодиректоряктион**](cim-todirectoryaction.md) и [**CIM \_ фромдиректоряктион**](cim-fromdirectoryaction.md) . Первый набор используется, когда исходный или целевой объект должен существовать до выполнения каких-либо действий. Второй набор используется, когда исходный или целевой объект создается как часть предыдущего действия. В последнем случае действие для создания каталога должно быть выполнено до объекта **CIM \_ копифилеактион** .
...
Класс **CIM \_ копифилеактион** имеет следующие типы членов:
...
Класс **CIM \_ копифилеактион** содержит следующие методы.
...
Класс **CIM \_ копифилеактион** имеет следующие свойства.
...
Уникальный идентификатор, назначенный конкретному действию для программного элемента.
...
Если **значение — true**, исходный файл удаляется после операции копирования.
...
Указывает, является ли конкретный объект [**\_ действия CIM**](cim-action.md) частью последовательности действий, чтобы перевести текущий программный элемент в следующее состояние, например "установить", или удалить текущий программный элемент, например "Удалить".
...
Определяет программный элемент.
...
Инструментарий WMI не реализует этот класс. Дополнительные сведения о классах, производных от **CIM \_ копифилеактион**, см. в разделе [Классы Win32](win32-provider.md).
