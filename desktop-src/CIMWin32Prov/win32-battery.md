...
ms.locfileid: "119917994"
---
# <a name="win32_battery-class"></a>\_Класс аккумулятора Win32

[Класс WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ батареи Win32** представляет батарею, подключенную к компьютерной системе.
...
Класс **\_ аккумулятора Win32** имеет следующие типы членов:
...
Класс **\_ аккумулятора Win32** содержит следующие методы.



| Метод            | Описание                                                                                                                                                                                          |
|:------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Сброс**         | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**Reset**](reset-method-in-class-cim-controller.md) в [**\_ батарее CIM**](cim-battery.md) .<br/>                 |
| **SetPowerState** | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) в разделе [**\_ аккумулятора CIM**](cim-battery.md) .<br/> |
...
Класс **\_ аккумулятора Win32** имеет следующие свойства.
...
Квалификаторы: не [**рекомендуется**](/windows/desktop/WmiSdk/standard-wmi-qualifiers), [**Маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("hKey \_ Local \_ Machine \\ \\ System \\ \\ CurrentControlSet \\ \\ Services \| речаржерате"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("минуты")
</dt> </dl>

Время, необходимое для полной зарядки аккумулятора. Данное свойство не поддерживается. **Баттериречаржетиме** не имеет свойства замены и теперь считается устаревшим.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Портативная батарея DMTF \| 002,14 ")
</dt> </dl>

Состояние аккумулятора. Значение 10 (undefine) недопустимо в схеме CIM, так как в DMI оно представляет, что батарея не установлена. В этом случае не следует создавать экземпляры объекта.

Это свойство наследуется [**от \_ аккумулятора CIM**](cim-battery.md).
...
Система имеет доступ к AC, поэтому батарея не взимается. Однако батарея не обязательно заряжается.

</dd> <dt>

<span id="Fully_Charged"></span><span id="fully_charged"></span><span id="FULLY_CHARGED"></span>

<span id="Fully_Charged"></span><span id="fully_charged"></span><span id="FULLY_CHARGED"></span>**Полностью заряжена** (3)
...
<span id="Low"></span><span id="low"></span><span id="LOW"></span>**Низкий** (4)
...
<span id="Charging"></span><span id="charging"></span><span id="CHARGING"></span>**Зарядка** (6)
...
<span id="Charging_and_High"></span><span id="charging_and_high"></span><span id="CHARGING_AND_HIGH"></span>**Зарядка и высокая** (7)
...
<span id="Charging_and_Low"></span><span id="charging_and_low"></span><span id="CHARGING_AND_LOW"></span>**Зарядка и низкая** (8)
...
<span id="Charging_and_Critical"></span><span id="charging_and_critical"></span><span id="CHARGING_AND_CRITICAL"></span>**Зарядка и критическая** ошибка (9)
...
<span id="Undefined"></span><span id="undefined"></span><span id="UNDEFINED"></span>Не **определено** (10)
...
<span id="Partially_Charged"></span><span id="partially_charged"></span><span id="PARTIALLY_CHARGED"></span>**Частично заряжена** (11)
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Портативная батарея DMTF \| 002,7 ")
</dt> </dl>

Перечисление, описывающее химия батареи.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Портативная батарея DMTF \| 002,8 "), [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) (" диаграмме милливаттах-hours ")
</dt> </dl>

Разработайте емкость аккумулятора в диаграмме милливаттах-hours. Если свойство не поддерживается, введите 0 (ноль).
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Переносная батарея DMTF \| 002,9 "), [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) (" милливольтах ")
</dt> </dl>
...
Произвольная строка, которая предоставляет дополнительные сведения об ошибке, записанной в свойстве **ластерроркоде** , и сведения о любых корректирующих действиях, которые могут быть предприняты.

Это свойство наследуется [**от \_ CIM**](cim-logicaldevice.md)-унаследованной модели.

</dd> <dt>

**естиматедчаржеремаининг**
</dt> <dd> <dl> <dt>
...
Квалификаторы: [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) ("процент")
</dt> </dl>

Оценка оставшегося процента от полной зарядки.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Портативная батарея DMTF \| 002,15 "), [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) (" минуты ")
</dt> </dl>

Оцените в минутах время, в течение которого заряд батареи заряжается при условиях текущей нагрузки, если работа программы отключена или потеряна и не работает, или ноутбук отключен от источника питания.

Это свойство наследуется [**от \_ аккумулятора CIM**](cim-battery.md).

</dd> <dt>

**експектедбаттерилифе**
</dt> <dd> <dl> <dt>
...
Квалификаторы: не [**рекомендуется**](/windows/desktop/WmiSdk/standard-wmi-qualifiers), [**Маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("hKey \_ Local \_ Machine \\ \\ System \\ \\ CurrentControlSet \\ \\ Services \| баттерилифе"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("минуты")
</dt> </dl>

Время, необходимое для полной очистки аккумулятора после полной оплаты. Это свойство больше не используется и считается устаревшим.
...
Ожидаемое время существования аккумулятора в минутах, предполагая, что батарея полностью заряжена. Свойство представляет общее ожидаемое время работы батареи, а не текущий оставшийся срок жизни, которое указывается свойством **EstimatedRunTime** .
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Портативная батарея DMTF \| 002,11 "), [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) (" диаграмме милливаттах-hours ")
</dt> </dl>

Полная плата за батарею в диаграмме милливаттах-часах. Сравнение значения со свойством **десигнкапаЦити** определяет, когда батарея требует замены. Окончание работы батареи обычно происходит, когда свойство **фуллчаржекапаЦити** становится менее 80% от свойства **десигнкапаЦити** . Если свойство не поддерживается, введите 0 (ноль).
...
Максимальное время (в минутах) для полной зарядки аккумулятора. Свойство представляет время, которое будет выплатить полностью исчерпанный аккумулятор, а не текущее оставшееся время, которое указывается в свойстве **тиметофуллчарже** .
...
Определяет метку, по которой известен объект. При создании подклассов свойство может быть переопределено как ключевое свойство.
...
Квалификаторы: [**maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (64), [**Маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Портативная батарея DMTF \| 002,10 ")
</dt> </dl>

Номер версии спецификации данных, поддерживаемый батареей. Если батарея не поддерживает эту функцию, значение должно остаться пустым.
...
Квалификаторы: [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) ("секунды")
</dt> </dl>

Время в секундах, прошедшее с момента последнего переключения ИБП компьютера на питание от аккумулятора или с момента последнего перезапуска системы или ИБП, в зависимости от того, какое значение меньше. Если батарея находится в строке, возвращается значение 0 (нуль).
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Портативная батарея DMTF \| 002,16 "), [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) (" минуты ")
</dt> </dl>

Оставшееся время для полной зарядки аккумулятора в минутах по текущей ставке и использованию.
...
Класс **\_ батареи Win32** является производным от [**\_ аккумулятора CIM**](cim-battery.md) , который является производным от CIM-класса. [**\_**](cim-logicaldevice.md)

Windows Сервер 2008 содержит драйверы ИБП (APC) в ОС, что позволяет рассматривать ИБП как питание от аккумулятора. Это позволяет отслеживать состояние ИБП с помощью сценария и предпринимать необходимые действия.

## <a name="examples"></a>Примеры

[Toggle-Wireless.ps1](https://Gallery.TechNet.Microsoft.Com/Toggle-Wirelessps1-2d244a8f) пример кода PowerShell запрашивает **\_ батарею Win32** , чтобы определить, следует ли переключать беспроводную сеть для экономии энергии.

В списке примеров Perl для [получения сведений](https://Gallery.TechNet.Microsoft.Com/7196121e-97de-4290-9939-26d0ce266270) о источниках питания, подключенных к компьютеру.
