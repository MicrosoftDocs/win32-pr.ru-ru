...
[Класс WMI](../wmisdk/retrieving-a-class.md) **\_ пнпентити для Win32** представляет свойства устройства Самонастраивающийся. Самонастраивающийся сущности отображаются как записи в диспетчер устройств, расположенном в панели управления.
...
Класс **Win32 \_ пнпентити** содержит следующие методы.



| Метод                                                             | Описание                                                                                                                                                                                    |
|:-------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**Отключить**](disable-win32-pnpentity.md)                         | Отключает это устройство самонастраивающийся.<br/>                                                                                                                                                 |
| [**Включить**](enable-win32-pnpentity.md)                           | Включает это самонастраивающийся устройство.<br/>                                                                                                                                                  |
| [**жетдевицепропертиес**](getdeviceproperties-win32-pnpentity.md) | Возвращает указанные свойства данного устройства самонастраивающийся.<br/>                                                                                                                         |
| **Сброс**                                                          | Не реализован. Чтобы реализовать этот метод, см. метод [**Reset**](reset-method-in-class-cim-controller.md) в [**CIM \_**](cim-logicaldevice.md)с параметром.<br/>                 |
| **SetPowerState**                                                  | Не реализован. Чтобы реализовать этот метод, см. метод [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) в [**CIM \_**](cim-logicaldevice.md)с параметром.<br/> |
...
Класс **Win32 \_ пнпентити** имеет следующие свойства.
...
Глобальный уникальный идентификатор (GUID) этого устройства самонастраивающийся.
...
Определяемая поставщиком строка идентификации, используемая программой установки для сопоставления устройства с INF-файлом. У устройства может быть список совместимых кодов, связанных с ним. Совместимые идентификаторы должны быть перечислены в порядке убывания пригодности. Если программе установки не удается выполнить поиск INF-файла, соответствующего одному из идентификаторов оборудования устройства, для поиска INF-файла используются совместимые идентификаторы. Совместимый идентификатор имеет тот же формат, что и **хардвареид**. Дополнительные сведения см. в разделе [Windows Driver Kit](https://developer.microsoft.com/windows/hardware/download-kits-windows-hardware-development).
...
Идентификатор устройства самонастраивающийся.
...
Определяемая поставщиком строка идентификации, используемая программой установки для сопоставления устройства с INF-файлом. Как правило, устройство имеет связанный список идентификаторов оборудования. Исключением является драйвер шины 1394, который не использует идентификаторы оборудования. Первым ИДЕНТИФИКАТОРом оборудования в списке должен быть идентификатор устройства. Остальные идентификаторы должны быть перечислены в порядке убывания пригодности.

Идентификаторы оборудования отображаются в одном из следующих форматов:

-   Перечислитель перечислителя \\ -Специальный-Device-ID

    Это наиболее распространенный формат для отдельных устройств PnP. Примером перечислителя может быть BIOS или ISAPNP.

-   \*Идентификатор, зависящий от перечислителя

    Звездочка ( \* ) указывает на использование более чем одним перечислителем.
...
<dl> <dd>корневой \\ \* PNPOF08</dd> <dd>PC \\ типов ven \_ 1000&DEV \_ 001&СУБСИС \_ 00000000&версия \_ 02</dd> </dl>

Дополнительные сведения см. в подразделе [Windows Driver Kit](https://developer.microsoft.com/windows/hardware/download-kits-windows-hardware-development).
...
Имя производителя устройства самонастраивающийся.
...
> [!WARNING]
> Это свойство, несмотря на то, что указано в MOF-файле, фактически не существует в классе. Это свойство описывается только в целях полноты, а также для уточнения самого MOF-файла.

 

Имя типа этого самонастраивающийся устройства.
...
Поддерживается метод **SetPowerState** . Этот метод находится в родительском классе [**класса \_ CIM**](cim-logicaldevice.md) и может быть реализован. Дополнительные сведения см. в разделе [Конструирование классов MOF-файл (MOF)](../wmisdk/designing-managed-object-format--mof--classes.md).
...
Является ли данное устройство самонастраивающийся в системе в данный момент.
...
Имя службы, которая поддерживает это устройство самонастраивающийся. Дополнительные сведения см. в разделе [**Win32 \_ системдриверпнпентити**](win32-systemdriverpnpentity.md).
...
Пример [Get-BrokenHardware.ps1](https://Gallery.TechNet.Microsoft.Com/dbb678f4-b95b-45c3-bc8b-2ae2d052448e) PowerShell в коллекции TechNet использует для получения списка нерабочего оборудования, использующего инструментарий WMI, в **Win32 \_ пнпентити** .

Следующий пример кода VBScript подключается к группе удаленных компьютеров в том же домене, создавая массив имен удаленных компьютеров, а затем отображая имена самонастраивающийся устройств — экземпляры Win32 \_ пнпентити — на каждом компьютере.
...
[Подключение к инструментарию WMI на удаленном компьютере](../wmisdk/connecting-to-wmi-on-a-remote-computer.md)
</dt> <dt>
