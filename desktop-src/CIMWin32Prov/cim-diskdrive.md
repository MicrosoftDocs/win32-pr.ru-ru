...
Класс **CIM \_ дискдриве** содержит следующие методы.
...
Класс **CIM \_ дискдриве** имеет следующие свойства.
...
Устройство не работает, но его можно быстро включить в полный режим.
...
Массив строк произвольной формы, которые содержат подробные пояснения для доступа к функциям устройств, указанным в массиве **capabilities** . Это свойство наследуется от [**CIM \_ медиаакцессдевице**](cim-mediaaccessdevice.md).
...
Если **значение равно true**, то ошибка, указанная в свойстве **ластерроркоде** , удаляется.
...
Произвольная строка, описывающая тип обнаружения и исправления ошибок, поддерживаемых устройством.
...
Если устройство доступа к носителю поддерживает несколько отдельных носителей, это свойство определяет максимальное число, которое может быть поддерживается или вставлено.
...
Поддерживается метод [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) .
...
Метод [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) может быть вызван с параметром *PowerState* , установленным в значение 5 ("цикл электропитания").
...
Метод [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) может быть вызван с параметром *PowerState* , установленным в значение 5 ("цикл электропитания"), и параметром *времени* , установленным на определенную дату и время (или интервал) для включения питания.
...
Инструментарий WMI не реализует этот класс. См. раздел [Классы Win32](win32-provider.md) для классов, производных от **CIM \_ дискдриве**.
