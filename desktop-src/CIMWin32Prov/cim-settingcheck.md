...
Класс **CIM \_ сеттингчекк** определяет сведения, необходимые для проверки конкретного файла параметров для конкретной записи, которая содержит значение, равное указанному значению. Предполагается, что все сравнения не чувствительны к регистру.
...
Класс **CIM \_ сеттингчекк** имеет следующие типы членов:
...
Класс **CIM \_ сеттингчекк** содержит следующие методы.
...
Класс **CIM \_ сеттингчекк** имеет следующие свойства.
...
Идентификатор, используемый в сочетании с другими ключами для уникальной идентификации проверки. Это свойство наследуется [**от \_ проверки CIM**](cim-check.md).
...
Если **значение равно true**, то условие должно существовать в среде (например, если файл находится в системе, метод [**Invoke**](invoke-method-in-class-cim-settingcheck.md) должен возвращать **значение true**). Если **значение равно false**, условие не должно существовать (например, если файл не находится в системе, метод **Invoke** должен возвращать **значение false**).
...
Значение, связанное с именованной записью для проверки.
...
Имя файла параметров для проверки.
...
Имя, используемое для распознавания программного элемента. Это свойство наследуется [**от \_ проверки CIM**](cim-check.md).
...
Ключ раздела, содержащий параметры для проверки.
