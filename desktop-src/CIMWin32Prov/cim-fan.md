...
---
# <a name="cim_fan-class"></a>\_Класс вентилятора CIM

Класс **\_ вентилятора CIM** представляет возможности и управление устройством охлаждения вентилятора.
...
Класс **\_ вентилятора CIM** имеет следующие типы членов:
...
Класс **\_ вентилятора CIM** имеет следующие методы.



| Метод                                                         | Описание                                                                                                                              |
|:---------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Перезапуск**](reset-method-in-class-cim-fan.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-fan.md) | Определяет требуемое состояние электропитания для логического устройства, а также когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
| [**сетспид**](setspeed-method-in-class-cim-fan.md)           | Задает скорость вращения вентилятора. Не реализовано инструментарием WMI.<br/>                                                                                   |
...
Класс **\_ вентилятора CIM** имеет эти свойства.
...
Запрошенная скорость вентилятора, определенная в революции в минуту, при поддержке вентилятора с переменной скоростью. Текущая скорость определяется датчиком ([**CIM \_ тахометр**](cim-tachometer.md)), связанным с вентилятором с помощью отношения [**CIM \_ ассоЦиатедсенсор**](cim-associatedsensor.md) .
...
Класс **\_ вентилятора CIM** является производным от [**CIM \_ кулингдевице**](cim-coolingdevice.md).

Инструментарий WMI не реализует этот класс. Классы, производные **от \_ вентилятора CIM**, см. в разделе [Классы Win32](win32-provider.md).
