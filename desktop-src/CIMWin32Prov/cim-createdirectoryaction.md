...
Класс **CIM \_ креатедиректоряктион** создает пустые каталоги для программных элементов, которые должны быть установлены локально.
...
Класс **CIM \_ креатедиректоряктион** содержит следующие методы.
...
Имя каталога, к которому применяется действие.

Это свойство наследуется от [**CIM \_ директоряктион**](cim-directoryaction.md).
...
Инструментарий WMI не реализует этот класс. Классы, производные от **CIM \_ креатедиректоряктион**, см. в разделе [Классы Win32](win32-provider.md).
