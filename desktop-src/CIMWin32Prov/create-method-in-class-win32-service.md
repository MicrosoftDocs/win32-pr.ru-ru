...
Запрошенный управляющий код не может быть отправлен в службу, так как состояние службы (свойство **State** класса [**Win32 \_ басесервице**](win32-baseservice.md) ) равно 0, 1 или 2.
...
Метод **CREATE** аналогичен методу [**Change**](change-method-in-class-win32-service.md) . В обоих случаях свойства службы передаются в качестве параметров в метод. Как и в случае с параметрами, используемыми в методе **Change** , порядок, в котором передаются эти параметры, очень важен.
...
Следующий сценарий VBScript устанавливает службу с именем Дбсервице
