...
[Класс WMI](../wmisdk/retrieving-a-class.md) **\_ UserAccount для Win32** содержит сведения об учетной записи пользователя в компьютерной системе под Windows.
...
Учетная запись компьютера для компьютера под Windows, который является членом этого домена.
