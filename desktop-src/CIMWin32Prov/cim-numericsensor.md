...
ms.locfileid: "119820934"
---
# <a name="cim_numericsensor-class"></a>\_Класс CIM NumericSensor

Класс **CIM \_ NumericSensor** представляет числовой датчик, который возвращает числовые показания и при необходимости поддерживает параметры пороговых значений.
...
Класс **CIM \_ NumericSensor** имеет следующие типы членов:
...
Класс **CIM \_ NumericSensor** имеет следующие методы.



| Метод                                                                   | Описание                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Перезапуск**](reset-method-in-class-cim-numericsensor.md)                 | Запрашивает сброс логического устройства. Не реализовано инструментарием WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-numericsensor.md) | Определяет требуемое состояние электропитания для логического устройства, а также когда устройство должно быть переведено в это состояние. Не реализовано инструментарием WMI.<br/> |
...
Класс **CIM \_ NumericSensor** имеет эти свойства.
...
Класс или подкласс, используемый при создании экземпляра. При использовании с другими ключевыми свойствами класса это свойство позволяет однозначно идентифицировать все экземпляры класса и его подклассов.
...
Если **значение — true**, то ошибка, сообщаемая в свойстве **ластерроркоде** , теперь удаляется.
...
Это свойство наследуется от **CIM \_ NumericSensor**.
...
Класс **CIM \_ NumericSensor** является производным от [**\_ датчика CIM**](cim-sensor.md).

Инструментарий WMI не реализует этот класс. Классы, производные от **CIM \_ NumericSensor**, см. в разделе [Классы Win32](win32-provider.md).
