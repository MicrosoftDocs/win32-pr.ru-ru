...
ms.locfileid: "119986354"
---
# <a name="win32_cachememory-class"></a>\_Класс Win32 качемемори

[Класс WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ качемемори для Win32** представляет внутреннюю и внешнюю кэш-память в компьютерной системе.
...
Класс **Win32 \_ качемемори** имеет следующие типы членов:
...
Класс **Win32 \_ качемемори** содержит следующие методы.



| Метод            | Описание                                                                                                                                                                                                  |
|:------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Сброс**         | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**Reset**](reset-method-in-class-cim-controller.md) в [**CIM \_ качемемори**](cim-cachememory.md) .<br/>                 |
| **SetPowerState** | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) в [**CIM \_ качемемори**](cim-cachememory.md) .<br/> |
...
Класс **Win32 \_ качемемори** имеет следующие свойства.
...
Массив октетов, содержащих дополнительные сведения об ошибке. Например, синдром ECC или возврат битовой проверки, если используется методология ошибок на основе CRC. В последнем случае, если обнаружена Одноразрядная ошибка и известен алгоритм CRC, можно определить точный бит, в котором произошел сбой. Этот тип данных (ECC синдром, бит проверки, битные данные четности или другие предоставленные поставщиком сведения) включен в это поле. Если свойство **errorInfo** равно 3 (ОК), это свойство не имеет смысла.
...
Целочисленное перечисление, определяющее ассоциативность системного кэша.

Это значение берется из элемента **ассоциативности** в структуре **сведений о кэше** в сведениях SMBIOS.

Это свойство наследуется от [**CIM \_ качемемори**](cim-cachememory.md).
...
Это значение берется из элемента **конфигурации кэша** структуры сведений о **кэше** в сведениях SMBIOS.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (" \| скорость кэша SMBIOS Type 7 \| "), [**единиц**](/windows/desktop/WmiSdk/standard-qualifiers) ("наносекундах")
</dt> </dl>

Скорость кэша.

Это значение берется из элемента **скорость кэширования** в структуре **сведений о кэше** в сведениях SMBIOS.
...
Это значение берется из члена **типа системного кэша** в структуре **сведений о кэше** в сведениях SMBIOS.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (" \| тип SMBIOS 7, \| текущий тип SRAM")
</dt> </dl>

Массив типов памяти (SRAM), используемых для кэш-памяти.

Это значение берется из **текущего элемента типа SRAM** в структуре **сведений о кэше** в сведениях SMBIOS.
...
Уникальный идентификатор кэша, представленного экземпляром **Win32 \_ качемемори**.
...
Конечный адрес, на который ссылается приложение или операционная система, сопоставленный контроллером памяти для этого объекта памяти. Конечный адрес указывается в килобайтах.
...
Операция доступа к памяти, которая привела к последней ошибке. Тип ошибки описан в свойстве **errorInfo** . Если значение **errorInfo** равно 3 (ОК), это свойство не имеет смысла.
...
Адрес последней ошибки памяти. Тип ошибки описан в свойстве **errorInfo** . Если значение **errorInfo** равно 3 (ОК), это свойство не имеет смысла.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("тип SMBIOS \| 7 типа \| исправления ошибок")
</dt> </dl>

Метод исправления ошибок, используемый кэшовой памятью.

Это значение берется из элемента **типа исправления ошибок** в структуре **сведений о кэше** в сведениях SMBIOS.
...
Массив данных, захваченных во время последней ошибочной операции доступа к памяти. Данные занимают первые *n* октетов массива, необходимых для хранения числа битов, заданных свойством **еррортрансферсизе** . Если **еррортрансферсизе** имеет значение 0 (ноль), это свойство не имеет смысла.
...
Упорядочивание данных, хранящихся в свойстве **ErrorData** . Если **еррортрансферсизе** имеет значение 0 (ноль), это свойство не имеет смысла.
...
Тип ошибки, которая возникла в последнее время. Значения 12-14 в схеме CIM не определены, поскольку в DMI они сочетают семантику типа ошибки и правильность ее выполнения. Последнее указывается в свойстве **корректаблиррор**.
...
Подробные сведения о алгоритмах четности и CRC, а также об ECC или других используемых механизмах.
...
Размер передачи данных в битах, которая привела к последней ошибке. 0 (ноль) указывает на отсутствие ошибки. Если свойство **errorInfo** равно 3 (ОК), то этому свойству должно быть присвоено значение 0 (ноль).
...
Максимальное количество времени (в секундах), в течение которого изменившиеся строки или контейнеры могут остаться в кэше перед очисткой. Значение 0 (ноль) указывает, что очистка кэша не контролируется таймером записи на диск.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (" \| \| Размер установленного SMBIOS типа 7"), [**единиц**](/windows/desktop/WmiSdk/standard-qualifiers) ("килобайтах")
</dt> </dl>

Текущий размер установленной кэш памяти.

Это значение берется из элемента **установленный размер** структуры **сведений о кэше** в сведениях SMBIOS.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (" \| тип SMBIOS 7 \| Location")
</dt> </dl>

Физическое расположение кэшовой памяти.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (" \| тип SMBIOS 7: \| максимальный размер кэша"), [**единиц**](/windows/desktop/WmiSdk/standard-qualifiers) ("килобайтах")
</dt> </dl>

Максимальный размер кэша, устанавливаемый для этого конкретного кэша памяти.

Это значение берется из параметра **максимального размера кэша** в структуре **сведений о кэше** в сведениях SMBIOS.
...
Строка произвольной формы, предоставляющая дополнительные сведения, если свойство **ErrorType** имеет значение 1, другое. В противном случае эта строка не имеет смысла.
...
Это значение берется из элемента **обозначение сокета** в структуре **сведений о кэше** в сведениях SMBIOS.
...
Политика, которая должна применяться кэшем для обработки запросов на чтение.
...
**Определение для операций ввода-вывода** (6)
...
Алгоритм для определения того, какие строки или контейнеры кэша следует использовать повторно.
...
**Наименее недавно использованный (LRU)** (3)
...
**Наименее часто используемые (ЛФУ)** (6)
...
**Несколько алгоритмов, зависящих от данных** (8)
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (" \| тип SMBIOS 7 \| поддерживаемый тип SRAM")
</dt> </dl>

Массив поддерживаемых типов статических оперативных памяти (SRAM), которые могут использоваться для кэш-памяти.

Это значение берется из **поддерживаемого члена типа SRAM** в структуре **сведений о кэше** в сведениях SMBIOS.
...
Если **значение — true**, сведения об адресе в свойстве **еррораддресс** являются адресом системного уровня. Если **значение равно false**, это физический адрес. Если свойство **errorInfo** равно 3, то это свойство не имеет смысла.
...
Класс **Win32 \_ качемемори** является производным от [**CIM \_ качемемори**](cim-cachememory.md).
