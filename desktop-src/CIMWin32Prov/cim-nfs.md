...
Класс **\_ NFS CIM** представляет удаленную файловую систему, подключенную с помощью протокола NFS, из компьютерной системы. Свойства объекта NFS соответствуют рабочим аспектам подключения и представляют конфигурацию на стороне клиента для доступа NFS. Необходимо задать тип файловой системы, чтобы указать тип файловой системы, отображаемой клиенту.
...
Класс **\_ NFS CIM** имеет следующие типы членов:
...
Если **значение — true**, кэширование атрибутов управления включено. Если задано **значение false**, кэширование атрибутов элемента управления отключено.

</dd> <dt>

**аттрибутекачингфордиректориесмакс**
</dt> <dd> <dl> <dt>
...
**аттрибутекачингфордиректориесмин**
</dt> <dd> <dl> <dt>
...
**аттрибутекачингфоррегуларфилесмакс**
</dt> <dd> <dl> <dt>
...
**аттрибутекачингфоррегуларфилесмин**
</dt> <dd> <dl> <dt>
...
Общий объем свободного пространства в байтах для файловой системы. Если значение неизвестно, введите 0.
...
Файловые системы могут считывать или записывать данные в блоках, которые определены независимо от базовых экстентов хранения. Это свойство захватывает размер блока файловой системы для хранения и извлечения данных.
...
Общий размер файловой системы в байтах. Если значение неизвестно, введите 0 (ноль).
...
Если **значение равно true**, повторные попытки выполняются на переднем плане. Если **значение равно false** и первая попытка подключения завершается неудачно, повторные попытки выполняются в фоновом режиме.
...
Если **значение равно true**, то после подключения файловой системы запросы NFS повторяются до тех пор, пока система размещения не ответит. Если **значение равно false**, то после подключения файловой системы возвращается сообщение об ошибке, если система размещения не отвечает.
...
**Значение true** показывает, что прерывания разрешены для жестких подключений. Если **значение равно false**, прерывания не учитываются для жестких подключений.
...
Максимально допустимое число повторных попыток подключения.
...
Квалификаторы: [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) ("десятые доли секунды")
</dt> </dl>

Время ожидания NFS (в десятых долях секунды).
