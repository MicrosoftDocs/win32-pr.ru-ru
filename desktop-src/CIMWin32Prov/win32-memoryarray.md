...
[Класс WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ меморяррай для Win32** представляет свойства массива памяти компьютерной системы и сопоставленных адресов.
...
Класс **Win32 \_ меморяррай** имеет следующие типы членов:
...
Класс **Win32 \_ меморяррай** содержит следующие методы.



| Метод            | Описание                                                                                                                                                                                                        |
|:------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Сброс**         | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**Reset**](reset-method-in-class-cim-controller.md) в [**Win32 \_ смбиосмемори**](win32-smbiosmemory.md) .<br/>                 |
| **SetPowerState** | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) в [**Win32 \_ смбиосмемори**](win32-smbiosmemory.md) .<br/> |
...
Класс **Win32 \_ меморяррай** имеет следующие свойства.
...
Массив дополнительных сведений об ошибке. Примером является проверка ошибок и исправление (ECC) синдром или возвращение битов проверки, если используется циклическая проверка избыточности (CRC) на основе значения **еррормесодологи** . В последнем случае, если обнаружена Одноразрядная ошибка и известен алгоритм CRC, можно определить точный бит, в котором произошел сбой. Этот тип данных (ECC синдром, бит проверки, битные данные четности или другие сведения, предоставленные поставщиком) включен в это поле. Это свойство используется только в том случае, если свойство **errorInfo** не равно 3.
...
Это значение берется из структуры **сопоставленного адреса массива памяти** в сведениях о версии SMBIOS. Для SMBIOS версии 2,1 по 2,6 значение берется из конечного элемента **адреса** . Для SMBIOS версии 2.7 + значение берется из элемента **расширенного конечного адреса** .
...
**4** (уровень секционирования памяти)
...
Тип ошибки, которая возникла в последнее время. Значения 12-14, которые указывают, является ли ошибка исправимой, не используются с этим свойством, но эти сведения находятся в свойстве **корректаблиррор** .
...
Это свойство наследуется от [**CIM \_ сторажеекстент**](cim-storageextent.md). Значения качества производительности:

<dl> <dd>Иной</dd> <dd>Неизвестный</dd> <dd>"None"</dd> <dd>Суммы</dd> <dd>"Одноразрядный код коррекции ошибок"</dd> <dd>"Многоразрядный ECC"</dd> <dd>СУММЫ</dd> </dl>
...
Это значение берется из структуры **сопоставленного адреса массива памяти** в сведениях о версии SMBIOS. Для SMBIOS версии 2,1 по 2,6 значение берется из элемента **начальный адрес** . Для SMBIOS версии 2.7 + значение берется из **расширенного элемента начального адреса** .
