...
Класс **CIM \_ волатилестораже** представляет возможности и управление энергозависимым хранилищем.
...
Класс **CIM \_ волатилестораже** содержит следующие методы.
...
Класс **CIM \_ волатилестораже** имеет следующие свойства.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Сведения о памяти системного ресурса DMTF \| 001,5 ")
</dt> </dl>

Если **значение равно true**, то эта память может быть кэширована.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) (MIF. \|Сведения о памяти системного ресурса DMTF \| 001,6 ")
</dt> </dl>

Тип кэша, совместимый с памятью. Если свойство **cacheed** имеет значение **false**, то это свойство не имеет значения и должно иметь значение 4 ("неприменимо").
...
Данные, захваченные во время последнего ошибочного доступа к памяти. Данные занимают первые *n* октетов массива, необходимых для хранения числа битов, заданных свойством **еррортрансферсизе** . Если **еррортрансферсизе** имеет значение 0, это свойство не имеет смысла.
...
Упорядочивание данных, хранящихся в массиве **ErrorData** . Если **еррортрансферсизе** имеет значение 0, это свойство не имеет смысла.
...
Тип ошибки, которая возникла в последнее время. Значения 12-14 не определены в схеме CIM, так как семантика типа ошибки и вне зависимости от того, были ли они исправлены, смешиваются в DMI. Свойство **корректаблиррор** указывает, было ли оно исправлено.
...
Произвольная строка, описывающая тип обнаружения ошибок и исправления, поддерживаемые экстентом хранения.
...
Общее число последовательных блоков, каждый из которых блокирует размер значения, содержащегося в **свойстве, которое** формирует эту область хранения. Общий размер области хранения можно вычислить путем умножения **значения свойства размерности на значение** этого свойства. Если **значение параметра DataSize равно 1** (один), то это свойство является общим размером экстента хранения.
...
Произвольная строка, которая предоставляет дополнительные сведения, если свойство **ErrorType** имеет значение 1 ("другое"). Если значение, отличное от 1 (одно), эта строка не имеет смысла.
