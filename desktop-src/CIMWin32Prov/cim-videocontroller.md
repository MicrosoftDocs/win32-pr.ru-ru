...
Класс **CIM \_ Видеоконтроллер** представляет возможности и управление видеоконтроллером.
...
Класс **CIM \_ Видеоконтроллер** содержит следующие методы.
...
Класс **CIM \_ Видеоконтроллер** имеет следующие свойства.
...
Квалификаторы: [**моделкорреспонденце**](/windows/desktop/WmiSdk/standard-qualifiers) ("**CIM \_ Видеоконтроллер**.**Капабилитидескриптионс**")
</dt> </dl>
...
Квалификаторы: [**arrayType**](/windows/desktop/WmiSdk/standard-qualifiers) ("индексированный"), [**Моделкорреспонденце**](/windows/desktop/WmiSdk/standard-qualifiers) ("**CIM \_ Видеоконтроллер**.**Акцелераторкапабилитиес**")
</dt> </dl>

Строки произвольной формы, содержащие подробные описания функций ускорителя видео, указанных в массиве **акцелераторкапабилитиес** .

> [!Note]  
> Каждая запись этого массива связана с записью в массиве **акцелераторкапабилитиес** , расположенной по тому же индексу.
...
Если в символьном режиме, число строк для видеоконтроллера; в противном случае введите 0.
...
Максимальная частота обновления контроллера видео в герцах.
...
Идентификатор устройства Windows самонастраивающийся логического устройства. Это свойство наследуется [**от \_ CIM**](cim-logicaldevice.md)-унаследованной модели.
...
Дата и время последнего сброса контроллера, означающее, что контроллер был отключен или инициализирован повторно.
...
Произвольная строка, описывающая обработчик видео или контроллер.
...
Инструментарий WMI не реализует этот класс. Классы WMI, производные от **CIM \_ Видеоконтроллер**, см. в разделе [Классы Win32](win32-provider.md).
