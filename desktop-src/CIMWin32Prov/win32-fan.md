...
ms.locfileid: "120085804"
---
# <a name="win32_fan-class"></a>\_Класс вентилятора Win32

[Класс WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ вентилятора Win32** представляет свойства устройства вентиляторов в компьютерной системе. Например, Вентилятор охлаждения процессора.
...
Класс **\_ вентилятора Win32** имеет следующие типы членов:
...
Класс **\_ вентилятора Win32** имеет следующие методы.



| Метод            | Описание                                                                                                                                                                                  |
|:------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Сброс**         | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**Reset**](reset-method-in-class-cim-controller.md) в разделе " [**\_ вентилятор CIM**](cim-fan.md) ".<br/>                 |
| **SetPowerState** | Не реализован. Чтобы реализовать этот метод, см. документацию по методу [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) в разделе " [**\_ вентилятор CIM**](cim-fan.md) ".<br/> |
| **сетспид**      | Не реализован.<br/>                                                                                                                                                                  |
...
Класс **\_ вентилятора Win32** имеет следующие свойства.
...
Если **значение равно true**, устройство охлаждения обеспечивает охлаждение (в отличие от пассивного).

Это свойство наследуется от [**CIM \_ кулингдевице**](cim-coolingdevice.md).
...
Имя первого конкретного класса, который будет отображаться в цепочке наследования, используемой при создании экземпляра. При использовании с другими ключевыми свойствами класса свойство позволяет уникально идентифицировать все экземпляры этого класса и его подклассов.
...
Квалификаторы: [**единицы**](/windows/desktop/WmiSdk/standard-qualifiers) ("революция в минуту")
</dt> </dl>

Текущая запрошенная скорость вентилятора, определенная в революциях в минуту, при поддержке вентилятора с переменной скоростью (**вариаблеспид** имеет **значение true**). Текущая скорость определяется датчиком ([**CIM \_ тахометр**](cim-tachometer.md)), связанным с вентилятором с помощью отношения [**CIM \_ ассоЦиатедсенсор**](cim-associatedsensor.md) .

Это свойство наследуется [**от \_ вентилятора CIM**](cim-fan.md).
...
Идентифицирует устройство вентиляторов. Это свойство наследуется [**от \_ CIM**](cim-logicaldevice.md)-унаследованной модели.
...
Произвольная строка, предоставляя дополнительные сведения об ошибке, записанной в свойстве **ластерроркоде** , и сведения о любых корректирующих действиях, которые могут быть предприняты.
...
**Значение true** показывает, что вентилятор поддерживает переменные скорости.
...
Класс **\_ вентилятора Win32** является производным от [**\_ вентилятора CIM**](cim-fan.md).

## <a name="examples"></a>Примеры

Пример " [список сведений о вентиляторе компьютера](https://Gallery.TechNet.Microsoft.Com/d1534503-704f-4450-8dab-f3e760bf818c) " извлекает сведения о вентиляторах охлаждения, установленных на компьютере.

В следующем примере извлекаются сведения о охлаждающих вентиляторах, установленных на компьютере.
