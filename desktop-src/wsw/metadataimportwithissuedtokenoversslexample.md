...
---
# <a name="metadataimportwithissuedtokenoversslexample"></a>метадатаимпортвисиссуедтокеноверсслексампле

В этом примере показано, как импортировать метаданные из конечной точки, поддерживающей использование выданного маркера, полученного из выдающей службы маркеров безопасности (STS), с помощью [**всрекуестсекурититокен**](/windows/desktop/api/WebServices/nf-webservices-wsrequestsecuritytoken) и предоставляемых службе с помощью [**\_ \_ \_ \_ \_ привязки безопасности сообщений токена WS XML**](/windows/desktop/api/WebServices/ns-webservices-ws_xml_token_message_security_binding) с привязкой безопасности [**\_ транспорта WS SSL \_ \_ \_**](/windows/desktop/api/WebServices/ns-webservices-ws_ssl_transport_security_binding). Дополнительные сведения см. в разделе [Федерация](federation.md) .

## <a name="metadataimportwithissuedtokenoversslexamplecpp"></a>Метадатаимпортвисиссуедтокеноверсслексампле. cpp
