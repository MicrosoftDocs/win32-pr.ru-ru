---
title: хттпклиентвисусернамеоверсслексампле
description: В этом примере показан HTTP-клиент, отправляющий сообщения типа "запрос-ответ" с именем пользователя, защищенным смешанным режимом SSL.
ms.assetid: aaaad584-d861-4abd-9d76-95aba5090676
keywords:
- Хттпклиентвисусернамеоверсслексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104068165"
---
# <a name="httpclientwithusernameoversslexample"></a><span data-ttu-id="fd63e-106">хттпклиентвисусернамеоверсслексампле</span><span class="sxs-lookup"><span data-stu-id="fd63e-106">HttpClientWithUsernameOverSslExample</span></span>

<span data-ttu-id="fd63e-107">В этом примере показан HTTP-клиент, отправляющий сообщения типа "запрос-ответ" с именем пользователя, защищенным смешанным режимом SSL.</span><span class="sxs-lookup"><span data-stu-id="fd63e-107">This example shows a HTTP client that sends request-reply messages, with username over SSL mixed-mode security.</span></span> <span data-ttu-id="fd63e-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="fd63e-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="fd63e-109">Проверка подлинности клиента обеспечивается парой имени пользователя и пароля в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="fd63e-109">Client authentication is provided by a username/password pair in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="fd63e-110">Хттпклиентвисусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="fd63e-110">HttpClientWithUsernameOverSsl.cpp</span></span>](#httpclientwithusernameoversslcpp)
-   [<span data-ttu-id="fd63e-111">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="fd63e-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="fd63e-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="fd63e-112">Makefile</span></span>](#makefile)

## <a name="httpclientwithusernameoversslcpp"></a><span data-ttu-id="fd63e-113">Хттпклиентвисусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="fd63e-113">HttpClientWithUsernameOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="fd63e-114">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="fd63e-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="fd63e-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="fd63e-115">Makefile</span></span>
