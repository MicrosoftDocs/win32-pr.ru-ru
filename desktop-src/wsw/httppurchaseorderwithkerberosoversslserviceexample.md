---
title: хттппурчасеордервискерберосоверсслсервицеексампле
description: В этом примере показано, как использовать узел службы для размещения службы PurchaseOrder по протоколу HTTP с защитой Kerberos через SSL в смешанном режиме.
ms.assetid: 9fbf5577-6a54-4ce1-ad44-80b9ee91c0d3
keywords:
- Хттппурчасеордервискерберосоверсслсервицеексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105700607"
---
# <a name="httppurchaseorderwithkerberosoversslserviceexample"></a><span data-ttu-id="2082c-106">хттппурчасеордервискерберосоверсслсервицеексампле</span><span class="sxs-lookup"><span data-stu-id="2082c-106">HttpPurchaseOrderWithKerberosOverSslServiceExample</span></span>

<span data-ttu-id="2082c-107">В этом примере показано, как использовать узел службы для размещения службы PurchaseOrder по протоколу HTTP с защитой Kerberos через SSL в смешанном режиме.</span><span class="sxs-lookup"><span data-stu-id="2082c-107">This example shows how to use service host for hosting a PurchaseOrder service over HTTP, with kerberos over SSL mixed-mode security.</span></span> <span data-ttu-id="2082c-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="2082c-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="2082c-109">Проверка подлинности клиента предоставляется билетом Kerberos АПРЕК в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="2082c-109">Client authentication is provided by a kerberos APREQ ticket in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="2082c-110">Пурчасеордерсервицекерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="2082c-110">PurchaseOrderServiceKerberosOverSsl.cpp</span></span>](#purchaseorderservicekerberosoversslcpp)
-   [<span data-ttu-id="2082c-111">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="2082c-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="2082c-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="2082c-112">Makefile</span></span>](#makefile)

## <a name="purchaseorderservicekerberosoversslcpp"></a><span data-ttu-id="2082c-113">Пурчасеордерсервицекерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="2082c-113">PurchaseOrderServiceKerberosOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="2082c-114">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="2082c-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="2082c-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="2082c-115">Makefile</span></span>
