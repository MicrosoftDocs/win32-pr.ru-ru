...
---
# <a name="httppurchaseorderwithkerberosoversslserviceexample"></a>хттппурчасеордервискерберосоверсслсервицеексампле

В этом примере показано, как использовать узел службы для размещения службы PurchaseOrder по протоколу HTTP с защитой Kerberos через SSL в смешанном режиме. В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера. Проверка подлинности клиента предоставляется билетом Kerberos АПРЕК в заголовке WS-Security сообщения.

-   [Пурчасеордерсервицекерберосоверссл. cpp](#purchaseorderservicekerberosoversslcpp)
-   [PurchaseOrder. WSDL](#purchaseorderwsdl)
-   [Makefile](#makefile)

## <a name="purchaseorderservicekerberosoversslcpp"></a>Пурчасеордерсервицекерберосоверссл. cpp
