...
В этом примере показан HTTP-клиент, использующий прокси-сервер службы для взаимодействия со службой калькулятора, с маркером безопасности XML, защищенным в смешанном режиме безопасности SSL. В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера. Проверка подлинности клиента обеспечивается WS-Security парой имя пользователя и пароль, которая используется в качестве маркера безопасности XML в примере.

Обратите внимание, что этот пример на стороне клиента использует [**\_ \_ \_ \_ \_ привязку безопасности сообщений токена WS XML**](/windows/desktop/api/WebServices/ns-webservices-ws_xml_token_message_security_binding). Аналогичный способ выполнения той же безопасности на стороне клиента с помощью [**\_ \_ \_ \_ привязки безопасности сообщений имени пользователя (WS username**](/windows/desktop/api/WebServices/ns-webservices-ws_username_message_security_binding) ) показан в примере [хттпкалкулаторвисусернамеоверсслклиентексампле](httpcalculatorwithusernameoversslclientexample.md).

Соответствующий серверный пример для обоих примеров на стороне клиента предоставляется в примере [хттпкалкулаторвисусернамеоверсслсервицеексампле](httpcalculatorwithusernameoversslserviceexample.md).
...
[хттпкалкулаторвисусернамеоверсслклиентексампле](httpcalculatorwithusernameoversslclientexample.md)
</dt> <dt>
