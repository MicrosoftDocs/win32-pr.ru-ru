---
title: хттпкалкулаторвисбасиковерсслклиентексампле
description: В этом примере показано, как использовать прокси-сервер службы для взаимодействия со службой калькулятора с безопасностью транспорта SSL и обычной проверкой подлинности заголовка HTTP.
ms.assetid: be1a44e6-642c-42fa-8b95-bd8c6f335696
keywords:
- Хттпкалкулаторвисбасиковерсслклиентексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105710170"
---
# <a name="httpcalculatorwithbasicoversslclientexample"></a><span data-ttu-id="a2bff-106">хттпкалкулаторвисбасиковерсслклиентексампле</span><span class="sxs-lookup"><span data-stu-id="a2bff-106">HttpCalculatorWithBasicOverSslClientExample</span></span>

<span data-ttu-id="a2bff-107">В этом примере показано, как использовать прокси-сервер службы для взаимодействия со службой калькулятора с безопасностью транспорта SSL и обычной проверкой подлинности заголовка HTTP.</span><span class="sxs-lookup"><span data-stu-id="a2bff-107">This example shows how to use the service proxy to talk to a calculator service, with SSL transport security and HTTP header basic authentication.</span></span> <span data-ttu-id="a2bff-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="a2bff-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="a2bff-109">Проверка подлинности клиента обеспечивается парой имен пользователей и паролей, указанной в заголовках HTTP, с использованием базовой схемы.</span><span class="sxs-lookup"><span data-stu-id="a2bff-109">Client authentication is provided by a username/password pair supplied in HTTP headers using the basic scheme.</span></span>

-   [<span data-ttu-id="a2bff-110">Хттпкалкулаторвисбасиковерсслклиент. cpp</span><span class="sxs-lookup"><span data-stu-id="a2bff-110">HttpCalculatorWithBasicOverSslClient.cpp</span></span>](#httpcalculatorwithbasicoversslclientcpp)
-   [<span data-ttu-id="a2bff-111">CalculatorService. WSDL</span><span class="sxs-lookup"><span data-stu-id="a2bff-111">CalculatorService.wsdl</span></span>](#calculatorservicewsdl)
-   [<span data-ttu-id="a2bff-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="a2bff-112">Makefile</span></span>](#makefile)

## <a name="httpcalculatorwithbasicoversslclientcpp"></a><span data-ttu-id="a2bff-113">Хттпкалкулаторвисбасиковерсслклиент. cpp</span><span class="sxs-lookup"><span data-stu-id="a2bff-113">HttpCalculatorWithBasicOverSslClient.cpp</span></span>
...
## <a name="calculatorservicewsdl"></a><span data-ttu-id="a2bff-114">CalculatorService. WSDL</span><span class="sxs-lookup"><span data-stu-id="a2bff-114">CalculatorService.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="a2bff-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="a2bff-115">Makefile</span></span>
