---
title: хттппурчасеордервисусернамеоверсслсервицеексампле
description: В этом примере показано, как использовать узел службы для размещения службы PurchaseOrder по протоколу HTTP с именем пользователя в смешанном режиме защиты SSL.
ms.assetid: 3dfedab1-d508-4cff-b179-591871078adb
keywords:
- Хттппурчасеордервисусернамеоверсслсервицеексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103777786"
---
# <a name="httppurchaseorderwithusernameoversslserviceexample"></a><span data-ttu-id="56223-106">хттппурчасеордервисусернамеоверсслсервицеексампле</span><span class="sxs-lookup"><span data-stu-id="56223-106">HttpPurchaseOrderWithUserNameOverSslServiceExample</span></span>

<span data-ttu-id="56223-107">В этом примере показано, как использовать узел службы для размещения службы PurchaseOrder по протоколу HTTP с именем пользователя в смешанном режиме защиты SSL.</span><span class="sxs-lookup"><span data-stu-id="56223-107">This example shows how to use service host for hosting a PurchaseOrder service over HTTP, with username over SSL mixed-mode security.</span></span> <span data-ttu-id="56223-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="56223-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="56223-109">Проверка подлинности клиента обеспечивается парой имени пользователя и пароля в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="56223-109">Client authentication is provided by a username/password pair in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="56223-110">Пурчасеордерсервицеусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="56223-110">PurchaseOrderServiceUserNameOverSsl.cpp</span></span>](#purchaseorderserviceusernameoversslcpp)
-   [<span data-ttu-id="56223-111">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="56223-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="56223-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="56223-112">Makefile</span></span>](#makefile)

## <a name="purchaseorderserviceusernameoversslcpp"></a><span data-ttu-id="56223-113">Пурчасеордерсервицеусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="56223-113">PurchaseOrderServiceUserNameOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="56223-114">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="56223-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="56223-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="56223-115">Makefile</span></span>
