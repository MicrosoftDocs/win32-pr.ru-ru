...
Филереп извлекает файлы с сервера и копирует их в клиент. Для этого используются три компонента: служба сервера, работающая на компьютере с исходным файлом, служба клиента, работающая на компьютере, где будет храниться конечный файл, и средство командной строки для управления копированием. Службы клиента и сервера постоянно работают с веб-службами, а программа командной строки запускается пользователем и завершается после одного запроса.

В этом примере демонстрируется использование уровня канала и сериализации.

Это служба. Программу командной строки можно найти [здесь](filereptoolexample.md). Служба имеет клиент и режим сервера, где сервер отправляет файлы, а клиент получает файлы.
...
<span id="Client"></span><span id="client"></span><span id="CLIENT"></span>Компьютера
</dt> <dd>

Обязательный. Указывает, что служба выполняется как клиент.

</dd> <dt>

<span id="Service_Url"></span><span id="service_url"></span><span id="SERVICE_URL"></span>URL-адрес службы
</dt> <dd>

Обязательный. Обозначает URL-адрес, который прослушивает служба.

</dd> <dt>

<span id="Encoding"></span><span id="encoding"></span><span id="ENCODING"></span>Шифрования
</dt> <dd>

Необязательный элемент. Указывает кодировку, используемую при обмене данными со средством командной строки. Обратите внимание, что текущее средство не поддерживает указание кодировки для этого перемещения, поэтому изменение этого параметра, скорее всего, приведет к ошибке. Этот параметр позволяет изменить и расширить средство независимо от сервера.

</dd> <dt>

<span id="Reporting"></span><span id="reporting"></span><span id="REPORTING"></span>Отчёт
</dt> <dd>

Необязательный элемент. Включает отчеты об ошибках, сведения или уровень детализации. Значение по умолчанию — Error. Сообщения выводятся на консоль.

</dd> <dt>

<span id="Connections"></span><span id="connections"></span><span id="CONNECTIONS"></span>Соединение
</dt> <dd>

Необязательный элемент. Указывает максимальное количество одновременных запросов, которые будут обрабатываться. Если этот параметр опущен, значение по умолчанию — 100.
...
<span id="Server"></span><span id="server"></span><span id="SERVER"></span>Сервером
</dt> <dd>

Обязательный. Указывает, что служба выполняется как файловый сервер.

</dd> <dt>

<span id="Chunk"></span><span id="chunk"></span><span id="CHUNK"></span>Фрагмент
</dt> <dd>

Необязательный элемент. Передаваемые файлы разбиваются на блоки указанного размера. Каждое сообщение содержит один фрагмент. Значение по умолчанию — 32768 байт.

</dd> </dl>

Сведения о реализации. Так как этот пример написан на уровне канала, он должен вручную выполнять определенные задачи, которые уровень модели службы может сделать автоматически. Одной из таких задач является управление каналами. Чтобы обеспечить быстрый отклик на запросы, всегда существует (до ограничения) несколько каналов, готовых принимать запросы. Если несколько каналов готовы к работе, то в случае нескольких запросов более производительнее, чем наличие только одного канала, так как создание канала занимает некоторое время. Кроме того, повторное использование состояния повышает производительность. Sapphire предоставляет API-интерфейсы для сброса большинства состояний, чтобы не пришлось освобождать и повторно создавать его. Этот пример использует преимущества, так как по возможности повторно использует канал и состояние запроса, а также создает или уничтожает состояние, когда передаются определенные пороговые значения. Код, связанный с управлением каналами, можно найти в Кчаннелманажер.

Основной цикл обработки сообщений находится в Крекуест. Этот класс содержит независимое от приложения состояние и методы, необходимые для асинхронного цикла обработки обмена сообщениями Sapphire. Код конкретного приложения находится в Кфилерепклиент (служба клиента) и Кфилерепсервер (служба сервера). Оба эти класса наследуют от Кфилереп, который содержит код, связанный со службой.

В примере используется сериализатор и выполняется пользовательская сериализация. Настраиваемая сериализация используется при работе с большими блоками данных для снижения потребления памяти за счет оптимизации выделения памяти для конкретной цели вручную. Так что это приводит к сложному коду низкого уровня, выполняющему ручную сериализацию, следует выполнять только в случае крайней необходимости.

Шаблон обмена сообщениями:

-   Служба клиента получает сообщение запроса от программы командной строки.
-   Значение, если запрос асинхронно отправляет подтверждение.
-   Служба клиента отправляет запрос сведений о файле службе сервера. Запрос на обнаружение обозначается позицией фрагмента данных, равной-1.
-   Служба сервера возвращает сведения о файле.
-   Служба клиента запрашивает отдельные блоки последовательно по одному на сервере. Блоки идентифицируются по их положению в файле.
-   Повторяйте, пока не завершится перенос файла или произошла ошибка.
-   Значение, если запрос является синхронным сообщением об успешной отправке или сбое для программы командной строки.

Сведения о отдельных структурах данных, связанных с каждым сообщением, см. в разделе Common. h.

-   [Service. cpp](#servicecpp)
-   [Service. h](#serviceh)
-   [Кчаннелманажер. cpp](#cchannelmanagercpp)
-   [Кфилереп. cpp](#cfilerepcpp)
-   [Крекуест. cpp](#crequestcpp)
-   [Кфилерепклиент. cpp](#cfilerepclientcpp)
-   [Кфилерепсервер. cpp](#cfilerepservercpp)
-   [Common. h](#commonh)
...
## <a name="cchannelmanagercpp"></a>Кчаннелманажер. cpp
...
## <a name="cfilerepcpp"></a>Кфилереп. cpp
...
## <a name="crequestcpp"></a>Крекуест. cpp
...
## <a name="cfilerepclientcpp"></a>Кфилерепклиент. cpp
...
## <a name="cfilerepservercpp"></a>Кфилерепсервер. cpp
