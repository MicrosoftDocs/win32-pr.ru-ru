---
title: филерепсервицеексампле
description: Филереп извлекает файлы с сервера и копирует их в клиент.
ms.assetid: 9f446999-8f10-4ce4-86eb-e9289e131733
keywords:
- Веб-службы Филерепсервицеексампле для Windows
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
---
# <a name="filerepserviceexample"></a><span data-ttu-id="57ae2-106">филерепсервицеексампле</span><span class="sxs-lookup"><span data-stu-id="57ae2-106">FileRepServiceExample</span></span>

<span data-ttu-id="57ae2-107">Филереп извлекает файлы с сервера и копирует их в клиент.</span><span class="sxs-lookup"><span data-stu-id="57ae2-107">FileRep retrieves files from a server and copies them to a client.</span></span> <span data-ttu-id="57ae2-108">Для этого используются три компонента: служба сервера, работающая на компьютере с исходным файлом, служба клиента, работающая на компьютере, где будет храниться конечный файл, и средство командной строки для управления копированием.</span><span class="sxs-lookup"><span data-stu-id="57ae2-108">To do so it employs three components - the server service running on the machine with the source file, the client service running on the machine where the destination file will be stored and a command line tool to control the copying.</span></span> <span data-ttu-id="57ae2-109">Службы клиента и сервера постоянно работают с веб-службами, а программа командной строки запускается пользователем и завершается после одного запроса.</span><span class="sxs-lookup"><span data-stu-id="57ae2-109">The client and server services are constantly running web services while the command line tool is started by the user and exits after one request.</span></span>

<span data-ttu-id="57ae2-110">В этом примере демонстрируется использование уровня канала и сериализации.</span><span class="sxs-lookup"><span data-stu-id="57ae2-110">This sample illustrates the use of the channel and serialization layer.</span></span>

<span data-ttu-id="57ae2-111">Это служба.</span><span class="sxs-lookup"><span data-stu-id="57ae2-111">This is the service.</span></span> <span data-ttu-id="57ae2-112">Программу командной строки можно найти [здесь](filereptoolexample.md). Служба имеет клиент и режим сервера, где сервер отправляет файлы, а клиент получает файлы.</span><span class="sxs-lookup"><span data-stu-id="57ae2-112">The command line tool can be found [here](filereptoolexample.md).The service has a client and a server mode, where the server sends files and the client receives files.</span></span>

<span data-ttu-id="57ae2-113">Параметры командной строки для режима клиента приведены ниже.</span><span class="sxs-lookup"><span data-stu-id="57ae2-113">The command line parameters for the client mode are as follows:</span></span>

<span data-ttu-id="57ae2-114">**КлиентWsFileRepService.exe** *<Service Url>* **\[ /reporting: <Error/info/verbose>\] \[ /Encoding: <Text/binary/MTOM>\] \[ /коннектионс: ***<number of connections>*** \]**</span><span class="sxs-lookup"><span data-stu-id="57ae2-114">**WsFileRepService.exe client** *<Service Url>* **\[/reporting:<error/info/verbose>\] \[/encoding:<text/binary/MTOM>\] \[/connections:***<number of connections>***\]**</span></span>

<dl> <dt>

<span data-ttu-id="57ae2-115"><span id="Client"></span><span id="client"></span><span id="CLIENT"></span>Компьютера</span><span class="sxs-lookup"><span data-stu-id="57ae2-115"><span id="Client"></span><span id="client"></span><span id="CLIENT"></span>Client</span></span>
</dt> <dd>

<span data-ttu-id="57ae2-116">Обязательный.</span><span class="sxs-lookup"><span data-stu-id="57ae2-116">Required.</span></span> <span data-ttu-id="57ae2-117">Указывает, что служба выполняется как клиент.</span><span class="sxs-lookup"><span data-stu-id="57ae2-117">Denotes that the service runs as client.</span></span>

</dd> <dt>

<span data-ttu-id="57ae2-118"><span id="Service_Url"></span><span id="service_url"></span><span id="SERVICE_URL"></span>URL-адрес службы</span><span class="sxs-lookup"><span data-stu-id="57ae2-118"><span id="Service_Url"></span><span id="service_url"></span><span id="SERVICE_URL"></span>Service Url</span></span>
</dt> <dd>

<span data-ttu-id="57ae2-119">Обязательный.</span><span class="sxs-lookup"><span data-stu-id="57ae2-119">Required.</span></span> <span data-ttu-id="57ae2-120">Обозначает URL-адрес, который прослушивает служба.</span><span class="sxs-lookup"><span data-stu-id="57ae2-120">Denotes the URL the service listens on.</span></span>

</dd> <dt>

<span data-ttu-id="57ae2-121"><span id="Encoding"></span><span id="encoding"></span><span id="ENCODING"></span>Шифрования</span><span class="sxs-lookup"><span data-stu-id="57ae2-121"><span id="Encoding"></span><span id="encoding"></span><span id="ENCODING"></span>Encoding</span></span>
</dt> <dd>

<span data-ttu-id="57ae2-122">Необязательный элемент.</span><span class="sxs-lookup"><span data-stu-id="57ae2-122">Optional.</span></span> <span data-ttu-id="57ae2-123">Указывает кодировку, используемую при обмене данными со средством командной строки.</span><span class="sxs-lookup"><span data-stu-id="57ae2-123">Specifies the encoding used when communicating with the command line tool.</span></span> <span data-ttu-id="57ae2-124">Обратите внимание, что текущее средство не поддерживает указание кодировки для этого перемещения, поэтому изменение этого параметра, скорее всего, приведет к ошибке.</span><span class="sxs-lookup"><span data-stu-id="57ae2-124">Note that the current tool does not support specifying an encoding for this transfer, so changing this setting will likely produce an error.</span></span> <span data-ttu-id="57ae2-125">Этот параметр позволяет изменить и расширить средство независимо от сервера.</span><span class="sxs-lookup"><span data-stu-id="57ae2-125">The setting is there so that the tool can be changed and extended independently of the server.</span></span>

</dd> <dt>

<span data-ttu-id="57ae2-126"><span id="Reporting"></span><span id="reporting"></span><span id="REPORTING"></span>Отчёт</span><span class="sxs-lookup"><span data-stu-id="57ae2-126"><span id="Reporting"></span><span id="reporting"></span><span id="REPORTING"></span>Reporting</span></span>
</dt> <dd>

<span data-ttu-id="57ae2-127">Необязательный элемент.</span><span class="sxs-lookup"><span data-stu-id="57ae2-127">Optional.</span></span> <span data-ttu-id="57ae2-128">Включает отчеты об ошибках, сведения или уровень детализации.</span><span class="sxs-lookup"><span data-stu-id="57ae2-128">Enables error, information or verbose level reporting.</span></span> <span data-ttu-id="57ae2-129">Значение по умолчанию — Error.</span><span class="sxs-lookup"><span data-stu-id="57ae2-129">The default is error.</span></span> <span data-ttu-id="57ae2-130">Сообщения выводятся на консоль.</span><span class="sxs-lookup"><span data-stu-id="57ae2-130">Messages are printed to the console.</span></span>

</dd> <dt>

<span data-ttu-id="57ae2-131"><span id="Connections"></span><span id="connections"></span><span id="CONNECTIONS"></span>Соединение</span><span class="sxs-lookup"><span data-stu-id="57ae2-131"><span id="Connections"></span><span id="connections"></span><span id="CONNECTIONS"></span>Connections</span></span>
</dt> <dd>

<span data-ttu-id="57ae2-132">Необязательный элемент.</span><span class="sxs-lookup"><span data-stu-id="57ae2-132">Optional.</span></span> <span data-ttu-id="57ae2-133">Указывает максимальное количество одновременных запросов, которые будут обрабатываться.</span><span class="sxs-lookup"><span data-stu-id="57ae2-133">Specifies the maximum number of concurrent requests that will be processed.</span></span> <span data-ttu-id="57ae2-134">Если этот параметр опущен, значение по умолчанию — 100.</span><span class="sxs-lookup"><span data-stu-id="57ae2-134">If omitted the default is 100.</span></span>

</dd> </dl>

<span data-ttu-id="57ae2-135">Ниже приведены параметры командной строки для режима сервера.</span><span class="sxs-lookup"><span data-stu-id="57ae2-135">The command line parameters for the server mode are as follows:</span></span>

<span data-ttu-id="57ae2-136">**СерверWsFileRepService.exe** *<Service Url>* **\[ /Reporting: <Error/info/verbose>\] \[ /Encoding: <Text/binary/MTOM>\] \[ /коннектионс: ***<number of connections>*** \] \[ /Чунк: ***<size of the payload per message in bytes>*** \]**</span><span class="sxs-lookup"><span data-stu-id="57ae2-136">**WsFileRepService.exe server** *<Service Url>* **\[/reporting:<error/info/verbose>\] \[/encoding:<text/binary/MTOM>\] \[/connections:***<number of connections>***\] \[/chunk:***<size of the payload per message in bytes>***\]**</span></span>

<dl> <dt>

<span data-ttu-id="57ae2-137"><span id="Server"></span><span id="server"></span><span id="SERVER"></span>Сервером</span><span class="sxs-lookup"><span data-stu-id="57ae2-137"><span id="Server"></span><span id="server"></span><span id="SERVER"></span>Server</span></span>
</dt> <dd>

<span data-ttu-id="57ae2-138">Обязательный.</span><span class="sxs-lookup"><span data-stu-id="57ae2-138">Required.</span></span> <span data-ttu-id="57ae2-139">Указывает, что служба выполняется как файловый сервер.</span><span class="sxs-lookup"><span data-stu-id="57ae2-139">Denotes that the service runs as file server.</span></span>

</dd> <dt>

<span data-ttu-id="57ae2-140"><span id="Chunk"></span><span id="chunk"></span><span id="CHUNK"></span>Фрагмент</span><span class="sxs-lookup"><span data-stu-id="57ae2-140"><span id="Chunk"></span><span id="chunk"></span><span id="CHUNK"></span>Chunk</span></span>
</dt> <dd>

<span data-ttu-id="57ae2-141">Необязательный элемент.</span><span class="sxs-lookup"><span data-stu-id="57ae2-141">Optional.</span></span> <span data-ttu-id="57ae2-142">Передаваемые файлы разбиваются на блоки указанного размера.</span><span class="sxs-lookup"><span data-stu-id="57ae2-142">The transferred files are broken into chunks of the specified size.</span></span> <span data-ttu-id="57ae2-143">Каждое сообщение содержит один фрагмент.</span><span class="sxs-lookup"><span data-stu-id="57ae2-143">Each message contains one chunk.</span></span> <span data-ttu-id="57ae2-144">Значение по умолчанию — 32768 байт.</span><span class="sxs-lookup"><span data-stu-id="57ae2-144">The default is 32768 bytes.</span></span>

</dd> </dl>

<span data-ttu-id="57ae2-145">Сведения о реализации.</span><span class="sxs-lookup"><span data-stu-id="57ae2-145">Implementation details.</span></span> <span data-ttu-id="57ae2-146">Так как этот пример написан на уровне канала, он должен вручную выполнять определенные задачи, которые уровень модели службы может сделать автоматически.</span><span class="sxs-lookup"><span data-stu-id="57ae2-146">As this sample is written against the channel layer, it has to manually perform certain tasks that the service model layer could do automatically.</span></span> <span data-ttu-id="57ae2-147">Одной из таких задач является управление каналами.</span><span class="sxs-lookup"><span data-stu-id="57ae2-147">One such task is channel management.</span></span> <span data-ttu-id="57ae2-148">Чтобы обеспечить быстрый отклик на запросы, всегда существует (до ограничения) несколько каналов, готовых принимать запросы.</span><span class="sxs-lookup"><span data-stu-id="57ae2-148">In order to provide a fast response to requests, there are always (up to a limit) several channels ready to accept requests.</span></span> <span data-ttu-id="57ae2-149">Если несколько каналов готовы к работе, то в случае нескольких запросов более производительнее, чем наличие только одного канала, так как создание канала занимает некоторое время.</span><span class="sxs-lookup"><span data-stu-id="57ae2-149">Having multiple channels ready is more performant in the case of multiple requests than having only one channel as the creation of a channel takes time.</span></span> <span data-ttu-id="57ae2-150">Кроме того, повторное использование состояния повышает производительность.</span><span class="sxs-lookup"><span data-stu-id="57ae2-150">Further, reuse of state also improves performance.</span></span> <span data-ttu-id="57ae2-151">Sapphire предоставляет API-интерфейсы для сброса большинства состояний, чтобы не пришлось освобождать и повторно создавать его.</span><span class="sxs-lookup"><span data-stu-id="57ae2-151">Sapphire provides APIs to reset most state to avoid having to free and recreate it.</span></span> <span data-ttu-id="57ae2-152">Этот пример использует преимущества, так как по возможности повторно использует канал и состояние запроса, а также создает или уничтожает состояние, когда передаются определенные пороговые значения.</span><span class="sxs-lookup"><span data-stu-id="57ae2-152">This sample takes advantage of that by reusing channel and request state wherever possible and only creating or destroying state when certain thresholds are passed.</span></span> <span data-ttu-id="57ae2-153">Код, связанный с управлением каналами, можно найти в Кчаннелманажер.</span><span class="sxs-lookup"><span data-stu-id="57ae2-153">The code related to channel management can be found in CChannelManager.</span></span>

<span data-ttu-id="57ae2-154">Основной цикл обработки сообщений находится в Крекуест.</span><span class="sxs-lookup"><span data-stu-id="57ae2-154">The main message processing loop is in CRequest.</span></span> <span data-ttu-id="57ae2-155">Этот класс содержит независимое от приложения состояние и методы, необходимые для асинхронного цикла обработки обмена сообщениями Sapphire.</span><span class="sxs-lookup"><span data-stu-id="57ae2-155">That class contains the application-independent state and methods needed for an asynchronous Sapphire messaging processing loop.</span></span> <span data-ttu-id="57ae2-156">Код конкретного приложения находится в Кфилерепклиент (служба клиента) и Кфилерепсервер (служба сервера).</span><span class="sxs-lookup"><span data-stu-id="57ae2-156">The application-specific code is in CFileRepClient (client service) and CFileRepServer (server service).</span></span> <span data-ttu-id="57ae2-157">Оба эти класса наследуют от Кфилереп, который содержит код, связанный со службой.</span><span class="sxs-lookup"><span data-stu-id="57ae2-157">Both those classes inherit from CFileRep, which contains generic service-related code.</span></span>

<span data-ttu-id="57ae2-158">В примере используется сериализатор и выполняется пользовательская сериализация.</span><span class="sxs-lookup"><span data-stu-id="57ae2-158">The sample both uses the serializer and performs custom serialization.</span></span> <span data-ttu-id="57ae2-159">Настраиваемая сериализация используется при работе с большими блоками данных для снижения потребления памяти за счет оптимизации выделения памяти для конкретной цели вручную.</span><span class="sxs-lookup"><span data-stu-id="57ae2-159">Custom serialization is used when dealing with large chunks of data to minimize memory consumption by manually optimizing memory allocation for the specific purpose.</span></span> <span data-ttu-id="57ae2-160">Так что это приводит к сложному коду низкого уровня, выполняющему ручную сериализацию, следует выполнять только в случае крайней необходимости.</span><span class="sxs-lookup"><span data-stu-id="57ae2-160">As this leads to complex, low-level code doing manual serialization should only be done when absolutely necessary.</span></span>

<span data-ttu-id="57ae2-161">Шаблон обмена сообщениями:</span><span class="sxs-lookup"><span data-stu-id="57ae2-161">Message exchange pattern:</span></span>

-   <span data-ttu-id="57ae2-162">Служба клиента получает сообщение запроса от программы командной строки.</span><span class="sxs-lookup"><span data-stu-id="57ae2-162">The client service gets a request message from the command line tool.</span></span>
-   <span data-ttu-id="57ae2-163">Значение, если запрос асинхронно отправляет подтверждение.</span><span class="sxs-lookup"><span data-stu-id="57ae2-163">If the request is asynchronous send back a confirmation immediately.</span></span>
-   <span data-ttu-id="57ae2-164">Служба клиента отправляет запрос сведений о файле службе сервера.</span><span class="sxs-lookup"><span data-stu-id="57ae2-164">The client service sends a request for file information to the server service.</span></span> <span data-ttu-id="57ae2-165">Запрос на обнаружение обозначается позицией фрагмента данных, равной-1.</span><span class="sxs-lookup"><span data-stu-id="57ae2-165">A discovery request is denoted by a chunk position of -1.</span></span>
-   <span data-ttu-id="57ae2-166">Служба сервера возвращает сведения о файле.</span><span class="sxs-lookup"><span data-stu-id="57ae2-166">The server service returns the file information.</span></span>
-   <span data-ttu-id="57ae2-167">Служба клиента запрашивает отдельные блоки последовательно по одному на сервере.</span><span class="sxs-lookup"><span data-stu-id="57ae2-167">The client service requests the individual chunks sequentially one by one from the server.</span></span> <span data-ttu-id="57ae2-168">Блоки идентифицируются по их положению в файле.</span><span class="sxs-lookup"><span data-stu-id="57ae2-168">Chunks are identified by their position within the file.</span></span>
-   <span data-ttu-id="57ae2-169">Повторяйте, пока не завершится перенос файла или произошла ошибка.</span><span class="sxs-lookup"><span data-stu-id="57ae2-169">Repeat until the file transfer is completed or a failure occurred.</span></span>
-   <span data-ttu-id="57ae2-170">Значение, если запрос является синхронным сообщением об успешной отправке или сбое для программы командной строки.</span><span class="sxs-lookup"><span data-stu-id="57ae2-170">If the request is synchronous send success or failure message to the command line tool.</span></span>

<span data-ttu-id="57ae2-171">Сведения о отдельных структурах данных, связанных с каждым сообщением, см. в разделе Common. h.</span><span class="sxs-lookup"><span data-stu-id="57ae2-171">For the individual data structures associated with each message, see common.h.</span></span>

-   [<span data-ttu-id="57ae2-172">Service. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-172">Service.cpp</span></span>](#servicecpp)
-   [<span data-ttu-id="57ae2-173">Service. h</span><span class="sxs-lookup"><span data-stu-id="57ae2-173">Service.h</span></span>](#serviceh)
-   [<span data-ttu-id="57ae2-174">Кчаннелманажер. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-174">CChannelManager.cpp</span></span>](#cchannelmanagercpp)
-   [<span data-ttu-id="57ae2-175">Кфилереп. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-175">CFileRep.cpp</span></span>](#cfilerepcpp)
-   [<span data-ttu-id="57ae2-176">Крекуест. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-176">CRequest.cpp</span></span>](#crequestcpp)
-   [<span data-ttu-id="57ae2-177">Кфилерепклиент. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-177">CFileRepClient.cpp</span></span>](#cfilerepclientcpp)
-   [<span data-ttu-id="57ae2-178">Кфилерепсервер. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-178">CFileRepServer.cpp</span></span>](#cfilerepservercpp)
-   [<span data-ttu-id="57ae2-179">Common. h</span><span class="sxs-lookup"><span data-stu-id="57ae2-179">common.h</span></span>](#commonh)
-   [<span data-ttu-id="57ae2-180">FileRep.mc</span><span class="sxs-lookup"><span data-stu-id="57ae2-180">FileRep.mc</span></span>](#filerepmc)
-   [<span data-ttu-id="57ae2-181">Филереп. RC</span><span class="sxs-lookup"><span data-stu-id="57ae2-181">FileRep.rc</span></span>](#filereprc)
-   [<span data-ttu-id="57ae2-182">Makefile</span><span class="sxs-lookup"><span data-stu-id="57ae2-182">Makefile</span></span>](#makefile)
-   [<span data-ttu-id="57ae2-183">См. также</span><span class="sxs-lookup"><span data-stu-id="57ae2-183">Related topics</span></span>](#related-topics)

## <a name="servicecpp"></a><span data-ttu-id="57ae2-184">Service. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-184">Service.cpp</span></span>
...
## <a name="serviceh"></a><span data-ttu-id="57ae2-185">Service. h</span><span class="sxs-lookup"><span data-stu-id="57ae2-185">Service.h</span></span>
...
## <a name="cchannelmanagercpp"></a><span data-ttu-id="57ae2-186">Кчаннелманажер. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-186">CChannelManager.cpp</span></span>
...
## <a name="cfilerepcpp"></a><span data-ttu-id="57ae2-187">Кфилереп. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-187">CFileRep.cpp</span></span>
...
## <a name="crequestcpp"></a><span data-ttu-id="57ae2-188">Крекуест. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-188">CRequest.cpp</span></span>
...
## <a name="cfilerepclientcpp"></a><span data-ttu-id="57ae2-189">Кфилерепклиент. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-189">CFileRepClient.cpp</span></span>
...
## <a name="cfilerepservercpp"></a><span data-ttu-id="57ae2-190">Кфилерепсервер. cpp</span><span class="sxs-lookup"><span data-stu-id="57ae2-190">CFileRepServer.cpp</span></span>
...
## <a name="commonh"></a><span data-ttu-id="57ae2-191">common.h</span><span class="sxs-lookup"><span data-stu-id="57ae2-191">common.h</span></span>
...
## <a name="filerepmc"></a><span data-ttu-id="57ae2-192">FileRep.mc</span><span class="sxs-lookup"><span data-stu-id="57ae2-192">FileRep.mc</span></span>
...
## <a name="filereprc"></a><span data-ttu-id="57ae2-193">Филереп. RC</span><span class="sxs-lookup"><span data-stu-id="57ae2-193">FileRep.rc</span></span>
...
## <a name="makefile"></a><span data-ttu-id="57ae2-194">Makefile</span><span class="sxs-lookup"><span data-stu-id="57ae2-194">Makefile</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="57ae2-195">См. также</span><span class="sxs-lookup"><span data-stu-id="57ae2-195">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="57ae2-196">филерептулексампле</span><span class="sxs-lookup"><span data-stu-id="57ae2-196">FileRepToolExample</span></span>](filereptoolexample.md)
</dt> </dl>
