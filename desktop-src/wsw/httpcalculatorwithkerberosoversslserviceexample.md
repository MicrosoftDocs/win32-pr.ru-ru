---
title: хттпкалкулаторвискерберосоверсслсервицеексампле
description: В этом примере показано, как использовать узел службы для размещения службы калькулятора по протоколу HTTP с защитой Kerberos через SSL в смешанном режиме.
ms.assetid: a006ce15-9470-49f7-80d6-8d40ffec8651
keywords:
- Хттпкалкулаторвискерберосоверсслсервицеексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104411432"
---
# <a name="httpcalculatorwithkerberosoversslserviceexample"></a><span data-ttu-id="88784-106">хттпкалкулаторвискерберосоверсслсервицеексампле</span><span class="sxs-lookup"><span data-stu-id="88784-106">HttpCalculatorWithKerberosOverSslServiceExample</span></span>

<span data-ttu-id="88784-107">В этом примере показано, как использовать узел службы для размещения службы калькулятора по протоколу HTTP с защитой Kerberos через SSL в смешанном режиме.</span><span class="sxs-lookup"><span data-stu-id="88784-107">This example shows how to use service host for hosting a calculator service over HTTP, with kerberos over SSL mixed-mode security.</span></span> <span data-ttu-id="88784-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="88784-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="88784-109">Проверка подлинности клиента предоставляется билетом Kerberos АПРЕК в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="88784-109">Client authentication is provided by a kerberos APREQ ticket in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="88784-110">Калкулаторсервицекерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="88784-110">CalculatorServiceKerberosOverSsl.cpp</span></span>](#calculatorservicekerberosoversslcpp)
-   [<span data-ttu-id="88784-111">CalculatorService. WSDL</span><span class="sxs-lookup"><span data-stu-id="88784-111">CalculatorService.wsdl</span></span>](#calculatorservicewsdl)
-   [<span data-ttu-id="88784-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="88784-112">Makefile</span></span>](#makefile)

## <a name="calculatorservicekerberosoversslcpp"></a><span data-ttu-id="88784-113">Калкулаторсервицекерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="88784-113">CalculatorServiceKerberosOverSsl.cpp</span></span>
...
## <a name="calculatorservicewsdl"></a><span data-ttu-id="88784-114">CalculatorService. WSDL</span><span class="sxs-lookup"><span data-stu-id="88784-114">CalculatorService.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="88784-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="88784-115">Makefile</span></span>
