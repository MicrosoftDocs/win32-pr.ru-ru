---
title: хттпклиентвискерберосоверсслексампле
description: В этом примере показан HTTP-клиент, отправляющий сообщения типа "запрос-ответ" с проверкой подлинности Kerberos через SSL в смешанном режиме.
ms.assetid: 48b20c9b-38e7-472c-9606-b454850e14c1
keywords:
- Хттпклиентвискерберосоверсслексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104068638"
---
# <a name="httpclientwithkerberosoversslexample"></a><span data-ttu-id="60257-106">хттпклиентвискерберосоверсслексампле</span><span class="sxs-lookup"><span data-stu-id="60257-106">HttpClientWithKerberosOverSslExample</span></span>

<span data-ttu-id="60257-107">В этом примере показан HTTP-клиент, отправляющий сообщения типа "запрос-ответ" с проверкой подлинности Kerberos через SSL в смешанном режиме.</span><span class="sxs-lookup"><span data-stu-id="60257-107">This example shows a HTTP client that sends request-reply messages, with kerberos over SSL mixed-mode security.</span></span> <span data-ttu-id="60257-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="60257-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="60257-109">Проверка подлинности клиента предоставляется билетом Kerberos АПРЕК в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="60257-109">Client authentication is provided by a kerberos APREQ ticket in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="60257-110">Хттпклиентвискерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="60257-110">HttpClientWithKerberosOverSsl.cpp</span></span>](#httpclientwithkerberosoversslcpp)
-   [<span data-ttu-id="60257-111">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="60257-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="60257-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="60257-112">Makefile</span></span>](#makefile)

## <a name="httpclientwithkerberosoversslcpp"></a><span data-ttu-id="60257-113">Хттпклиентвискерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="60257-113">HttpClientWithKerberosOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="60257-114">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="60257-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="60257-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="60257-115">Makefile</span></span>
