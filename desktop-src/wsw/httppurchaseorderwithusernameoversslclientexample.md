---
title: хттппурчасеордервисусернамеоверсслклиентексампле
description: В этом примере показан HTTP-клиент, использующий прокси-сервер службы для взаимодействия со службой PurchaseOrder, с именем пользователя в смешанном режиме защиты SSL.
ms.assetid: 9874f00d-1efa-47ed-8029-f93962046789
keywords:
- Хттппурчасеордервисусернамеоверсслклиентексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104258754"
---
# <a name="httppurchaseorderwithusernameoversslclientexample"></a><span data-ttu-id="fa809-106">хттппурчасеордервисусернамеоверсслклиентексампле</span><span class="sxs-lookup"><span data-stu-id="fa809-106">HttpPurchaseOrderWithUsernameOverSslClientExample</span></span>

<span data-ttu-id="fa809-107">В этом примере показан HTTP-клиент, использующий прокси-сервер службы для взаимодействия со службой PurchaseOrder, с именем пользователя в смешанном режиме защиты SSL.</span><span class="sxs-lookup"><span data-stu-id="fa809-107">This example shows a HTTP client that uses service proxy to talk to a PurchaseOrder service, with username over SSL mixed-mode security.</span></span> <span data-ttu-id="fa809-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="fa809-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="fa809-109">Проверка подлинности клиента обеспечивается парой имени пользователя и пароля в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="fa809-109">Client authentication is provided by a username/password pair in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="fa809-110">Пурчасеордерклиентусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="fa809-110">PurchaseOrderClientUserNameOverSsl.cpp</span></span>](#purchaseorderclientusernameoversslcpp)
-   [<span data-ttu-id="fa809-111">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="fa809-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="fa809-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="fa809-112">Makefile</span></span>](#makefile)

## <a name="purchaseorderclientusernameoversslcpp"></a><span data-ttu-id="fa809-113">Пурчасеордерклиентусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="fa809-113">PurchaseOrderClientUserNameOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="fa809-114">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="fa809-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="fa809-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="fa809-115">Makefile</span></span>
