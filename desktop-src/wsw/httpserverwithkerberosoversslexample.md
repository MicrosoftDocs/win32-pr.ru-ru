---
title: хттпсервервискерберосоверсслексампле
description: В этом примере показан TCP-сервер, принимающий канал и принимающий сообщения и отвечающий на него, с защитой Kerberos через SSL в смешанном режиме.
ms.assetid: 2910f594-2f38-4c1e-bb47-89258b752572
keywords:
- Хттпсервервискерберосоверсслексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103986825"
---
# <a name="httpserverwithkerberosoversslexample"></a><span data-ttu-id="5b831-106">хттпсервервискерберосоверсслексампле</span><span class="sxs-lookup"><span data-stu-id="5b831-106">HttpServerWithKerberosOverSslExample</span></span>

<span data-ttu-id="5b831-107">В этом примере показан TCP-сервер, принимающий канал и принимающий сообщения и отвечающий на него, с защитой Kerberos через SSL в смешанном режиме.</span><span class="sxs-lookup"><span data-stu-id="5b831-107">This example shows a TCP server that accepts a channel and receives and replies to messages, with kerberos over SSL mixed-mode security.</span></span> <span data-ttu-id="5b831-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="5b831-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="5b831-109">Проверка подлинности клиента предоставляется билетом Kerberos АПРЕК в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="5b831-109">Client authentication is provided by a kerberos APREQ ticket in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="5b831-110">Хттпсервервискерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="5b831-110">HttpServerWithKerberosOverSsl.cpp</span></span>](#httpserverwithkerberosoversslcpp)
-   [<span data-ttu-id="5b831-111">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="5b831-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="5b831-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="5b831-112">Makefile</span></span>](#makefile)

## <a name="httpserverwithkerberosoversslcpp"></a><span data-ttu-id="5b831-113">Хттпсервервискерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="5b831-113">HttpServerWithKerberosOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="5b831-114">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="5b831-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="5b831-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="5b831-115">Makefile</span></span>
