---
title: хттпкалкулаторвисусернамеоверсслклиентексампле
description: В этом примере показан HTTP-клиент, использующий прокси-сервер службы для взаимодействия со службой калькулятора, с именем пользователя в смешанном режиме защиты SSL.
ms.assetid: 82f27502-0f18-4b30-9da1-9191e34366c6
keywords:
- Хттпкалкулаторвисусернамеоверсслклиентексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105700513"
---
# <a name="httpcalculatorwithusernameoversslclientexample"></a><span data-ttu-id="63c9b-106">хттпкалкулаторвисусернамеоверсслклиентексампле</span><span class="sxs-lookup"><span data-stu-id="63c9b-106">HttpCalculatorWithUsernameOverSslClientExample</span></span>

<span data-ttu-id="63c9b-107">В этом примере показан HTTP-клиент, использующий прокси-сервер службы для взаимодействия со службой калькулятора, с именем пользователя в смешанном режиме защиты SSL.</span><span class="sxs-lookup"><span data-stu-id="63c9b-107">This example shows a HTTP client that uses service proxy to talk to a calculator service, with username over SSL mixed-mode security.</span></span> <span data-ttu-id="63c9b-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="63c9b-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="63c9b-109">Проверка подлинности клиента обеспечивается парой имени пользователя и пароля в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="63c9b-109">Client authentication is provided by a username/password pair in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="63c9b-110">Калкулаторклиентусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="63c9b-110">CalculatorClientUserNameOverSsl.cpp</span></span>](#calculatorclientusernameoversslcpp)
-   [<span data-ttu-id="63c9b-111">CalculatorService. WSDL</span><span class="sxs-lookup"><span data-stu-id="63c9b-111">CalculatorService.wsdl</span></span>](#calculatorservicewsdl)
-   [<span data-ttu-id="63c9b-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="63c9b-112">Makefile</span></span>](#makefile)

## <a name="calculatorclientusernameoversslcpp"></a><span data-ttu-id="63c9b-113">Калкулаторклиентусернамеоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="63c9b-113">CalculatorClientUserNameOverSsl.cpp</span></span>
...
## <a name="calculatorservicewsdl"></a><span data-ttu-id="63c9b-114">CalculatorService. WSDL</span><span class="sxs-lookup"><span data-stu-id="63c9b-114">CalculatorService.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="63c9b-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="63c9b-115">Makefile</span></span>
