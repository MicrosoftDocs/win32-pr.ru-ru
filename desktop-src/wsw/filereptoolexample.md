...
Это средство командной строки, которое управляет [службой филереп](filerepserviceexample.md). Более подробное описание этого примера можно найти здесь.
...
URL-адрес службы клиента: обязательный параметр. URL-адрес службы клиента.

URL-адрес службы сервера: обязательный. URL-адрес службы сервера.

Исходный файл: обязательный параметр. Полное локальное имя исходного файла. Файл находится на компьютере, где запущена служба сервера.

Целевой файл: обязательный параметр. Полное локальное имя целевого файла. Файл будет расположен на компьютере, на котором запущена служба клиента.

Кодировка: необязательный. Указывает кодировку сообщений для сообщений, передаваемых между службами клиента и сервера. Кодировка, используемая для обмена данными между программой командной строки и службой клиента, не может быть изменена по соображениям простоты. Допустимые параметры: binary, Text и MTOM. Если параметр не указан, используется кодировка по умолчанию для транспорта.

Синхронизация: необязательно. Если этот параметр задан, запрос завершается синхронно. В противном случае запрос будет выполнен асинхронно.
