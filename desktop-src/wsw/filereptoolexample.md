---
title: филерептулексампле
description: Это средство командной строки, которое управляет службой Филереп.
ms.assetid: e6273036-2e31-4cbb-b72b-8f4d3ade6336
keywords:
- Филерептулексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
---
# <a name="filereptoolexample"></a><span data-ttu-id="e4d41-106">филерептулексампле</span><span class="sxs-lookup"><span data-stu-id="e4d41-106">FileRepToolExample</span></span>

<span data-ttu-id="e4d41-107">Это средство командной строки, которое управляет [службой филереп](filerepserviceexample.md).</span><span class="sxs-lookup"><span data-stu-id="e4d41-107">This is the command line tool that drives the [FileRep service](filerepserviceexample.md).</span></span> <span data-ttu-id="e4d41-108">Более подробное описание этого примера можно найти здесь.</span><span class="sxs-lookup"><span data-stu-id="e4d41-108">A more detailed description of this sample can be found there.</span></span>

<span data-ttu-id="e4d41-109">Ниже приведены параметры командной строки.</span><span class="sxs-lookup"><span data-stu-id="e4d41-109">The command line parameters are as follows:</span></span>

<span data-ttu-id="e4d41-110">WsFileRep.exe <Client Service Url> <Server Service Url> <Source File> <Destination File> \[ /Encoding: <binary/Text/MTOM>\] \[ /Sync\]</span><span class="sxs-lookup"><span data-stu-id="e4d41-110">WsFileRep.exe <Client Service Url> <Server Service Url> <Source File> <Destination File> \[/encoding:<binary/text/MTOM>\] \[/sync\]</span></span>

<span data-ttu-id="e4d41-111">URL-адрес службы клиента: обязательный параметр.</span><span class="sxs-lookup"><span data-stu-id="e4d41-111">Client Service Url: Mandatory.</span></span> <span data-ttu-id="e4d41-112">URL-адрес службы клиента.</span><span class="sxs-lookup"><span data-stu-id="e4d41-112">Url of the client service.</span></span>

<span data-ttu-id="e4d41-113">URL-адрес службы сервера: обязательный.</span><span class="sxs-lookup"><span data-stu-id="e4d41-113">Server Service Url: Mandatory.</span></span> <span data-ttu-id="e4d41-114">URL-адрес службы сервера.</span><span class="sxs-lookup"><span data-stu-id="e4d41-114">Url of the server service.</span></span>

<span data-ttu-id="e4d41-115">Исходный файл: обязательный параметр.</span><span class="sxs-lookup"><span data-stu-id="e4d41-115">Source File: Mandatory.</span></span> <span data-ttu-id="e4d41-116">Полное локальное имя исходного файла.</span><span class="sxs-lookup"><span data-stu-id="e4d41-116">Fully qualified local name of the source file.</span></span> <span data-ttu-id="e4d41-117">Файл находится на компьютере, где запущена служба сервера.</span><span class="sxs-lookup"><span data-stu-id="e4d41-117">The file is located on the machine where the server service runs.</span></span>

<span data-ttu-id="e4d41-118">Целевой файл: обязательный параметр.</span><span class="sxs-lookup"><span data-stu-id="e4d41-118">Destination file: Mandatory.</span></span> <span data-ttu-id="e4d41-119">Полное локальное имя целевого файла.</span><span class="sxs-lookup"><span data-stu-id="e4d41-119">Fully qualified local name of the destination file.</span></span> <span data-ttu-id="e4d41-120">Файл будет расположен на компьютере, на котором запущена служба клиента.</span><span class="sxs-lookup"><span data-stu-id="e4d41-120">The file will be located on the machine where the client service runs.</span></span>

<span data-ttu-id="e4d41-121">Кодировка: необязательный.</span><span class="sxs-lookup"><span data-stu-id="e4d41-121">Encoding: Optional.</span></span> <span data-ttu-id="e4d41-122">Указывает кодировку сообщений для сообщений, передаваемых между службами клиента и сервера.</span><span class="sxs-lookup"><span data-stu-id="e4d41-122">Specifies the message encoding for the messages sent between the client and server services.</span></span> <span data-ttu-id="e4d41-123">Кодировка, используемая для обмена данными между программой командной строки и службой клиента, не может быть изменена по соображениям простоты.</span><span class="sxs-lookup"><span data-stu-id="e4d41-123">The encoding used for the communication between the command line tool and the client service cannot be changed for simplicity reasons.</span></span> <span data-ttu-id="e4d41-124">Допустимые параметры: binary, Text и MTOM.</span><span class="sxs-lookup"><span data-stu-id="e4d41-124">Valid parameters are binary, text and MTOM.</span></span> <span data-ttu-id="e4d41-125">Если параметр не указан, используется кодировка по умолчанию для транспорта.</span><span class="sxs-lookup"><span data-stu-id="e4d41-125">If the parameter is not specified the default encoding for transport is used.</span></span>

<span data-ttu-id="e4d41-126">Синхронизация: необязательно.</span><span class="sxs-lookup"><span data-stu-id="e4d41-126">Sync: Optional.</span></span> <span data-ttu-id="e4d41-127">Если этот параметр задан, запрос завершается синхронно.</span><span class="sxs-lookup"><span data-stu-id="e4d41-127">When set, the request completes synchronously.</span></span> <span data-ttu-id="e4d41-128">В противном случае запрос будет выполнен асинхронно.</span><span class="sxs-lookup"><span data-stu-id="e4d41-128">Otherwise the request will complete asynchronously.</span></span>

-   [<span data-ttu-id="e4d41-129">Tool. cpp</span><span class="sxs-lookup"><span data-stu-id="e4d41-129">Tool.cpp</span></span>](#toolcpp)
-   [<span data-ttu-id="e4d41-130">Common. h</span><span class="sxs-lookup"><span data-stu-id="e4d41-130">common.h</span></span>](#commonh)
-   [<span data-ttu-id="e4d41-131">См. также</span><span class="sxs-lookup"><span data-stu-id="e4d41-131">Related topics</span></span>](#related-topics)

## <a name="toolcpp"></a><span data-ttu-id="e4d41-132">Tool. cpp</span><span class="sxs-lookup"><span data-stu-id="e4d41-132">Tool.cpp</span></span>
...
## <a name="commonh"></a><span data-ttu-id="e4d41-133">common.h</span><span class="sxs-lookup"><span data-stu-id="e4d41-133">common.h</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="e4d41-134">См. также</span><span class="sxs-lookup"><span data-stu-id="e4d41-134">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="e4d41-135">филерепсервицеексампле</span><span class="sxs-lookup"><span data-stu-id="e4d41-135">FileRepServiceExample</span></span>](filerepserviceexample.md)
</dt> </dl>
