---
title: хттппурчасеордервискерберосоверсслклиентексампле
description: В этом примере показан HTTP-клиент, использующий прокси-сервер службы для взаимодействия со службой PurchaseOrder, с защищенным режимом Kerberos через SSL в смешанном режиме.
ms.assetid: 6756043c-ef4c-4fa6-a27b-7ecb900601f9
keywords:
- Хттппурчасеордервискерберосоверсслклиентексампле Native-Web-Services
- ввсапи
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104411096"
---
# <a name="httppurchaseorderwithkerberosoversslclientexample"></a><span data-ttu-id="92c60-106">хттппурчасеордервискерберосоверсслклиентексампле</span><span class="sxs-lookup"><span data-stu-id="92c60-106">HttpPurchaseOrderWithKerberosOverSslClientExample</span></span>

<span data-ttu-id="92c60-107">В этом примере показан HTTP-клиент, использующий прокси-сервер службы для взаимодействия со службой PurchaseOrder, с защищенным режимом Kerberos через SSL в смешанном режиме.</span><span class="sxs-lookup"><span data-stu-id="92c60-107">This example shows a HTTP client that uses service proxy to talk to a PurchaseOrder service, with kerberos over SSL mixed-mode security.</span></span> <span data-ttu-id="92c60-108">В этой настройке транспортное соединение защищено (подписано, зашифровано) по протоколу SSL, которое также обеспечивает проверку подлинности сервера.</span><span class="sxs-lookup"><span data-stu-id="92c60-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="92c60-109">Проверка подлинности клиента предоставляется билетом Kerberos АПРЕК в заголовке WS-Security сообщения.</span><span class="sxs-lookup"><span data-stu-id="92c60-109">Client authentication is provided by a kerberos APREQ ticket in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="92c60-110">Пурчасеордерклиенткерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="92c60-110">PurchaseOrderClientKerberosOverSsl.cpp</span></span>](#purchaseorderclientkerberosoversslcpp)
-   [<span data-ttu-id="92c60-111">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="92c60-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="92c60-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="92c60-112">Makefile</span></span>](#makefile)

## <a name="purchaseorderclientkerberosoversslcpp"></a><span data-ttu-id="92c60-113">Пурчасеордерклиенткерберосоверссл. cpp</span><span class="sxs-lookup"><span data-stu-id="92c60-113">PurchaseOrderClientKerberosOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="92c60-114">PurchaseOrder. WSDL</span><span class="sxs-lookup"><span data-stu-id="92c60-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="92c60-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="92c60-115">Makefile</span></span>
