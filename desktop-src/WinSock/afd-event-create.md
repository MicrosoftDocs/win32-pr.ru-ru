...
в следующей таблице перечислены возможные значения параметра *type* , поддерживаемые для сокетов Windows 2:
