---
description: Ниже приведен код Simple. c, который является сервером сокетов Windows, поддерживающим только IPv4 (в приложении б можно найти версию файла simple. c с поддержкой IPv6).
ms.assetid: 02e599e9-8652-4ca1-9355-b989dadedcdf
title: Серверный код только для IPv4
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105711027"
---
# <a name="ipv4-only-server-code"></a><span data-ttu-id="2b865-103">Серверный код только для IPv4</span><span class="sxs-lookup"><span data-stu-id="2b865-103">IPv4-only Server Code</span></span>

<span data-ttu-id="2b865-104">Ниже приведен код Simple. c, который является сервером сокетов Windows, поддерживающим только IPv4 (в приложении б можно найти версию файла simple. c с поддержкой IPv6).</span><span class="sxs-lookup"><span data-stu-id="2b865-104">The following code is the Simples.c file, which is an IPv4-only Windows Sockets server (an IPv6-enabled version of the Simples.c file can be found in Appendix B).</span></span> <span data-ttu-id="2b865-105">Этот код предоставляется только для сравнения — используйте приложение б, чтобы получить пример того, как создать сервер с поддержкой IPv6.</span><span class="sxs-lookup"><span data-stu-id="2b865-105">This code is provided for comparison purposes only — use Appendix B for an example of how to write an IPv6-enabled server.</span></span>
