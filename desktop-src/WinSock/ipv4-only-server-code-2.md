---
description: следующий код представляет собой файл simple. c, который является сервером Windows сокетов только для IPv4 (версия файла simple. c с поддержкой IPv6 доступна в приложении б).
...
ms.locfileid: "119051462"
---
# <a name="ipv4-only-server-code"></a>Серверный код только для IPv4

следующий код представляет собой файл simple. c, который является сервером Windows сокетов только для IPv4 (версия файла simple. c с поддержкой IPv6 доступна в приложении б). Этот код предоставляется только для сравнения — используйте приложение б, чтобы получить пример того, как создать сервер с поддержкой IPv6.
