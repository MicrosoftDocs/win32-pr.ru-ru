...
---
# <a name="ipv4-only-server-code"></a>Серверный код только для IPv4

Ниже приведен код Simple. c, который является сервером сокетов Windows, поддерживающим только IPv4 (в приложении б можно найти версию файла simple. c с поддержкой IPv6). Этот код предоставляется только для сравнения — используйте приложение б, чтобы получить пример того, как создать сервер с поддержкой IPv6.
