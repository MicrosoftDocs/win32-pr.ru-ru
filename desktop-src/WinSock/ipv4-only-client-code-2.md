...
---
# <a name="ipv4-only-client-code"></a>Код клиента только для IPv4

Следующий код — это файл Симплек. c, который является клиентом сокетов Windows, поддерживающим только IPv4 (версия файла Симплек. c с поддержкой IPv6 доступна в приложении б). Этот код предоставляется только в целях сравнения — используйте приложение б, чтобы получить пример того, как создать клиент с поддержкой IPv6.
