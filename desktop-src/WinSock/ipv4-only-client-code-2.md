---
description: следующий код представляет собой файл симплек. c, который является клиентом сокетов Windows только IPv4 (версия симплек. c с поддержкой IPv6 доступна в приложении б).
...
ms.locfileid: "120097734"
---
# <a name="ipv4-only-client-code"></a>Код клиента только для IPv4

следующий код представляет собой файл симплек. c, который является клиентом сокетов Windows только IPv4 (версия симплек. c с поддержкой IPv6 доступна в приложении б). Этот код предоставляется только в целях сравнения — используйте приложение б, чтобы получить пример того, как создать клиент с поддержкой IPv6.
