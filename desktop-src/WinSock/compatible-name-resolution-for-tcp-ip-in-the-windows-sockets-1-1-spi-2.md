...
---
# <a name="compatible-name-resolution-for-tcpip-in-the-windows-sockets-11-spi"></a>Совместимое разрешение имен для TCP/IP в Windows Sockets 1,1 SPI

Сокеты Windows 1,1 определяют ряд подпрограмм, которые использовались для разрешения имен IPv4 в сетях TCP/IP. Эти функции также называются функциями **жетксбии** и включают следующее.
...
Эти функции относятся к сетям TCP/IP. разработчикам приложений, не зависящих от протокола, не рекомендуется продолжать использовать эти функции, связанные с транспортом. Тем не менее, чтобы обеспечить максимальную обратную совместимость с сокетами Windows 1,1, предыдущие функции продолжают поддерживаться до тех пор, пока имеется хотя бы один поставщик пространства имен, поддерживающий \_ семейство адресов AF inet.
