...
---
# <a name="receiving-event-notification"></a>Получение уведомления о событии

В следующем примере показано, как использовать функцию [**нотифичанжеевентлог**](/windows/desktop/api/Winbase/nf-winbase-notifychangeeventlog) для получения уведомлений при записи события в журнал. Этот пример фильтрует события, записанные в примере в [сообщении о событии](reporting-an-event.md).
