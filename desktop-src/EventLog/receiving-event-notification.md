---
description: Получение уведомления о событии
ms.assetid: 3200d666-d927-4198-b1f6-1636971f5f07
title: Получение уведомления о событии
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103812886"
---
# <a name="receiving-event-notification"></a><span data-ttu-id="e0b03-103">Получение уведомления о событии</span><span class="sxs-lookup"><span data-stu-id="e0b03-103">Receiving Event Notification</span></span>

<span data-ttu-id="e0b03-104">В следующем примере показано, как использовать функцию [**нотифичанжеевентлог**](/windows/desktop/api/Winbase/nf-winbase-notifychangeeventlog) для получения уведомлений при записи события в журнал.</span><span class="sxs-lookup"><span data-stu-id="e0b03-104">The following example shows how to use the [**NotifyChangeEventLog**](/windows/desktop/api/Winbase/nf-winbase-notifychangeeventlog) function to receive notification when an event is logged.</span></span> <span data-ttu-id="e0b03-105">Этот пример фильтрует события, записанные в примере в [сообщении о событии](reporting-an-event.md).</span><span class="sxs-lookup"><span data-stu-id="e0b03-105">This example filters for the events written by the example in [Reporting an Event](reporting-an-event.md).</span></span>
