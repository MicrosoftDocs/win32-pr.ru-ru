...
---
# <a name="registering-for-device-notification"></a>Регистрация для получения уведомлений устройства

Операционная система может отсылать сообщения уведомления, представляющие события, вызванные устройством. Система отправляет эти уведомления либо в окно, либо в службу Windows. И вы можете зарегистрироваться для получения этих уведомлений, вызвав функцию [**регистердевиценотификатион**](/windows/win32/api/winuser/nf-winuser-registerdevicenotificationw) .

В этом разделе приведен пример кода, показывающий, как зарегистрироваться для получения уведомлений устройства. Вы можете определить устройство для получения уведомлений, указав класс устройства при вызове **регистердевиценотификатион**. И вы определяете класс устройства по его идентификатору GUID.

## <a name="source-code-listing"></a>Листинг исходного кода

Если вы хотите собрать и запустить этот пример исходного кода как приложение, начните с создания нового проекта в Microsoft Visual Studio. Создайте новый проект на основе шаблона проекта **классическое приложение Windows** . Назовите проект *регистердевиценотификатион*.

Откройте файл `RegisterDeviceNotification.cpp`, удалите все его содержимое и вставьте список ниже.
...
## <a name="about-the-code-example"></a>Сведения о примере кода

В примере кода используется маркер окна для уведомлений об изменении устройства, который содержит большой объем кода, просто для управления инфраструктурой окон. Приложение, предназначенное для установки в качестве службы Windows, будет содержать другой код инфраструктуры.

Приложение получит сообщение [**WM_DEVICECHANGE**](./wm-devicechange.md) всякий раз, когда отправляется уведомление о событии интерфейса USB-устройства.

Система передает набор событий изменения устройства по умолчанию всем приложениям и службам. Для получения этих событий по умолчанию не нужно регистрироваться. Дополнительные сведения см. в подразделе **"Примечания"** в [**регистердевиценотификатион**](/windows/win32/api/winuser/nf-winuser-registerdevicenotificationw).
