...
---
# <a name="winsat-schema"></a>Схема WinSAT

\[Перечисления WinSAT могут быть изменены или недоступны для выпусков после Windows 8.1.\]

Для определения допустимых запросов XPath можно использовать схему средства оценки системы Windows (WinSAT). Чтобы выполнить запрос XPath, ознакомьтесь с одним из следующих методов.

-   [**Икуеряллвинсатассессментс:: Get \_ аллксмл**](/windows/desktop/api/Winsatcominterfacei/nf-winsatcominterfacei-iqueryallwinsatassessments-get_allxml)
-   [**Икуерирецентвинсатассессмент:: Get \_ XML**](/windows/desktop/api/Winsatcominterfacei/nf-winsatcominterfacei-iqueryrecentwinsatassessment-get_xml)

Элемент **WinSAT** может иметь элемент **винсатассессментс** в качестве родительского элемента при вызове метода [**Get \_ аллксмл**](/windows/desktop/api/Winsatcominterfacei/nf-winsatcominterfacei-iqueryallwinsatassessments-get_allxml) .

Ниже приведена схема WinSAT, которую можно использовать для определения XPath.
