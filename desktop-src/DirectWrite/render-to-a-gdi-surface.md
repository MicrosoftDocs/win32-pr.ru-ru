...
в некоторых случаях может потребоваться отображать [DirectWrite](direct-write-portal.md) текст на поверхности GDI. Интерфейс [**идвритебитмапрендертаржет**](/windows/win32/api/dwrite/nn-dwrite-idwritebitmaprendertarget) инкапсулирует растровое изображение и контекст устройства для отрисовки текста. Вы создаете **идвритебитмапрендертаржет** с помощью метода [**Идвритегдиинтероп:: креатебитмапрендертаржет**](/windows/win32/api/dwrite/nf-dwrite-idwritegdiinterop-createbitmaprendertarget) , как показано в следующем коде.
...
ограничивающий прямоугольник можно получить с помощью функции [**жетбаундсрект**](/windows/win32/api/wingdi/nf-wingdi-getboundsrect) , а затем использовать ограничивающий прямоугольник с функцией [**BitBlt**](/windows/win32/api/wingdi/nf-wingdi-bitblt) , чтобы скопировать отображаемый [DirectWrite](direct-write-portal.md) текст с контроллера домена памяти на поверхность GDI, как показано в следующем коде.
