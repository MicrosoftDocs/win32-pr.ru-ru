...
description: в этой статье описаны некоторые основные понятия, с которыми разработчики игр для Windows могут и должны реализовываться для улучшения общего опыта работы.
...
ms.locfileid: "120042480"
---
# <a name="simplifying-game-installation"></a>Упрощение установки игр

одним из основных преимуществ игр, работающих на консоли, а не на Windows является процесс установки или недостаток. При первом запуске игры на консоли игрок создает несколько вариантов или подтверждений и может начать играть практически сразу же. установка игры на Windows более сложная, по сравнению с тем, что она требует значительных пользовательских данных и потенциально длительного процесса установки. однако этот процесс установки можно улучшить, чтобы обеспечить лучшую работу для игроков в игры на основе Windows. в этой статье описаны некоторые основные понятия, с которыми разработчики игр для Windows могут и должны реализовываться для улучшения общего опыта работы.
...
-   [Упрощенная установка игр](#simplified-game-installation)
    ...
    -   [регистрация в Windows брандмауэре во время установки](#register-with-windows-firewall-during-installation)
...
При сравнении простоты установки и количества времени, необходимого для начала игры, типичный интерфейс Xbox гораздо лучше, чем Windows. в блоковой диаграмме на рис. 1 показаны стандартные процессы установки на Xbox и Windows для сравнения.
...
однако дополнительные требования к установке игры на Windows не обязательно должны быть назначены пользователю. Реализуя следующие основные понятия, вы уменьшите число шагов, которое должен выполнить пользователь, что может сократить время, необходимое для установки.
...
в идеале Windows установщики игр должны предоставлять только полностью автоматические и пользовательские режимы установки, а не между ними.
...
### <a name="register-with-windows-firewall-during-installation"></a>регистрация в Windows брандмауэре во время установки

если ваша игра может работать в качестве сервера или модель сетевой сети является одноранговой, зарегистрируйте игру в Windows брандмауэре во время установки. Это предотвратит появление диалогового окна брандмауэра в середине игры, когда пользователь пытается получить доступ к сети. Если игра является чистым клиентом, то установщик не должен добавлять игру в список исключений брандмауэра.

дополнительные сведения см. в подWindows брандмауэре для разработчиков игр.
...
Игроки хотят как можно быстрее играть в игру. Последнее, что необходимо сделать для игр, — это продирание с помощью диалоговых окон и выбора того же варианта, что и для всех остальных игр, на которых он установлен. реализация этих идей может сократить время, затрачиваемое игроками на установку игр на Windows и улучшить общее качество Windows игр.
