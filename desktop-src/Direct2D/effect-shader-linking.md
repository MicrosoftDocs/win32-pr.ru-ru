...
> [!Note]  
> При компиляции воздействия с помощью Visual Studio следует создать пакетный файл, выполняющий обе команды FXC, и запустить этот пакетный файл в качестве настраиваемого этапа сборки, запускаемого до этапа компиляции.
...
Спецификации для полных шейдеров являются теми же, что и в предыдущих версиях Windows. Вкратце, входные параметры шейдера пикселей должны быть \_ ПОЛОЖЕНИЕМ SV, \_ позицией сцены и одним текскурд на входные данные.
...
-   Сложный ввод
    ...
    Для сложных входов D2D будет передавать только координату текстуры, как описано в документации по Windows 8.
