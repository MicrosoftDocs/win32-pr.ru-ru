...
Приложение HTTP может включать поле заголовка авторизации с запросом, который отправляется на сервер. Заголовок Authorization содержит схему проверки подлинности и соответствующий ответ, необходимый для этой схемы. Например, заголовок «Authorization: Basic <имя_пользователя: пароль>» будет добавлен в запрос и отправлен на сервер, если клиент получил заголовок ответа «WWW-Authenticate: Basic realm =» example».
