...
Свойство **ResponseStream** извлекает тело сущности ответа в виде [**IStream**](/windows/desktop/api/objidl/nn-objidl-istream).
...
**Вариант** , получающий указатель на интерфейс [**IUnknown**](/windows/win32/api/unknwn/nn-unknwn-iunknown) , который может запрашивать интерфейс [**IStream**](/windows/desktop/api/objidl/nn-objidl-istream) . Этот поток возвращает необработанные данные, полученные непосредственно с сервера.
...
В следующем примере показано, как открыть HTTP-соединение, отправить HTTP-запрос и прочитать ответ в виде [**IStream**](/windows/desktop/api/objidl/nn-objidl-istream). Данные из **IStream** записываются в файл Temp1.gif.
