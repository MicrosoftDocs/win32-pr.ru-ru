...
ms.locfileid: "117744460"
---
# <a name="iwinhttprequeststatus-property"></a>Свойство Ивинхттпрекуест:: Status

Свойство **Status** получает код состояния HTTP от последнего ответа.
...
Значение типа **Long** , которое получает возвращенный код состояния.
...
Результаты этого свойства действительны только после успешного завершения метода [**Send**](iwinhttprequest-send.md) . Список кодов состояния см. в разделе [**коды состояния HTTP**](http-status-codes.md).
...
В следующем примере показано, как открыть HTTP-соединение, отправить HTTP-запрос, отобразить **состояние** и [**StatusText**](iwinhttprequest-statustext.md), а также прочитать заголовки ответа. Этот пример необходимо запустить из командной строки.
...
В следующем примере скрипта показано, как открыть HTTP-соединение, отправить HTTP-запрос, отобразить **состояние** и [**StatusText**](iwinhttprequest-statustext.md), а также прочитать текст ответа.
