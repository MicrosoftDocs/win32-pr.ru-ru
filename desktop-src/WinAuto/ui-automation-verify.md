...
Программа **UIA-проверки** устанавливается вместе с пакетом средств разработки программного обеспечения (SDK) для Windows. Он находится в \\ папке bin \\ < *версии* > \\ < *Platform* > \\ уиаверифи пути установки пакета SDK (VisualUIAVerifyNative.exe).
...
> [!NOTE]
> **Проверка пользовательского интерфейса** — это устаревший инструмент. Вместо этого рекомендуется использовать [аналитические сведения о специальных возможностях](https://accessibilityinsights.io/) .
