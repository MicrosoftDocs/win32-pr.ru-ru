...
Компилятор ресурсов Microsoft Windows 32 (RC) требует запятую между параметрами *события* и *idvalue* в операторе [**ускорителей**](accelerators-resource.md) .
...
Компилятору ресурсов Microsoft Windows (RC) не удалось определить, какой ключ был настроен для ускорителя. Параметр *события* в операторе [**ускорителей**](accelerators-resource.md) может быть недопустимым.
