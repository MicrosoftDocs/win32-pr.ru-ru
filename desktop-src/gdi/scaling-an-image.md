---
description: Некоторые приложения масштабируют изображения; то есть они отображают масштабные или уменьшенные представления изображения. Например, приложение для рисования может предоставить функцию масштабирования, позволяющую пользователю просматривать и редактировать рисунок на основе попикселной.
...
ms.locfileid: "104080649"
---
# <a name="scaling-an-image"></a><span data-ttu-id="45daf-104">Масштабирование изображения</span><span class="sxs-lookup"><span data-stu-id="45daf-104">Scaling an Image</span></span>

<span data-ttu-id="45daf-105">Некоторые приложения масштабируют изображения; то есть они отображают масштабные или уменьшенные представления изображения.</span><span class="sxs-lookup"><span data-stu-id="45daf-105">Some applications scale images; that is, they display zoomed or reduced views of an image.</span></span> <span data-ttu-id="45daf-106">Например, приложение для рисования может предоставить функцию масштабирования, позволяющую пользователю просматривать и редактировать рисунок на основе попикселной.</span><span class="sxs-lookup"><span data-stu-id="45daf-106">For example, a drawing application may provide a zoom feature that enables the user to view and edit a drawing on a pixel-by-pixel basis.</span></span>

<span data-ttu-id="45daf-107">Приложения масштабируют изображения путем вызова функции [**стретчблт**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .</span><span class="sxs-lookup"><span data-stu-id="45daf-107">Applications scale images by calling the [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) function.</span></span> <span data-ttu-id="45daf-108">Как и функция [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) , **стретчблт** копирует растровые данные из точечного рисунка в исходном контексте устройства в точечный рисунок на целевом контроллере [**домена.**](/windows/desktop/api/Winuser/nf-winuser-getdcex)</span><span class="sxs-lookup"><span data-stu-id="45daf-108">Like the [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) function, **StretchBlt** copies bitmap data from a bitmap in a source device context ([**DC**](/windows/desktop/api/Winuser/nf-winuser-getdcex)) into a bitmap in a target DC.</span></span> <span data-ttu-id="45daf-109">Однако, в отличие от функции **BitBlt** , **стретчблт** масштабирует изображение на основе указанных размеров исходного и целевого прямоугольников.</span><span class="sxs-lookup"><span data-stu-id="45daf-109">However, unlike the **BitBlt** function, **StretchBlt** scales the image based on the specified dimensions of the source and target rectangles.</span></span> <span data-ttu-id="45daf-110">Если исходный прямоугольник больше целевого прямоугольника, результирующий образ будет выглядеть как сжатое; Если исходный прямоугольник меньше целевого прямоугольника, результирующий образ будет выглядеть как развернутый.</span><span class="sxs-lookup"><span data-stu-id="45daf-110">If the source rectangle is larger than the target rectangle, the resultant image will appear to have shrunk; if the source rectangle is smaller than the target rectangle, the resultant image will appear to have expanded.</span></span>

<span data-ttu-id="45daf-111">Если целевой прямоугольник меньше, чем исходный прямоугольник, [**стретчблт**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) удаляет цветовые данные из изображения в соответствии с заданным режимом растяжения, как показано в следующей таблице.</span><span class="sxs-lookup"><span data-stu-id="45daf-111">If the target rectangle is smaller than the source rectangle, [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) removes color data from the image according to a specified stretch mode as shown in the following table.</span></span>



| <span data-ttu-id="45daf-112">Режим Stretch</span><span class="sxs-lookup"><span data-stu-id="45daf-112">Stretch mode</span></span> | <span data-ttu-id="45daf-113">Метод</span><span class="sxs-lookup"><span data-stu-id="45daf-113">Method</span></span>                                                                                                                    |
|--------------|---------------------------------------------------------------------------------------------------------------------------|
| <span data-ttu-id="45daf-114">блакконвхите</span><span class="sxs-lookup"><span data-stu-id="45daf-114">BLACKONWHITE</span></span> | <span data-ttu-id="45daf-115">Выполняет логическую операцию и над данными цвета для исключающих пикселов и цветовых данных для оставшихся пикселов.</span><span class="sxs-lookup"><span data-stu-id="45daf-115">Performs a logical AND operation on the color data for the eliminated pixels and the color data for the remaining pixels.</span></span> |
| <span data-ttu-id="45daf-116">вхитеонблакк</span><span class="sxs-lookup"><span data-stu-id="45daf-116">WHITEONBLACK</span></span> | <span data-ttu-id="45daf-117">Выполняет логическую операцию OR с данными цвета для исключающих пикселов и цветовых данных для оставшихся пикселов.</span><span class="sxs-lookup"><span data-stu-id="45daf-117">Performs a logical OR operation on the color data for the eliminated pixels and the color data for the remaining pixels.</span></span>  |
| <span data-ttu-id="45daf-118">колоронколор</span><span class="sxs-lookup"><span data-stu-id="45daf-118">COLORONCOLOR</span></span> | <span data-ttu-id="45daf-119">Полностью удаляет данные цвета удаленных пикселей.</span><span class="sxs-lookup"><span data-stu-id="45daf-119">Eliminates the color data of the deleted pixels completely.</span></span>                                                               |
| <span data-ttu-id="45daf-120">НАСТРОЕК</span><span class="sxs-lookup"><span data-stu-id="45daf-120">HALFTONE</span></span>     | <span data-ttu-id="45daf-121">Приблизительный исходный (исходный) цвет данных в назначении.</span><span class="sxs-lookup"><span data-stu-id="45daf-121">Approximates the original (source) color data in the destination.</span></span>                                                         |



 

<span data-ttu-id="45daf-122">Режим Stretch устанавливается путем вызова функции [**сетстретчблтмоде**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) .</span><span class="sxs-lookup"><span data-stu-id="45daf-122">You set the stretch mode by calling the [**SetStretchBltMode**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) function.</span></span>

<span data-ttu-id="45daf-123">Следующий пример кода взят из приложения, в котором демонстрируются все четыре режима растяжения, доступные с помощью функции [**стретчблт**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .</span><span class="sxs-lookup"><span data-stu-id="45daf-123">The following example code is taken from an application that demonstrates all four of the stretch modes available with the [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) function.</span></span>
