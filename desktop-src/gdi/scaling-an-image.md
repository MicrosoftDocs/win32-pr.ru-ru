...
Некоторые приложения масштабируют изображения; то есть они отображают масштабные или уменьшенные представления изображения. Например, приложение для рисования может предоставить функцию масштабирования, позволяющую пользователю просматривать и редактировать рисунок на основе попикселной.

Приложения масштабируют изображения путем вызова функции [**стретчблт**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) . Как и функция [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) , **стретчблт** копирует растровые данные из точечного рисунка в исходном контексте устройства в точечный рисунок на целевом контроллере [**домена.**](/windows/desktop/api/Winuser/nf-winuser-getdcex) Однако, в отличие от функции **BitBlt** , **стретчблт** масштабирует изображение на основе указанных размеров исходного и целевого прямоугольников. Если исходный прямоугольник больше целевого прямоугольника, результирующий образ будет выглядеть как сжатое; Если исходный прямоугольник меньше целевого прямоугольника, результирующий образ будет выглядеть как развернутый.

Если целевой прямоугольник меньше, чем исходный прямоугольник, [**стретчблт**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) удаляет цветовые данные из изображения в соответствии с заданным режимом растяжения, как показано в следующей таблице.



| Режим Stretch | Метод                                                                                                                    |
|--------------|---------------------------------------------------------------------------------------------------------------------------|
| блакконвхите | Выполняет логическую операцию и над данными цвета для исключающих пикселов и цветовых данных для оставшихся пикселов. |
| вхитеонблакк | Выполняет логическую операцию OR с данными цвета для исключающих пикселов и цветовых данных для оставшихся пикселов.  |
| колоронколор | Полностью удаляет данные цвета удаленных пикселей.                                                               |
| НАСТРОЕК     | Приблизительный исходный (исходный) цвет данных в назначении.                                                         |



 

Режим Stretch устанавливается путем вызова функции [**сетстретчблтмоде**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) .

Следующий пример кода взят из приложения, в котором демонстрируются все четыре режима растяжения, доступные с помощью функции [**стретчблт**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .
