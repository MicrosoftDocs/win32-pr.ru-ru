...
Хотя при создании пера можно указать любой цвет, система использует только те цвета, которые доступны на устройстве. Это означает, что система использует ближайший совпадающий цвет, когда он реализует перо для рисования. При создании кистей система создает цвет с оттенокм при указании цвета, который не поддерживается устройством. В любом случае можно использовать макрос [**RGB**](/windows/desktop/api/Wingdi/nf-wingdi-rgb) , чтобы указать цвет при создании пера или кисти.
