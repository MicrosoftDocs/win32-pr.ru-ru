...
В этом разделе представлен полный листинг исходного кода для минимального Дкскоре приложения (написанного на [C++/WinRT](/windows/uwp/cpp-and-winrt-apis)). Большая часть кода, показанного ниже, объясняется в разделе [Использование дкскоре для перечисления адаптеров](dxcore-enum-adapters.md).
