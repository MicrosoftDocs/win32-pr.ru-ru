...
ms.locfileid: "122984337"
---
# <a name="jetsetcurrentindex-function"></a>Функция JetSetCurrentIndex
...
Функцию **жетсеткуррентиндекс** можно использовать для задания текущего индекса курсора. Текущий индекс курсора определяет, какие записи в таблице видимы для этого курсора, и порядок, в котором они отображаются, выбрав набор записей индекса, который будет использоваться для предоставления этих записей.
...
| <p>JET_errBadItagSequence</p> | <p>Вторичный индекс выбирается с параметром JET_bitNoMove и для первого многозначного ключевого столбца в определении нового индекса, который соответствует указанному порядковому номеру, отсутствует значение.</p> | 
...
| <p><strong>Юникод</strong></p> | <p>Реализуется как <strong>жетсеткуррентиндексв</strong> (Юникод) и <strong>жетсеткуррентиндекса</strong> (ANSI).</p> | 
