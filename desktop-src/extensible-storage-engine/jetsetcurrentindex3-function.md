...
---
# <a name="jetsetcurrentindex3-function"></a>Функция JetSetCurrentIndex3
...
Функция **JetSetCurrentIndex3** используется для задания текущего индекса курсора. Текущий индекс курсора определяет, какие записи в таблице видимы для этого курсора, и порядок, в котором они отображаются, выбрав набор записей индекса, который будет использоваться для предоставления этих записей.
...
<td><p>Вторичный индекс выбирается с параметром JET_bitNoMove и для первого многозначного ключевого столбца в определении нового индекса, который соответствует указанному порядковому номеру, отсутствует значение.</p></td>
</tr>
<tr class="odd">
...
<td><p>Содержимое идентификатора индекса недопустимо или устарело, и его необходимо обновить. Это может произойти для <strong>JetSetCurrentIndex3</strong> в следующих случаях:</p>
<ul>
...
<li><p>Все курсоры, ссылающиеся на таблицу, содержащую индекс, соответствующий ИДЕНТИФИКАТОРу индекса, были закрыты и подсистема удалила определение этого индекса из кэша схемы.</p></li>
...
<td><p>Реализуется как <strong>JetSetCurrentIndex3W</strong> (Юникод) и <strong>JetSetCurrentIndex3A</strong> (ANSI).</p></td>
</tr>
</tbody>
</table>
