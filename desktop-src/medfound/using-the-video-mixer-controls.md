---
description: использование элементов управления видео Mixer
ms.assetid: 475996c6-a205-4133-8882-f55beaf9f8fd
title: использование элементов управления видео Mixer
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "119887153"
---
# <a name="using-the-video-mixer-controls"></a>использование элементов управления видео Mixer
...
правильный способ получения указателей на эти интерфейсы зависит от того, используется ли DirectShowная версия евр или версия Media Foundation. Для Media Foundation Евр это также зависит от того, используется ли Евр напрямую или с помощью [сеанса мультимедиа](media-session.md). (Обычно приложение будет использовать Евр через сеанс мультимедиа, а не напрямую).
...
1.  Получите указатель на интерфейс [**имфжетсервице**](/windows/desktop/api/mfidl/nn-mfidl-imfgetservice) в Евр.
    ...
    -   если вы используете DirectShow фильтр евр, вызовите **QueryInterface** в фильтре.
...
## <a name="video-processor-settings"></a>процессор видео Параметры
...
-   **Прокампконтролкапс**. В этом поле указываются доступные элементы управления коррекцией цвета. список возможных вариантов цветовой коррекции см. в разделе [Параметрые camp](procamp-settings.md). Если драйвер не может выполнить настройку цвета, это поле будет равно нулю.
...
5.  Если драйвер поддерживает фильтрацию изображений, то каждый тип фильтра (шум и сведения) поддерживает три параметра: уровень, радиус и пороговое значение — в чрома и яркости. (см. раздел [дксва Image Filter Параметры](dxva-image-filter-settings.md).) Для каждого параметра вызовите [**имфвидеопроцессор:: жетфилтерингранже**](/windows/desktop/api/evr9/nf-evr9-imfvideoprocessor-getfilteringrange) , чтобы получить диапазон возможных значений, и вызовите [**Имфвидеопроцессор:: жетфилтерингвалуе**](/windows/desktop/api/evr9/nf-evr9-imfvideoprocessor-getfilteringvalue) , чтобы получить текущее значение.
