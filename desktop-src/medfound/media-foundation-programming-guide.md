---
description: Media Foundationое программное руководством
ms.assetid: 5258b2ff-96d5-490f-9a18-c01dd6924f1c
title: Media Foundationое программное руководством
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 281cc30c84f422e5ba620be977b338bd6a11282d805ba50539d99e530363d5f0
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/11/2021
ms.locfileid: "120114134"
---
# <a name="media-foundation-programming-guide"></a>Media Foundationое программное руководством

В этом разделе содержатся обзорные разделы по использованию Microsoft Media Foundation в приложении C++.

### <a name="getting-started"></a>Приступая к работе

-   [Основные понятия](media-foundation-programming--essential-concepts.md)
-   [Архитектура Media Foundation](media-foundation-architecture.md)

### <a name="writing-media-applications"></a>Написание мультимедийных приложений

-   [Воспроизведение звука/видео](audio-video-playback.md)
-   [Захват аудио- и видеоданных](audio-video-capture.md)
-   [Кодировка и разработка файлов](encoding-and-file-authoring.md)
-   [Метаданные мультимедиа](media-metadata.md)
-   [Сетевые подключения в Media Foundation](networking-in-media-foundation.md)

### <a name="media-format-support"></a>Поддержка формата мультимедиа

-   [Поддерживаемые форматы мультимедиа в Media Foundation](supported-media-formats-in-media-foundation.md)
-   [Media Foundation объекты кодека](codecobjects.md)
-   [Поддержка ASF в Media Foundation](asf-support-in-media-foundation.md)
-   [Поддержка MPEG-4 в Media Foundation](mpeg-4-support-in-media-foundation.md)
-   [Кодеки Windows Media](windows-media-codecs.md)

### <a name="low-level-video-apis"></a>API-интерфейсы Low-Level видео

-   [API-интерфейсы видео Direct3D 9](direct3d-video-apis.md)
-   [API-интерфейсы видео Direct3D 11](direct3d-11-video-apis.md)
-   [Ускорение видео DirectX 2,0](directx-video-acceleration-2-0.md)
-   [Диспетчер выходной защиты](output-protection-manager.md)

### <a name="writing-media-foundation-plug-ins"></a>Написание подключаемых модулей Media Foundation

-   [Преобразования Media Foundation](media-foundation-transforms.md)
-   [Приемники носителей](media-sinks.md)
-   [Источники мультимедиа](media-sources.md)

## <a name="related-topics"></a>Связанные темы

<dl> <dt>

[Архитектура Media Foundation](media-foundation-architecture.md)
</dt> <dt>

[Microsoft Media Foundation](microsoft-media-foundation-sdk.md)
</dt> </dl>

 

 



