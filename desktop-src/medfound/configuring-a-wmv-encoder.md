...
Чтобы создать допустимый тип выходных данных для кодировщика видео Windows Media (WMV), необходимо иметь следующие сведения:
...
Свойства конфигурации описаны в документации по API-интерфейсам аудио-и видеокодеков Windows Media и DSP. Дополнительные сведения см. в разделе "Свойства потока видео" раздела [Свойства кодировки](configuring-the-encoder.md).
...
Данные частного кодека — это непрозрачная структура данных, которую необходимо получить из кодировщика WMV и добавить к типу сжатия, прежде чем задавать тип сжатия кодировщика. Для получения закрытых данных необходимо использовать интерфейс **ивмкодекприватедата** , который описан в пакете SDK для Windows Media Format 11.
...
3.  Преобразуйте тип мультимедиа в структуру [**\_ \_ типа мультимедиа DMO**](/previous-versions/windows/desktop/api/mediaobj/ns-mediaobj-dmo_media_type) , вызвав функцию [**мфинитаммедиатипефроммфмедиатипе**](/windows/desktop/api/mfapi/nf-mfapi-mfinitammediatypefrommfmediatype) .
