...
Метод [**Initialize**](/windows/desktop/api/wmcontainer/nf-wmcontainer-imfasfmultiplexer-initialize) анализирует объект контентинфо, чтобы получить сведения о конфигурации потока, такие как число потоков, размер пакета, предпроход. Кроме того, мультиплексору также могут потребоваться параметры сегмента утечки и единицы расширения полезных данных. Эти сведения необходимы для создания пакетов данных, соответствующих требованиям, определенным в объекте заголовка ASF. Метод **Initialize** настраивает мультиплексор на основе типа носителя и параметров конфигурации потоков. например, если поток настроен на наличие расширений полезных данных (см. раздел [создание и настройка ASF Потоки](creating-and-configuring-asf-streams.md)), а затем мультиплексор настраивается для добавления этих значений к созданным пакетам данных.
...
сведения о функции, используемой в полном приложении, см. в разделе [учебник. копирование ASF Потоки из одного файла в другой](tutorial--copying-asf-streams-from-one-file-to-another.md).

## <a name="multiplexer-initialization-and-leaky-bucket-settings"></a>инициализация мультиплексора и Параметрыа утечки
