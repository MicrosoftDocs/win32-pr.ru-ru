...
После настройки типов мультимедиа для входных и выходных потоков задайте их, вызвав [**имедиаобжект:: сетинпуттипе**](/previous-versions/windows/desktop/api/mediaobj/nf-mediaobj-imediaobject-setinputtype) и [**Имедиаобжект:: сетаутпуттипе**](/previous-versions/windows/desktop/api/mediaobj/nf-mediaobj-imediaobject-setoutputtype) соответственно. оба этих метода возвращают **DMO \_ \_ тип E \_ не \_ принимается** , если указанный тип является недопустимым.
...
ниже приведен общий порядок операций для настройки DMO кодирования.

1.  Настройте необходимые функции кодека с помощью методов **ипропертибаг**.
2.  при необходимости используйте кодек DMO интерфейсов для настройки дополнительных компонентов.
...
> [!Note]  
> Важно использовать тот же тип носителя для ввода декодера, что использовалось для выходных данных кодировщика. это связано с тем, что видеокодеки Windows media аудио и видео используют форматы мультимедиа с дополнительными данными. эти данные добавляются к структуре, на которую указывает элемент **пбформат** структуры [**\_ \_ типа мультимедиа DMO**](/previous-versions/windows/desktop/api/mediaobj/ns-mediaobj-dmo_media_type) (обычно [**видеоинфохеадер**](/previous-versions/windows/desktop/api/amvideo/ns-amvideo-videoinfoheader) или [**вавеформатекс**](/previous-versions/dd757713(v=vs.85))). Для некоторых типов дополнительные данные являются частью типа выходных данных кодировщика перечисления. Другие типы потребует добавления этих данных вручную. Без данных расширенного формата декодировать сжатое содержимое нельзя.
