...
ms.locfileid: "118742383"
---
# <a name="evrconfig_allowbatching-attribute"></a>Еврконфиг \_ алловбатчинг, атрибут

Позволяет расширенному обработчику видео (Евр) выполнять пакетный вызов метода повторного вызова Microsoft Direct3D **IDirect3DDevice9::P** .
...
Установка этого атрибута оказывает тот же результат, что и установка флага **мфвидеорендерпрефс \_ АЛЛОВБАТЧИНГ** в Евр. Описание этого флага см. в разделе [**мфвидеорендерпрефс**](/windows/desktop/api/evr/ne-evr-mfvideorenderprefs) .
