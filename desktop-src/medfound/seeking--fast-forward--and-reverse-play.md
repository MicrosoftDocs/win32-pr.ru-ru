...
При использовании сеанса мультимедиа для воспроизведения приложение может искать и изменять скорость воспроизведения следующим образом:

-   Чтобы найти, вызовите метод [**имфмедиасессион:: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start) и укажите положение поиска.
-   Чтобы изменить скорость воспроизведения, используйте интерфейс [**имфратеконтрол**](/windows/desktop/api/mfidl/nn-mfidl-imfratecontrol) , как описано в разделе [Управление скоростью](rate-control.md).
-   Чтобы получить обновленные видеокадры во время поиска, установите нулевую скорость воспроизведения перед вызовом [**имфмедиасессион:: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start). Эта операция называется *очистке*. (См. раздел [как выполнить очистку](how-to-perform-scrubbing.md).)

Однако для повышения удобства работы пользователей необходимо учитывать следующие особенности.

-   Поиск осуществляется асинхронно, а сеанс мультимедиа ставит в очередь все запросы поиска в очереди FIFO. Если вы отправляете несколько запросов поиска, Пользовательский интерфейс может запуститься впереди фактического состояния воспроизведения. Например, предположим, что приложение реализует панель поиска. Если пользователь перетаскивает панель поиска вперед и назад, то во время выполнения прямых поисков может возникнуть задержка. Пока выполняется поиск, необходимо кэшировать запросы пользователя на поиск. По завершении текущей операции поиска Отправьте пользователю последний запрос поиска и удалите другие.
-   В некоторых состояниях транспорта не допускаются некоторые скорости передачи. Например, при воспроизведении не допускается переход с прямого воспроизведения на обратное воспроизведение. Поддерживаемые переходы описаны в [**имфратеконтрол:: сетрате**](/windows/desktop/api/mfidl/nf-mfidl-imfratecontrol-setrate). Изменения частоты также являются асинхронными.

Следующий вспомогательный класс можно использовать для управления запросами поиска и изменения скорости. Пока асинхронная операция находится в состоянии ожидания, класс кэширует любые запросы поиска или изменения скорости. По завершении текущей операции класс отправляет последний запрос, если таковой имеется. Класс также управляет состоянием транспорта, чтобы избежать недопустимых изменений скорости.

Ниже приведено объявление класса Плайерсикинг.
...
Ниже приведена реализация класса Плайерсикинг.
