...
ms.locfileid: "120012364"
---
# <a name="sequencer-source-example-code"></a>Исходный пример кода Sequencer

В этом разделе показан пример кода для использования [источника Sequencer](sequencer-source.md) в Microsoft Media Foundation. В него входят следующие разделы.

-   [Класс Кплайлист](#cplaylist-class)
-   [Создание экземпляра Кплайлист](#creating-an-instance-of-cplaylist)
-   [Добавление и удаление сегментов списка воспроизведения](#adding-and-removing-playlist-segments)
-   [Обработка событий сеанса](#handling-session-events)
-   [Связанные темы](#related-topics)

## <a name="cplaylist-class"></a>Класс Кплайлист

`CPlaylist`Класс является производным от `CPlayer` класса, который показан в руководстве [Воспроизведение мультимедийных файлов с помощью Media Foundation](how-to-play-unprotected-media-files.md).
...
## <a name="creating-an-instance-of-cplaylist"></a>Создание экземпляра Кплайлист

`CPlaylist::CreateInstance`Метод создает новый `CPlaylist` объект. На внутреннем уровне этот метод вызывает `CPlaylist::Initialize` для инициализации объекта. `Initialize`Метод вызывает [**мфкреатесекуенцерсаурце**](/windows/desktop/api/mfidl/nf-mfidl-mfcreatesequencersource) для создания источника последовательности. Он также вызывает [**имфмедиасессион::-Clock**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-getclock) , чтобы получить указатель на часы презентации.
...
## <a name="adding-and-removing-playlist-segments"></a>Добавление и удаление сегментов списка воспроизведения

`AddSegment`Метод добавляет новый сегмент списка воспроизведения.

Этот метод выполняет следующие действия:

1.  Создает топологию воспроизведения. Код этого шага показан в разделе [Создание топологий воспроизведения](creating-playback-topologies.md).
2.  Вызывает [**имфсекуенцерсаурце:: аппендтопологи**](/windows/desktop/api/mfidl/nf-mfidl-imfsequencersource-appendtopology) , чтобы добавить топологию в список воспроизведения.
3.  В первом сегменте получает значение атрибута « [**\_ \_ Продолжительность MF PD**](mf-pd-duration-attribute.md) », которое содержит длительность воспроизведения.
4.  Хранит идентификатор сегмента и идентификатор топологии в таблице уточняющих запросов.
...
`AddSegment`Метод является закрытым для `CPlaylist` и вызывается из следующего открытого метода:
...
При создании первого сегмента списка воспроизведения необходимо поставить в очередь топологию сегмента в сеансе мультимедиа следующим образом:

1.  Запросите источник Sequencer для интерфейса [**имфмедиасаурцетопологипровидер**](/windows/desktop/api/mfidl/nn-mfidl-imfmediasourcetopologyprovider) .
2.  Передайте дескриптор презентации в метод [**имфмедиасаурцетопологипровидер:: жетмедиасаурцетопологи**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasourcetopologyprovider-getmediasourcetopology) . Этот метод получает указатель на топологию сегмента. Обратите внимание, что эта топология не совпадает с топологией воспроизведения, созданной ранее. Вместо этого это измененная версия этой топологии. Дополнительные сведения см. [в разделе о источнике Sequencer](about-the-sequencer-source.md).
3.  Постановка топологии в очередь в сеансе мультимедиа путем вызова [**имфмедиасессион:: сеттопологи**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-settopology).

Эти действия показаны в следующем коде. Этот же код также используется, когда список воспроизведения выполняет откат следующего сегмента.
...
Чтобы удалить сегмент списка воспроизведения, вызовите [**имфсекуенцерсаурце::D елететопологи**](/windows/desktop/api/mfidl/nf-mfidl-imfsequencersource-deletetopology). Сегмент задается ИДЕНТИФИКАТОРом сегмента. (По этой причине приложение должно кэшировать список идентификаторов сегментов.)
...
## <a name="handling-session-events"></a>Обработка событий сеанса
