---
description: Оболочки
ms.assetid: 0AADEF94-9C6F-4FE9-B272-579423FCB46E
title: Оболочки
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 8fd47f3935adf16c25776e61d84340e033f61a9665bbc7e1fbc1b19645d61b35
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/11/2021
ms.locfileid: "119537994"
---
# <a name="wrappers"></a>Оболочки

## <a name="in-this-section"></a>Содержание раздела

-   [**\_CreateFile**](-createfile.md)
-   [**\_CreateFontIndirect**](-createfontindirect.md)
-   [**\_DeleteFile**](-deletefile.md)
-   [**\_FindResource**](-findresource.md)
-   [**\_FormatMessage**](-formatmessage.md)
-   [**\_GetComputerName**](-getcomputername.md)
-   [**\_жетдискфриспацеекс**](-getdiskfreespaceex.md)
-   [**\_GetModuleFileName**](-getmodulefilename.md)
-   [**\_GetObject**](-getobject.md)
-   [**\_GetProcAddress\_**](-getprocaddress-.md)
-   [**\_GetUserName**](-getusername.md)
-   [**\_GetVersionEx**](-getversionex.md)
-   [**\_жетвиндовтекст**](-getwindowtext.md)
-   [**\_лоадкурсор**](-loadcursor.md)
-   [**\_LoadLibrary**](-loadlibrary.md)
-   [**\_лоадстринг**](-loadstring.md)
-   [**\_MessageBox**](-messagebox.md)
-   [**\_MoveFile**](-movefile.md)
-   [**\_PeekMessage**](-peekmessage.md)
-   [**\_сенддлгитеммессаже**](-senddlgitemmessage.md)
-   [**\_SendMessage**](-sendmessage.md)
-   [**\_сетдлгитемтекст**](-setdlgitemtext.md)
-   [**\_SetWindowText**](-setwindowtext.md)

 

 



