...
Указывает ноль или несколько деклараторов указателей. Декларатор указателя аналогичен декларатору указателя, используемому в языке C, созданному из **\*** указателя, модификаторов, таких как **FAR**, и квалификатора [**const**](const.md).
...
Задает имя массива, за которым следуют одна из следующих конструкций для каждого измерения массива: "**\[ \]**", " **\[\*\]** ", "**\[ ***Const1*** \]**" или "**\[ ***Lower...*** Upper \]**", где" *снизу* "и" *верхний* "являются константными значениями, представляющими нижнюю и верхнюю границы. Константа *ниже* должна иметь значение 0.
