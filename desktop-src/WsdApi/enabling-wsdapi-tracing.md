---
description: Журналы WSDAPI содержат отладочную информацию, которую можно использовать для поиска основной причины сбоев приложений WSDAPI.
ms.assetid: 28b4c032-1c9a-4b3a-9a6a-2948456572b2
title: Включение трассировки WSDAPI
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: bd8f765249f4888a1dcfd2c6a44a81d3e2652a75bb983e85a881093d3c266b5d
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/11/2021
ms.locfileid: "119049702"
---
# <a name="enabling-wsdapi-tracing"></a>Включение трассировки WSDAPI

Журналы WSDAPI содержат отладочную информацию, которую можно использовать для поиска основной причины сбоев приложений WSDAPI. Если трассировка включена, данные журнала хранятся в ETL-файле в указанном пользователем расположении. Этот ETL-файл можно отправить в службу поддержки разработчиков Майкрософт для анализа основных причин. Сведения о связи со службой поддержки см. по адресу [https://support.microsoft.com](https://support.microsoft.com) .

Эту процедуру следует выполнять дважды: один раз на клиенте и один раз на узле.

**Включение трассировки WSDAPI**

1.  с помощью Блокнот или другого текстового редактора создайте текстовый файл со следующим текстом:

    ``` syntax
    "{480217a9-f824-4bd4-bbe8-f371caaf9a0d}" 0xFF 0xFF
    "{649e3596-2620-4d58-a01f-17aefe8185db}" 0xFF 0xFF
    "{96ab095a-9519-4f5c-81ee-c510b0a45463}" 0xFF 0xFF
    "{f9be9c98-10db-4318-bb61-cb0ddea08bf7}" 0xFF 0xFF
    "{db1d0418-105a-4c77-9a25-8f96a19716a4}" 0xFF 0xFF
    "{7e2dbfc7-41e8-4987-bca7-76cadfad765f}" 0xFF 0xFF
    "{8b20d3e4-581f-4a27-8109-df01643a7a93}" 0xFF 0xFF
    "{6d04bf88-60a5-4d02-bc5c-94a20ba490ec}" 0xFF 0xFF
    "{75454210-b231-4fea-b2b4-2cc66d7ae8aa}" 0xFF 0xFF
    "{e176aa66-5cc8-4321-9624-f9c1d2b7bf06}" 0xFF 0xFF
    "{836767a6-af31-4938-b4c0-ef86749a9aef}" 0xFF 0xFF
    ```

2.  Сохраните текстовый файл как `C:\temp\traceguids.txt` , а затем закройте файл.
3.  Откройте окно командной строки с повышенными привилегиями.
4.  Выполните следующую команду: **logman.exe Create Trace всдлог-o c: \\ temp \\ WSD**
5.  Выполните следующую команду: **logman.exe Update всдлог-PF c: \\ temp \\traceguids.txt**
6.  Выполните следующую команду: **logman.exe Start всдлог**
7.  Воспроизведите ошибку, запустив узел и клиент или нажав клавишу F5 в обозревателе сети.

**Отключение трассировки WSDAPI**

1.  Откройте окно командной строки с повышенными привилегиями.
2.  Выполните следующую команду: **logman.exe всдлог** .

После записи сбоя приложения \* ETL-файлы могут отправляться в службу поддержки Майкрософт. Эти файлы находятся в папке `C:\temp\wsd_*.etl` .

## <a name="related-topics"></a>Связанные темы

<dl> <dt>

[Процедуры диагностики WSDAPI](wsdapi-diagnostic-procedures.md)
</dt> <dt>

[начало работы с разрешениями WSDAPI](getting-started-with-wsdapi-troubleshooting.md)
</dt> <dt>

[https://support.microsoft.com](https://support.microsoft.com)
</dt> </dl>

 

 



