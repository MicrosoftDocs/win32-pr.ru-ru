...
![Graph, в котором показана функция веса для компонента J метрики.](images/gmmp-image119.png)
...
Процесстриангле реализует предыдущую стратегию II. Получив обычный вектор из предварительно вычисленного вектора базовой единицы к треугольнику по отношению к стандартному продукту с точкой, он вычислит расстояние от точки запроса до бесконечной плоскости, содержащей треугольник, образуя скалярное произведение вектора базовой единицы и Куеривектор, вектор из одной из вершин треугольника.  vertex1, в точку запроса Куерипоинт.
