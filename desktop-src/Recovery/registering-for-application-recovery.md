...
В этом разделе приводятся сведения о реализации функции восстановления в приложении. Рекомендуется реализовать эту функцию для решения следующих задач:

-   [Восстановление, когда приложение испытывает необработанное исключение или перестает отвечать на запросы](#recovering-when-an-application-experiences-an-unhandled-exception-or-stops-responding)

    Предотвращает потерю данных при внезапной остановке работы приложения.

-   [Сохранение данных и состояния приложения при закрытии приложения из-за обновления программного обеспечения](#saving-data-and-application-state-when-application-is-being-closed-due-to-a-software-update)

    Позволяет пользователю легко получать данные приложений, когда приложение закрывается из-за установки обновления программного обеспечения (что может произойти, не давая пользователю возможности сохранять данные).

## <a name="recovering-when-an-application-experiences-an-unhandled-exception-or-stops-responding"></a>Восстановление, когда приложение испытывает необработанное исключение или перестает отвечать на запросы

Чтобы зарегистрировать обратный вызов восстановления, вызовите функцию [**регистераппликатионрековерикаллбакк**](/windows/win32/api/winbase/nf-winbase-registerapplicationrecoverycallback) . [отчеты об ошибках Windows (WER)](/windows/desktop/wer/windows-error-reporting) вызывает обратный вызов восстановления до завершения работы приложения из-за необработанного исключения или неотвечающего приложения.

Используйте обратный вызов восстановления, чтобы сохранить данные и сведения о состоянии до завершения работы приложения. После перезапуска приложения можно использовать сохраненные данные и сведения о состоянии.

Во время процесса восстановления необходимо вызвать функцию [**аппликатионрековеринпрогресс**](/windows/win32/api/winbase/nf-winbase-applicationrecoveryinprogress) в течение указанного интервала опроса; в противном случае процесс восстановления завершится. Вызов **аппликатионрековеринпрогресс** позволяет WER понять, что вы по-прежнему активно восстанавливаете данные. После завершения процесса восстановления вызовите функцию [**аппликатионрековерифинишед**](/windows/win32/api/winbase/nf-winbase-applicationrecoveryfinished) . Обратите внимание, что функция **аппликатионрековерифинишед** должна быть последним вызовом перед выходом, поскольку функция немедленно завершает работу приложения.

Рекомендуется периодически сохранять временные копии данных и сведения о состоянии в ходе обычного процесса приложения. Периодическое сохранение данных может сэкономить время в процессе восстановления.

## <a name="saving-data-and-application-state-when-application-is-being-closed-due-to-a-software-update"></a>Сохранение данных и состояния приложения при закрытии приложения из-за обновления программного обеспечения

если приложение Windows можно обновить, оно также должно обрабатывать сообщения [**WM \_ куерендсессион**](/windows/desktop/Shutdown/wm-queryendsession) и [**wm \_ ENDSESSION**](/windows/desktop/Shutdown/wm-endsession) . Установщик отправляет эти сообщения, когда установщику требуется завершить работу приложения для завершения установки или когда требуется перезагрузка для завершения установки. Обратите внимание, что в этом случае приложение имеет меньше времени на выполнение восстановления. Например, приложение должно отвечать на каждое сообщение в течение пяти секунд.

Для консольных приложений, которые могут быть обновлены, следует рассмотреть возможность обработки \_ \_ уведомлений о событиях CTRL C. Пример см. в разделе [Регистрация для перезапуска приложения](registering-for-application-restart.md). Установщик отправляет это уведомление, когда ему требуется завершить работу приложения, чтобы завершить обновление. Приложение обрабатывает уведомление через 30 секунд.

В следующем примере показано, как зарегистрировать для восстановления, простую реализацию обратного вызова восстановления и как обрабатывать сообщения [**WM \_ Куерендсессион**](/windows/desktop/Shutdown/wm-queryendsession) и [**WM \_ ENDSESSION**](/windows/desktop/Shutdown/wm-endsession) .
...
Ниже приведен файл recover. h include для примера восстановления.
...
Ниже приведен файл ресурсов Recover. RC для примера восстановления.
...
Ниже приведен файл Resource. h, в котором содержится пример восстановления.
