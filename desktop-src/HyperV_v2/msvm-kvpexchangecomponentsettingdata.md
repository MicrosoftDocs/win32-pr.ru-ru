...
ms.locfileid: "119522084"
---
# <a name="msvm_kvpexchangecomponentsettingdata-class"></a>\_Класс мсвм квпексчанжекомпонентсеттингдата

Представляет настроенное состояние службы обмена ключами и значением.
...
Класс **мсвм \_ квпексчанжекомпонентсеттингдата** имеет следующие типы членов:

-   [Свойства](#properties)

### <a name="properties"></a>Элемент Property

Класс **мсвм \_ квпексчанжекомпонентсеттингдата** имеет следующие свойства.
...
Пользователи могут видеть выделенный ресурс. Это свойство наследуется от [**CIM \_ ресаурцеаллокатионсеттингдата**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).



| Значение                                                                        | Значение                |
|------------------------------------------------------------------------------|------------------------|
| <dl> <dt>3</dt> </dl> | Виртуализированных<br/> |
...
Это свойство отключает узел от автоматически популатингхост имя и сведения о ОС в гостевой системе.
...
Массив внедренных экземпляров [**мсвм \_ квпексчанжедатаитем**](msvm-kvpexchangedataitem.md) , представляющих пары "ключ-значение".
...
Массив экземпляров [**мсвм \_ квпексчанжедатаитем**](msvm-kvpexchangedataitem.md) , содержащих пары "ключ-значение", которые хранятся в файле конфигурации, но не обмениваются данными с гостевой операционной системой. Это позволяет приложениям хранить данные, относящиеся к виртуальной машине, которые не должны быть видимыми для гостевой операционной системы. Элементы форматируются так же, как элементы в свойстве **хостексчанжеитемс** , за исключением того, что свойство **Source** экземпляра **мсвм \_ квпексчанжедатаитем** имеет значение 4. Каждый файл конфигурации ограничен 128 парами "ключ-значение", где каждое поле значения может иметь размер до 16 КБ, а ключевое поле может содержать до 512 байт.
...
Объем ресурсов, которые гарантированно будут доступны для этого выделения. Это свойство наследуется от [**CIM \_ ресаурцеаллокатионсеттингдата**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
Строка, описывающая конкретный подтип реализации для этого ресурса. Это свойство наследуется от [**CIM \_ ресаурцеаллокатионсеттингдата**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
Доступ к классу **\_ квпексчанжекомпонентсеттингдата мсвм** может быть ограничен фильтром контроля учетных записей. Дополнительные сведения см. в разделе [Управление учетными записями пользователей и инструментарий WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
