...
ms.locfileid: "120107424"
---
# <a name="msvm_transparentbridgingservice-class"></a>\_Класс мсвм транспарентбридгингсервице

Служит заполнителем для службы внутри коммутатора, который изучает MAC-адреса и служит мостом между классами [**мсвм \_ Виртуалесернетсвитч**](msvm-virtualethernetswitch.md) и [**мсвм \_ динамикфорвардинжентри**](msvm-dynamicforwardingentry.md) .
...
Класс **мсвм \_ транспарентбридгингсервице** имеет следующие типы членов:
...
Класс **мсвм \_ транспарентбридгингсервице** содержит следующие методы.



| Метод                                                                           | Описание                         |
|:---------------------------------------------------------------------------------|:------------------------------------|
| [**Равен**](msvm-transparentbridgingservice-requeststatechange.md) | Запрашивает изменение состояния.<br/> |
| [**StartService**](msvm-transparentbridgingservice-startservice.md)             | запускает службу.<br/>      |
| [**StopService**](msvm-transparentbridgingservice-stopservice.md)               | останавливает службу.<br/>       |
...
Класс **мсвм \_ транспарентбридгингсервице** имеет следующие свойства.
...
Время ожидания в секундах для динамически изученных MAC-адресов. Это свойство наследуется от [**CIM \_ транспарентбридгингсервице**](/previous-versions/windows/desktop/clushyperv/cim-transparentbridgingservice).
...
Краткое описание объекта. Это свойство наследуется от [**CIM \_ манажеделемент**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)и всегда является "виртуальным коммутатором со прозрачным мостом".
...
Краткое описание объекта. Это свойство наследуется [**от \_ службы CIM**](/windows/desktop/CIMWin32Prov/cim-service).
...
Описание объекта. Это свойство наследуется от [**CIM \_ манажеделемент**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)и всегда имеет значение "служба виртуального коммутатора со прозрачным мостом (Майкрософт)".
...
Отображаемое имя объекта. Это свойство позволяет каждому экземпляру определить отображаемое имя в дополнение к его ключевым свойствам, идентификационным данным и сведениям о описании. Это свойство наследуется от [**CIM \_ манажеделемент**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement).
...
Идентификатор базы данных фильтрации, используемый коммутаторами, которые поддерживают виртуальную ЛС и имеют более одной фильтрации базы данных. Это свойство наследуется от [**CIM \_ транспарентбридгингсервице**](/previous-versions/windows/desktop/clushyperv/cim-transparentbridgingservice).
...
Указывает время установки объекта. Отсутствие значения не означает, что объект не установлен. Это свойство наследуется от [**CIM \_ манажедсистемелемент**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)и не используется.
...
Не используйте. Массив строк произвольной формы, который предоставляет описательные слова и фразы, которые можно использовать в запросах. Это свойство не реализовано, так как оно не стандартизовано. Если это свойство является необходимой конструкцией запроса, то в иерархии наследования потребуется больше. Это свойство наследуется [**от \_ сетевой NetworkService CIM**](/previous-versions//cc136875(v=vs.85)).
...
Имя, однозначно идентифицирующее службу и предоставляющее сведения об управляемых функциях. Это свойство наследуется [**от \_ службы CIM**](/windows/desktop/CIMWin32Prov/cim-service).
...
Состояние включения или отключения элемента, если свойство **EnabledState** имеет значение 1 (другое). Это свойство наследуется от [**CIM \_ енабледлогикалелемент**](/previous-versions//cc136818(v=vs.85))и не используется.
...
Тип перенаправляемого протокола, если значение **ProtocolType** равно 1 (другое). Это свойство наследуется от [**CIM \_ форвардингсервице**](/previous-versions/windows/desktop/clushyperv/cim-forwardingservice).
...
Строка, которая предоставляет сведения о том, как можно достичь основного владельца службы. Это свойство наследуется [**от \_ службы CIM**](/windows/desktop/CIMWin32Prov/cim-service).
...
Имя основного владельца службы, если она определена. Это свойство наследуется [**от \_ службы CIM**](/windows/desktop/CIMWin32Prov/cim-service).
...
Тип перенаправляемого протокола. Это свойство наследуется от [**CIM \_ форвардингсервице**](/previous-versions/windows/desktop/clushyperv/cim-forwardingservice).
...
Не используйте. URL-адрес, предоставляющий протокол, сетевое расположение и другие сведения, относящиеся к службе, необходимые для доступа к службе. Вместо этого используйте класс **сервицеакцессури** , который правильно позиционирует семантику доступа к службе и уточняет формат информации. Это свойство наследуется [**от \_ сетевой NetworkService CIM**](/previous-versions//cc136875(v=vs.85)).
...
Указывает, была ли служба запущена (**true**) или остановлена (**false**). Это свойство наследуется [**от \_ службы CIM**](/windows/desktop/CIMWin32Prov/cim-service).
...
Указывает, запускается ли служба автоматически системой, операционной системой и т. д. или запускается только по запросу. Это свойство наследуется [**от \_ службы CIM**](/windows/desktop/CIMWin32Prov/cim-service).
...
Не используйте. Массив строк произвольной формы, указывающий все конкретные предусловия, которые должны быть выполнены для правильной работы этой службы. Это свойство нецелесообразно, так как оно не является стандартизированным. Если это свойство является необходимой конструкцией, оно должно быть выше в иерархии наследования (для службы). Это свойство наследуется [**от \_ сетевой NetworkService CIM**](/previous-versions//cc136875(v=vs.85)).
...
Не используйте. Массив строк произвольной формы, указывающий все конкретные параметры, которые должны быть передаваться методу **StartService** для запуска службы. Если этот метод был уточнен, его параметры более формально сообщают эту информацию. Это свойство наследуется [**от \_ сетевой NetworkService CIM**](/previous-versions//cc136875(v=vs.85)).
...
Имя класса создания для системы области. Это свойство наследуется [**от \_ службы CIM**](/windows/desktop/CIMWin32Prov/cim-service).
...
Дата или время последнего изменения включенного состояния элемента. Это свойство наследуется от [**CIM \_ енабледлогикалелемент**](/previous-versions//cc136818(v=vs.85))и не используется.
...
Доступ к классу **\_ транспарентбридгингсервице мсвм** может быть ограничен фильтром контроля учетных записей. Дополнительные сведения см. в разделе [Управление учетными записями пользователей и инструментарий WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
...
[**\_ТРАНСПАРЕНТБРИДГИНГСЕРВИЦЕ CIM**](cim-transparentbridgingservice.md)
</dt> <dt>

[**\_ТРАНСПАРЕНТБРИДГИНГСЕРВИЦЕ CIM**](/previous-versions/windows/desktop/clushyperv/cim-transparentbridgingservice)
</dt> </dl>
