...
---
# <a name="querying-networking-objects"></a>Запрос сетевых объектов

В следующем примере кода C# извлекается и отображаются сведения о физических и виртуальных сетях для указанного коммутатора. Этот код взят из [примера сети Hyper-V](https://github.com/microsoft/Windows-classic-samples/tree/master/Samples/Hyper-V/Networking).

Для выполнения этого примера используется следующий синтаксис командной строки:

**NetworkingSamples.exe енумератесвитч** *SwitchName*

где *SwitchName* — имя параметра, для которого нужно отобразить сведения. Имя переключателя содержится в свойстве [**мсвм \_ Виртуалесернетсвитч. ElementName**](msvm-virtualethernetswitch.md) .
