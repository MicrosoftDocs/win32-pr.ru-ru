...
Представляет настроенное состояние искусственного Fibre Channel порта.
...
Класс **мсвм \_ синсетикфкпортсеттингдата** имеет следующие типы членов:
...
Указывает, что этот порт настроен с использованием общего секрета с помощью DH-CHAP, что позволяет Fibre Channel структуре проверить подлинность этой виртуальной машины, которая может использовать WWN-имена, назначенные этому порту.
...
Строка, описывающая тип ресурса, если четко определенное значение недоступно, а [**ResourceType**](msvm-processorsettingdata.md) имеет значение 1 (другое). Это свойство наследуется от [**CIM \_ ресаурцеаллокатионсеттингдата**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata), но не используется.
...
Объем ресурсов, зарезервированных для использования виртуальной машиной. Это свойство наследуется от [**CIM \_ ресаурцеаллокатионсеттингдата**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata), но не используется.
...
Указывает имя узла виртуального адаптера шины, используемого во время динамической миграции виртуальной машины.
...
Указывает WWN имя порта виртуального адаптера шины, которое будет использоваться во время динамической миграции виртуальной машины.
...
Указывает WWN имени узла для виртуального порта HBA.
...
Указывает WWN имя порта виртуального адаптера шины.
...
Указывает количество ресурсов, представленных потребителю. Это свойство наследуется от [**CIM \_ ресаурцеаллокатионсеттингдата**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
Строковый массив идентификаторов этого ресурса, представленный операционной системе виртуальной машины. Индексы и значения для каждого индекса определяются для каждого ресурса (то есть для каждого перечислимого **значения типа ресурса)** . Это свойство не используется
...
Целое число, определяющее относительный вес для каждого процессора виртуальной машины. Это свойство наследуется от [**CIM \_ ресаурцеаллокатионсеттингдата**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata), но не используется.
