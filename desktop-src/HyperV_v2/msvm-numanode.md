...
---
# <a name="msvm_numanode-class"></a>\_Класс мсвм NumaNode

Представляет узел виртуальной системы на основе неоднородного доступа к памяти (NUMA).
...
Класс **мсвм \_ NumaNode** имеет следующие типы членов:
...
Класс **мсвм \_ NumaNode** содержит следующие методы.
...
Класс **мсвм \_ NumaNode** имеет следующие свойства.
...
**куррентликонсумаблемемориблоккс**
</dt> <dd> <dl> <dt>
...
Количество блоков памяти, доступных для использования виртуальными машинами в настоящее время.
...
Общее число ядер логических процессоров в этом узле.
...
Общее число ядер процессора в этом узле NUMA. Это может отличаться от количества объектов [**\_ процессора мсвм**](msvm-processor.md) , связанных с этим узлом, если каждый процессорный ядро поддерживает несколько потоков вычислений.
