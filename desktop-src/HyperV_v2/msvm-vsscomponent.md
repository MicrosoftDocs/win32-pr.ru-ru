...
Представляет состояние службы служба теневого копирования томов (VSS), которая реализует инициатор запроса VSS в гостевой операционной системе.
...
Класс **мсвм \_ всскомпонент** имеет следующие типы членов:
...
Класс **мсвм \_ всскомпонент** содержит следующие методы.
...
| [**Равен**](msvm-vsscomponent-requeststatechange.md) | Запрашивает изменение состояния.<br/>      |
| [**Перезапуск**](msvm-vsscomponent-reset.md)                           | Сбрасывает виртуальное устройство.<br/>    |
...
Класс **мсвм \_ всскомпонент** имеет следующие свойства.
...
Строка, которая предоставляет дополнительные сведения об ошибке, записанной в **ластерроркоде**, и сведения о любых корректирующих действиях, которые могут быть выполнены. Это свойство наследуется от [**CIM \_ , но**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) не используется.
...
Дата или время последнего изменения **EnabledState** элемента. Это свойство наследуется от [**CIM \_ енабледлогикалелемент**](/previous-versions//cc136818(v=vs.85))и всегда имеет значение **null**.
