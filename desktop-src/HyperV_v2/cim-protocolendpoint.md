...
Точка обмена данными, используемая для отправки и получения данных между системами, интерфейсами компьютеров и логическими сетями.
...
Класс **CIM \_ протоколендпоинт** имеет следующие типы членов:
...
Квалификаторы: [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("Описание"), [**Маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ифдескр ")
</dt> </dl>
...
Квалификаторы: [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("EnabledState"), [**Маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ифадминстатус ")
</dt> </dl>
...
Квалификаторы: [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("имя"), [**maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (256)
</dt> </dl>

Уникальный идентификатор конечной точки протокола, указывающий управляемые функциональные возможности. Соглашение об именовании для этого свойства определено в свойстве **намеформат** .
...
Соглашение об именовании, используемое свойством **Name** для обеспечения уникальности значений **имен** . Например, можно добавить значение свойства **протоколифтипе** в начало имени, за которым следует символ подчеркивания.
...
Квалификаторы: [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("OperationalStatus"), [**Маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ифоперстатус ")
</dt> </dl>

Массив, содержащий текущее состояние элемента. Первое значение массива **OperationalStatus** должно содержать первичное состояние элемента.
...
Квалификаторы: [**maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (64), [**Моделкорреспонденце**](/windows/desktop/WmiSdk/standard-qualifiers) ("**CIM \_ протоколендпоинт**.**ProtocolType**","**CIM \_ протоколендпоинт**.**Протоколифтипе**")
</dt> </dl>

Описание типа сетевого протокола, используемого, если свойство **протоколифтипе** имеет значение 1 (другое); в противном случае это значение должно быть равно null.
...
Квалификаторы: [**маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ифтипе "), [**моделкорреспонденце**](/windows/desktop/WmiSdk/standard-qualifiers) ("**CIM \_ протоколендпоинт**.**Осертипедескриптион**")
</dt> </dl>

Перечисление, используемое для категоризации и классификации различных экземпляров этого класса. Возможные значения для этого свойства синхронизируются с Ифтипе MIB-модулем (основной информацией об управлении), который поддерживается в https://www.iana.org/assignments/ianaiftype-mib . Включаются дополнительные значения, определяемые DMTF.

> [!Note]  
> Если для **протоколифтипе** задано значение 1 (другое), то сведения о типе протокола должны быть предоставлены в свойстве **осертипедескриптион** .
...
**Собственная последовательное подключение "точка — точка** " (22)
...
**Замыкание программного обеспечения** (24)
...
**Собственная виртуальная/внутренняя** (53)
...
**Переключение каналов передачи данных** (74)
...
**Мост удаленных маршрутов удаленного источника** (79)
...
**Система отчетов ext POS Loc** (87)
...
**Протокол удаленного доступа AppleTalk** (88)
...
**Собственная без подключения** (89)
...
**Клавиатура терминала ITU X. 3** (91)
...
**Получение и передача голоса** (100)
...
**ISO 802.5 j Fibre Token Ring** (115)
...
**IP-адрес (для АППН ХПР в IP-сетях)** (126)
...
**Виртуальная ЛС уровня 2 с помощью 802.1 q** (135)
...
**Виртуальная ЛС уровня 3 с использованием IP-адреса** (136)
...
**Виртуальная ЛС уровня 3 с использованием IPX** (137)
...
**Цифровая линия электропитания** (138)
...
**Мультимедийная почта через IP-адрес** (139)
...
**Вышестоящее телевидение DVB-RCC** (148)
...
Передача **голоса по сети ATM** (152)
...
Передача **голоса через Frame Relay** (153)
...
**Агрегирование ссылок AD IEEE 802.3** (161)
...
**ФРФ .16 многоканальная связь fr** (163)
...
**Многочастотная ссылка на сигнал** (167)
...
**Ссылка на данные DS1-устройства** (170)
...
**Связанные сигналы без устройства** (175)
...
**Частный уровень беспроводной сети Mac** (180)
...
**Собственная беспроводная сеть** (181)
...
**Частные Беспроводные потоки** (182)
...
**Собственная широкополосная беспроводная точка доступа к мулипоинт** (184)
...
**Канал накладных расходов Сонет** (185)
...
**Канал накладных расходов цифровых оболочек** (186)
...
**Межмашинная магистраль между компьютерами** (190)
...
**Общедоступный для чтения DSL** (192)
...
**Конечная точка DLCI Frame Relay** (193)
...
**Виртуальная магистральная группа** (202)
...
**Конечная точка канала ISDN B** (4106)
...
**Конечная точка D канала ISDN** (4107)
...
Квалификаторы: не [**рекомендуется**](/windows/desktop/WmiSdk/standard-wmi-qualifiers) ("**CIM \_ протоколендпоинт**.**Протоколифтипе**"), [**моделкорреспонденце**](/windows/desktop/WmiSdk/standard-qualifiers) ("**CIM \_ протоколендпоинт**.**Осертипедескриптион**")
</dt> </dl>

> [!Note]  
> Нерекомендуемое описание: перечисление, используемое для категоризации и классификации различных экземпляров этого класса.

 

Это свойство использовать не рекомендуется. Вместо этого используйте свойство **протоколифтипе** .
...
**Конечная точка канала ISDN B** (20)
...
**Конечная точка D канала ISDN** (21)
...
Квалификаторы: [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("тимеофластстатечанже"), [**Маппингстрингс**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| If-MIB. ифластчанже ")
</dt> </dl>

Дата и время последнего изменения свойства **EnabledState** . Если параметр **EnabledState** не изменился и это свойство заполнено, то для него должно быть задано нулевое значение интервала. Если изменение состояния было запрошено, но отклонено или еще не обработано, свойство не должно обновляться.
