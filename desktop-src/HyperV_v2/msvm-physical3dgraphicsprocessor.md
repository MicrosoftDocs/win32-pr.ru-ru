...
Описывает физический модуль обработки трехмерных графики (GPU).
...
Класс **мсвм \_ Physical3dGraphicsProcessor** имеет следующие типы членов:
...
Класс **мсвм \_ Physical3dGraphicsProcessor** содержит следующие методы.
...
Класс **мсвм \_ Physical3dGraphicsProcessor** имеет следующие свойства.
...
Указывает идентификатор индекса адаптера, выделенный для этого устройства.
...
Указывает объем видеопамяти в байтах, доступный GPU.
...
Указывает объем видеопамяти в байтах, выделенной для GPU.
...
Указывает виртуализации DirectX, поддерживаемый GPU.
...
Указывает виртуализации шейдера пикселей, поддерживаемое графическим процессором.
...
Указывает объем общей системной памяти в байтах, доступный для GPU.
...
Указывает общий объем видеопамяти (в байтах), имеющейся на GPU.
