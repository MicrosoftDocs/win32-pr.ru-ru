...
---
# <a name="msvm_processorpool-class"></a>\_Класс мсвм процессорпул

Статистическая обработка ресурсов процессора, которые могут быть выделены виртуальной машине.
...
Класс **мсвм \_ процессорпул** имеет следующие типы членов:
...
Класс **мсвм \_ процессорпул** содержит следующие методы.



| Метод                                                                          | Описание                                           |
|:--------------------------------------------------------------------------------|:------------------------------------------------------|
| [**калкулатепоссиблересерве**](calculatepossiblereserve-msvm-processorpool.md) | Используется для поиска действительного резерва процессора.<br/> |
...
Класс **мсвм \_ процессорпул** имеет следующие свойства.
...
Строка, описывающая тип ресурса, если четко определенное значение недоступно, а **ResourceType** имеет значение 0 ("Other"). Это свойство наследуется от [**CIM \_ ResourcePool**](/previous-versions//cc136903(v=vs.85))и имеет значение **null**.
...
Доступ к классу **\_ процессорпул мсвм** может быть ограничен фильтром контроля учетных записей. Дополнительные сведения см. в разделе [Управление учетными записями пользователей и инструментарий WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
