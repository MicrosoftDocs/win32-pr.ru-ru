...
NLS также определяет несколько функций API, которые можно использовать для устранения некоторых угроз безопасности, предоставляемых технологией IDN. В Windows Vista и более поздних версиях следующие функции используются для проверки того, что символы в заданном IDN были полностью выведены из сценариев, связанных с определенным языковым стандартом или региональными стандартами. Пример использования этих функций см. в разделе [NLS — пример Межязыкового доменного имени (IDN)](nls--internationalized-domain-name--idn--mitigation-sample.md).
