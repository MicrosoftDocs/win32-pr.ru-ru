...
-   Не поддерживается в основных шрифтах Windows и поэтому может потребовать [отката шрифта](#font-fallback).
