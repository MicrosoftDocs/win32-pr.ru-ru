...
Приложение вызывает функцию [**маппингрекогнизетекст**](/windows/desktop/api/Elscore/nf-elscore-mappingrecognizetext) для запроса распознавания текста из определенной службы ELS. Служба должна быть обнаружена в предыдущем вызове [**маппингжетсервицес**](/windows/desktop/api/Elscore/nf-elscore-mappinggetservices), как описано в разделе [**перечисление и освобождение служб**](enumerating-and-freeing-services.md).

> [!Note]  
> Платформа может обрабатывать вызовы [**маппингрекогнизетекст**](/windows/desktop/api/Elscore/nf-elscore-mappingrecognizetext) синхронно или асинхронно.

 

[**Маппингрекогнизетекст**](/windows/desktop/api/Elscore/nf-elscore-mappingrecognizetext) обрабатывает следующие типы текста:

-   Обнаружение языка Майкрософт. UTF-16, форма нормализации C, текст, для которого определяется язык.
-   Обнаружение сценариев Майкрософт. Текст UTF-16, для которого необходимо определить диапазоны скриптов.
-   Службы транслитерации. Текст в кодировке UTF-16, написанный в исходном скрипте (система записи).

## <a name="use-synchronous-text-recognition"></a>Использовать синхронное распознавание текста

В этом разделе приводятся инструкции по нескольким способам выполнения синхронного распознавания текста.

**Синхронное распознавание текста с помощью службы Microsoft распознавание языка**

В следующем примере показано использование [**маппингрекогнизетекст**](/windows/desktop/api/Elscore/nf-elscore-mappingrecognizetext) со службой Microsoft распознавание языка и вывод всех результатов, полученных службой. Формат вывода этой службы — это единая структура [**\_ \_ диапазона данных сопоставления**](/windows/desktop/api/Elscore/ns-elscore-mapping_data_range) с элементом **pData** , указывающим на массив строк в формате Юникода, заканчивающийся двойным нулем, Каждая строка массива завершается нулем, а для указания конца массива используется пустая строка. Содержимое массива — это имена языков, отсортированные по достоверности.
...
**Синхронное распознавание текста со службой обнаружения сценариев (Майкрософт)**
...
**Синхронное распознавание текста с помощью службы транслитерации Microsoft кириллицы и латиницы**

В следующем примере показано использование [**маппингрекогнизетекст**](/windows/desktop/api/Elscore/nf-elscore-mappingrecognizetext) со службой Microsoft кириллицы to Latin транслитерации и вывод полученных результатов. Обратите внимание на два разных способа перечисления этой службы: по GUID или по категориям и входным сценариям.

Формат выходных данных одинаков для всех доступных служб транслитерации. Это единая структура [**\_ \_ диапазона данных сопоставления**](/windows/desktop/api/Elscore/ns-elscore-mapping_data_range) с элементом **pData** , указывающим на массив символов Юникода, представляющий исходный текст, преобразованный в выходной скрипт, применяя только правила конкретной службы транслитерации. Эта служба не имеет значения NULL — завершает свои выходные данные, если входные данные не содержат завершающего нуль-символа.
...
**Синхронное распознавание текста с помощью вызова всех доступных служб**

В следующем примере показано использование [**маппингрекогнизетекст**](/windows/desktop/api/Elscore/nf-elscore-mappingrecognizetext) со всеми доступными службами и вывод полученных результатов для всех служб. В этом примере приведена хорошая иллюстрация работы каждой службы. Просматривая выходные данные примера приложения, можно легко определить, что происходит внутри служб. В этом примере также показано, что почти весь код, используемый для вызова любой из ELS служб, одинаков.
...
## <a name="use-asynchronous-text-recognition"></a>Использовать асинхронное распознавание текста

В следующем примере показано использование [**маппингрекогнизетекст**](/windows/desktop/api/Elscore/nf-elscore-mappingrecognizetext) для асинхронного распознавания текста. При использовании обратного вызова приложение должно убедиться, что контейнер свойств, входной текст, параметры и служба действительны до завершения выполнения обратного вызова.

Приложение должно вызвать [**маппингфрипропертибаг**](/windows/desktop/api/Elscore/nf-elscore-mappingfreepropertybag) сразу после того, как контейнер будет использован функцией обратного вызова. Дополнительные сведения см. в разделе [предоставление обратных вызовов для служб ELS Services](providing-callbacks-for-els-services.md).
...
[Предоставление обратных вызовов для служб ELS Services](providing-callbacks-for-els-services.md)
</dt> <dt>
