...
description: В следующем примере кода демонстрируются отдельные представления реестра, предоставляемые перенаправитель реестра в 64-разрядной Windows.
ms.assetid: b3ca2a47-402d-4e91-88bc-ddda6c776468
keywords:
- примеры 64-разрядных Windows программировании
- 64-разрядные Windows примеры программного кода с 64-разрядной Windows
- 64-bit Windows примеры программного кода с 64-разрядной Windows, перенаправление реестра на WOW64
- пример перенаправления реестра 64-bit Windows программирование
- 64-разрядное Windows guide 64-разрядное Windows программирование, примеры см. в статье примеры по программированию для 64-bit Windows
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "119680233"
---
# <a name="example-of-registry-redirection-on-wow64"></a>Пример перенаправления реестра в эмуляторе WOW64

В следующем примере кода демонстрируются отдельные представления реестра, предоставляемые перенаправитель реестра в 64-разрядной Windows. Здесь также показано, как задаются значения ключей в зависимости от того, является ли ключ общим или перенаправленным. Дополнительные сведения см. в разделе [разделы реестра, затрагиваемые WOW64](shared-registry-keys.md).

Скомпилируйте следующий код отдельно для 32-разрядных и 64-разрядных Windows. запустите каждый полученный исполняемый файл в 64-разрядном Windows и сравните выходные данные. Пример выходных данных для обеих версий приведен ниже исходного кода.
