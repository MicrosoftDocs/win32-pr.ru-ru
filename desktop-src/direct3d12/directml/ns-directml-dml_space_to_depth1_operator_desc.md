...
Тензорные, из которого производится чтение. К измерениям входного тензорные присвоено значение `{ Batch, Channels, Height, Width }` .
...
Объект тензорные, в который записываются результаты. Размеры выходных тензорные: `{ Batch, Channels / (BlockSize * BlockSize), Height * BlockSize, Width * BlockSize }` .
...
Если для параметра *Order* задано значение [DML_DEPTH_SPACE_ORDER_DEPTH_COLUMN_ROW](./ne-directml-dml_depth_space_order.md), этот оператор эквивалентен [DML_SPACE_TO_DEPTH_OPERATOR_DESC](/windows/win32/api/directml/ns-directml-dml_space_to_depth_operator_desc).
