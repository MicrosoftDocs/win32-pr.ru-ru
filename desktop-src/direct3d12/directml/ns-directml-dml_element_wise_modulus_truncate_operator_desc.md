...
---
# <a name="dml_element_wise_modulus_truncate_operator_desc-structure-directmlh"></a>Структура DML_ELEMENT_WISE_MODULUS_TRUNCATE_OPERATOR_DESC (директмл. h)

Рассчитывает оператор модуля C для каждой пары соответствующих элементов входных десятков, помещая результат в соответствующий элемент *аутпуттенсор*.

Так как значение частного округляется в сторону 0, результат будет иметь тот же знак, что и делимое.
