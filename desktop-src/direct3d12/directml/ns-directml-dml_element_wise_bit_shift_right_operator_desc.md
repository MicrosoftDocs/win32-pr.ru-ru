...
---
# <a name="dml_element_wise_bit_shift_right_operator_desc-structure-directmlh"></a>Структура DML_ELEMENT_WISE_BIT_SHIFT_RIGHT_OPERATOR_DESC (директмл. h)

Выполняет логическую сдвиг вправо каждого элемента *атенсор* на количество битов, заданное соответствующим элементом *бтенсор*, помещая результат в соответствующий элемент *аутпуттенсор*.
...
| Тензорные | Вид | Поддерживаемые счетчики измерений | Поддерживаемые типы данных |
| ------ | ---- | -------------------------- | -------------------- |
| атенсор | Входные данные | 4 | UINT32, UINT16, UINT8 |
| бтенсор | Входные данные | 4 | UINT32, UINT16, UINT8 |
| аутпуттенсор | Выходные данные | 4 | UINT32, UINT16, UINT8 |
