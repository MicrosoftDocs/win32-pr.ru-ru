...
---
# <a name="dml_element_wise_atan_yx_operator_desc-directmlh"></a>DML_ELEMENT_WISE_ATAN_YX_OPERATOR_DESC (директмл. h)

Выполняет вычисление двух аргументов-арктангенс для каждого элемента *атенсор* и *Бтенсор*, где *атенсор* — это *ось Y* , а *бтенсор* — *ось X*, помещая результат в соответствующий элемент *аутпуттенсор*. Этот оператор не определен для источника (т. е. Если *атенсор* и *бтенсор* равны 0 для соответствующих элементов).
...
Этот оператор поддерживает выполнение на месте, что означает, что выходному тензорныеу может быть присвоен псевдоним *атенсор* или *бтенсор* во время привязки.
...
Входной тензорные для считывания значений оси Y из.

`BTensor`

Тип: **const [DML_TENSOR_DESC](/windows/win32/api/directml/ns-directml-dml_tensor_desc) \***

Входной тензорные для считывания значений оси X из.
...
| ------ | ---- | -------------------------- | -------------------- |
| атенсор | Входные данные | от 1 до 8 | FLOAT32, FLOAT16 |
| бтенсор | Входные данные | от 1 до 8 | FLOAT32, FLOAT16 |
