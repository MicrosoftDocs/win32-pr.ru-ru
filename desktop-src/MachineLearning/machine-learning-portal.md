---
description: Просмотрите целевую страницу пакета SDK для Windows Машинное обучение. Он содержит ссылку на страницу справки по Машинное обучение Windows.
ms.assetid: 75dc6fc9-0e24-4403-8243-a46f0f9538bb
title: Windows Machine Learning
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: f711092371095baa0e7fcec082b6e1b53117e90c
ms.sourcegitcommit: 91530c19d26ba4c57a6af1f37b57f211f580464e
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/19/2021
ms.locfileid: "112395598"
---
# <a name="windows-machine-learning"></a>Windows Machine Learning

В следующих разделах описываются справочные страницы по COM для пакета SDK для Windows Машинное обучение.

## <a name="in-this-section"></a>В этом разделе



| Раздел                                                                                       | Описание                                                                                            |
|---------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|
| [Справочник по Машинное обучение Windows](./machine-learning-reference.md)<br/> | В следующих разделах описываются справочные страницы по COM для пакета SDK для Windows Машинное обучение.<br/> |



 

 

 
