...
-   [Изменения заголовка](#header-changes)
-   [Постоянные изменения](#constant-changes)
-   [Пространства имен](#namespaces)
-   [Частичные загрузки](#partial-loads)
-   [Удаление специальных типов для Xbox 360](#removal-of-xbox-360-specific-types)
-   [Встроенные функции ARM-NEON](#arm-neon-intrinsics)
-   [Переставляют](#permute)
-   [Формы шаблонов](#template-forms)
-   [Исключенные функции](#eliminated-functions)
...
Замените заголовок кснамас. h на Директксмас. h и добавьте Директкспаккедвектор. h для типов "GPU Pack".

Ограничивающие типы из примера конфликта DirectX SDK в кснаколлисион. h теперь входят в библиотеку Директксмас в Директксколлисион. h. Они были изменены для использования классов C++, а не API-интерфейсов в стиле языка C.
...
Библиотека Директксмас использует пространства имен C++ для организации типов. XNA Math использовал только глобальное пространство имен. Типы Директксмас, общие с XNA Math, находятся в пространстве имен DirectX или DirectX::P Аккедвектор.

В исходных файлах C++ простое решение заключается в добавлении операторов using:
...
Для заголовков не считается рекомендациями по добавлению операторов using. Вместо этого добавьте полные пространства имен:
