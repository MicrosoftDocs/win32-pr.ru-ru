...
---
# <a name="creating-a-connection-request-policy"></a>Создание политики запросов на подключение

Следующий Visual Basic код создает политику запросов на подключение. Он добавляет условие в политику, соответствующее имени пользователя, запросившего доступ. Она связывает политику с профилем. Он настраивает этот профиль для использования RADIUS как для проверки подлинности, так и для учета, а также для указания групп серверов, которые профиль должен использовать для проверки подлинности и учета.

> [!Note]  
> Этот пример встроенного кода основан на управляемых Visual Basic.
...
[Порядок использования API SDO](/windows/desktop/Nps/sdo-order-in-which-to-use-the-sdo-api)
</dt> <dt>
