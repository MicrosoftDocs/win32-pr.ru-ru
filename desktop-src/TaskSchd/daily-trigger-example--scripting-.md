...
В этом примере сценария показано, как создать задачу, которая запускает приложение «Блокнот» в 8:00 AM каждый день. Задача содержит ежедневный триггер, указывающий начальную границу для активации триггера, а также задание времени суток, в течение которого задача выполняется каждый день, и конечную границу для деактивации триггера. В примере также показано, как задать шаблон повторения для триггера, чтобы повторить задачу. Задача также содержит исполняемое действие, запускающее Блокнот.
...
**Чтобы запланировать запуск программы "Блокнот" в 8:00 AM каждый день**
...
6.  Зарегистрируйте задачу с помощью метода [**таскфолдер. регистертаскдефинитион**](taskfolder-registertaskdefinition.md) . В этом примере задача запустится в блокноте каждый день в 8:00 AM.
