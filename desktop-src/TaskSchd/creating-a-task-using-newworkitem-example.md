...
> [!Note]  
> по умолчанию только члены групп «администраторы», «операторы архива» и «операторы сервера» могут создавать задачи на Windows Server 2003. член группы "администраторы" может изменить дескриптор безопасности Windows \\ папки задач, чтобы разрешить другим пользователям создавать задачи.
