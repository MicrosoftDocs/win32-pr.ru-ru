...
description: в этом примере C++ показано, как создать задачу, запланированную на выполнение Блокнот еженедельно.
...
ms.locfileid: "119001882"
---
# <a name="weekly-trigger-example-c"></a>Пример еженедельного триггера (C++)

в этом примере C++ показано, как создать задачу, запланированную на выполнение Блокнот еженедельно. Задача содержит еженедельный триггер, указывающий начальную границу, интервал недели и день недели для запуска задачи. задача также содержит действие, указывающее задачу, выполняемую Блокнот.

Следующая процедура описывает, как запланировать задачу запуска исполняемого файла еженедельно.

**планирование запуска Блокнот еженедельно**
...
5.  Создайте еженедельный триггер с помощью [**свойства Triggers объекта итаскдефинитион**](/windows/desktop/api/taskschd/nf-taskschd-itaskdefinition-get_triggers) , чтобы получить доступ к интерфейсу [**ITriggerCollection**](/windows/desktop/api/taskschd/nn-taskschd-itriggercollection) для задачи.

    Используйте метод [**ITriggerCollection:: Create**](/windows/desktop/api/taskschd/nf-taskschd-itriggercollection-create) , чтобы указать, что необходимо создать еженедельный триггер. Вы можете задать начальную границу, недельный интервал и день недели для триггера [**ивиклитригжер**](/windows/desktop/api/taskschd/nn-taskschd-iweeklytrigger) , чтобы действия, запланированные для задачи, выполнялись в указанное время в определенный день недели.
...
в следующем примере C++ показано, как запланировать выполнение задачи Блокнот еженедельно.
