...
Указывает, что задача выполняется в декабре.
...
Элемент **декабря** определяется сложным типом [**монсстипе**](taskschedulerschema-monthstype-complextype.md) .
...
Следующий XML-код определяет календарь месяцев, который запускает задачу в декабре.
