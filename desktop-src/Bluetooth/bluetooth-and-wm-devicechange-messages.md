...
description: Bluetooth включает специальные \_ сообщения WM девицечанже, которые позволяют разработчикам получать сообщения при изменении состояния Bluetooth устройств.
...
ms.locfileid: "119588414"
---
# <a name="bluetooth-and-wm_devicechange-messages"></a>сообщения Bluetooth и WM \_ девицечанже

Bluetooth включает специальные сообщения [**WM \_ девицечанже**](/windows/desktop/DevIO/wm-devicechange) , которые позволяют разработчикам получать сообщения при изменении состояния Bluetooth устройств. в этом разделе описано, как получить сообщения Bluetooth **WM \_ девицечанже** и списки сообщений Bluetooth.
...
при открытии Bluetoothного маркера вызовите функцию [**регистердевиценотификатион**](/windows/desktop/api/winuser/nf-winuser-registerdevicenotificationa) и зарегистрируйте уведомления об этом маркере с помощью **дбт \_ девтип \_ handle** в качестве типа устройства. При регистрации отправляются следующие идентификаторы GUID, и элемент **\_ данных** [**dev \_ Broadcast \_ Handle**](/windows/desktop/api/dbt/ns-dbt-dev_broadcast_handle):: дбч является связанным буфером.

## <a name="bluetooth-specific-messages"></a>сообщения, относящиеся к Bluetooth

в следующей таблице перечислены Bluetooth сообщения [**WM \_ девицечанже**](/windows/desktop/DevIO/wm-devicechange) .

| GUID                                   | BUFFER                                                  | Описание                                                                                                                                                                                                                                                                                                                                                      |
|----------------------------------------|---------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| GUID \_ Bluetooth \_ хЦи, \_ событие            | [**\_ \_ сведения о событии БС хЦи \_**](/windows/desktop/api/Bthdef/ns-bthdef-bth_hci_event_info)     | это сообщение отправляется, когда удаленный Bluetooth устройство подключается к уровню ACL или отключается от него.                                                                                                                                                                                                                                                                    |
...
| GUID \_ \_ запрос ПИН-кода Bluetooth \_          | Неприменимо.                                         | Это сообщение должно игнорироваться приложением. Если приложение должно получать запросы на получение ПИН-кода, следует использовать функцию [**блуетусрегистерфораусентикатион**](/windows/desktop/api/BluetoothAPIs/nf-bluetoothapis-bluetoothregisterforauthentication) .                                                                                                                                                   |
