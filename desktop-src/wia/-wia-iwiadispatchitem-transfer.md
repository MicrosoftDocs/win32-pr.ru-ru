...
присвойте параметру *асинктрансфер* значение **false** для передачи в любом приложении или сценарии, работающем в среде, завершающей процесс в конце скрипта, например Windows сервера сценариев (WSH). В противном случае скрипт может завершиться, а процесс завершится до завершения перемещения.
