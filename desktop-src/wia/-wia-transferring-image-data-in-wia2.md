---
description: так как передача данных изображения выполняется в потоковом режиме Windows получения изображений (WIA) 2,0, указывать тип назначения не нужно (например,
...
ms.locfileid: "119592964"
---
# <a name="transferring-image-data-in-wia-20"></a>Передача данных изображения в WIA 2,0

> [!Note]  
> в этом руководстве показано, как передавать данные образа в приложения, работающие в Windows Vista или более поздней версии. сведения о передаче данных изображений в приложениях, работающих под управлением Windows XP и более ранних версий, см. [в разделе передача данных изображения в WIA 1,0](-wia-transferring-image-data.md) .

 

так как передача данных изображения осуществляется на основе потоков в Windows получения изображений (WIA) 2,0, указывать целевой тип (например, память или файл) не нужно. Приложение просто предоставляет WIA 2,0 для использования в потоке, а драйвер считывает или выполняет запись в поток. Поток может быть файловым потоком, потоком памяти или любым другим типом потока, а также прозрачным для драйвера. Использование потоков также обеспечивает простую интеграцию с фильтром обработки изображений.
