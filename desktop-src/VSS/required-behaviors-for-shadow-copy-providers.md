...
VSS устанавливает флаги на каждом затронутом LUN таким, что результирующие тома теневых копий будут скрыты и доступны только для чтения при обнаружении компьютером, на котором работает Windows Server 2003. Буквы дисков и подключенные папки не назначаются автоматически. VSS поддерживает эти флаги в течение жизненного цикла теневой копии.
