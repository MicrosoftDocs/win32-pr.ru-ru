...
<p>Вторым параметром является поток треугольника, определяемый типом Трианглестреам <GSPS_INPUT> . Это означает, что параметр является массивом треугольников, каждый из которых состоит из трех вершин (которые содержат данные из членов GSPS_INPUT).</p>
