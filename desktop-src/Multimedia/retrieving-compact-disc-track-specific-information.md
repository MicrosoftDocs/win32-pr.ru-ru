...
---
# <a name="retrieving-compact-disc-track-specific-information"></a><span data-ttu-id="d4bd0-104">Получение сведений о Track-Specific компактных дисков</span><span class="sxs-lookup"><span data-stu-id="d4bd0-104">Retrieving Compact Disc Track-Specific Information</span></span>

<span data-ttu-id="d4bd0-105">Для устройств с компакт-дисков можно получить начальное расположение и длину дорожки, указав \_ флаг MCI Track и установив для элемента **Двтракк** [**\_ состояния MCI \_ пармс**](mci-status-parms.md) значение нужного номера дорожки.</span><span class="sxs-lookup"><span data-stu-id="d4bd0-105">For CD audio devices, you can get the starting location and length of a track by specifying the MCI\_TRACK flag and setting the **dwTrack** member of [**MCI\_STATUS\_PARMS**](mci-status-parms.md) to the desired track number.</span></span> <span data-ttu-id="d4bd0-106">Чтобы получить начальное расположение записи, установите для элемента **двитем** \_ положение состояния MCI \_ .</span><span class="sxs-lookup"><span data-stu-id="d4bd0-106">To get the starting location of a track, set the **dwItem** member to MCI\_STATUS\_POSITION.</span></span> <span data-ttu-id="d4bd0-107">Чтобы получить длину записи, задайте для **двитем** значение \_ длины состояния MCI \_ .</span><span class="sxs-lookup"><span data-stu-id="d4bd0-107">To get the length of a track, set **dwItem** to MCI\_STATUS\_LENGTH.</span></span> <span data-ttu-id="d4bd0-108">Например, в следующем примере извлекается общее число дорожек на компактном диске и начальное расположение каждой дорожки. Затем она использует функцию [MessageBox](/windows/win32/api/winuser/nf-winuser-messagebox) для сообщения начальных расположений дорожек.</span><span class="sxs-lookup"><span data-stu-id="d4bd0-108">For example, the following example retrieves the total number of tracks on the compact disc and the starting location of each track. Then, it uses the [MessageBox](/windows/win32/api/winuser/nf-winuser-messagebox) function to report the starting locations of the tracks.</span></span>
