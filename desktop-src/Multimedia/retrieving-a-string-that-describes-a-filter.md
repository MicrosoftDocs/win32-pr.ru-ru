...
---
# <a name="retrieving-a-string-that-describes-a-filter"></a><span data-ttu-id="b16fb-109">Получение строки, описывающей фильтр</span><span class="sxs-lookup"><span data-stu-id="b16fb-109">Retrieving a String That Describes a Filter</span></span>

<span data-ttu-id="b16fb-110">Приложению часто требуется отобразить строку, описывающую текущий формат.</span><span class="sxs-lookup"><span data-stu-id="b16fb-110">An application often needs to display a string that describes the current format.</span></span> <span data-ttu-id="b16fb-111">Эту задачу можно легко выполнить с помощью функций [**акмфилтертагдетаилс**](/windows/desktop/api/Msacm/nf-msacm-acmfiltertagdetails) и [**акмфилтердетаилс**](/windows/desktop/api/Msacm/nf-msacm-acmfilterdetails) .</span><span class="sxs-lookup"><span data-stu-id="b16fb-111">This task can be accomplished easily with the [**acmFilterTagDetails**](/windows/desktop/api/Msacm/nf-msacm-acmfiltertagdetails) and [**acmFilterDetails**](/windows/desktop/api/Msacm/nf-msacm-acmfilterdetails) functions.</span></span> <span data-ttu-id="b16fb-112">Эти функции должны вызываться с соответствующим фильтром или тегом фильтра.</span><span class="sxs-lookup"><span data-stu-id="b16fb-112">These functions must be called with the appropriate filter or filter tag.</span></span> <span data-ttu-id="b16fb-113">В следующем примере показано, как использовать эти функции.</span><span class="sxs-lookup"><span data-stu-id="b16fb-113">The following example shows how to use these functions.</span></span>
