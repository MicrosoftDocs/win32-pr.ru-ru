...
---
# <a name="creating-a-virtual-function-table-for-a-stream-handler"></a><span data-ttu-id="628af-103">Создание таблицы виртуальной функции для обработчика потока</span><span class="sxs-lookup"><span data-stu-id="628af-103">Creating a Virtual Function Table for a Stream Handler</span></span>

<span data-ttu-id="628af-104">В следующем примере (написан на языке C) показано, как приложение (Авибалл) создает таблицу виртуальной функции, используемую для ссылки на службы.</span><span class="sxs-lookup"><span data-stu-id="628af-104">The following example (written in C) shows how an application (AVIBall) creates the virtual function table used to reference its services.</span></span>
...
<span data-ttu-id="628af-105">Обработчики файлов используют аналогичную процедуру, за исключением того, что они используют другое определение для таблицы виртуальной функции.</span><span class="sxs-lookup"><span data-stu-id="628af-105">File handlers use a similar procedure, except they use a different definition for the virtual function table.</span></span>
