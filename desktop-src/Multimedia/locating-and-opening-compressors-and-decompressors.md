...
---
# <a name="locating-and-opening-compressors-and-decompressors"></a><span data-ttu-id="bb322-106">Поиск и открытие компрессоров и декомпрессоров</span><span class="sxs-lookup"><span data-stu-id="bb322-106">Locating and Opening Compressors and Decompressors</span></span>

<span data-ttu-id="bb322-107">В следующем примере функция [**иклокате**](/windows/desktop/api/Vfw/nf-vfw-iclocate) используется для поиска компрессора, который может сжимать точечный рисунок размером 8 бит на пиксель.</span><span class="sxs-lookup"><span data-stu-id="bb322-107">The following example uses the [**ICLocate**](/windows/desktop/api/Vfw/nf-vfw-iclocate) function to find a compressor that can compress an 8-bits-per-pixel bitmap.</span></span>
...
<span data-ttu-id="bb322-108">В следующем примере выполняется перечисление декомпрессоров в системе, чтобы найти тот, который может работать с форматом его изображений.</span><span class="sxs-lookup"><span data-stu-id="bb322-108">The following example enumerates the decompressors in the system to find one that can handle the format of its images.</span></span> <span data-ttu-id="bb322-109">В этом примере **используется \_ видео иктипе** (которое эквивалентно 4-символьному коду "vidc") и макрос [**икдекомпресскуери**](/windows/desktop/api/Vfw/nf-vfw-icdecompressquery) для определения того, поддерживает ли программа сжатия или распаковку формат изображения.</span><span class="sxs-lookup"><span data-stu-id="bb322-109">This example uses **ICTYPE\_VIDEO** (which is equivalent to the "VIDC" four-character code) and the [**ICDecompressQuery**](/windows/desktop/api/Vfw/nf-vfw-icdecompressquery) macro to determine if a compressor or decompressor supports the image format.</span></span>
...
<span data-ttu-id="bb322-110">В следующем примере предпринимается попытка указать конкретный компрессор для сжатия 8-разрядного формата RGB до 8-разрядного формата RLE.</span><span class="sxs-lookup"><span data-stu-id="bb322-110">The following example attempts to locate a specific compressor to compress the 8-bit RGB format to an 8-bit RLE format.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="bb322-111">См. также</span><span class="sxs-lookup"><span data-stu-id="bb322-111">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="bb322-112">Использование диспетчера сжатия видео</span><span class="sxs-lookup"><span data-stu-id="bb322-112">Using the Video Compression Manager</span></span>](using-the-video-compression-manager.md)
</dt> </dl>
