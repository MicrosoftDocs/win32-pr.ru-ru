...
ms.locfileid: "119036832"
---
# <a name="displaying-a-print-dialog-box"></a>Отображение диалогового окна «Печать»

Одним из способов получения маркера контекста устройства для принтера является отображение диалогового окна Печать и предоставление пользователю возможности выбора принтера. Функция [принтдлг](/previous-versions/windows/desktop/legacy/ms646940(v=vs.85)) (которая отображает диалоговое окно) имеет один параметр, который является адресом структуры [принтдлг](/windows/win32/api/commdlg/ns-commdlg-printdlga?redirectedfrom=MSDN) . Структура ПРИНТДЛГ имеет несколько членов, но большинство из них можно оставить в качестве значений по умолчанию. Необходимо задать два члена — **лструктсизе** и **flags**. Задайте для **лструктсизе** размер переменной принтдлг и установите **Флаги** PD \_ ретурндк. Установка **флагов** для PC \_ ретурндк указывает, что необходимо, чтобы функция принтдлг заполнила поле **HDC** с помощью маркера контекста устройства для принтера, выбранного пользователем.
