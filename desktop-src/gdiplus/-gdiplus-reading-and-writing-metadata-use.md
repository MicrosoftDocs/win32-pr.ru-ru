...
ms.locfileid: "120114934"
---
# <a name="reading-and-writing-metadata"></a>Чтение и запись метаданных

Некоторые файлы изображений содержат метаданные, которые можно прочитать, чтобы определить характеристики изображения. Например, цифровая фотография может содержать метаданные, которые можно прочитать для определения марки и модели камеры, используемой для записи образа. с помощью Windows GDI+ можно считывать существующие метаданные. кроме того, можно записывать новые метаданные в файлы изображений.

GDI+ предоставляет единообразный способ хранения и извлечения метаданных из файлов изображений в различных форматах. в GDI+ часть метаданных называется *элементом свойства*. Метаданные можно хранить и извлекать путем вызова методов **сетпропертитем** и **жетпропертитем** класса [**Image**](/windows/desktop/api/gdiplusheaders/nl-gdiplusheaders-image) , и вам не нужно беспокоиться о том, как конкретный формат файлов сохраняет эти метаданные.
