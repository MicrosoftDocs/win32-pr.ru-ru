...
ms.locfileid: "119778634"
---
# <a name="setting-jpeg-compression-level"></a>Задание уровня сжатия JPEG

Чтобы указать уровень сжатия при сохранении изображения JPEG, инициализируйте объект [**EncoderParameters**](/windows/win32/api/gdiplusimaging/nl-gdiplusimaging-encoderparameters) и передайте адрес этого объекта методу [Save](/windows/win32/api/gdiplusheaders/nf-gdiplusheaders-image-save(inistream_inconstclsid_inconstencoderparameters)) класса [**Image**](/windows/win32/api/gdiplusheaders/nl-gdiplusheaders-image) . Инициализируйте объект **EncoderParameters** , чтобы он был массивом, состоящим из одного объекта [**EncoderParameter**](/windows/win32/api/gdiplusimaging/nl-gdiplusimaging-encoderparameter) . Инициализируйте этот один объект **EncoderParameter** , чтобы его **значение** указывало на значение **ulong** от 0 до 100. Задайте для элемента **GUID** объекта **EncoderParameter** значение енкодеркуалити.

Следующее консольное приложение сохраняет три изображения JPEG, каждое из которых имеет разный уровень качества. Уровень качества 0 соответствует максимальному сжатию, а уровень качества 100 — минимальному сжатию.
