...
В сценарии политики IPsec режима транспорта требуется защита режима транспорта IPsec для всего соответствующего трафика. Любой соответствующий трафик с открытым текстом удаляется до успешного завершения согласования IKE или AuthIP. В случае сбоя согласования подключение с соответствующим IP-адресом останется неработоспособным.

Примером возможного режима транспорта может быть "защита всего трафика одноадресных данных, за исключением ICMP, с использованием транспортного режима IPsec".
...
1.  Добавьте один или оба следующих контекста поставщика политики транспортного режима QM.  
...
[**Типы контекста поставщика**](/windows/desktop/api/Fwpmtypes/ne-fwpmtypes-fwpm_provider_context_type)
</dt> <dt>
