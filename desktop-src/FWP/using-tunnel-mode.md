...
---
# <a name="using-tunnel-mode"></a><span data-ttu-id="00b63-103">Использование режима туннелирования</span><span class="sxs-lookup"><span data-stu-id="00b63-103">Using Tunnel Mode</span></span>

<span data-ttu-id="00b63-104">В следующем примере кода показано, как настроить туннель IPsec типа "точка — точка" с помощью API платформы фильтрации Windows (WFP).</span><span class="sxs-lookup"><span data-stu-id="00b63-104">The following sample code demonstrates how to configure a point-to-point IPsec tunnel using the Windows Filtering Platform (WFP) API.</span></span>
