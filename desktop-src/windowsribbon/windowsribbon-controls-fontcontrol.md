...
description: для упрощения интеграции и настройки поддержки шрифтов в приложениях, требующих обработки текста и редактирования текстов, платформа Windows Ribbon предоставляет специализированный элемент управления "шрифт", предоставляющий широкий спектр свойств шрифта, таких как имя гарнитуры, стиль, размер точки и эффекты.
...
-   автоматическая, в Windows 7, активация шрифта, основанная на параметре **показать** или **скрыть** для каждого шрифта на панели управления **шрифты** . Элемент управления "Шрифт" отображает только те шрифты, которые установлены для **отображения**.
    > [!Note]  
...
-   Раскрытие шрифтов, которые наиболее важны для пользователя, например
    ...
    > [!Note]  
    > поддержка этой функции недоступна на всех платформах, более ранних, чем Windows 7.
...
рекомендуемым префиксом для ленты многоязычный пользовательский интерфейс (MUI) EN-US является "F", как показано в следующем примере.
