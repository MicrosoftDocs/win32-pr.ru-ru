...
Часть LCID предиката указывает языковой стандарт поиска. Это указывает поисковому механизму использовать соответствующие средства разбиения по словам и формы форм для поискового запроса. чтобы указать языковой стандарт, укажите Windows стандартный идентификатор языка (LCID). Например, 1033 — это LCID для США-English. Поместите LCID в качестве последнего элемента в скобках предложения CONTAINS. Важные сведения о поиске и языках см. [в разделе Использование локализованных поисков](-search-sql-usinglocsearches.md).
