...
Пользователи могут сохранить поисковый запрос как папку поиска, виртуальную папку, отображаемую в проводнике Windows в папке "Поиск". При открытии папки поиска выполняется сохраненный поиск и отображаются последние результаты. Сохраненный файл поиска хранит запрос в формате, в котором может работать Windows Search, указывая, что искать, где искать, и как представлять результаты.
