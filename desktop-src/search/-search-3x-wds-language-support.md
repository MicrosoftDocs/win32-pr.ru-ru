---
description: в этом разделе описывается, как Windows поиск поддерживает несколько языков.
...
в этом разделе описывается, как Windows поиск поддерживает несколько языков.

## <a name="tokenization-wordbreakers-and-language-resources"></a>Разметка, разбиение по словам и языковые ресурсы

Windows Поиск не зависит от языка, но точность поиска по языкам может отличаться в зависимости от способа разделения текста в виде средств разбиения по словам. Средства разбиения по словам реализуют различные правила разметки для языков и разбивают текст на отдельные маркеры или слова для индексирования или поиска.
...
Windows Поиск поставляется с хорошо определенным набором средств разбиения по словам. классические компоненты средства разбиения на слова и средства извлечения корней поддерживаются в Windows Vista и более поздних версиях. если не удается определить язык документа, Windows поиск пытается определить язык для определения наиболее подходящего средства разбиения по словам. Windows Поиск пытается определить язык, вызвав функцию [**жетсистемпреферредуилангуажес**](/windows/win32/api/winnls/nf-winnls-getsystempreferreduilanguages) для определения первого языка многоязыкового интерфейса (MUI) (обычно это системный язык пользовательского интерфейса, если не установлены языковые пакеты MUI). Если этот вызов будет выполнен, используется средство разбиения по словам для первого языка MUI. если вызов **жетсистемпреферредуилангуажес** завершается ошибкой, Windows поиск получает языковой стандарт системы, вызывая функцию [**жетсистемдефаултлЦид**](/windows/win32/api/winnls/nf-winnls-getsystemdefaultlcid) и использующий средство разбиения по словам, связанное с этим языковым стандартом.

если для языка не установлено средство разбиения по словам, Windows поиска на пустом пробеле с помощью **нейтрального** средства разбиения на слова.
...
если для Windows поиска требуется новое средство разбиения по словам, считывается идентификатор класса (CLSID), а также кэшируется экземпляр средства разбиения по словам.

Вы можете создать пользовательское средство разбиения по словам для языка, реализовав интерфейс [**ивордбреакер**](/windows/desktop/api/Indexsrv/nn-indexsrv-iwordbreaker) . Windows Затем Поиск вызывает методы **ивордбреакер** при построении индексов содержимого и выполнении запросов.
...
Windows Поиск включает в себя разбиение по словам для поддержки следующих языков.
...
-   сведения о том, как реализовать и использовать пользовательские средства разбиения по словам и парадигматические модули для дополнительных языков и национальных настроек, см. [в разделе расширение языковых ресурсов в Windows поиска](extending-language-resources-in-windows-search.md).
...
-   дополнительные сведения об управлении, выполнении запросов и расширении индекса см. в разделе [Windows поиска](-search-developers-guide-entry-page.md)в документации разработчика.
