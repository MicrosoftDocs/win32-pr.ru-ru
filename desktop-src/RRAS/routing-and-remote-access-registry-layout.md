...
Следующий синтаксис показывает пример макета реестра для службы маршрутизатора.
...
Каждый диспетчер маршрутизаторов, установленный в системе, имеет раздел реестра, созданный в разделе ключа маршрутизатора. Переменная DLLPath указывает расположение библиотеки DLL, которая соответствует диспетчеру маршрутизации, а переменная Протоколид указывает идентификатор семейства протоколов для диспетчера маршрутизаторов.
