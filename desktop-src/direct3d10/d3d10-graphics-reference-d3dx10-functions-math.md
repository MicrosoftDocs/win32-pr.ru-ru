---
description: Сведения о математической библиотеке, предоставляемой библиотекой служебной программы D3DX в графике Direct3D 10. Библиотека предоставляет функции для вычислений трехмерных математических операций.
...
> [!Note]  
> Математические функции библиотеки служебной программы D3DX являются устаревшими для Windows 8. Вместо этого рекомендуется использовать [директксмас](../dxmath/directxmath-portal.md) .

 

Математическая библиотека, предоставляемая библиотекой служебной программы D3DX, предоставляет функции для вычисления трехмерных математических операций. Каждая из функций может принимать тот же объект, что и переданный параметр, \[ \] и возвращать \[ выходные \] Параметры. Кроме того, параметры out обычно возвращаются в виде возвращаемых значений, чтобы выходные данные одной математической функции можно было использовать в качестве параметра для другой математической функции.
...
## <a name="resolving-link-errors-with-d3dx-math-functions"></a>Разрешение ошибок ссылок с помощью математических функций D3DX

Математические функции D3DX реализуются одинаково в D3DX10 (D3DX10math. h) и D3DX9 (D3DX9math. h). Это может привести к ошибкам связи, если проект реализует как код DirectX 9, так и DirectX 10, и пытается связать функцию из одного заголовка с противоположной библиотекой.

Чтобы устранить проблему, связанную с включением обоих заголовков, D3DX10math. h включает следующие \# определения:
