...
Начиная с Windows Vista, данные принтера, возвращаемые методом **PrintOut** , получаются из локального кэша, когда *хпринтер* ссылается на принтер, размещенный на сервере печати, и существует по крайней мере одно открытое соединение с сервером печати. Во всех остальных конфигурациях данные принтера запрашиваются с сервера печати.
