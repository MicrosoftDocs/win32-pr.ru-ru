...
Вызовите этот метод после вызова метода [**креатевиртуалчаннелс**](imstscax-createvirtualchannels.md) , но перед установкой соединения с помощью метода [**Connect**](imstscax-connect.md) . Этот метод задает дополнительные параметры для виртуальных каналов, созданных с помощью **креатевиртуалчаннелс**.
