...
CNG впервые доступен в Windows Vista и располагается для замены существующих применений [*CryptoAPI*](/windows/desktop/SecGloss/c-gly) во всем стеке программного обеспечения Майкрософт. Сторонние разработчики найдут в CNG множество новых функций, включая:
...
CNG проверяется на Федеральный стандарт обработки информации (FIPS) 140-2 и является частью цели оценки сертификации Windows с общим критерием. CNG был разработан для использования в качестве компонента в проверенной системе уровня FIPS 2.
...
CNG включает поддержку набора B, который распространяется на все необходимые алгоритмы: AES (все размеры ключей), Семейство SHA-2 (SHA-256, SHA-384 и SHA-512) для алгоритмов хэширования, ECDH и DSA (ECDSA) на основе стандартных кривых стандарта NIST P-256, P-384 и P-521. Двоичные кривые, кривые Коблитз, пользовательские простые кривые и эллиптическая кривая Менезес-qu-Ванстоне (ЕКМКВ) не поддерживаются поставщиками алгоритмов Майкрософт, входящими в состав Windows Vista.
...
| Режим работы           | \_Значение режима цепочки BCRYPT \_ | Алгоритмы              | Standard  |
|-----------------------------|------------------------------|-------------------------|-----------|
...
> [!Note]  
> В Windows Vista определены только режимы работы в ECB, CBC и CFB. Для GCM и CCM требуется Windows Vista с пакетом обновления 1 (SP1) или Windows Server 2008.
