...
---
# <a name="pkcs-10-encoded-asn1"></a><span data-ttu-id="0f7a9-105">Формат \# ASN. 1 в кодировке PKCS 10</span><span class="sxs-lookup"><span data-stu-id="0f7a9-105">PKCS \#10 Encoded ASN.1</span></span>

<span data-ttu-id="0f7a9-106">Следующий пример содержит \# запрос PKCS 10, показанный в формате ASN. 1.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-106">The following example contains a PKCS \#10 request shown in ASN.1 format.</span></span> <span data-ttu-id="0f7a9-107">Пример был создан с помощью средств Certreq.exe и Certutil.exe.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-107">The example was generated by using the Certreq.exe and Certutil.exe tools.</span></span> <span data-ttu-id="0f7a9-108">INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-108">The .inf file used as input to Certreq.exe contains the following configuration.</span></span>
...
<span data-ttu-id="0f7a9-109">Эта конфигурация создает следующий пример выходных данных.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-109">This configuration generates the following sample output.</span></span> <span data-ttu-id="0f7a9-110">В конфигурации указывается субъект, тип запроса (PKCS \# 10) и шаблон сертификата (пользователь).</span><span class="sxs-lookup"><span data-stu-id="0f7a9-110">The configuration specifies the subject, type of request (PKCS \#10), and certificate template (User).</span></span> <span data-ttu-id="0f7a9-111">В пользовательском шаблоне указывается следующее:</span><span class="sxs-lookup"><span data-stu-id="0f7a9-111">The User template specifies that:</span></span>

-   <span data-ttu-id="0f7a9-112">В запросе должен использоваться базовый поставщик служб шифрования Microsoft 1,0 или Microsoft Улучшенный поставщик служб шифрования 1,0.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-112">The request must use either the Microsoft Base Cryptographic Provider 1.0 or the Microsoft Enhanced Cryptographic Provider 1.0.</span></span>
-   <span data-ttu-id="0f7a9-113">Имя субъекта должно быть создано из Active Directory.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-113">The subject name must be built from Active Directory.</span></span>
-   <span data-ttu-id="0f7a9-114">Запрос включает имя шаблона сертификата, расширенное использование ключа (EKU) и расширения использования ключа.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-114">The request includes the Certificate Template Name, Enhanced Key Usage (EKU), and Key Usage extensions.</span></span> <span data-ttu-id="0f7a9-115">Расширение EKU указывает, что выданный сертификат можно использовать для шифрованная файловая система (EFS) (EFS), безопасной электронной почты и проверки подлинности клиента.</span><span class="sxs-lookup"><span data-stu-id="0f7a9-115">The EKU extension specifies that the issued certificate can be used for Encrypting File System (EFS), secure email, and client authentication.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="0f7a9-116">См. также</span><span class="sxs-lookup"><span data-stu-id="0f7a9-116">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="0f7a9-117">Примеры запросов</span><span class="sxs-lookup"><span data-stu-id="0f7a9-117">Sample Requests</span></span>](sample-requests.md)
</dt> </dl>
