---
description: В следующем примере содержится запрос обновления CMC, показанный в формате ASN. 1. Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
...
В следующем примере содержится запрос обновления CMC, показанный в формате ASN. 1. Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
...
В конфигурации указывается тип запроса (CMC) и серийный номер сертификата, на основе которого основан запрос. При этом создаются следующие выходные данные.
