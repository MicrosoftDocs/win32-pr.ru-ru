---
description: В следующем примере содержится регистрация CMC от имени запроса (ЕОБО). Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
ms.assetid: 05ec0b2c-56a5-4d9c-999b-23324134153b
title: Запрос ЕОБО CMC
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103815784"
---
# <a name="cmc-eobo-request"></a>Запрос ЕОБО CMC

В следующем примере содержится регистрация CMC от имени запроса (ЕОБО). Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
...
Эта конфигурация создает следующий пример выходных данных. В конфигурации указывается тип запроса (CMC), имя сущности, запрашивающей регистрацию, и имя шаблона. В пользовательском шаблоне указывается следующее:
