...
---
# <a name="cmc-encoded-asn1"></a><span data-ttu-id="28a7d-105">В формате CMC с кодировкой ASN. 1</span><span class="sxs-lookup"><span data-stu-id="28a7d-105">CMC Encoded ASN.1</span></span>

<span data-ttu-id="28a7d-106">В следующем примере содержится запрос CMC, показанный в формате ASN. 1.</span><span class="sxs-lookup"><span data-stu-id="28a7d-106">The following example contains a CMC request shown in ASN.1 format.</span></span> <span data-ttu-id="28a7d-107">Пример был создан с помощью средств Certreq.exe и Certutil.exe.</span><span class="sxs-lookup"><span data-stu-id="28a7d-107">The example was generated by using the Certreq.exe and Certutil.exe tools.</span></span> <span data-ttu-id="28a7d-108">INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.</span><span class="sxs-lookup"><span data-stu-id="28a7d-108">The .inf file used as input to Certreq.exe contains the following configuration.</span></span>
...
<span data-ttu-id="28a7d-109">Эта конфигурация создает следующий пример выходных данных.</span><span class="sxs-lookup"><span data-stu-id="28a7d-109">This configuration generates the following sample output.</span></span> <span data-ttu-id="28a7d-110">В конфигурации указывается субъект, тип запроса (CMC) и шаблон сертификата (пользователь).</span><span class="sxs-lookup"><span data-stu-id="28a7d-110">The configuration specifies the subject, type of request (CMC), and certificate template (User).</span></span> <span data-ttu-id="28a7d-111">В пользовательском шаблоне указывается следующее:</span><span class="sxs-lookup"><span data-stu-id="28a7d-111">The User template specifies that:</span></span>

-   <span data-ttu-id="28a7d-112">В запросе должен использоваться базовый поставщик служб шифрования Microsoft 1,0 или Microsoft Улучшенный поставщик служб шифрования 1,0.</span><span class="sxs-lookup"><span data-stu-id="28a7d-112">The request must use either the Microsoft Base Cryptographic Provider 1.0 or the Microsoft Enhanced Cryptographic Provider 1.0.</span></span>
-   <span data-ttu-id="28a7d-113">Имя субъекта должно быть создано из Active Directory.</span><span class="sxs-lookup"><span data-stu-id="28a7d-113">The subject name must be built from Active Directory.</span></span>
-   <span data-ttu-id="28a7d-114">Запрос включает имя шаблона сертификата, расширенное использование ключа (EKU) и расширения использования ключа.</span><span class="sxs-lookup"><span data-stu-id="28a7d-114">The request includes the Certificate Template Name, Enhanced Key Usage (EKU), and Key Usage extensions.</span></span> <span data-ttu-id="28a7d-115">Расширение EKU указывает, что выданный сертификат можно использовать для шифрованная файловая система (EFS) (EFS), безопасной электронной почты и проверки подлинности клиента.</span><span class="sxs-lookup"><span data-stu-id="28a7d-115">The EKU extension specifies that the issued certificate can be used for Encrypting File System (EFS), secure email, and client authentication.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="28a7d-116">См. также</span><span class="sxs-lookup"><span data-stu-id="28a7d-116">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="28a7d-117">Примеры запросов</span><span class="sxs-lookup"><span data-stu-id="28a7d-117">Sample Requests</span></span>](sample-requests.md)
</dt> </dl>
