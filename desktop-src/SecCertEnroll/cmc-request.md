---
description: В следующем примере содержится запрос на сертификат CMC. Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
...
ms.locfileid: "119670344"
---
# <a name="cmc-request"></a>Запрос CMC

В следующем примере содержится запрос на сертификат CMC. Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
...
Эта конфигурация создает следующий пример выходных данных. В конфигурации указывается субъект, тип запроса (CMC) и шаблон сертификата (пользователь). В пользовательском шаблоне указывается следующее:
...
Запрос на сертификат CMC представлен интерфейсом [**IX509CertificateRequestCmc**](/windows/desktop/api/CertEnroll/nn-certenroll-ix509certificaterequestcmc) .
