...
---
# <a name="pkcs-7-eobo-request"></a><span data-ttu-id="52688-105">\#Запрос PKCS 7 еобо</span><span class="sxs-lookup"><span data-stu-id="52688-105">PKCS \#7 EOBO Request</span></span>

<span data-ttu-id="52688-106">В следующем примере содержится запрос PKCS \# 7 "регистрация от имени" (еобо).</span><span class="sxs-lookup"><span data-stu-id="52688-106">The following example contains a PKCS \#7 Enroll On Behalf Of (EOBO) request.</span></span> <span data-ttu-id="52688-107">Пример был создан с помощью средств Certreq.exe и Certutil.exe.</span><span class="sxs-lookup"><span data-stu-id="52688-107">The example was generated by using the Certreq.exe and Certutil.exe tools.</span></span> <span data-ttu-id="52688-108">INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.</span><span class="sxs-lookup"><span data-stu-id="52688-108">The .inf file used as input to Certreq.exe contains the following configuration.</span></span>
...
<span data-ttu-id="52688-109">Эта конфигурация создает следующий пример выходных данных.</span><span class="sxs-lookup"><span data-stu-id="52688-109">This configuration generates the following sample output.</span></span> <span data-ttu-id="52688-110">В конфигурации указывается тип запроса (PKCS \# 7), имя сущности, запрашивающей регистрацию, и имя шаблона.</span><span class="sxs-lookup"><span data-stu-id="52688-110">The configuration specifies the type of request (PKCS \#7), the name of the entity requesting the enrollment, and the name of the template.</span></span> <span data-ttu-id="52688-111">В пользовательском шаблоне указывается следующее:</span><span class="sxs-lookup"><span data-stu-id="52688-111">The User template specifies that:</span></span>

-   <span data-ttu-id="52688-112">В запросе должен использоваться базовый поставщик служб шифрования Microsoft 1,0 или Microsoft Улучшенный поставщик служб шифрования 1,0.</span><span class="sxs-lookup"><span data-stu-id="52688-112">The request must use either the Microsoft Base Cryptographic Provider 1.0 or the Microsoft Enhanced Cryptographic Provider 1.0.</span></span>
-   <span data-ttu-id="52688-113">Имя субъекта должно быть создано из Active Directory.</span><span class="sxs-lookup"><span data-stu-id="52688-113">The subject name must be built from Active Directory.</span></span>
-   <span data-ttu-id="52688-114">Запрос включает имя шаблона сертификата, расширенное использование ключа (EKU) и расширения использования ключа.</span><span class="sxs-lookup"><span data-stu-id="52688-114">The request includes the Certificate Template Name, Enhanced Key Usage (EKU), and Key Usage extensions.</span></span> <span data-ttu-id="52688-115">Расширение EKU указывает, что выданный сертификат можно использовать для шифрованная файловая система (EFS) (EFS), безопасной электронной почты и проверки подлинности клиента.</span><span class="sxs-lookup"><span data-stu-id="52688-115">The EKU extension specifies that the issued certificate can be used for Encrypting File System (EFS), secure email, and client authentication.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="52688-116">См. также</span><span class="sxs-lookup"><span data-stu-id="52688-116">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="52688-117">Примеры запросов</span><span class="sxs-lookup"><span data-stu-id="52688-117">Sample Requests</span></span>](sample-requests.md)
</dt> </dl>
