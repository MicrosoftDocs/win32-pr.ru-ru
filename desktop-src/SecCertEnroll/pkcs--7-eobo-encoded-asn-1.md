...
---
# <a name="pkcs-7-eobo-encoded-asn1"></a><span data-ttu-id="c85b7-105">PKCS \# 7 еобо в кодировке ASN. 1</span><span class="sxs-lookup"><span data-stu-id="c85b7-105">PKCS \#7 EOBO Encoded ASN.1</span></span>

<span data-ttu-id="c85b7-106">В следующем примере приведен запрос PKCS \# 7 "регистрация от имени" (еобо), показанный в формате ASN. 1.</span><span class="sxs-lookup"><span data-stu-id="c85b7-106">The following example contains a PKCS \#7 Enroll On Behalf Of (EOBO) request shown in ASN.1 format.</span></span> <span data-ttu-id="c85b7-107">Пример был создан с помощью средств Certreq.exe и Certutil.exe.</span><span class="sxs-lookup"><span data-stu-id="c85b7-107">The example was generated by using the Certreq.exe and Certutil.exe tools.</span></span> <span data-ttu-id="c85b7-108">INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.</span><span class="sxs-lookup"><span data-stu-id="c85b7-108">The .inf file used as input to Certreq.exe contains the following configuration.</span></span>
...
<span data-ttu-id="c85b7-109">Эта конфигурация создает следующий пример выходных данных.</span><span class="sxs-lookup"><span data-stu-id="c85b7-109">This configuration generates the following sample output.</span></span> <span data-ttu-id="c85b7-110">В конфигурации указывается тип запроса (PKCS \# 7), имя сущности, запрашивающей регистрацию, и имя шаблона.</span><span class="sxs-lookup"><span data-stu-id="c85b7-110">The configuration specifies the type of request (PKCS \#7), the name of the entity requesting the enrollment, and the name of the template.</span></span> <span data-ttu-id="c85b7-111">В пользовательском шаблоне указывается следующее:</span><span class="sxs-lookup"><span data-stu-id="c85b7-111">The User template specifies that:</span></span>

-   <span data-ttu-id="c85b7-112">В запросе должен использоваться базовый поставщик служб шифрования Microsoft 1,0 или Microsoft Улучшенный поставщик служб шифрования 1,0.</span><span class="sxs-lookup"><span data-stu-id="c85b7-112">The request must use either the Microsoft Base Cryptographic Provider 1.0 or the Microsoft Enhanced Cryptographic Provider 1.0.</span></span>
-   <span data-ttu-id="c85b7-113">Имя субъекта должно быть создано из Active Directory.</span><span class="sxs-lookup"><span data-stu-id="c85b7-113">The subject name must be built from Active Directory.</span></span>
-   <span data-ttu-id="c85b7-114">Запрос включает имя шаблона сертификата, расширенное использование ключа (EKU) и расширения использования ключа.</span><span class="sxs-lookup"><span data-stu-id="c85b7-114">The request includes the Certificate Template Name, Enhanced Key Usage (EKU), and Key Usage extensions.</span></span> <span data-ttu-id="c85b7-115">Расширение EKU указывает, что выданный сертификат можно использовать для шифрованная файловая система (EFS) (EFS), безопасной электронной почты и проверки подлинности клиента.</span><span class="sxs-lookup"><span data-stu-id="c85b7-115">The EKU extension specifies that the issued certificate can be used for Encrypting File System (EFS), secure email, and client authentication.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="c85b7-116">См. также</span><span class="sxs-lookup"><span data-stu-id="c85b7-116">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="c85b7-117">Примеры запросов</span><span class="sxs-lookup"><span data-stu-id="c85b7-117">Sample Requests</span></span>](sample-requests.md)
</dt> </dl>
