---
description: В следующем примере приведен запрос PKCS \# 7 "регистрация от имени" (еобо), показанный в формате ASN. 1. Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
ms.assetid: b6945071-8019-40bb-9f71-13ae3bd57948
title: PKCS \# 7 еобо в кодировке ASN. 1
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105664253"
---
# <a name="pkcs-7-eobo-encoded-asn1"></a>PKCS \# 7 еобо в кодировке ASN. 1

В следующем примере приведен запрос PKCS \# 7 "регистрация от имени" (еобо), показанный в формате ASN. 1. Пример был создан с помощью средств Certreq.exe и Certutil.exe. INF-файл, используемый в качестве входных данных для Certreq.exe, содержит следующую конфигурацию.
...
Эта конфигурация создает следующий пример выходных данных. В конфигурации указывается тип запроса (PKCS \# 7), имя сущности, запрашивающей регистрацию, и имя шаблона. В пользовательском шаблоне указывается следующее:

-   В запросе должен использоваться базовый поставщик служб шифрования Microsoft 1,0 или Microsoft Улучшенный поставщик служб шифрования 1,0.
-   Имя субъекта должно быть создано из Active Directory.
-   Запрос включает имя шаблона сертификата, расширенное использование ключа (EKU) и расширения использования ключа. Расширение EKU указывает, что выданный сертификат можно использовать для шифрованная файловая система (EFS) (EFS), безопасной электронной почты и проверки подлинности клиента.
