...
используя API [**INetFwPolicy2**](/previous-versions/windows/desktop/api/netfw/nn-netfw-inetfwpolicy2) Windows брандмауэра для перечисления всех правил с установленным флагом обхода, все приложения, которые хотят прослушивать незапрошенный трафик, перечисляются для исключения брандмауэра. Конкретные сведения об использовании параметра обхода узлов [см. в](receiving-unsolicited-traffic-over-teredo.md)этой статьи.
