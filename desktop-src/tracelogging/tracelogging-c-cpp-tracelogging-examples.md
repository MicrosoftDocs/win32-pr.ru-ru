---
title: Примеры Трацелоггинг в C/C++
description: Этот раздел содержит примеры Трацелоггинг C/C++.
...
ms.locfileid: "104410821"
---
# <a name="cc-tracelogging-examples"></a><span data-ttu-id="170ee-103">Примеры Трацелоггинг в C/C++</span><span class="sxs-lookup"><span data-stu-id="170ee-103">C/C++ Tracelogging Examples</span></span>

<span data-ttu-id="170ee-104">Этот раздел содержит примеры Трацелоггинг C/C++.</span><span class="sxs-lookup"><span data-stu-id="170ee-104">This topic contains C/C++ Tracelogging examples.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="170ee-105">Свяжите advapi32. lib при компиляции этих примеров.</span><span class="sxs-lookup"><span data-stu-id="170ee-105">Link advapi32.lib when compiling these examples.</span></span>

 

-   [<span data-ttu-id="170ee-106">Встроенные типы данных журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-106">Log intrinsic data types</span></span>](#log-intrinsic-data-types)
-   [<span data-ttu-id="170ee-107">Поля "имя события"</span><span class="sxs-lookup"><span data-stu-id="170ee-107">Name event fields</span></span>](#name-event-fields)
-   [<span data-ttu-id="170ee-108">Уровень детализации событий журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-108">Log events verbosity level</span></span>](#log-event-verbosity-level)
-   [<span data-ttu-id="170ee-109">Ключевые слова событий Log</span><span class="sxs-lookup"><span data-stu-id="170ee-109">Log events keywords</span></span>](#log-event-keywords)
-   [<span data-ttu-id="170ee-110">Данные события массива журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-110">Log array event data</span></span>](#log-array-event-data)
-   [<span data-ttu-id="170ee-111">Данные события структуры журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-111">Log structure event data</span></span>](#log-structure-event-data)

### <a name="log-intrinsic-data-types"></a><span data-ttu-id="170ee-112">Встроенные типы данных журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-112">Log intrinsic data types</span></span>

<span data-ttu-id="170ee-113">В этом примере показано, как вести журнал встроенных типов данных, таких как целые числа, логические значения и т. д.</span><span class="sxs-lookup"><span data-stu-id="170ee-113">This example shows how to log intrinsic data types such as integers, booleans, and so on.</span></span>
...
### <a name="name-event-fields"></a><span data-ttu-id="170ee-114">Поля "имя события"</span><span class="sxs-lookup"><span data-stu-id="170ee-114">Name event fields</span></span>

<span data-ttu-id="170ee-115">В примере показано, как присвоить имя полям событий.</span><span class="sxs-lookup"><span data-stu-id="170ee-115">The example shows how to name event fields.</span></span>
...
### <a name="log-event-verbosity-level"></a><span data-ttu-id="170ee-116">Уровень детализации событий журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-116">Log event verbosity level</span></span>

<span data-ttu-id="170ee-117">В этом примере показано, как регистрировать события по уровню детализации.</span><span class="sxs-lookup"><span data-stu-id="170ee-117">This example shows how to log events by verbosity level.</span></span>
...
### <a name="log-event-keywords"></a><span data-ttu-id="170ee-118">Ключевые слова событий Log</span><span class="sxs-lookup"><span data-stu-id="170ee-118">Log event keywords</span></span>

<span data-ttu-id="170ee-119">В этом примере показано, как задать ключевые слова событий.</span><span class="sxs-lookup"><span data-stu-id="170ee-119">This example illustrates how to set event keywords.</span></span> <span data-ttu-id="170ee-120">Фильтрацию событий можно выполнять по уровню и ключевому слову.</span><span class="sxs-lookup"><span data-stu-id="170ee-120">Event filtering can be done by level and keyword.</span></span> <span data-ttu-id="170ee-121">Например, события завершения сценария или события сбоя могут быть сгруппированы под отдельными ключевыми словами, чтобы можно было легко фильтровать эти события.</span><span class="sxs-lookup"><span data-stu-id="170ee-121">For example, scenario completion events or failure events might be grouped under separate keywords so that you can easily filter those events.</span></span>
...
### <a name="log-array-event-data"></a><span data-ttu-id="170ee-122">Данные события массива журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-122">Log array event data</span></span>

<span data-ttu-id="170ee-123">В этом примере показано, как записывать в журнал массивы как данные событий.</span><span class="sxs-lookup"><span data-stu-id="170ee-123">This example shows how to log arrays as event data.</span></span>
...
### <a name="log-structure-event-data"></a><span data-ttu-id="170ee-124">Данные события структуры журнала</span><span class="sxs-lookup"><span data-stu-id="170ee-124">Log structure event data</span></span>

<span data-ttu-id="170ee-125">В этом примере показано, как записывать данные события структуры в журнал.</span><span class="sxs-lookup"><span data-stu-id="170ee-125">This example shows how to log structure event data.</span></span>
...
### <a name="how-to-fix-tracelogging-cc-build-errors"></a><span data-ttu-id="170ee-126">Исправление ошибок сборки C/C++ Трацелоггинг</span><span class="sxs-lookup"><span data-stu-id="170ee-126">How to fix TraceLogging C/C++ build errors</span></span>

<span data-ttu-id="170ee-127">Ошибка сборки</span><span class="sxs-lookup"><span data-stu-id="170ee-127">Build Error</span></span>
...
<span data-ttu-id="170ee-128">Fix</span><span class="sxs-lookup"><span data-stu-id="170ee-128">Fix</span></span>

<span data-ttu-id="170ee-129">Устраните эту ошибку, связав advapi32. lib в проекте.</span><span class="sxs-lookup"><span data-stu-id="170ee-129">Fix this error by linking advapi32.lib in your project.</span></span>
