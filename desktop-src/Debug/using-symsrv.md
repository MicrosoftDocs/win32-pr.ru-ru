...
|  Тип хранилища символов       |  Описание |
|----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| https://SecureInternetSite | URL-адрес защищенного веб-сайта, на котором размещены символы. это может поддерживать пароли, Windows учетные данные входа, сертификаты и smartcards. Должно быть крайним правым магазином в списке и не должно быть единственным хранилищем в списке.                                                                                                                              |
| &lt;пусто&gt;              | Если между двумя звездочками нет текста, это означает, что *по умолчанию используется нисходящий магазин*. Расположение задается путем вызова [**симсесомедиректори**](/windows/desktop/api/Dbghelp/nf-dbghelp-symsethomedirectory). Значение по умолчанию — каталог с именем «sym» непосредственно под каталогом программы вызывающего приложения. Иногда это называется *локальным кэшем по умолчанию*. |
...
сервер символов интернета заполняется множеством Windows символов для операционных систем Microsoft Windows, включая исправления, пакеты обновления, пакеты исправлений безопасности и розничные выпуски. символы также доступны на сервере для текущих бета-версий и кандидатов на выпуски для Windows продуктов, а также различных продуктов майкрософт, таких как microsoft Internet Explorer.
...
сервер символов SymSrv (symsrv.dll) включен в средства отладки для пакета Windows. Он должен быть установлен в том же каталоге, что и копия dbghelp.dll, которую вы загружаете. Дополнительные сведения см. [в разделе вызов библиотеки DBGHELP](calling-the-dbghelp-library.md).
