---
description: В следующем образце кода показано, как использовать векторную обработку исключений. Он использует функцию Аддвекторедексцептионхандлер для добавления нескольких обработчиков, проверяет обработчики, а затем использует функцию Ремовевекторедексцептионхандлер для удаления обработчиков.
ms.assetid: dbf7016b-09ac-4ca7-9b47-38b0dd763462
title: Использование векторного обработчика исключений
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103895902"
---
# <a name="using-a-vectored-exception-handler"></a><span data-ttu-id="a384f-104">Использование векторного обработчика исключений</span><span class="sxs-lookup"><span data-stu-id="a384f-104">Using a Vectored Exception Handler</span></span>

<span data-ttu-id="a384f-105">В следующем образце кода показано, как использовать векторную обработку исключений.</span><span class="sxs-lookup"><span data-stu-id="a384f-105">The following sample code demonstrates how to use vectored exception handling.</span></span> <span data-ttu-id="a384f-106">Он использует функцию [**аддвекторедексцептионхандлер**](/windows/win32/api/errhandlingapi/nf-errhandlingapi-addvectoredexceptionhandler) для добавления нескольких обработчиков, проверяет обработчики, а затем использует функцию [**ремовевекторедексцептионхандлер**](/windows/win32/api/errhandlingapi/nf-errhandlingapi-removevectoredexceptionhandler) для удаления обработчиков.</span><span class="sxs-lookup"><span data-stu-id="a384f-106">It uses the [**AddVectoredExceptionHandler**](/windows/win32/api/errhandlingapi/nf-errhandlingapi-addvectoredexceptionhandler) function to add several handlers, tests the handlers, then uses the [**RemoveVectoredExceptionHandler**](/windows/win32/api/errhandlingapi/nf-errhandlingapi-removevectoredexceptionhandler) function to remove the handlers.</span></span>

<span data-ttu-id="a384f-107">**64-разрядная версия Windows:** Этот код не подходит для 64-разрядных Windows.</span><span class="sxs-lookup"><span data-stu-id="a384f-107">**64-bit Windows:** This code is not suitable for 64-bit Windows.</span></span>
