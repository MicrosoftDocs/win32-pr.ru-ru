...
| SSINDEX. cmd | Этот сценарий создает список файлов, возвращенных в систему управления версиями, и сведения о версии каждого файла. Он сохраняет подмножество этих сведений в PDB-файлах, созданных при построении приложения. Скрипт использует один из следующих модулей Perl для взаимодействия с системой управления версиями: P4.pm (Перфорце) или Vss.pm (Visual Source Сейф). Для получения дополнительных сведений запустите сценарий с параметром-?. или-?? (подробное описание справки) или просмотрите скрипт.<br/>                                                                                                                                                                                                                                                                                                             |
...
чтобы использовать исходный сервер с WinDbg, KD, NTSD или CDB, убедитесь, что установлена последняя версия средств отладки для пакета Windows (версии 6,3 или более поздней). Затем включите SRV \* в команду. сркпас следующим образом:
