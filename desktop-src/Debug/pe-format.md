---
description: эта спецификация описывает структуру исполняемых файлов (образов) и объектных файлов в семействе Windows операционных систем. Эти файлы называются переносимыми исполняемыми (PE) и файловыми файлами формата COFF соответственно.
...
ms.locfileid: "121812197"
---
# <a name="pe-format"></a>Формат PE

эта спецификация описывает структуру исполняемых файлов (образов) и объектных файлов в семействе Windows операционных систем. Эти файлы называются переносимыми исполняемыми (PE) и файловыми файлами формата COFF соответственно.

> [!Note]  
> этот документ предназначен для помощи в разработке средств и приложений для Windows, но не гарантируется полной спецификацией во всех отношениях. Корпорация Майкрософт оставляет за собой право изменять этот документ без предварительного уведомления.
...
В начале объектного файла или непосредственно после подписи файла изображения является стандартным заголовком COFF-файла в следующем формате. обратите внимание, что Windows загрузчик ограничивает количество разделов до 96.
...
|------------------------------------------------------|--------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| \_неочищенный файл изображения \_ reloc \_ <br/>            | 0x0001 <br/> | только изображения, Windows CE и Microsoft Windows NT и более поздние версии. Это означает, что файл не содержит базовых перемещений и поэтому должен быть загружен по предпочтительному базовому адресу. Если базовый адрес недоступен, загрузчик сообщает об ошибке. Поведение компоновщика по умолчанию заключается в том, чтобы удалить базовые перемещения из исполняемых файлов (EXE). <br/> |
...
| 64/64 <br/>    | 4 <br/>      | Вычислен <br/>                    | Контрольная сумма файла образа. Алгоритм вычисления контрольной суммы включен в IMAGHELP.DLL. следующие проверки проверяются во время загрузки: все драйверы, все библиотеки dll, загруженные во время загрузки, и любая библиотека dll, которая загружается в критический Windows процесс. <br/>                                          |
| 68/68 <br/>    | 2 <br/>      | Subsystem <br/>                   | Подсистема, необходимая для выполнения этого образа. дополнительные сведения см. в разделе [Windowsная подсистема](#windows-subsystem). <br/>                                                                                                                                                                                       |
...
##### <a name="windows-subsystem"></a>Windows Подсистемы
...
| \_собственная ПОДСИСТЕМА \_ образа <br/>                      | 1 <br/>  | драйверы устройств и процессы машинного Windows <br/>          |
| \_ \_ графический интерфейс Windows для ПОДсистемы изображения \_ <br/>                | 2 <br/>  | Windows подсистемы графического пользовательского интерфейса <br/> |
| \_ПОДСИСТЕМА образов \_ Windows \_ несекретная <br/>                | 3 <br/>  | подсистема Windows символов <br/>                      |
...
| \_ \_ \_ приложение загрузки Windows для подсистемы образа \_ <br/>  | 16 <br/> | приложение Windows boot. <br/>                            |
...
| IMAGE \_ DLLCHARACTERISTICS \_ без \_ SEH <br/>                      | 0x0400 <br/> | не использует обработку структурированных исключений (SE). в этом образе невозможно вызвать обработчик SE. <br/> |
...
| IMAGE \_ DLLCHARACTERISTICS \_ Guard \_ CF <br/>                     | 0x4000 <br/> | образ поддерживает управление Flow Guard. <br/>                                                          |
...
каждый каталог данных предоставляет адрес и размер таблицы или строки, которые Windows использует. Эти записи каталога данных загружаются в память, чтобы система могла их использовать во время выполнения. Каталог данных — это 8-байтовое поле, которое имеет следующее объявление:
...
| 152/168 <br/> | 8 <br/> | Architecture <br/>            | Зарезервировано, должно быть равно 0 <br/>                                                                                                                                                      |
...
| 176/192 <br/> | 8 <br/> | Загрузить таблицу конфигурации <br/>       | Адрес и размер таблицы конфигурации загрузки. Дополнительные сведения см. [в разделе Структура конфигурации загрузки (только для образа)](#the-load-configuration-structure-image-only).<br/>       |
...
- [Таблица символов COFF](#coff-symbol-table)
  ...
  - [служба хранилища См](#storage-class)
...
##### <a name="ibm-powerpc-processors"></a>IBM PowerPC процессоры

для PowerPC процессоров определены следующие индикаторы типа перемещения.
...
| IMAGE \_ rel \_ ia64 \_ GPREL32 <br/>    | 0x011B <br/> | Адресная привязка для 32-разрядного элемента GP. <br/>                                                                                                                                                                                                                                                                                                         |
...
| 16 <br/> | 1 <br/> | сторажекласс <br/>       | Перечислимое значение, представляющее класс хранения. дополнительные сведения см. в разделе [класс служба хранилища](#storage-class). <br/>                                                                                                                   |
...
\_Элемент **бцертификате** структуры сертификата Win содержит массив байтов переменной длины с типом содержимого, заданным параметром **вцертификатетипе**. Тип, поддерживаемый Authenticode, — это \_ сертификат Win \_ \_ Data типа PKCS \_ \_ , \# Структура PKCS 7 **сигнеддата** . дополнительные сведения о формате цифровой подписи authenticode см. в разделе [Windows формат переносимого исполняемого файла authenticode](https://download.microsoft.com/download/9/c/5/9c5b2167-8017-4bae-9fde-d599bac8184a/Authenticode_PE.docx).
...
например, в Kernel32.dll в Windows XP экспорт с именем "хеапаллок" перенаправляется в строку "NTDLL. Ртлаллокатехеап ". это позволяет приложениям использовать Ntdll.dll модуль Windows XP без фактического указания на него ссылок на импорт. Таблица импорта приложения относится только к Kernel32.dll. поэтому приложение не предназначено для Windows XP и может выполняться в любой системе Win32.
...
| Биты            | Размер           | Битовое поле                       | Описание                                                                                                                                                               |
|-------------------|----------------|---------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| 72/112 <br/>  | 4/8 <br/> | гуардкфчеккфунктионпоинтер <br/>    | указатель на функцию проверки Flow Guard, где хранится элемент управления. <br/>                                                                                                                                                                                                               |
| 76/120 <br/>  | 4/8 <br/> | гуардкфдиспатчфунктионпоинтер <br/> | объект ва, где хранится указатель функции управления Flow Guard. <br/>                                                                                                                                                                                                            |
| 80/128 <br/>  | 4/8 <br/> | гуардкффунктионтабле <br/>           | порядок сортировки отсортированной таблицы по rva каждого элемента управления Flow функции Guard в изображении. <br/>                                                                                                                                                                                            |
| 84/136 <br/>  | 4/8 <br/> | гуардкффунктионкаунт <br/>           | Число уникальных RVA в таблице выше. <br/>                                                                                                                                                                                                                                    |
| 88/144 <br/>  | 4 <br/>   | гуардфлагс <br/>                     | управление флагами, связанными с Flow Guard. <br/>                                                                                                                                                                                                                                               |
| 92/148 <br/>  | 12 <br/>  | CodeIntegrity <br/>                  | Сведения о целостности кода. <br/>                                                                                                                                                                                                                                                     |
| 104/160 <br/> | 4/8 <br/> | гуардаддресстакениатентритабле <br/> | адрес ва, где элемент управления Flow охранник, потратил таблицу IAT. <br/>                                                                                                                                                                                                              |
| 108/168 <br/> | 4/8 <br/> | гуардаддресстакениатентрикаунт <br/> | Число уникальных RVA в таблице выше. <br/>                                                                                                                                                                                                                                    |
| 112/176 <br/> | 4/8 <br/> | гуардлонгжумптаржеттабле <br/>       | адрес, где хранится целевая таблица перехода Flow Guard в долгосрочной перспективе. <br/>                                                                                                                                                                                                               |
...
-   маска для вложенного поля, содержащего шаг элементов управления Flow таблицы функции Guard (то есть дополнительное количество байтов на запись таблицы).

    ` #define IMAGE_GUARD_CF_FUNCTION_TABLE_SIZE_MASK  0xF0000000`

кроме того, заголовок Windows SDK winnt. h определяет этот макрос на количество битов, сдвигев значение гуардфлагс вправо, чтобы выровнять по правому краю элемент управления с таблицей function Flow Guard с шагом:
...
| 28 <br/> | 6 <br/>  | User ID <br/>       | Десятичное представление идентификатора пользователя в формате ASCII. это поле не содержит осмысленного значения на Windows платформах, так как средства майкрософт выдают все пробелы. <br/>                                    |
| 34 <br/> | 6 <br/>  | Идентификатор группы. <br/>      | Десятичное представление идентификатора группы в формате ASCII. это поле не содержит осмысленного значения на Windows платформах, так как средства майкрософт выдают все пробелы. <br/>                                   |
...
- **поле контрольной суммы файла для полей, относящихся к Windows необязательного заголовка.** Эта контрольная сумма включает в себя весь файл (включая все сертификаты атрибутов в файле). При любой вероятности контрольная сумма будет отличаться от первоначального значения после вставки подписи Authenticode.
...
с помощью средств makecert и signtool, представленных в Windows Platform SDK, можно поэкспериментировать с созданием и проверкой подписей Authenticode. Дополнительные сведения см. в разделе Reference ниже.

## <a name="references"></a>Ссылки

[загружаемые файлы и средства для Windows (включая Windows SDK)](https://developer.microsoft.com/windows/downloads)
...
[Windows Формат переносимых исполняемых сигнатур Authenticode (.docx)](https://download.microsoft.com/download/9/c/5/9c5b2167-8017-4bae-9fde-d599bac8184a/Authenticode_PE.docx)
