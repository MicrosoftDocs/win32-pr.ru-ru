...
Запустите служебную программу [Pktextract.exe](pktextract-exe.md) , предоставляемую в пакете средств разработки программного обеспечения (SDK) для Microsoft Windows, чтобы извлечь маркер открытого ключа из файла сертификата. Для правильной работы Пктекстракт файл сертификата должен находиться в том же каталоге, что и программа. Используйте извлеченное значение токена открытого ключа, чтобы обновить атрибут **PublicKeyToken** элемента **assemblyIdentity** в файле манифеста.
