---
description: этот пример программы работает с клиентской программой, использующей SSPI, с клиентом Windowsных сокетов. Оба образца программ используют заголовочный файл Сспиексампле. h, который можно найти в файле заголовка для примеров клиента и сервера SSPI.
...
ms.locfileid: "118915177"
---
# <a name="using-sspi-with-a-windows-sockets-server"></a>использование SSPI с сервером сокетов Windows

этот пример программы работает с клиентской программой, [использующей SSPI, с клиентом Windowsных сокетов](using-sspi-with-a-windows-sockets-client.md). Оба образца программ используют заголовочный файл Сспиексампле. h, который можно найти в [файле заголовка для примеров клиента и сервера SSPI](header-file-for-sspi-client-and-server.md).

Эта программа включает вызовы функций в Secur32. lib и Ws2 \_ 32. lib, которые должны быть включены в библиотеки ссылок.

Эта программа демонстрирует следующее.

-   установка соединения сокетов Windows с клиентом.
-   Инициализация сеанса.
-   Соединение с клиентом и установление безопасного сеанса связи. Здесь сервер предоставляет поставщик общих служб, который может отвечать на клиент с помощью NTLM или Kerberos.
-   Использует олицетворение клиента и меняет его на Self.
-   Отправляет клиенту зашифрованное сообщение.

В этом образце программы используется ограниченная обработка ошибок.
