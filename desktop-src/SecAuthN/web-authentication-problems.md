...
1.  Так как AuthHost работает в собственном контейнере приложения, чтобы предоставить ему возможность частной сети, необходимо настроить раздел реестра: редактор реестра Windows версии 5.00
    ...
    **HKey \_ \_** \\  \\  \\  \\  \\ **параметры выполнения файла изображения** Microsoft Windows NT CurrentVersion по локальному компьютеру \\ **authhost.exe** \\ **енаблеприватенетворк** = 00000001<dl> <dt>
...
дополнительные сведения см. [в блоге об использовании веб-отладчика Fiddler с приложениями Windows Store](/archive/blogs/fiddler/revisiting-fiddler-and-win8-immersive-applications).
