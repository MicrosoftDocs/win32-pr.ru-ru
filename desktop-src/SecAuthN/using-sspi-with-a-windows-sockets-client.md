...
---
# <a name="using-sspi-with-a-windows-sockets-client"></a><span data-ttu-id="dbd2c-103">Использование SSPI с клиентом сокетов Windows</span><span class="sxs-lookup"><span data-stu-id="dbd2c-103">Using SSPI with a Windows Sockets Client</span></span>

<span data-ttu-id="dbd2c-104">Этот пример программы работает с серверной программой с [помощью SSPI и сервера Windows Sockets](using-sspi-with-a-windows-sockets-server.md).</span><span class="sxs-lookup"><span data-stu-id="dbd2c-104">This sample program works with the server program [Using SSPI with a Windows Sockets Server](using-sspi-with-a-windows-sockets-server.md).</span></span> <span data-ttu-id="dbd2c-105">Примеры программ клиента и сервера предназначены для совместной работы.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-105">The client and server sample programs are designed to work together.</span></span> <span data-ttu-id="dbd2c-106">Обе программы используют заголовочный файл Сспиексампле. h, который можно найти в [файле заголовка для примеров клиента и сервера SSPI](header-file-for-sspi-client-and-server.md).</span><span class="sxs-lookup"><span data-stu-id="dbd2c-106">Both programs use the header file SspiExample.h, which can be found in [Header File for SSPI Client and Server Samples](header-file-for-sspi-client-and-server.md).</span></span> <span data-ttu-id="dbd2c-107">Эта программа включает вызовы функций в Secur32. lib и Ws2 \_ 32. lib, которые должны быть включены в библиотеки ссылок.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-107">This program includes calls to functions in Secur32.lib and Ws2\_32.lib, which must be included among the link libraries.</span></span>

<span data-ttu-id="dbd2c-108">Эта программа демонстрирует следующее.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-108">This program demonstrates the following:</span></span>

-   <span data-ttu-id="dbd2c-109">Установка подключения Windows Sockets к серверу.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-109">Establishing a Windows Sockets connection with a server.</span></span>
-   <span data-ttu-id="dbd2c-110">Инициализация сеанса SSPI с проверкой подлинности с помощью поставщика общих служб Negotiate.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-110">Initializing an authenticated SSPI session with the Negotiate SSP.</span></span>
-   <span data-ttu-id="dbd2c-111">Соединение с сервером и установление безопасного сеанса связи.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-111">Connecting with a server and establishing a secure communication session.</span></span>
-   <span data-ttu-id="dbd2c-112">Получение и расшифровка сообщения с сервера в безопасном сеансе.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-112">Receiving and decrypt a message from the server within the secure session.</span></span>

<span data-ttu-id="dbd2c-113">В этом образце программы используется ограниченная обработка ошибок.</span><span class="sxs-lookup"><span data-stu-id="dbd2c-113">This sample program uses limited error handling.</span></span>
