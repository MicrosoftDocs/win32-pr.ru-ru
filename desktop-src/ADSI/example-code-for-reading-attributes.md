---
title: Пример кода для чтения атрибутов
description: В следующем примере кода выполняется перечисление свойств указанного пользователя в текущем домене путем поиска пользователя и последующего использования Иадспропертилист для перечисления его свойств.
...
---
# <a name="example-code-for-reading-attributes"></a><span data-ttu-id="7485e-104">Пример кода для чтения атрибутов</span><span class="sxs-lookup"><span data-stu-id="7485e-104">Example Code for Reading Attributes</span></span>

<span data-ttu-id="7485e-105">В следующем примере кода выполняется перечисление свойств указанного пользователя в текущем домене путем поиска пользователя и последующего использования [**иадспропертилист**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) для перечисления его свойств.</span><span class="sxs-lookup"><span data-stu-id="7485e-105">The following code example enumerates the properties of the specified user in the current domain, by searching for the user and then using [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) to enumerate its properties.</span></span> <span data-ttu-id="7485e-106">Учтите, что значения времени и даты, как и большие целые числа, обрабатываются и как обрабатываются строки октета для **objectSid** и **objectGUID** .</span><span class="sxs-lookup"><span data-stu-id="7485e-106">Be aware that time and date values, as large integers, are handled and how octet strings for **objectSID** and **objectGUID** are handled.</span></span>
...
<span data-ttu-id="7485e-107">В следующем Visual Basic примере кода показано, как получить свойства объекта пользователя.</span><span class="sxs-lookup"><span data-stu-id="7485e-107">The following Visual Basic code example shows how to get the properties of a user object.</span></span> <span data-ttu-id="7485e-108">Чтобы использовать этот пример кода, создайте ссылку на библиотеку типов Active Directory и библиотеку Microsoft объекты данных ActiveX в проекте Visual Basic.</span><span class="sxs-lookup"><span data-stu-id="7485e-108">To use this code example, create a reference to the Active DS Type Library and the Microsoft ActiveX Data Objects Library in your Visual Basic project.</span></span>
