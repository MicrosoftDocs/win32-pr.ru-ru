...
Может возникнуть вопрос, что произойдет, если Терри изменит свое имя, перейдет в другую организацию или покидает компанию. Кто поддерживает эту ссылку "руководитель — прямой отчет"? Дополнительные сведения и решение этой проблемы см. в разделе [reorganization](reorganization.md). Поскольку схема Active Directory является расширяемой, вы можете моделировать объекты для включения аналогичных связей между руководителями и подчиненными стилями.
