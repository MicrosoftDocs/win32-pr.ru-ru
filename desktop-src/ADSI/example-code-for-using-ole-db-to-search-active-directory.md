...
В следующем примере кода показано, как выполнять поиск Active Directory с помощью C++, COM и OLE DB. Это пример функции, которая принимает различающееся имя контейнера для поиска, а также учетные данные, используемые для поиска. В примере выполняется поиск по поддереву в указанном контейнере для всех объектов, имеющих **объект класса** "User". В этом примере атрибуты **Name** и **ADsPath** каждого пользователя выводятся в окне консоли.

Существует два диалекта запроса, которые можно использовать с поставщиком OLE DB ADSI, LDAP и SQL. Диалект задается в методе [ICommandText:: SetCommandText](/previous-versions/windows/desktop/ms709757(v=vs.85)) с одним из следующих идентификаторов GUID:

-   **DBGUID \_ SQL** для диалекта SQL.
-   **DBGUID \_ Лдапдиалект** для диалекта LDAP.
