...
description: Windows Удаленное управление 2,0 (WinRM) представляет собой платформу размещения. Для использования платформы записываются подключаемые модули WinRM, которые предоставляют данные управления приложением в инфраструктуре WinRM.
...
ms.locfileid: "119795274"
---
# <a name="infrastructure-for-managing-hosted-services"></a>Инфраструктура для управления размещенными службами

Windows Удаленное управление 2,0 (WinRM) представляет собой платформу размещения. Для использования платформы записываются подключаемые модули WinRM, которые предоставляют данные управления приложением в инфраструктуре WinRM. Поддерживаются две модели размещения. одна из них — службы IIS (IIS), а другая — на основе службы. Модель на основе WinRM использует HTTP.sys и не зависит от служб IIS.
