...
<tr class="odd">
<td>Версия 2,0</td>
<td>Большая часть API была переработана. Большая часть функциональных возможностей версии 1,0 по-прежнему доступна в версии 2,0. При написании приложений для создания образов или разработке новых устройств и форматов рекомендуется использовать версию 2,0 вместо версии 1,0.<br/> IMAPI 2,0 входит в Windows Vista. Чтобы включить те же функциональные возможности для Windows XP и Windows Server 2003, необходимо установить пакет обновления <a href="https://support.microsoft.com/kb/932716">KB932716</a> .<br/> Примечания Point-Release:<br/>
<ul>
<li>В Windows Vista с пакетом обновления 1 (SP1) и Windows Server 2008 появилась поддержка многократной загрузки с поддержкой нескольких загрузок через интерфейс <a href="/windows/desktop/api/imapi2fs/nn-imapi2fs-ifilesystemimage2"><strong>IFileSystemImage2</strong></a> .<br/></li>
<li>В <a href="https://www.microsoft.com/downloads/details.aspx?FamilyID=63ab51ea-99c9-45c0-980a-c556746fcf05">пакет дополнительных компонентов Windows для хранения</a>включена поддержка поддержки основных компонентов для формата БД-р\бд-ре, создания образа необработанного компакт-диска с CD, а также проверки записи. Это обновление компонента поддерживается в Windows Vista с пакетом обновления 1 (SP1), Windows XP с пакетом обновления 2 (SP2) и более поздних версий и Windows Server 2003 с пакетом обновления 1 (SP1) и более поздних версий. Кроме того, эти функции включены в Windows 7 и Windows Server 2008.<br/></li>
<li>Функции IMAPI 2,0, встроенные в Windows 7 и Windows Server 2008, включают в себя запись неперекрывающееся для звуковых компакт-дисков и исключение "двойного скрытия" во время операций записи. Двойная разскрытая процедура — это процесс в операции с большим объемом записи, в котором каждый файл скрыт перед записью на диск. В последней версии IMAPI 2,0 файлы выборочно выбранной для скрытия, что оставит оставшиеся файлы (в основном большие файлы) без скрытой. Конечный результат сохраняет место на диске и время операции.<br/></li>
</ul></td>
</tr>
<tr class="even">
<td>Версия 1.0</td>
<td>Начальный выпуск. Позволяет этапу приложения и записывать простой образ аудио или данных на устройства CD-R и CD-RW. API поддерживает формат Жолиет и ISO 9660 для дисков аудио и данных Редбук. Дополнительные сведения о версии 1,0 см. в разделе <a href="imapiv1.md">Поддержка IMAPIv1</a>. Входит в Windows XP.<br/></td>
</tr>
</tbody>
</table>
...
-   В состав Windows Vista с пакетом обновления 1 (SP1) и Windows Server 2008 IMAPI версия требуемая 2,0 предлагает поддержку многозагрузочного интерфейса через интерфейс [**IFileSystemImage2**](/windows/desktop/api/imapi2fs/nn-imapi2fs-ifilesystemimage2) .
...
-   С [пакетом дополнительных компонентов Windows для хранилища](https://www.microsoft.com/downloads/details.aspx?FamilyID=63ab51ea-99c9-45c0-980a-c556746fcf05)доступны параметры проверки записи, доступные для свойства, доступного через [**ибурнверификатион**](/windows/desktop/api/imapi2/nn-imapi2-iburnverification).
