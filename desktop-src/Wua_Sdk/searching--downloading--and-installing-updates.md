---
description: В примере сценариев в этом разделе показано, как использовать агент Центр обновления Windows (WUA) для проверки, загрузки и установки обновлений.
ms.assetid: 4b2b1898-64f1-4908-98b7-ea87a6fcb71d
title: Поиск, скачивание и установка обновлений
ms.topic: article
ms.date: 01/16/2020
...
ms.locfileid: "105694364"
---
# <a name="searching-downloading-and-installing-updates"></a><span data-ttu-id="a6b2d-103">Поиск, скачивание и установка обновлений</span><span class="sxs-lookup"><span data-stu-id="a6b2d-103">Searching, Downloading, and Installing Updates</span></span>

<span data-ttu-id="a6b2d-104">В примере сценариев в этом разделе показано, как использовать агент Центр обновления Windows (WUA) для проверки, загрузки и установки обновлений.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-104">The scripting sample in this topic shows you how to use Windows Update Agent (WUA) to scan, download, and install updates.</span></span>

<span data-ttu-id="a6b2d-105">В примере выполняется поиск всех применимых обновлений программного обеспечения, а затем список этих обновлений.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-105">The sample searches for all the applicable software updates and then lists those updates.</span></span> <span data-ttu-id="a6b2d-106">Затем он создает коллекцию обновлений для загрузки, а затем загружает их.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-106">Next, it creates a collection of updates to download and then downloads them.</span></span> <span data-ttu-id="a6b2d-107">Наконец, он создает коллекцию обновлений для установки, а затем устанавливает их.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-107">Finally, it creates a collection of updates to install and then installs them.</span></span>

<span data-ttu-id="a6b2d-108">Если вы хотите найти, скачать и установить определенное обновление, которое вы определяете с помощью заголовка обновления, см. [статью Поиск, скачивание и установка конкретных обновлений](searching--downloading--and-installing-specific-updates.md).</span><span class="sxs-lookup"><span data-stu-id="a6b2d-108">If you want to search, download, and install a specific update that you identify by using the update title, see [Searching, Downloading, and Installing Specific Updates](searching--downloading--and-installing-specific-updates.md).</span></span>

<span data-ttu-id="a6b2d-109">Прежде чем пытаться выполнить этот пример, обратите внимание на следующее:</span><span class="sxs-lookup"><span data-stu-id="a6b2d-109">Before you attempt to run this sample, note the following:</span></span>

-   <span data-ttu-id="a6b2d-110">На компьютере должен быть установлен WUA.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-110">WUA must be installed on the computer.</span></span> <span data-ttu-id="a6b2d-111">Дополнительные сведения о том, как определить версию установленного агента WUA, см. [в разделе Определение текущей версии WUA](determining-the-current-version-of-wua.md).</span><span class="sxs-lookup"><span data-stu-id="a6b2d-111">For more information about how to determine the version of WUA that is installed, see [Determining the Current Version of WUA](determining-the-current-version-of-wua.md).</span></span>
-   <span data-ttu-id="a6b2d-112">Этот пример может скачивать обновления только с помощью WUA.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-112">The sample can download updates only by using WUA.</span></span> <span data-ttu-id="a6b2d-113">Он не может скачивать обновления с сервера SUS 1,0.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-113">It cannot download updates from a Software Update Services (SUS) 1.0 server.</span></span>
-   <span data-ttu-id="a6b2d-114">Для работы с этим примером требуется сервер сценариев Windows (WSH).</span><span class="sxs-lookup"><span data-stu-id="a6b2d-114">Running this sample requires Windows Script Host (WSH).</span></span> <span data-ttu-id="a6b2d-115">Дополнительные сведения о WSH см. в разделе «WSH» пакета средств разработки программного обеспечения (SDK) платформы.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-115">For more information about WSH, see the WSH section of the Platform Software Development Kit (SDK).</span></span> <span data-ttu-id="a6b2d-116">Если пример копируется в файл с именем WUA \_SearchDownloadInstall.vbs, можно запустить пример, открыв окно командной строки и введя в командной строке следующую команду.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-116">If the sample is copied to a file named WUA\_SearchDownloadInstall.vbs, you can run the sample by opening a Command Prompt window and typing the following command at the command prompt.</span></span>

    <span data-ttu-id="a6b2d-117">**cscript WUA \_SearchDownloadInstall.vbs**</span><span class="sxs-lookup"><span data-stu-id="a6b2d-117">**cscript WUA\_SearchDownloadInstall.vbs**</span></span>

## <a name="example"></a><span data-ttu-id="a6b2d-118">Пример</span><span class="sxs-lookup"><span data-stu-id="a6b2d-118">Example</span></span>

> [!IMPORTANT]
> <span data-ttu-id="a6b2d-119">Этот сценарий предназначен для демонстрации использования API-интерфейсов агента Центр обновления Windows и предоставления примера того, как разработчики могут использовать эти API для решения проблем.</span><span class="sxs-lookup"><span data-stu-id="a6b2d-119">This script is intended to demonstrate the use of the Windows Update Agent APIs, and provide an example of how developers can use these APIs to solve problems.</span></span> <span data-ttu-id="a6b2d-120">Этот сценарий не предназначен для рабочего кода, и сам сценарий не поддерживается корпорацией Майкрософт (хотя поддерживаются базовые API-интерфейсы Центр обновления Windows агента).</span><span class="sxs-lookup"><span data-stu-id="a6b2d-120">This script is not intended as production code, and the script itself is not supported by Microsoft (though the underlying Windows Update Agent APIs are supported).</span></span>
