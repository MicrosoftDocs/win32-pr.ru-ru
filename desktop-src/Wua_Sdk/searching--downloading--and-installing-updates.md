...
ms.locfileid: "119994245"
---
# <a name="searching-downloading-and-installing-updates"></a>Поиск, скачивание и установка обновлений

в примере сценариев в этом разделе показано, как использовать агент Центр обновления Windows (WUA) для проверки, загрузки и установки обновлений.

В примере выполняется поиск всех применимых обновлений программного обеспечения, а затем список этих обновлений. Затем он создает коллекцию обновлений для загрузки, а затем загружает их. Наконец, он создает коллекцию обновлений для установки, а затем устанавливает их.

Если вы хотите найти, скачать и установить определенное обновление, которое вы определяете с помощью заголовка обновления, см. [статью Поиск, скачивание и установка конкретных обновлений](searching--downloading--and-installing-specific-updates.md).
...
-   Этот пример может скачивать обновления только с помощью WUA. Он не может скачивать обновления с сервера SUS 1,0.
-   для выполнения этого примера требуется Windows сервер сценариев (WSH). Дополнительные сведения о WSH см. в разделе «WSH» пакета средств разработки программного обеспечения (SDK) платформы. Если пример копируется в файл с именем WUA \_SearchDownloadInstall.vbs, можно запустить пример, открыв окно командной строки и введя в командной строке следующую команду.
