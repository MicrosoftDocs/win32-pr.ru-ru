...
Реализация **допроцессаутпут** , предоставляемая в примере кода мастера подключаемых модулей, создает цикл while, который выполняет итерацию один раз для каждого образца во входном буфере, предоставленном проигрывателем Windows Media. Этот цикл работает одинаково как для 8-разрядных, так и для 16-разрядных аудио, хотя для каждого из них требуется отдельный цикл. В каждом случае цикл инициируется следующим тестом:
