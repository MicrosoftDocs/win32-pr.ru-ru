...
ms.locfileid: "118995534"
---
# <a name="retrieving-device-attributes"></a>Получение атрибутов устройства

проигрыватель Windows Media хранит сведения об устройствах, подключенных к проигрывателю. Некоторые атрибуты доступны путем вызова отдельных методов **ивмпсинкдевице**. Некоторые из них можно извлечь с помощью **Ивмпсинкдевице:: getItemInfo**, а некоторые — с помощью любой из этих методик.

В следующем примере кода элемент управления "список" заполняется доступными атрибутами для указанного устройства. Первая часть функции извлекает свойства, доступные с помощью конкретных методов. Вторая часть функции извлекает значения атрибутов с помощью **ивмпсинкдевице:: getItemInfo**. Параметр функции *Линдекс*— это индекс устройства в пользовательском массиве устройств, на который указывает m \_ ппвмпдевицес.
...
[**Ивмпсинкдевице:: getItemInfo**](/previous-versions/windows/desktop/api/wmp/nf-wmp-iwmpsyncdevice-getiteminfo)
</dt> <dt>
