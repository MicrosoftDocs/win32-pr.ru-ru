...
В следующем примере кода используется **сетпроксинаме** для указания имени прокси-сервера проигрывателя Windows Media для протокола MMS. При нажатии кнопки новое имя извлекается из текстового поля. Объект **аксвмплиб. аксвиндовсмедиаплайер** представлен переменной с именем Player.
