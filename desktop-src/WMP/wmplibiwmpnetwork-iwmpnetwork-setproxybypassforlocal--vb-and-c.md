...
В следующем примере кода используется **сетпроксибипассфорлокал** , чтобы указать, пропускается ли прокси-сервер проигрывателя Windows Media при использовании протокола MMS, если сервер-источник находится в локальной сети. Объект **аксвмплиб. аксвиндовсмедиаплайер** представлен переменной с именем Player.
