...
-   Для RGB 32 каждый пиксель является **ргбкуад**. Каждый цвет имеет один байт со значением от 0 до 255 включительно. Макет памяти: 
    ...
    Если подтипом является МЕДИАСУБТИПЕ \_ ARGB32, то Byte 3 содержит значение для альфа-канала. Если подтип имеет значение МЕДИАСУБТИПЕ \_ RGB32, то байт 3 следует игнорировать.

-   A2R10G10B10 использует следующий макет: 

    |       | Layout     | Layout      | Layout     | Layout |
    |-------|-------|---------|---------|---------|
    | **Версий**   | 0–9 | 10–19 | 20 - 29 | 30 - 31 |
