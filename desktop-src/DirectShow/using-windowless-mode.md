...
-   Диспетчер графов фильтров должен пересылать определенные окна сообщений, например WM \_ Paint, в модуль подготовки видео. Приложение должно использовать реализацию [**ивидеовиндов**](/windows/desktop/api/Control/nn-control-ivideowindow) для диспетчера графа фильтров (а не для модуля подготовки видео), чтобы диспетчер графа фильтров сохранит правильное внутреннее состояние.
...
1.  Создайте диспетчер графов фильтров.
...
Теперь создайте остальную часть графа фильтра, вызвав [**играфбуилдер:: renderFile**](/windows/desktop/api/Strmif/nf-strmif-igraphbuilder-renderfile) или другие методы построения графа. Диспетчер графов фильтров автоматически использует экземпляр VMR, добавленный в граф. (Дополнительные сведения о том, почему это происходит, см. в разделе [Intelligent Connect](intelligent-connect.md).)
