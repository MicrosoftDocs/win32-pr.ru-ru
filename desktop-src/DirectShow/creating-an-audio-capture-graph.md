---
description: Создание Graph записи звука
ms.assetid: 2302bb40-a5db-473a-afeb-71905ac41f47
title: Создание Graph записи звука
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "120108224"
---
# <a name="creating-an-audio-capture-graph"></a>Создание Graph записи звука
...
-   Windows Файл мультимедиа Audio (WMA): фильтр записи звука для фильтра [модуля записи WM ASF](wm-asf-writer-filter.md) .
...
граф фильтра можно построить с помощью объекта [Capture Graph Builder](capture-graph-builder.md) или построить граф вручную. Это значит, что приложение программным образом добавляет и подключает каждый фильтр. В этой статье описывается ручной подход. дополнительные сведения об использовании построителя Graph capture см. в разделе [capture видео](video-capture.md). Большая часть информации в этой статье относится к графикам только для звука.
...
в этом примере используется `AddFilterByCLSID` функция, описанная в разделе [добавление фильтра по CLSID](add-a-filter-by-clsid.md), и `ConnectFilters` функция, описанная в [Подключение два фильтра](connect-two-filters.md). ни один из них не является DirectShow API.
