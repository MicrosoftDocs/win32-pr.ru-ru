...
Чтобы получить изображения с устройства по-прежнему, рекомендуется использовать интерфейсы API для получения образа Windows (WIA). Дополнительные сведения см. в разделе "получение образа Windows" в документации по Platform SDK. Однако для записи образа можно также использовать DirectShow.
