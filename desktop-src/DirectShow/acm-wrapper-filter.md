...
В качестве фильтра распаковки оболочка ACM отображается в категории «фильтры DirectShow» (CLSID \_ легациамфилтеркатегори) и имеет нештатную подмножество \_ . Тип носителя подключения для входного ПИН-кода определяет, какой кодек использует фильтр. Как правило, приложению не нужно добавлять фильтр к графу фильтра; При необходимости он автоматически извлекается диспетчером графа фильтров. Распаковка применяется только к звуку PCM.
