...
В этом разделе описывается, как реализовать поиск в фильтре источника Microsoft DirectShow. В качестве отправной точки в нем используется пример [фильтра шарика](ball-filter-sample.md) и описывается дополнительный код, необходимый для поддержки поиска в этом фильтре.
...
Метод называется «неделегированным» из-за способа, которым базовые классы DirectShow поддерживают агрегирование объектной модели (COM). Дополнительные сведения см. в разделе "реализация IUnknown" в пакете SDK для DirectShow.
