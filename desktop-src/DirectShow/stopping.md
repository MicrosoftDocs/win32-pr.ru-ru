...
Метод " **останавливаться** " должен разблокировать метод **Receive** и отменить фиксацию распределителя фильтра. При отсчете распределителя происходит принудительное возвращение всех ожидающих **вызовов метода** noreturn, что разблокирует вышестоящее фильтры, ожидающие выборки. Метод « **Завершение** » удерживает блокировку фильтра, а затем вызывает метод [**Кбасефилтер:: останавливаться**](cbasefilter-stop.md) , который вызывает [**кбасепин:: Inactive**](cbasepin-inactive.md) для всех ПИН-кодов фильтра:
...
Переопределите **Неактивный** метод входного контакта следующим образом:
