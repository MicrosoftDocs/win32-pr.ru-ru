...
---
# <a name="building-the-recompression-graph"></a>Построение графа повторного сжатия
...
Самый простой способ создать диаграмму фильтра — использовать [Построитель диаграмм записи](capture-graph-builder.md), который является компонентом DirectShow для создания графов записи и других настраиваемых графов фильтра.

Начните с вызова CoCreateInstance для создания построителя графа записи:
...
Затем с помощью построителя графа записи создайте граф фильтра:
...
3.  Подключите фильтр источника к фильтру МУЛЬТИПЛЕКСОРа. Построитель диаграмм записи вставляет любые фильтры разделителя и декодера, необходимые для анализа исходного файла. Он также может маршрутизировать видео-и звуковые потоки с помощью фильтров сжатия.
...
Подключение источника к мультиплексору
