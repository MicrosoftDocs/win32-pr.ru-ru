---
description: В Windows 2000 обычно программа установки проверяет текущий и максимальный размер реестра, чтобы определить, достаточно ли свободного места для новых данных, которые он будет добавлять.
...
ms.locfileid: "103913840"
---
# <a name="determining-the-registry-size"></a><span data-ttu-id="8419a-103">Определение размера реестра</span><span class="sxs-lookup"><span data-stu-id="8419a-103">Determining the Registry Size</span></span>

<span data-ttu-id="8419a-104">В Windows 2000 обычно программа установки проверяет текущий и максимальный размер реестра, чтобы определить, достаточно ли свободного места для новых данных, которые он будет добавлять.</span><span class="sxs-lookup"><span data-stu-id="8419a-104">On Windows 2000, it is common for an installation utility to check the current and maximum size of the registry to determine whether there is enough available space for the new data it will be adding.</span></span> <span data-ttu-id="8419a-105">В этом примере показано, как сделать это программно с помощью счетчика производительности "% использования квоты реестра" в системном объекте.</span><span class="sxs-lookup"><span data-stu-id="8419a-105">This sample demonstrates how to do this programmatically using the "% Registry Quota In Use" performance counter within the System object.</span></span>

<span data-ttu-id="8419a-106">В следующем примере для получения значения счетчика используется модуль поддержки данных производительности (PDH). Он должен быть связан с PDH. lib.</span><span class="sxs-lookup"><span data-stu-id="8419a-106">The following sample uses performance data helper (PDH) to obtain the counter value; it must be linked with Pdh.lib.</span></span> <span data-ttu-id="8419a-107">PDH — это высокоуровневый набор интерфейсов API, используемых для получения данных о производительности.</span><span class="sxs-lookup"><span data-stu-id="8419a-107">PDH is a high-level set of APIs used to obtain performance data.</span></span>

> [!Note]  
> <span data-ttu-id="8419a-108">Нет необходимости реализовывать этот размер реестра — проверка на Windows Server 2003 или Windows XP, так как у них нет квоты реестра.</span><span class="sxs-lookup"><span data-stu-id="8419a-108">It is not necessary to implement this registry size-check on Windows Server 2003 or Windows XP because they do not have a registry quota limit.</span></span>
