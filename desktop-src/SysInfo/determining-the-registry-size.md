...
В Windows 2000 обычно программа установки проверяет текущий и максимальный размер реестра, чтобы определить, достаточно ли свободного места для новых данных, которые он будет добавлять. В этом примере показано, как сделать это программно с помощью счетчика производительности "% использования квоты реестра" в системном объекте.

В следующем примере для получения значения счетчика используется модуль поддержки данных производительности (PDH). Он должен быть связан с PDH. lib. PDH — это высокоуровневый набор интерфейсов API, используемых для получения данных о производительности.

> [!Note]  
> Нет необходимости реализовывать этот размер реестра — проверка на Windows Server 2003 или Windows XP, так как у них нет квоты реестра.
