...
-   **Лонгарр \_ из \_ локальной** подпрограммы подсчитывает узлы списка, выделяет объект лонгарр с размером **sizeof**(**лонгарр**) + Count \* **sizeof**(**Long**), устанавливает в поле **size** значение Count и копирует данные в поле **датаарр** .
