...
---
# <a name="itsdpget_description-method"></a>Метод Итсдп:: Get \_ Description
...
Метод **Get \_ Description** Возвращает описание сеанса (**BSTR**). Это должна быть строка, преобразуемая в строку ASCII, если кодировка — ASCII. (В противном случае это может быть любая строка **BSTR** .)
...
| <dl> <dt>**\_указатель E**</dt> </dl>     | Параметр *ппдескриптион* не является допустимым указателем.<br/> |
...
Приложение должно использовать [**сисфристринг**](/windows/win32/api/oleauto/nf-oleauto-sysfreestring) для высвобождения параметра *ппдескриптион* .
...
[**Итсдп: описание:p UT \_**](itsdp-put-description.md)
</dt> </dl>
