...
- **Добавление меток.** Всем командам присваиваются понятные метки, а исключения — только в том случае, если значки очень хорошо известны и место на уровне Premium.
...
- **Чтобы закрыть модальную вкладку, переведите &lt; &gt; на вкладку команду режим закрытия, которая была последней командой.** Используйте значок Закрыть, чтобы упростить поиск команды. Присвойте режиму в команде, чтобы избежать путаницы в том, что именно закрывается.
...
- **Developer.** Содержит команды, используемые только разработчиками. Если он используется, по умолчанию он скрыт и последняя обычная вкладка при отображении.
