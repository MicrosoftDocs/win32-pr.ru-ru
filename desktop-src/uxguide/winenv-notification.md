...
-   **Выйдите из программы.** Укажите более подходящие варианты:
    -   **Временное завершение работы программы.** при перезапуске Windows программа останавливается и перезапускается. Этот подход подходит для важных системных служебных программ, таких как программы безопасности.
...
4.  воспользуйтесь всеми преимуществами кнопок панели задач Windows 7, чтобы отобразить состояние и обеспечить эффективное выполнение наиболее часто выполняемых задач программы.
...
| | | <strong>Состояние системы и доступ</strong><br /> Отображается непрерывно, чтобы показать важное, но не критическое состояние системы и предоставить доступ к соответствующим функциям и параметрам. <br /> | Системные компоненты, для которых требуются значки области уведомлений, не имеют постоянного присутствия рабочего стола. Также может использоваться в качестве источника уведомления. <br /><img src="images/winenv-notification-image8.png" alt="Screenshot that shows a notification area and icons for system status." /><br /> В этом примере значки батареи, сети и тома отображаются постоянно, если это возможно.<br /> | | <strong>Состояние фоновой задачи и доступ к нему</strong><br /> Отображается во время выполнения фоновой задачи, чтобы отобразить состояние и предоставить доступ к функциям и параметрам. <br /> | Фоновым процессам требуются значки области уведомлений, если у них нет присутствия рабочего стола. Также может использоваться в качестве источника уведомления. <br /><img src="images/winenv-notification-image9.png" alt="Screenshot that shows notification area and icon for background task status." /><br /> В этом примере значок центра уведомлений позволяет пользователям проверить свое состояние, даже если у него нет присутствия рабочего стола.<br /> | | <strong>Состояние временного события</strong><br /> Программы с присутствием рабочего стола могут временно отображать значки для отображения важных событий или изменений в состоянии. <br /> | <img src="images/winenv-notification-image10.png" alt="Screenshot that shows notification area and icons for a temporary event status." /><br /> В этом примере значки для печати и установки обновлений временно отображаются для отображения важных событий или изменений в состоянии.<br /> | | <strong>Временный источник уведомления</strong><br /> Временно отображается для отображения уведомления. Удалено после истечения времени ожидания или при устранении базовой проблемы или выполнении задачи. <br /> | Для чистых источников уведомлений предпочтительно использовать временные значки. Не отображать значок, который не предоставляет полезное, соответствующее динамическое состояние, только потому что в будущем может потребоваться отображение уведомления. <br /><img src="images/winenv-notification-image11.png" alt="Screen shot of notification area install message " /><br /> В этом примере значок Plug-and-Play отображается, когда отображается новое уведомление об обнаружении оборудования.<br /> | | <strong>Несведенное приложение с одним экземпляром</strong><br /> Чтобы уменьшить помехи в панелях задач, можно свернуть приложение с одним экземпляром, длительно выполняющееся, в значок области уведомлений. <br /> | <img src="images/winenv-notification-image12.png" alt="Screen shot of notification area and icons " /><br /> в этом примере из Windows Vista Outlook и Windows Live Messenger являются приложениями с одним экземпляром, которые сворачиваются в значки области уведомлений.<br /> Этот шаблон рекомендуется использовать только в том случае, если применяются все следующие условия. <br /><ul><li>Приложение может иметь только один экземпляр.</li><li>Приложение запускается в течение продолжительного периода времени.</li><li>Значок показывает состояние.</li><li>Значок может быть источником уведомления.</li><li>Это необязательно, и пользователи должны <a href="glossary.md">принять участие</a>.</li></ul>Если все эти условия применяются, то при свертывании значка в значок не удаляются две точки доступа, когда требуется только один. <br /><strong>Примечание.</strong> этот шаблон значка больше не рекомендуется для Windows 7. Вместо этого используйте обычные кнопки панели задач, если в программе есть присутствие рабочего стола.<br /><img src="images/winenv-notification-image13.png" alt="Screen shot of Outlook and Messenger taskbar icons " /><br /> в этом примере из Windows 7 обычная кнопка на панели задач занимает немного места, но преимущества кнопок панели задач Windows 7, включая списки переходов, значки наложения и насыщенные эскизы.<br /> | 
