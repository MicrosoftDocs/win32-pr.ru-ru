...
Если насыщенность равна 0 в HSL и HSV пространствах, то при задании значения параметра значение "яркость" задается затенение серого цвета. в Windows пространства HSL и HSV обычно пересопоставляются с масштабом от 0 до 240, чтобы цвета можно было представить с помощью 32-разрядного значения.
...
-   **С.** Состояние объекта может быть указано цветом. например, Windows использует цвет для обозначения состояния выбора и наведения указателя. Ссылки внутри веб-страниц используют синий цвет для непосещенных и пурпурных ссылок.
...
-   **Уделяет.** Цвет можно использовать для привлечения внимания пользователей. например, Windows использует синие [основные инструкции](text-ui.md) , чтобы они могли выделяться из другого текста.
...
-   они отключены ("Windows классический стиль" в теме Параметры в элементе панели управления персонализации).
...
-   **Выберите темы и системные цвета в зависимости от их назначения.** Не выбирайте цвета в зависимости от их текущего вида, так как их внешний вид может изменить пользователь или будущие версии Windows.
-   **Сопоставление цветов переднего плана со связанными цветами фона.** Цвета переднего плана гарантированно должны быть удобочитаемыми только для соответствующих цветов фона. Не следует смешивать и сопоставлять цвета переднего плана с другими цветами фона или, что еще хуже, с другими цветами переднего плана.
-   **Не смешивать типы цветов.** То есть всегда сопоставлять цвета темы со связанными цветами темы, системными цветами со связанными цветами системы и прикован цветами с другими цветами прикован. Например, цвет текста темы не гарантирует разборчивость на фоне прикован.
...
-   **Обрабатывайте изменения темы.** Изменения в теме автоматически обрабатываются Windows со стандартными фреймами окон и общими элементами управления. Windows с пользовательскими кадрами окон, настраиваемыми или пользовательскими элементами управления, а также с другим использованием цвета должны быть явно обработаны изменения темы.
...
|--------------------------------------|-------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| синий/зеленый<br/>                | торговая марка Windows<br/>                                                      | фон: Windows фирменная символика.<br/>                                                                                                                        |
| стекло, черный, серый, белый<br/> | нейтральная тональность<br/>                                                            | фон: стандартные кадры окон, меню, панель задач, боковая панель.<br/> Передний план: нормальный текст.<br/>                                                |
...
| purple<br/>                    | страниц<br/>                                                            | передний план: посещенные ссылки (для ссылок в Windows Internet Explorer и документов).<br/>                                                                |
...
в неправильном примере сообщение относится к Windows панели информации Internet Explorer по ее цвету, а не по имени.
