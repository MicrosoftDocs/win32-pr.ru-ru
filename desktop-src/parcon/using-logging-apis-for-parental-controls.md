...
поставщик WPC называется Microsoft.com/Windows/паренталконтролс с GUID {01090065-B467-4503-9B28-533766761087}. локальный канал ведения журнала по умолчанию — Microsoft.com/Windows/паренталконтролс/локалевентс.

файлы журнала хранятся в \\ папке Windows System32 \\ Wpc \\ Logs.

### <a name="notification-of-impending-time-limits-logout"></a>Уведомление об ограничениях времени ожидания выхода из системы

Система родительского контроля запустит событие предупреждения через 15 минут и снова через 1 минуту, прежде чем выйти из контролируемого пользователя на основе временных ограничений. приложения могут подписываться на эти события, особенно если они работают в полноэкранном режиме DirectX, где не отображаются стандартные Windows уведомления. Приведен пример кода, который показывает, как подписаться на события, зарегистрировать функцию обратного вызова и получить события.
