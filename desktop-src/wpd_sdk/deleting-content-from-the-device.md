...
ms.locfileid: "119657934"
---
# <a name="deleting-content-from-the-device"></a>Удаление содержимого с устройства

Другой распространенной операцией, выполняемой приложением WPD, является удаление содержимого из расположения на устройстве.

Операции удаления содержимого выполняются с помощью интерфейсов, описанных в следующей таблице.



| Интерфейс                                                                                      | Описание                                      |
|------------------------------------------------------------------------------------------------|--------------------------------------------------|
| [**Интерфейс Ипортабледевицеконтент**](/windows/desktop/api/portabledeviceapi/nn-portabledeviceapi-iportabledevicecontent)                             | Предоставляет доступ к методам удаления содержимого. |
| [**Интерфейс Ипортабледевицепропвариантколлектион**](iportabledevicepropvariantcollection.md) | Предоставляет доступ к методам, относящимся к свойствам.    |



 

Функция Делетеконтентфромдевице в модуле Контенттрансфер. cpp примера приложения демонстрирует, как приложение может удалить содержимое на устройстве. Операции удаления содержимого очень похожи на операции по переносу содержимого. Единственное отличие состоит в том, что во время удаления приложение вызывает [**ипортабледевицеконтент::D удалить**](/windows/desktop/api/PortableDeviceApi/nf-portabledeviceapi-iportabledevicecontent-delete) , а не [**Ипортабледевицеконтент:: Move**](/windows/desktop/api/PortableDeviceApi/nf-portabledeviceapi-iportabledevicecontent-move). (Описание задач, ведущих к вызову метода Delete, см. в разделе [Перемещение содержимого в устройстве](moving-content-on-the-device.md) .)
