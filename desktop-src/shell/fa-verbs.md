...
-   [Потоковая передача нефайловых системных элементов и результатов OpenSearch.](#stream-non-file-system-items-and-opensearch-results)
...
Причина, по которой контекстное меню по умолчанию отображается для **MyFile.XYZ-MS** , связано с тем, что **. XYZ-MS** не является членом зарегистрированного типа файла. В отличие от этого, **txt** является зарегистрированным типом файла. Если щелкнуть правой кнопкой мыши **txt** -файл, в его верхнем разделе появится контекстное меню с тремя дополнительными командами: **Печать**, **Редактирование** и **Открытие с помощью**.
...
Глагол — это простая текстовая строка, которая используется оболочкой для обнаружения связанной команды. Каждая команда соответствует строке команды, используемой для запуска команды в окне консоли или в файле пакета (bat).
