...
Причина, по которой MyDocs4.xyz отображает контекстное меню по умолчанию, заключается в том, что она не является членом зарегистрированного [типа файла](fa-file-types.md). С другой стороны, .txt является зарегистрированным типом файлов. Если щелкнуть правой кнопкой мыши один из файлов .txt, в его верхней части появится контекстное меню с двумя дополнительными командами: **Открыть** и **напечатать**.
