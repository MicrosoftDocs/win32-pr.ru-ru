...
Данные буфера обмена имеют тип [**TYMED_HGLOBAL**](/windows/win32/api/objidl/ne-objidl-tymed). Это представление **HWND** в виде **DWORD** . Данные можно передать в функцию **улонгтохандле** , определенную в басетсд. h, чтобы предоставить 64-разрядный **HWND** для использования в 64-разрядной версии Windows.
