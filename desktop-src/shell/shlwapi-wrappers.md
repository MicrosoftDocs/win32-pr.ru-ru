---
description: в таблицах этого документа перечислены функции-оболочки из Shlwapi.dll, которые предоставляют ограниченную функциональность юникода Windows 95, Windows 98 и Windows Millennium Edition (Windows Me).
...
эта функция поддерживает только версию Microsoft Windows NT 4,0 для структуры [**менуитеминфов**](/windows/win32/api/winuser/ns-winuser-menuiteminfoa) . В этой структуре отсутствует элемент **хбмпитем** . Кроме того, функция не поддерживает \_ флаг битовой карты миим. МСЛУ не имеет этих ограничений.
