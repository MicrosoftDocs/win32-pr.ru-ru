...
-   [Автозапуск для других типов носителей хранилища](#autorun-for-other-types-of-storage-media)
...
В следующих фрагментах кода показано, как настроить и обработано это сообщение. Приложение должно работать в окне переднего плана. Сначала зарегистрируйте "Куериканцелаутоплай" как сообщение Windows:
...
В следующей таблице приводятся константы битов и битовой маски, которые можно задать в первом байте Нодриветипеауторун, чтобы отключить автозапуск для определенного типа диска. Чтобы изменения вступили в силу, необходимо перезапустить проводник Windows.
...
## <a name="autorun-for-other-types-of-storage-media"></a>Автозапуск для других типов носителей хранилища
