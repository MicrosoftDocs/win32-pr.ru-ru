...
существуют некоторые дополнительные факторы, которые применимы к Windowsным системам 2000. дополнительные сведения см. в разделе [регистрация обработчиков расширений оболочки в Windows 2000 Systems](#registering-shell-extension-handlers) .

как и в случае с объектами модели COM, необходимо создать идентификатор GUID для обработчика с помощью такого средства, как Guidgen.exe, которое предоставляется вместе с Windowsным пакетом средств разработки программного обеспечения (SDK). Создайте подраздел в разделе **\_ классы hKey \_ root** \\ **CLSID** , имя которого является строковым форматом этого GUID. Так как обработчики расширений оболочки являются внутрипроцессного серверами, необходимо также создать подраздел **InprocServer32** в этом подразделе GUID со значением (по умолчанию), равным пути к DLL-файлу обработчика. Используйте модель потоков подразделений. Пример показан далее:
...
|---------------------------|----------------------------------------------------------------------|--------------------------------------------------|
| **\***                    | Все файлы                                                            | Контекстное меню, страница свойств, команды (см. ниже) |
| **аллфилесистемобжектс**  | Все файлы и папки с файлами                                           | Контекстное меню, страница свойств, команды             |
...
| **десктопбаккграунд**     | фон рабочего стола (Windows 7 и выше)                            | Контекстное меню, команды                             |
| **Накопитель**                 | Все диски в MyComputer, например "C: \\ "                             | Контекстное меню, страница свойств, команды             |
| **Сеть**               | Вся сеть (в разделе Мое сетевое окружение)                             | Контекстное меню, страница свойств, команды             |
