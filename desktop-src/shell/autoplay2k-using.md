...
если содержимое не соответствует критериям автозапуска, событие затем передается в Windowsное получение изображений (WIA).
...
-   [Примеры сценариев](#sample-scenarios)
    -   [автозапуск для служба хранилища устройств с рисунком мультимедиа](#autoplay-for-storage-devices-with-picture-media)
    -   [автозапуск для устройств воспроизведения музыкальных файлов и служба хранилища устройств с музыкальными носителями](#autoplay-for-music-file-playback-devices-and-storage-devices-containing-music-media)
...
### <a name="autoplay-for-storage-devices-with-picture-media"></a>автозапуск для служба хранилища устройств с рисунком мультимедиа
...
### <a name="autoplay-for-music-file-playback-devices-and-storage-devices-containing-music-media"></a>автозапуск для устройств воспроизведения музыкальных файлов и служба хранилища устройств с музыкальными носителями
...
3.  автозапуск воспроизводит файлы с помощью зарегистрированного обработчика по умолчанию (например, проигрыватель Windows Media.

аналогичным образом, если пользователь вставит носитель, содержащий .mp3 файлы (например, CompactFlash, смартмедиа, память или компакт-диск), который 100 процентов от общего поддерживаемого содержимого на устройстве хранения, событие вставки носителя также приведет к воспроизведению файлов с помощью проигрыватель Windows Media. Пользователь может получить доступ к странице свойств устройства хранения и изменить действие по умолчанию на другое зарегистрированное приложение автозапуска, например винамп или Real Player.
...
|------------|--------------|
| Изображения | <ul><li>Слайд-шоу (по умолчанию)</li><li>Мастер настройки камеры и сканера</li><li>Мастер печати</li><li>Открытие папки</li></ul> | 
| Музыка | <ul><li>проигрыватель Windows Media (по умолчанию)</li><li>Открытие папки</li></ul> | 
| Видео | <ul><li>проигрыватель Windows Media (по умолчанию)</li><li>Открытие папки</li></ul> | 
...
Пользователь также может сохранить выбор в качестве действия по умолчанию для этого среднего, установив флажок **всегда выполнять выбранное действие** . После этого диалоговое окно снова не отображается. однако в Windows XP с пакетом обновления 1 (sp1), если на компьютер добавляется новое приложение, способное работать с определенным типом носителя, диалоговое окно снова становится пользователем, давая ему возможность выбрать новое приложение в качестве действия автозапуска по умолчанию. Приложения также могут устанавливаться по умолчанию при их установке.

Windows В пакете обновления 1 (SP1) также добавляется функция, сохраняющая пользовательское действие автоматического запуска, если не щелкнуть поле **всегда выполнять выбранное действие** . Если пользователь выбирает действие автозапуска для одного экземпляра, при следующем отображении этого диалогового окна для этого типа носителя это действие будет выбрано по умолчанию.
