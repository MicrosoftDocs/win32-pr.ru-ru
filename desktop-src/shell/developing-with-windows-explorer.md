---
description: Windows Обозреватель — это мощное приложение для обзора ресурсов и управления ими.
...
ms.locfileid: "118460192"
---
# <a name="developing-with-windows-explorer"></a>разработка с помощью проводника Windows

Windows Обозреватель — это мощное приложение для обзора ресурсов и управления ими. Windows Доступ к обозревателю можно получить в виде встроенного целого, используя Explorer.exe или интерфейс [**иексплорербровсер**](/windows/desktop/api/shobjidl_core/nn-shobjidl_core-iexplorerbrowser) . Windows Обозреватель (Explorer.exe) может быть порожден как отдельный процесс с помощью [**ShellExecuteEx**](/windows/desktop/api/Shellapi/nf-shellapi-shellexecuteexa) или аналогичной функции.

> [!Note]  
> параметры командной строки для Explorer.exe описаны на сайте поддержки Microsoft Windows в статье [Windows Explorer Command-Line options](https://support.microsoft.com/kb/152457).

 

открытые окна проводника можно обнаружить и программировать с помощью [**ишеллвиндовс**](/windows/desktop/api/Exdisp/nn-exdisp-ishellwindows) (clsid \_ шеллвиндовс), а новые экземпляры Windows explorer можно создать с помощью [**IWebBrowser2**](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa752127(v=vs.85)) (clsid \_ шеллбровсервиндов).

в следующем образце кода показано, как можно использовать модель автоматизации Windows explorer для создания и обнаружения работающих окон проводника.
...
клиентскую область обозревателя Windows можно разместить с помощью интерфейса [иексплорербровсер](/windows/desktop/api/shobjidl_core/nn-shobjidl_core-iexplorerbrowser) . элементы управления "клиент Windows Explorer" и "дерево пространства имен" являются стандартными компонентами Windows Vista и более поздних версий. Разработчики могут повторно использовать интерфейсы как сборки компонентов. Одним из распространенных способов использования этих элементов управления является создание настраиваемых обозревателей, подходящих для проблемного домена.

элементы управления в Windows Explorer относятся к следующим функциональным категориям:
...
Элементы управления навигацией помогают пользователям определить контекст и перемещаться по связанному пространству логического домена, называемому пажеспаце. например, пажеспаце для Windows Explorer является пространством имен оболочки. Пажеспацес состоят из нуля или более страниц.

в следующей таблице перечислены и описаны элементы управления навигацией, доступные в Windows Explorer в Windows Vista и более поздних операционных системах.
...
| Название                            | Отображает имя и контекст текущего обозревателя.                                                                                                                                            |
...
элементы управления командами объявляют функции и функциональные возможности обозревателя Windows пользователям. Эти элементы управления выполняют общие действия или действия, относящиеся к одному выбранному элементу или элементам.
...
| Элемент    | Описание                                                                                                                                                                                                                                        |
|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Предварительный просмотр    | Отображает предварительный просмотр выбранного элемента, например эскиза или активного значка.                                                                                                                                                                       |
...
> [!Caution]  
> хотя некоторые из этих элементов управления имеют имена и (или) функциональные возможности, аналогичные стандартным элементам управления Windows Presentation Foundation (WPF), найденным в системе. Windows. Управляет пространством имен, они являются отдельными классами.
