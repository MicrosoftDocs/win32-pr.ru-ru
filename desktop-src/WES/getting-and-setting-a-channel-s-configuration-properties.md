...
Изначально канал настраивается в манифесте (см. раздел [Определение каналов](defining-channels.md)). Чтобы получить настраиваемые свойства канала, вызовите функцию [**евтопенчаннелконфиг**](/windows/desktop/api/WinEvt/nf-winevt-evtopenchannelconfig) , чтобы получить маркер для канала. Затем вызовите функцию [**евтжетчаннелконфигпроперти**](/windows/desktop/api/WinEvt/nf-winevt-evtgetchannelconfigproperty) , чтобы получить значение настраиваемого свойства канала. Список настраиваемых свойств см. в разделе Перечисление [**\_ \_ \_ \_ идентификаторов свойств конфигурации канала evt**](/windows/desktop/api/WinEvt/ne-winevt-evt_channel_config_property_id) . Свойства строки имени, значения и сообщения канала считаются метаданными и не могут быть получены с помощью функции **евтжетчаннелконфигпроперти** . Дополнительные сведения о получении этих свойств см. в разделе [Получение метаданных поставщика](getting-a-provider-s-metadata-.md).

Во время выполнения можно настроить многие свойства канала. Перечисление [**\_ \_ \_ \_ идентификаторов свойств конфигурации канала evt**](/windows/desktop/api/WinEvt/ne-winevt-evt_channel_config_property_id) определяет те свойства, которые нельзя задать. Чтобы настроить свойства канала, пользователь должен входить в группу "Администраторы" и работать с повышенными привилегиями. Чтобы задать настраиваемые свойства канала, вызовите функцию [**евтопенчаннелконфиг**](/windows/desktop/api/WinEvt/nf-winevt-evtopenchannelconfig) , чтобы получить маркер канала. Затем вызовите функцию [**евтсетчаннелконфигпроперти**](/windows/desktop/api/WinEvt/nf-winevt-evtsetchannelconfigproperty) , чтобы задать значение настраиваемого свойства. После задания настраиваемых свойств вызовите функцию [**евтсавечаннелконфиг**](/windows/desktop/api/WinEvt/nf-winevt-evtsavechannelconfig) , чтобы сохранить и применить изменения.

Примеры, демонстрирующие получение и установку свойств канала, см. в следующих разделах:

-   [Перечисление каналов](#enumerating-channels)
-   [Получение свойств канала](#getting-and-setting-a-channels-configuration-properties)
-   [Настройка свойств канала](#getting-and-setting-a-channels-configuration-properties)

## <a name="enumerating-channels"></a>Перечисление каналов

В следующем примере показано, как перечислить каналы, зарегистрированные на компьютере.
...
## <a name="getting-channel-properties"></a>Получение свойств канала

В следующем примере показано, как получить настраиваемые свойства канала.
...
## <a name="setting-channel-properties"></a>Настройка свойств канала
