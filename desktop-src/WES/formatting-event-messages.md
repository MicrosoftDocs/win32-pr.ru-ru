...
---
# <a name="formatting-event-messages"></a>Форматирование сообщений о событиях

Событие может содержать локализованные строки сообщений, которые можно отформатировать для вывода. Чтобы получить строку сообщения из события, вызовите функцию [**евтформатмессаже**](/windows/desktop/api/WinEvt/nf-winevt-evtformatmessage) . Событие может содержать следующие строки сообщения:

-   Строка сообщения для события.
-   Строка сообщения, описывающая значение уровня, назначенное событию.
-   Строка сообщения, описывающая значение задачи, назначенное событию.
-   Строка сообщения, описывающая значение кода операции, присвоенное событию.
-   Строка сообщения, описывающая значения ключевых слов, назначенных событию.
-   Строка сообщения, описывающая значение канала, назначенное событию.

Можно также использовать [**евтформатмессаже**](/windows/desktop/api/WinEvt/nf-winevt-evtformatmessage) для получения строки сообщения для поставщика или XML-строки, содержащей событие и все строки сообщения.

Помимо получения строк сообщений из запрашиваемых событий, можно также получить строки сообщений из метаданных поставщика. Дополнительные сведения о форматировании сообщения на основе идентификатора сообщения, полученного из метаданных поставщика, см. в разделе [Получение метаданных поставщика](getting-a-provider-s-metadata-.md).

В следующем примере показано, как получить строки сообщения из события.
