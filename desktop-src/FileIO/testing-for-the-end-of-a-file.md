---
description: Пример кода, который показывает, как проверить конец файла во время синхронной операции чтения и во время асинхронной операции чтения.
ms.assetid: 93fa9e29-1ff1-496d-9551-99ae88ba7253
title: Проверка на конец файла
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103991595"
---
# <a name="testing-for-the-end-of-a-file"></a><span data-ttu-id="6fd17-103">Проверка на конец файла</span><span class="sxs-lookup"><span data-stu-id="6fd17-103">Testing for the End of a File</span></span>

<span data-ttu-id="6fd17-104">Функция [**ReadFile**](/windows/desktop/api/FileAPI/nf-fileapi-readfile) проверяет условие конца файла (EOF) по-разному для синхронных и асинхронных операций чтения.</span><span class="sxs-lookup"><span data-stu-id="6fd17-104">The [**ReadFile**](/windows/desktop/api/FileAPI/nf-fileapi-readfile) function checks for the end-of-file condition (EOF) differently for synchronous and asynchronous read operations.</span></span> <span data-ttu-id="6fd17-105">Когда операция синхронного чтения получается до конца файла, функция **ReadFile** возвращает **значение true** и задает для переменной, на которую указывает параметр *лпнумберофбитесреад* , значение 0.</span><span class="sxs-lookup"><span data-stu-id="6fd17-105">When a synchronous read operation gets to the end of a file, **ReadFile** returns **TRUE** and sets the variable pointed to by the *lpNumberOfBytesRead* parameter to zero.</span></span> <span data-ttu-id="6fd17-106">Асинхронная операция чтения может возникать в конце файла во время вызова метода **ReadFile** или во время последующих асинхронных операций, если указатель файла программным образом находится за пределами файла.</span><span class="sxs-lookup"><span data-stu-id="6fd17-106">An asynchronous read operation can encounter the end of a file during the initiating call to **ReadFile** or during subsequent asynchronous operations if the file pointer is programmatically advanced beyond the end of the file.</span></span>

<span data-ttu-id="6fd17-107">В следующем примере C++ показано, как проверить конец файла во время синхронной операции чтения.</span><span class="sxs-lookup"><span data-stu-id="6fd17-107">The following C++ example shows how to test for the end of a file during a synchronous read operation.</span></span>
...
<span data-ttu-id="6fd17-108">Проверка конца файла во время асинхронной операции чтения немного сложнее, чем для аналогичной синхронной операции чтения.</span><span class="sxs-lookup"><span data-stu-id="6fd17-108">The test for end-of-file during an asynchronous read operation is slightly more involved than for a similar synchronous read operation.</span></span> <span data-ttu-id="6fd17-109">Индикатор конца файла для асинхронных операций чтения — когда [**GetOverlappedResult**](/windows/desktop/api/ioapiset/nf-ioapiset-getoverlappedresult) возвращает **значение false** , а [**GetLastError**](/windows/desktop/api/errhandlingapi/nf-errhandlingapi-getlasterror) возвращает **ошибку с \_ маркером \_ EOF**.</span><span class="sxs-lookup"><span data-stu-id="6fd17-109">The end-of-file indicator for asynchronous read operations is when [**GetOverlappedResult**](/windows/desktop/api/ioapiset/nf-ioapiset-getoverlappedresult) returns **FALSE** and [**GetLastError**](/windows/desktop/api/errhandlingapi/nf-errhandlingapi-getlasterror) returns **ERROR\_HANDLE\_EOF**.</span></span>

<span data-ttu-id="6fd17-110">В следующем примере C++ показано, как проверить конец файла во время асинхронной операции чтения.</span><span class="sxs-lookup"><span data-stu-id="6fd17-110">The following C++ example shows how to test for the end of file during an asynchronous read operation.</span></span>
...
<span data-ttu-id="6fd17-111">Выходные данные этого примера кода выглядят следующим образом.</span><span class="sxs-lookup"><span data-stu-id="6fd17-111">The output from this sample code is as follows.</span></span>
