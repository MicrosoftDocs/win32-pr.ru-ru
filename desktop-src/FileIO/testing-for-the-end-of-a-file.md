...
---
# <a name="testing-for-the-end-of-a-file"></a>Проверка на конец файла

Функция [**ReadFile**](/windows/desktop/api/FileAPI/nf-fileapi-readfile) проверяет условие конца файла (EOF) по-разному для синхронных и асинхронных операций чтения. Когда операция синхронного чтения получается до конца файла, функция **ReadFile** возвращает **значение true** и задает для переменной, на которую указывает параметр *лпнумберофбитесреад* , значение 0. Асинхронная операция чтения может возникать в конце файла во время вызова метода **ReadFile** или во время последующих асинхронных операций, если указатель файла программным образом находится за пределами файла.
...
Выходные данные этого примера кода выглядят следующим образом.
