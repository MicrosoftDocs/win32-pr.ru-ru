...
Структуры данных автономного присоединение к домену (ODJ) не определены в файле заголовка К\К + +.  Вместо этого структуры определяются в формате языка определения интерфейса (IDL), который после компиляции используется для сериализации и десериализации. В случае сериализации и десериализации платформы Windows эти структуры автоматически обрабатываются следующими API-интерфейсами Win32:
